 ...
 * <p> An L5.Element harness for iframe elements.

  Adds L5.UpdateManager(Updater) support and a compatible 'update' method for
  ...
  * Usage:<br>
   ...
   * var i = new L5.ux.ManagedIFrame("myIframe");
   ...
   * // meaning, there (by default) will be no L5 namespace defined in it since the document is
   * // overwritten after each call to the update method, and no styleSheets.
  * </code></pre>
  * <br>
   * @cfg {Boolean/Object} autoCreate True to auto generate the IFRAME element, or a {@link L5.DomHelper} config of the IFRAME to create
   * @cfg {String} html Any markup to be applied to the IFRAME's document content when rendered.
   * @cfg {Object} loadMask An {@link L5.LoadMask} config or true to mask the iframe while using the update or setSrc methods (defaults to false).
 ...
 * Release: 1.2  (8/22/2008) FF3 Compatibility Fixes, loadMask tweaks
            ...
            1.1  (4/13/2008) Adds L5.Element, CSS Selectors (query,select) fly, and CSS
...
L5.ux.ManagedIFrame = function(){
    ...
    if(el && el.dom && el.dom.tagName == 'IFRAME'){
            ...
            L5.get(L5.DomHelper.append(config.autoCreate.parent||document.body,
                L5.apply({tag:'iframe', src:(L5.isIE&&L5.isSecure)?L5.SSL_SECURE_URL:''},config.autoCreate))):null;
    }
    ...
    this.addEvents({
         ...
         * @param {L5.ux.ManagedIFrame} this
         * @param {L5.Event}
    ...
    if(config.listeners){
        ...
        L5.ux.ManagedIFrame.superclass.constructor.call(this);
    }

    L5.apply(el,this);  // apply this class interface ( pseudo Decorator )
    ...
    var um = el.updateManager=new L5.UpdateManager(el,true);
    ...
    return L5.ux.ManagedIFrame.Manager.register(el);

};

var MIM = L5.ux.ManagedIFrame.Manager = function(){
  var frames = {};
  ...
  var implementation= {
    ...
    hideShims : function(){
        ...
        L5.select('.'+this.shimCls,true).removeClass(this.shimCls+'-on');
    ...
    showShims : function(){
       if(!this.shimApplied){
          ...
          L5.select('.'+this.shimCls,true).addClass(this.shimCls+'-on');
       }
    ...
    destroy : function(){
      if(this._domreadySignature  ){
        L5.EventManager.un.apply(L5.EventManager,this._domreadySignature);
       }
   ...
   if(document.addEventListener){  //for Gecko and Opera and any who might support it later
       L5.EventManager.on.apply(L5.EventManager,implementation._domreadySignature=[window,"DOMFrameContentLoaded", readyHandler , implementation]);
   }

   L5.EventManager.on(window,'beforeunload', implementation.destroy, implementation);
 ...
 //Provide an L5.Element interface to frame document elements
 ...
 L5.apply(MIM.El.prototype,L5.Element.prototype);

 L5.extend(L5.ux.ManagedIFrame , L5.util.Observable,
    {

    src : null ,

    resetUrl :L5.isIE&&L5.isSecure?L5.SSL_SECURE_URL: 'about:blank' ,
      /**
    ...
    update : function(content,loadScripts,callback){
        ...
        content = L5.DomHelper.markup(content||'');
    ...
    ,fly : function(el, named){
        ...
        if(!MIM._flyweights[named]){
            MIM._flyweights[named] = new L5.Element.Flyweight();
        }
    ...
    ,select : function(selector, unique){
        var d;
        return (d = this.getDocument())?L5.Element.select(selector, unique, d):null;
     }
    ...
    ,query : function(selector){
        var d;
        return (d = this.getDocument())?L5.DomQuery.select(selector, d):null;
     }


    /**
     * Returns the current HTML document object as an {@link L5.Element}.
     * @return L5.Element The document
    */
    ,getDoc  : function(){
    ...
    ,_renderHook : function(){
        ...
        this._hooked = false;
        try{
           if(this.writeScript('(function(){(window.hostMIF = parent.L5.get("'+
                                this.dom.id+
                                '"))._windowContext='+
                                (L5.isIE?'window':'{eval:function(s){return eval(s);}}')+
                                ';})();')
                ){
    ...
    sendMessage : function (message,tag,origin){
         ...
         if(this.disableMessaging !== true && (win = this.getWindow())){
              ...
              tag || (tag= message.tag || '');
              tag = tag.toLowerCase();
              message = L5.applyIf(message.data?message:{data:message},
                                 {type   :L5.isEmpty(tag)?'message':'message:'+tag
                                 ,domain :origin || document.domain
                                 ,uri    : document.documentURI
                                 ,source : window
                                 ,tag    :tag || null
    ...
    */
    getDocument:function(){
      ...
      try{
        doc = (L5.isIE && win?win.document : null ) ||
    ...
    */
    ...
    destroy:function(){
        ...
        if(this.dom){
             ...
             L5.ux.ManagedIFrame.Manager.deRegister(this);
             this._windowContext = null;
             //IE Iframe cleanup
             if(L5.isIE && this.dom.src){
        ...
        if(this.loadMask){ L5.apply(this.loadMask,{masker :null ,maskEl : null});}
    ...
    ,writeScript  : function(block, attributes) {
        attributes = L5.apply({},attributes||{},{type :"text/javascript",text:block});
     ...
     * @param {String/Object} name of the function or
                              function map object: {name:'encodeHTML',fn:L5.util.Format.htmlEncode}
     * @param {Boolean} useDOM - if true inserts the fn into a dynamic script tag,
    ...
    ,showMask: function(msg,msgCls,forced){
          ...
          if((lmask = this.loadMask) && (!lmask.disabled|| forced)){
               ...
               lmask.masker || (lmask.masker = L5.get(lmask.maskEl||this.dom.parentNode||this.wrap({tag:'div',style:{position:'relative'}})));
               lmask._vis = true;
    ...
    */
    ,checkDOM : function(win){
        if(L5.isOpera || L5.isGecko || !this._frameAction )return;
...
var CSSInterface = function(hostDocument){
 ...
 /*
  * @class L5.ux.ManagedIFramePanel
  ...
  *
 */
 L5.ux.ManagedIframePanel = L5.extend(L5.Panel, {
    ...
    * Custom CSS styles to be applied to the ux.ManagedIframe element in the format expected by {@link L5.Element#applyStyles}
    * (defaults to CSS Rule {overflow:'auto'}).
    */
    frameStyle  : {overflow:'auto'},
    frameConfig : null,
    hideMode    : !L5.isIE?'display':'display',
    shimCls     : L5.ux.ManagedIFrame.Manager.shimCls,
    shimUrl     : null,
    loadMask    : false,
    stateful    : false,
    animCollapse: L5.isIE && L5.enableFx,
    autoScroll  : false,
    closable    : true, /* set True by default in the event a site times-out while loadMasked */
    ctype       : "L5.ux.ManagedIframePanel",
    showLoadIndicator : false,

    /**
    *@cfg {String/Object} unsupportedText Text (or L5.DOMHelper config) to display within the rendered iframe tag to indicate the frame is not supported
    */
   ...
   ,initComponent : function(){
        ...
        this.bodyCfg ||
                    ...
                    ,children:[
                        //L5.apply(
                          L5.apply({
                            ...
                            , L5.isIE && L5.isSecure?{src:L5.SSL_SECURE_URL}:false
                            )
                             //the shimming agent
                            ,{tag:'img', src:this.shimUrl||L5.BLANK_IMAGE_URL , cls: this.shimCls , galleryimg:"no"}
                         ]
           });
         ...
         L5.ux.ManagedIframePanel.superclass.initComponent.call(this);
    ...
    doLayout   :  function(){
        ...
        if(this.fitToParent && !this.ownerCt){
            var pos = this.getPosition(), size = (L5.get(this.fitToParent)|| this.getEl().parent()).getViewSize();
            this.setSize(size.width - pos[0], size.height - pos[1]);
        }
        L5.ux.ManagedIframePanel.superclass.doLayout.apply(this,arguments);
    ...
    beforeDestroy : function(){
        ...
        if(this.rendered){
             ...
             L5.each(['iframe','shim','header','topToolbar','bottomToolbar','footer','loadMask','body','bwrap'],
                function(elName){
                  if(this[elName]){
                    if(typeof this[elName].destroy == 'function'){
                         this[elName].destroy();
                    } else { L5.destroy(this[elName]); }
        ...
        L5.ux.ManagedIframePanel.superclass.beforeDestroy.call(this);
    },
    onDestroy : function(){
        //Yes, Panel.super (Component), since we're doing Panel cleanup beforeDestroy instead.
        L5.Panel.superclass.onDestroy.call(this);
    },
    // private
    onRender : function(ct, position){
        L5.ux.ManagedIframePanel.superclass.onRender.call(this, ct, position);
        ...
        if(this.iframe = this.body.child('iframe.x-managed-iframe')){
            ...
            if(this.loadMask){
                this.loadMask = L5.apply({disabled     :false
                                          ,maskEl       :this.body
                                          ,hideOnReady  :true}
                                          ,this.loadMask);
             }

            if(this.iframe = new L5.ux.ManagedIFrame(this.iframe, {
            ...
            while(ownerCt){
                ...
                ownerCt.on('afterlayout',function(container,layout){
                        var MIM = L5.ux.ManagedIFrame.Manager,st=false;
                        L5.each(['north','south','east','west'],function(region){
                            var reg;
        ...
        this.shim = L5.get(this.body.child('.'+this.shimCls));
    },
    ...
    afterRender : function(container){
        var html = this.html;
        delete this.html;
        L5.ux.ManagedIframePanel.superclass.afterRender.call(this);
        if(this.iframe){
            if(this.defaultSrc){
                this.setSrc();
            }
            else if(html){
                this.iframe.update(typeof html == 'object' ? L5.DomHelper.markup(html) : html);
            }
        }
    }
    ...
    ,on : function(name){
           ...
           L5.ux.ManagedIframePanel.superclass.on.apply(this, arguments);
    },
    /**
    * Sets the embedded Iframe src property.
    * @param {String/Function} url (Optional) A string or reference to a Function that returns a URI string when called
    ...
    */
    setSrc : function(url, discardUrl,callback){
         ...
         var src = url || (L5.isIE&&L5.isSecure?L5.SSL_SECURE_URL:'');
    ...
    getState: function(){
         ...
         return L5.apply(L5.ux.ManagedIframePanel.superclass.getState.call(this) || {},
             URI?{defaultSrc  : typeof URI == 'function'?URI():URI}:null );

    },
    /**
     * Get the {@link L5.Updater} for this panel's iframe/or body. Enables you to perform Ajax-based document replacement of this panel's iframe document.
     * @return {L5.Updater} The Updater
     */
    getUpdater : function(){
        return this.rendered?(this.iframe||this.body).getUpdater():null;
    },
    /**
     * Get the embedded iframe L5.Element for this panel
     * @return {L5.Element} The Panels ux.ManagedIFrame instance.
     */
    getFrame : function(){
    ...
    /**
     * Get the embedded iframe's document as an L5.Element.
     * @return {L5.Element object} or null if unavailable
     */
...
L5.reg('iframepanel', L5.ux.ManagedIframePanel);

L5.ux.ManagedIframePortlet = L5.extend(L5.ux.ManagedIframePanel, {
     anchor: '100%',
     frame:true,
     collapseEl:'bwrap',
     collapsible:true,
...
L5.reg('iframeportlet', L5.ux.ManagedIframePortlet);

/* override adds a third visibility feature to L5.Element:
...
L5.apply(L5.Element.prototype, {
  setVisible : function(visible, animate){
        if(!animate || !L5.lib.Anim){
            if(this.visibilityMode == L5.Element.DISPLAY){
                this.setDisplayed(visible);
            }else if(this.visibilityMode == L5.Element.VISIBILITY){
                this.fixDisplay();
            ...
            this.anim({opacity: { to: (visible?1:0) }},
                  ...
                  null, .35, 'easeIn', function(){
                     ...
                     if(!visible){
                         if(visMode == L5.Element.DISPLAY){
                             dom.style.display = "none";
                         }else if(visMode == L5.Element.VISIBILITY){
                             dom.style.visibility = "hidden";
                         }else {
                             L5.get(dom).addClass(String(visMode));
                         }
