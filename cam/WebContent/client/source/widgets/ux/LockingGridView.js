...
L5.ux.grid.LockingGridView = L5.extend(L5.grid.GridView, {
    lockText : 'Lock',
    unlockText : 'Unlock',
    rowBorderWidth : 1,
    lockedBorderWidth : 1,
    ...
    initTemplates : function(){
        ...
        if(!ts.master){
            ts.master = new L5.Template(
                '<div class="l-grid3" hidefocus="true">',
                    '<div class="l-grid3-locked">',
                        '<div class="l-grid3-header"><div class="l-grid3-header-inner"><div class="l-grid3-header-offset" style="{lstyle}">{lockedHeader}</div></div><div class="l-clear"></div></div>',
                        '<div class="l-grid3-scroller"><div class="l-grid3-body" style="{lstyle}">{lockedBody}</div><div class="l-grid3-scroll-spacer"></div></div>',
                    '</div>',
                    '<div class="l-grid3-viewport l-grid3-unlocked">',
                        '<div class="l-grid3-header"><div class="l-grid3-header-inner"><div class="l-grid3-header-offset" style="{ostyle}">{header}</div></div><div class="l-clear"></div></div>',
                        '<div class="l-grid3-scroller"><div class="l-grid3-body" style="{bstyle}">{body}</div><a href="#" class="l-grid3-focus" tabIndex="-1"></a></div>',
                    '</div>',
        ...
        this.templates = ts;
        L5.ux.grid.LockingGridView.superclass.initTemplates.call(this);
    },
    ...
    getCell : function(row, col){
        ...
        return L5.ux.grid.LockingGridView.superclass.getCell.call(this, row, col - llen);
    },
    ...
    getHeaderCell : function(index){
        ...
        return L5.ux.grid.LockingGridView.superclass.getHeaderCell.call(this, index - llen);
    },
    ...
    addRowClass : function(row, cls){
        ...
        L5.ux.grid.LockingGridView.superclass.addRowClass.call(this, row, cls);
    },
    ...
    removeRowClass : function(row, cls){
        ...
        L5.ux.grid.LockingGridView.superclass.removeRowClass.call(this, row, cls);
    },
    
    removeRow : function(row) {
        L5.removeNode(this.getLockedRow(row));
        L5.ux.grid.LockingGridView.superclass.removeRow.call(this, row);
    },
    ...
    removeRows : function(firstRow, lastRow){
        ...
        L5.ux.grid.LockingGridView.superclass.removeRows.call(this, firstRow, lastRow);
    },
    
    syncScroll : function(e){
        var mb = this.scroller.dom;
        this.lockedScroller.dom.scrollTop = mb.scrollTop;
        L5.ux.grid.LockingGridView.superclass.syncScroll.call(this, e);
    },
    ...
    doRender : function(cs, rs, ds, startRow, colCount, stripe){
        ...
        for(var j = 0, len = rs.length; j < len; j++){
            ...
            for(var i = 0; i < colCount; i++){
                ...
                p.attr = p.cellAttr = '';
                p.value = c.renderer(r.data[c.name], p, r, rowIndex, i, ds,c.name,this.grid);
                p.style = c.style;
                if(L5.isEmpty(p.value)){
                    p.value = '&#160;';
                }
                if(this.markDirty && r.dirty && L5.isDefined(r.modified[c.name])){
    ...
    processRows : function(startRow, skipStripe){
        ...
        for(var i = 0, len = rows.length; i < len; ++i){
            ...
            if(!skipStripe){
                ...
                if ((idx + 1) % 2 === 0){
                    ...
                    lrow.className += ' l-grid3-row-alt';
                }
            }
            if(this.syncHeights){
                var el1 = L5.get(row),
        ...
        if(startRow === 0){
            L5.fly(rows[0]).addClass(this.firstRowCls);
            L5.fly(lrows[0]).addClass(this.firstRowCls);
        }
        L5.fly(rows[rows.length - 1]).addClass(this.lastRowCls);
        L5.fly(lrows[lrows.length - 1]).addClass(this.lastRowCls);
    },
    ...
    renderUI : function(){
        ...
        L5.fly(this.lockedInnerHd).on('click', this.handleHdDown, this);
        this.mainHd.on({
            scope: this,
        ...
        if(g.enableColumnResize !== false){
            ...
            this.splitZone.setOuterHandleElId(L5.id(this.lockedHd.dom));
            this.splitZone.setOuterHandleElId(L5.id(this.mainHd.dom));
        }
        if(g.enableColumnMove){
            this.columnDrag = new L5.grid.GridView.ColumnDragZone(g, this.innerHd);
            this.columnDrag.setOuterHandleElId(L5.id(this.lockedInnerHd));
            this.columnDrag.setOuterHandleElId(L5.id(this.innerHd));
    ...
    getScrollOffset: function(){
        return L5.num(this.scrollOffset, L5.getScrollBarWidth());
    },
    ...
    getResolvedXY : function(resolved){
        ...
        return c ? L5.fly(c).getXY() : [this.scroller.getX(), L5.fly(r).getY()];
    },
    
    syncFocusEl : function(row, col, hscroll){
        L5.ux.grid.LockingGridView.superclass.syncFocusEl.call(this, row, col, col < this.cm.getLockedCount() ? false : hscroll);
    },
    
    ensureVisible : function(row, col, hscroll){
        return L5.ux.grid.LockingGridView.superclass.ensureVisible.call(this, row, col, col < this.cm.getLockedCount() ? false : hscroll);
    },
    ...
    insertRows : function(dm, firstRow, lastRow, isUpdate){
        ...
        if(!isUpdate && firstRow === 0 && lastRow >= last){
            ...
            if(before){
                ...
                L5.DomHelper.insertHtml('beforeBegin', before, html[0]);
                before = this.getLockedRow(firstRow);
                L5.DomHelper.insertHtml('beforeBegin', before, html[1]);
            }else{
                this.removeRowClass(last - 1, this.lastRowCls);
                L5.DomHelper.insertHtml('beforeEnd', this.mainBody.dom, html[0]);
                L5.DomHelper.insertHtml('beforeEnd', this.lockedBody.dom, html[1]);
            }
            if(!isUpdate){
    ...
    getColumnData : function(){
        ...
        for(var i = 0; i < colCount; i++){
            ...
            cs[i] = {
                name : (!L5.isDefined(name) ? this.ds.fields.get(i).name : name),
    ...
    refreshRow : function(record){
        L5.ux.grid.LockingGridView.superclass.refreshRow.call(this, record);
        var index = L5.num(record,this.ds.indexOf(record));
    ...
    initData : function(ds, cm){
        ...
        L5.ux.grid.LockingGridView.superclass.initData.call(this, ds, cm);
        if(this.cm){
    ...
    handleHdMenuClick : function(item){
        ...
        switch(id){
            ...
            break;
            default:
                return L5.ux.grid.LockingGridView.superclass.handleHdMenuClick.call(this, item);
        }
        return true;
    },
    
    handleHdDown : function(e, t){
        L5.ux.grid.LockingGridView.superclass.handleHdDown.call(this, e, t);
        if(this.grid.enableColLock !== false){
    ...
    updateLockedWidth: function(){
        var lw = this.cm.getTotalLockedWidth(),
            ...
            lp = L5.isBorderBox ? 0 : this.lockedBorderWidth,
            rp = L5.isBorderBox ? 0 : this.rowBorderWidth,
...
L5.ux.grid.LockingColumnModel = L5.extend(L5.grid.ColumnModel, {
    ...
    setConfig:function(config, initial){
    	...
    	for(var i=0;i<config.length;i++){
    		if(config[i].locked){
    			locked[locked.length]=config[i];
    		}else{
    			unlocked[unlocked.length]=config[i];
    		}		
    	}
    	config = locked.concat(unlocked);
        L5.ux.grid.LockingColumnModel.superclass.setConfig.call(this,config,initial);
    },
