 ...
 * <pre><code>
var resizer = new L5.Resizable("element-id", {
    handles: 'all',
    minWidth: 200,
    minHeight: 100,
    maxWidth: 500,
 ...
 * @param {Object} config configuration options
  */
L5.Resizable = function(el, config){
    this.el = L5.get(el);
     ...
     * This may be queried using {@link L5.Element#getBox} to provide the new area to resize to.
     * Read only.
     * @type L5.Element.
     * @property proxy
     */
    this.proxy = this.el.createProxy({tag: "div", cls: "l-resizable-proxy", id: this.el.id + "-rzproxy"}, L5.getBody());
    ...
    if(this.pinned){
        ...
        this.el.addClass("l-resizable-pinned");
    }
    ...
    var ps = L5.Resizable.positions;
    for(var i = 0, len = hs.length; i < len; i++){
        if(hs[i] && ps[hs[i]]){
            var pos = ps[hs[i]];
            this[pos] = new L5.Resizable.Handle(this, pos, this.disableTrackOver, this.transparent);
        }
    }
    // legacy
    ...
    if(this.resizeChild){
        if(typeof this.resizeChild == "boolean"){
            this.resizeChild = L5.get(this.el.dom.firstChild, true);
        }else{
            this.resizeChild = L5.get(this.resizeChild, true);
        }
    }
    ...
    L5.Resizable.superclass.constructor.call(this);
};

L5.extend(L5.Resizable, L5.util.Observable, {
        resizeChild : false,
        adjustments : [0, 0],
        minWidth : 5,
        minHeight : 5,
         ...
         */
        /**
         * @cfg {L5.lib.Region} resizeRegion Constrain the resize to a particular region
         */

        /**
         * @event beforeresize
         * Fired before resize is allowed. Set enabled to false to cancel resize.
         * @param {L5.Resizable} this
         * @param {L5.EventObject} e The mousedown event
         */
        /**
         * @event resize
         ...
         * @param {L5.EventObject} e The mouseup event
         */
    ...
    startSizing : function(e, handle){
        ...
        if(this.enabled){ // 2nd enabled check in case disabled before beforeresize handler
            ...
            if(!this.overlay){
                this.overlay = this.el.createProxy({tag: "div", cls: "l-resizable-overlay", html: "&#160;"}, L5.getBody());
            ...
            if(this.constrainTo) {
                var ct = L5.get(this.constrainTo);
...
new L5.Panel({
    ...
    handleOver : function(){
        if(this.enabled){
            this.el.addClass("l-resizable-over");
        }
    },
    ...
    handleOut : function(){
        if(!this.resizing){
            this.el.removeClass("l-resizable-over");
        }
    },
...
L5.Resizable.positions = {
    n: "north", s: "south", e: "east", w: "west", se: "southeast", sw: "southwest", nw: "northwest", ne: "northeast"
};

// private
L5.Resizable.Handle = function(rz, pos, disableTrackOver, transparent){
    if(!this.tpl){
        // only initialize the template if resizable is used
        var tpl = L5.DomHelper.createTemplate(
            {tag: "div", cls: "l-resizable-handle l-resizable-handle-{0}"}
        );
        tpl.compile();
        L5.Resizable.Handle.prototype.tpl = tpl;
    }
    this.position = pos;
    this.rz = rz;
