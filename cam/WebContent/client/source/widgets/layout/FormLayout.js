 ...
 * It is responsible for rendering the labels of {@link L5.form.Field Field}s.</p>
 * <p>This layout manager is used when a Container is configured with the layout:'form' {@link L5.Container#layout layout} config,
 * and should generally not need to be created directly via the new keyword. In an application,
 * it will usually be preferrable to use a {@link L5.form.FormPanel FormPanel} (which automatically uses FormLayout as its layout
 ...
 * the {@link L5.Container#layoutConfig layoutConfig} object which will then be applied internally to the layout.</p>
 * <p>The {@link L5.Container Container} <i>using</i> the FormLayout can also accept the following layout-specific config
 * properties:
 ...
 * </ul></div></p>
 * <p>Any type of components can be added to a FormLayout, but items that inherit from {@link L5.form.Field}
 ...
 * form field wrapper (defaults to 'l-form-clear-left')</div></li>
 ...
 * and field element (the default class is 'l-form-item' and itemCls will be added to that).  If supplied,
...
var form = new L5.form.FormPanel({
    labelWidth: 75,
    title: 'Form Layout',
 ...
 */
L5.layout.FormLayout = L5.extend(L5.layout.AnchorLayout, {
    /**
    ...
    setContainer : function(ct){
        L5.layout.FormLayout.superclass.setContainer.call(this, ct);

        if(ct.labelAlign){
            ct.addClass('l-form-label-'+ct.labelAlign);
        }
        ...
        if(!this.fieldTpl){
            ...
            var t = new L5.Template(
                '<div class="l-form-item {5}" tabIndex="-1">',
                    '<label for="{0}" style="{2}" class="l-form-item-label">{1}{4}</label>',
                    '<div class="l-form-element" id="l-form-el-{0}" style="{3}">',
                    '</div><div class="{6}"></div>',
                '</div>'
            );
            t.disableFormats = true;
            t.compile();
            L5.layout.FormLayout.prototype.fieldTpl = t;
        }
    },
    ...
    renderItem : function(c, position, target){
        if(c && !c.rendered && c.isFormField && c.inputType != 'hidden'){
            var args = [
                   ...
                   (c.itemCls||this.container.itemCls||'') + (c.hideLabel ? ' l-hide-label' : ''),
                   c.clearCls || 'l-form-clear-left' 
            ];
            ...
            c.render('l-form-el-'+c.id);
        }else {
            L5.layout.FormLayout.superclass.renderItem.apply(this, arguments);
        }
    },
...
L5.Container.LAYOUTS['form'] = L5.layout.FormLayout;