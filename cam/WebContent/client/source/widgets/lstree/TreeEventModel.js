...
L5.tree.TreeEventModel.prototype = {
    ...
    delegateClick : function(e, t){
        ...
        } else if(e.getTarget('input[type=radio]', 1)){//增加对radio的处理
			this.onCheckboxClick(e, this.getNode(e));
    ...
    onNodeClick : function(e, node){
		if(node.nodeModel){
			node.nodeModel.onNodeSelected(node);
		}
        node.ui.onClick(e);
    },
...
//		if(node.nodeModel){双击是由两个单击组成,对于节点选中事件来说,只要监听一次单击就够了
//			node.nodeModel.onNodeSelected(node);
//		}
    ...
    onNodeContextMenu : function(e, node){
		if(node.nodeModel){
			node.nodeModel.onNodeContextMenu(e,node);
		}
