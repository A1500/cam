/**
 * @class L5.form.CheckboxGroup
 * @extends L5.form.Field
 * A grouping container for {@link L5.form.Checkbox} controls.
 * @constructor
 * Creates a new CheckboxGroup
 * @param {Object} config Configuration options
 */
L5.form.CheckboxGroup = L5.extend(L5.form.Field, {
    /**
     * @cfg {Array} items An Array of {@link L5.form.Checkbox Checkbox}es or Checkbox config objects
    ...
    onRender : function(ct, position){
        if(!this.el){
            ...
            if(this.items[0].items){
                ...
                for(var i=0, len=this.items.length; i<len; i++){
                    L5.applyIf(this.items[i], colCfg);
                };
                ...
                for(var i=0; i<numCols; i++){
                    var cc = L5.apply({items:[]}, colCfg);
                    cc[this.columns[i] <= 1 ? 'columnWidth' : 'width'] = this.columns[i];
                    if(this.defaults){
                        cc.defaults = L5.apply(cc.defaults || {}, this.defaults)
                    }
                    cols.push(cc);
                };
            ...
            this.panel = new L5.Panel(panelCfg);
        ...
        L5.form.CheckboxGroup.superclass.onRender.call(this, ct, position);
    },
    ...
    reset : function(){
        L5.form.CheckboxGroup.superclass.reset.call(this);
     ...
     * @hide
     */
    setRawValue : L5.emptyFn
    
});

L5.reg('checkboxgroup', L5.form.CheckboxGroup);
