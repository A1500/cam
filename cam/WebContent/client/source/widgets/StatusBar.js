/**
 * @class L5.StatusBar
 * <p>Basic status bar component that can be used as the bottom toolbar of any {@link L5.Panel}.  In addition to
 * supporting the standard {@link L5.Toolbar} interface for adding buttons, menus and other items, the StatusBar
 * provides a greedy status element that can be aligned to either side and has convenient methods for setting the
 * status text and icon.  You can also indicate that something is processing using the {@link #showBusy} method.</p>
 * <p><b>Note:</b> Although StatusBar supports xtype:'statusbar', at this time L5.Toolbar (the base class) does
 ...
 */
L5.StatusBar = L5.extend(L5.Toolbar, {
    /**
...
// Example CSS rule:
.l-statusbar .l-status-custom {
    padding-left: 25px;
    background: transparent url(images/custom-icon.gif) no-repeat 3px 3px;
}

// Initializing the status bar:
var sb = new L5.StatusBar({
    defaultIconCls: 'l-status-custom'
});

// Setting it in code:
sb.setStatus({
    text: 'New status',
    iconCls: 'l-status-custom'
});
</code></pre>
     */
    ...
    /**
     * @cfg {String} cls
     * The base class applied to the containing element for this component on render (defaults to 'l-statusbar')
     */
    cls : 'l-statusbar',
    /**
     * @cfg {String} busyIconCls
     * The default {@link #iconCls} applied when calling {@link #showBusy} (defaults to 'l-status-busy'). It can be
    ...
    initComponent : function(){
        if(this.statusAlign=='right'){
            this.cls += ' l-status-right';
        }
        L5.StatusBar.superclass.initComponent.call(this);
    },
    
    // private
    afterRender : function(){
        L5.StatusBar.superclass.afterRender.call(this);
        
        var right = this.statusAlign=='right',
            td = L5.get(this.nextBlock());
        ...
        this.statusEl = td.createChild({
            cls: 'l-status-text ' + (this.iconCls || this.defaultIconCls || ''),
