...
* inline code execution and more.  XTemplate also provides the templating mechanism built into {@link L5.DataView}.</p>
...
* <li><b><tt>fm</tt></b>: An alias for <tt>L5.util.Format</tt>.</li>
* </ul>
...
*/
L5.XTemplate = function(){
    L5.XTemplate.superclass.constructor.apply(this, arguments);
    var s = this.html;
    ...
    while(m = s.match(re)){
       ...
       if(m3){
           ...
           if(exp){
               fn = new Function('values', 'parent', 'xindex', 'xcount', 'with(values){ return '+(L5.util.Format.htmlDecode(exp))+'; }');
           }
       }
       if(m4){
           exp = m4 && m4[1] ? m4[1] : null;
           if(exp){
               exec = new Function('values', 'parent', 'xindex', 'xcount', 'with(values){ '+(L5.util.Format.htmlDecode(exp))+'; }');
           }
       }
       if(name){
           switch(name){
...
L5.extend(L5.XTemplate, L5.Template, {
    // private
    re : /\{([\w-\.\#]+)(?:\:([\w\.]*)(?:\((.*?)?\))?)?(\s?[\+\-\*\\]\s?[\d\.\+\-\*\\\(\)]+)?\}/g,
    // private
    codeRe : /\{\[((?:\\\]|.|\n)*?)\]\}/g,
 ...
 * @method apply
 */
L5.XTemplate.prototype.apply = L5.XTemplate.prototype.applyTemplate;

/**
 * Creates a template from the passed element's value (<i>display:none</i> textarea, preferred) or innerHTML.
 * @param {String/HTMLElement} el A DOM element or its id
 * @return {L5.Template} The created template
 * @static
 */
L5.XTemplate.from = function(el){
    el = L5.getDom(el);
    return new L5.XTemplate(el.value || el.innerHTML);
};