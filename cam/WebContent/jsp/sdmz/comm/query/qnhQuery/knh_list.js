...
function getXzqhDs() {
	...
	xzqhDs.setParameter("filterSql"," id in('"+singleDs.getAllRecords()[0].get("town").trim()+"','"+singleDs.getAllRecords()[0].get("village")+"')");
	xzqhDs.load();
}
/**
 * value:是当前单元格的值
...
function shbzRenderer(value, cellmeta, record, rowIndex, columnIndex, store) {
	...
	if(value=="0") {
		...
		re = "<span style='color:red;'>审核未通过</span>";
	}else if (value=="3") {
