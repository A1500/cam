...
<next:Html>
		<form action="" name="form1" method="post" onsubmit="return false" class="L5Form" dataset="portalTemplateDataset">
			...
			<input type="hidden" id="templateId" name ="templateId" field="templateId"  />	
			<input type="hidden" id="type" name ="type" field="type"  />	
...
function init(){
	...
	portalTemplateDataset.setParameter("templateId",templateId);
	portalTemplateDataset.load();
...
function saveTemplate(){
	...
	command.execute("updatePortalTemplate");
	if (!command.error) {
...
function saveAndDesignTemplate(){
	...
	if (!command.error) {
		L5.MessageBox.alert("消息提示", "保存成功!", function(){
			var pageId=L5.getDom("pageId").value;
			var templateId = L5.getDom("templateId").value;
