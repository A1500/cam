...
<model:datasets>
	<model:dataset id="portalUserLayoutWidgetLayout" cmd="org.loushang.portal.layout.cmd.PortalUserLayoutQueryCmd" global="true" method="queryPortalUserLayoutWidgetLayout">
  ...
  <next:BorderLayout>
    <next:Center>
      <next:Panel bodyStyle="overflow-y:auto">
        ...
        <next:TopBar>
          ...
          <next:ToolBarItem iconCls="save"  text="保存页面布局" handler="saveWidgetLayout"/>
    ...
    <next:Right split="true" >
	    <next:Panel id="widgetSelector" width="200" height="100%" 
		  ...
		  <next:Html >
			 <iframe height="100%" width="100%" src="<%=webPath %>/jsp/portal/help/widgethelper.jsp">
			 </iframe>
		  </next:Html>
	    </next:Panel>
    </next:Right>
...
//初始化
function init(){
	portalUserLayoutWidgetLayout.setParameter("id",'<%=id%>');
	portalUserLayoutWidgetLayout.load();
	portalUserLayoutWidgetLayout.on("load",initWidgetLayout);
	// 微件选择器初始为折叠
    var widgetSelector = L5.getCmp("widgetSelector");
    widgetSelector.collapse(false);
	//设置当前页面
	document.getElementById("tbCurPageName").innerHTML="<font color='red'>[<%=portalPage.getName()%>]</font>";
}
...
{	  
	...
	// 4.向每列中添加widget
	// 4.1 widget布局json串
	widgetLayoutJson = portalUserLayoutWidgetLayout.getCustomData("widgetLayout");
	...
	for(var i=0;i<toAddWidgetColumnCount;i++){
		...
		for(var j = 0 ;j<widgetLayoutArray[i].length;j++){
			...
			var record = portalUserLayoutWidgetLayout.getById(widgetId);
			if(record)
			{
...
function saveWidgetLayout(){
	...
	if(widgetLayout!=widgetLayoutJson){
		...
		command.execute("updatePortalWidgetLayout",false);
	}else{
...
//返回到用户布局查询页面
function forReturn(){
	var url="jsp/portal/layout/userlayoutquery.jsp";
	L5.forward(url,"用户布局查询页面");
}
</script>
</html>