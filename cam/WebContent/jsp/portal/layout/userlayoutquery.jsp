...
<body>
<model:datasets>
	<model:dataset id="portalLayoutDataset" cmd="org.loushang.portal.layout.cmd.PortalUserLayoutQueryCmd" method="queryUserPortalLayout" autoLoad="false" pageSize="10" global="true">
		<model:record  >
...
<next:ViewPort>
              ...
              <form name="queryForm" id="queryForm" onsubmit="return false;" action="" method="post" class="L5form">
	            <table  border="1" width="100%">
				  ...
				  <td class="FieldLabel">用户Id：</td>
					<td class="FieldInput"><input type="text"  name="userId" id="userId" field="userId"/></td>
  ...
  <next:EditGridPanel id="layoutGridPanel" title="用户页面" name="layoutGridPanel" width="100%" height="100%" dataset="portalLayoutDataset">
	<next:Columns>
	...
	<next:BottomBar>
		<next:PagingToolBar dataset="portalLayoutDataset" displayMsg="从第{0}条到{1}条，一共{2}条" displayInfo="true"/>
	</next:BottomBar>
</next:EditGridPanel>
			</next:Panel>
		</next:Center>
...
function init(){
	portalLayoutDataset.load();
}
function save(){
	var isValidate = portalLayoutDataset.isValidate();
	if (isValidate != true) {
		L5.MessageBox.alert("消息提示", "校验未通过,不能保存!" + isValidate);
		return;
	}
	
	var records = portalLayoutDataset.getAllChangedRecords();
	...
	if (!command.error) {
		portalLayoutDataset.commitChanges();
		portalLayoutDataset.reload();
...
function update(){
	var layoutGridPanel=L5.getCmp("layoutGridPanel");
	var selecteds=layoutGridPanel.getSelectionModel().getSelections();
	...
	var url="jsp/portal/layout/userlayoutdesign.jsp?id="+id+"&pageId="+pageId;
	L5.forward(url,"设计页面");
}

function remove(){
	if(portalLayoutDataset.getCount()==0){
		L5.MessageBox.alert("消息提示",'没有要删除的记录!');
		return;
	}
	var layoutGridPanel=L5.getCmp('layoutGridPanel');
	var selected = layoutGridPanel.getSelectionModel().getSelections();
	...
	L5.MessageBox.confirm("消息提示", "你确定要删除吗?", function(state){
		if(state=="yes"){
			...
			for(var i=0;i<selected.length;i++){
				...
				portalLayoutDataset.remove(selected[i]);
			}
...
function query(){
	...
	portalLayoutDataset.setParameter("userId",userId);
	portalLayoutDataset.setParameter("pageName",pagename);
	portalLayoutDataset.load();
}
function reset(){
