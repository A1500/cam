...
<model:datasets>
	<model:dataset pageSize="10" id="propertyConfigDataset"
		cmd="org.loushang.workflow.ecgapform.ecgapformextendjsconfig.cmd.EcgapFormExtendJsConfigQueryCmd" 
		method="queryEcgapFormExtendJs"
		autoLoad="false" global="true">
		<model:record>
			<model:field name="id" type="string"/>
			<model:field name="propertyKey" type="string"/>
			<model:field name="propertyValue" type="string"/>
			<model:field name="propertyDescription" type="string"/>
		</model:record>
	</model:dataset>
</model:datasets>
...
function forSave()
{ 
	var isValidate = propertyConfigDataset.isValidate();
	if (isValidate != true) {
	...
	var command = new L5.Command(
			"org.loushang.workflow.ecgapform.ecgapformextendjsconfig.cmd.EcgapFormExtendJsConfigCmd");
	...
	L5.MessageBox.confirm('确定', '确定要删除选中的记录吗？',function(state){
		if(state=="yes"){  
			...
			if (deleteSelected.length < 1) {
				...
				var command = new L5.Command("org.loushang.workflow.ecgapform.ecgapformextendjsconfig.cmd.EcgapFormExtendJsConfigCmd");
