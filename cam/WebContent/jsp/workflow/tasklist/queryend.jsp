...
<model:datasets>
	<model:dataset pageSize="10" id="endTaskDataset" cmd="org.loushang.workflow.tasklist.taskinfo.cmd.EndTaskQueryCmd" autoLoad="false" global="true">
		<model:record>
			...
			<%
		if(null != defaultEndSubjectDefList && !defaultEndSubjectDefList.isEmpty() && defaultEndSubjectDefList.size() > 0){
			for(int i = 0; i<defaultEndSubjectDefList.size(); i++){
				DefaultSubjectDef defaultSubjectDef = (DefaultSubjectDef)defaultEndSubjectDefList.get(i);
			%>
						...
						<form onsubmit="return false;" class="L5form">				
							<table  border="1" width="100%">
								<tr>
									<%
									if(null != defaultEndSubjectDefList && !defaultEndSubjectDefList.isEmpty() && defaultEndSubjectDefList.size() > 0){
										if(null != defaultEndSubjectQueryDefList && !defaultEndSubjectQueryDefList.isEmpty() && defaultEndSubjectQueryDefList.size() > 0){
		 										for (int i = 0; i < defaultEndSubjectQueryDefList.size(); i++) {
...
function query()
{
	...
	%>
	 ...
	 {	
		 endTaskDataset.setParameter("<%=subjectKey%>",queryCondition); 
	 }
	<%
	}
}else{
	...
	endTaskDataset.setParameter("subject",subject); 
	<%	
}
	%>
	endTaskDataset.load();
...
function showState()
{
   ...
   showprocessstate(endTaskGridPanel,"end","jsp/workflow/tasklist/queryend.jsp");
}
</script>
</html>