...
<model:datasets>
	<model:dataset pageSize="10" id="yiBanProfileInfoDataset" cmd="org.loushang.workflow.tasklist.taskinfo.cmd.YiBanTaskQueryCmd" autoLoad="true" method="queryProcYiBanTaskTableProfileInfo" global="true">
		<model:record>
		...
		<next:Center floatable="true" cmargins="0 0 0 0" margins="0 0 0 0" >
			...
			<next:GridPanel id="yiBanTaskGridPanel" name="yiBanTaskGridPanel" anchor="100%" height="100%" dataset="yiBanProfileInfoDataset"     title="已办任务概要信息列表" notSelectFirstRow="true">
				<next:Columns>
					...
					<next:Column header="已办任务个数" field="TASK_COUNT" width="150" renderer="addLink">
						<next:TextField allowBlank="false"/>
					</next:Column>	
				</next:Columns>
				<next:TopBar>
...
function query(){
	...
	if(procDefQueryName!="")
	{
		yiBanProfileInfoDataset.setParameter("PROCESS_NAME",procDefQueryName);
	}
...
function addLink(value,second,record)
{
	...
	return "<a href=\"javascript:viewYiBanTaskDetaiInfo('"+procDefUniqueId+"')\">"+value+"</a>";
}
...
function handle()
{
	...
	var rec=selected[0];
	viewYiBanTaskDetaiInfo(rec.get("PROC_DEF_UNIQUE_ID"));
}

//查看已办任务详细信息
function viewYiBanTaskDetaiInfo(procDefUniqueId)
{			
	var url="jsp/workflow/tasklist/display/procsubject/queryyibandetailinfo.jsp";
