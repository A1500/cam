...
<html>
	<head><style>a{TEXT-DECORATION:none}</style>
		...
		<%
			//taskListType="0"界面没有返回按钮；taskListType="1"界面显示返回按钮。
	    ...
	    <% 
		  ...
		  //任务标题
	      List procSubjectDefList=TaskListJspUtil.getProcEndTaskSubjectDefListByProcDefUniqueId(procDefUniqueId);
		  //任务标题查询条件
	      List procSubjectQueryDefList=TaskListJspUtil.getProcSubjectEndTaskProcSubjectQueryDefListByProcDefUniqueId(procDefUniqueId);
	       //通用帮助查询条件
...
<model:datasets>
	<model:dataset pageSize="10" id="endDataset" cmd="org.loushang.workflow.tasklist.taskinfo.cmd.EndTaskQueryCmd" autoLoad="false" method="queryProcEndTaskTableDetailInfoByProc" global="true">
		<model:record>
...
<next:ViewPort>
	<next:BorderLayout>
		...
		<next:Center floatable="true" cmargins="0 0 0 0" margins="0 0 0 0" >
			...
			<next:GridPanel id="endTaskGridPanel" name="endTaskGridPanel" anchor="100%" height="100%" dataset="endDataset"     title="办结任务详细信息列表" notSelectFirstRow="true">
				<next:Columns>
			        ...
			        %>
						...
						<next:Column id="endTime" header="办理时间" field="endTime" width="150" sortable="true">
							<next:TextField allowBlank="false"/>
						</next:Column>						
						<next:Column id="procEndTime" header="流程结束时间" field="procEndTime" width="150" sortable="true">
							<next:TextField allowBlank="false"/>
						</next:Column>
			        <%
			        } %>
...
function showState()
{
   ...
   var backUrl="jsp/workflow/tasklist/display/procsubject/queryenddetailinfobyproc.jsp";
...
{
	var url="jsp/workflow/tasklist/display/procsubject/queryendprofileinfo.jsp";
