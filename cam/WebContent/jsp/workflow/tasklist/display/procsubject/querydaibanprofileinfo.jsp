...
<html>
	<head><style>a{TEXT-DECORATION:none}</style>
		<title>待办任务概要信息查询</title>
		<next:ScriptManager></next:ScriptManager>
	</head>
<body>

<model:datasets>
	<model:dataset pageSize="10" id="daiBanProfileInfoDataset" cmd="org.loushang.workflow.tasklist.taskinfo.cmd.DaiBanTaskQueryCmd" autoLoad="true" method="queryProcDaiBanTaskTableProfileInfo" global="true">
		<model:record>
...
<next:ViewPort>
	<next:BorderLayout>
		...
		<next:Center floatable="true" cmargins="0 0 0 0" margins="0 0 0 0" >
			...
			<next:GridPanel id="daiBanTaskGridPanel" name="daiBanTaskGridPanel" anchor="100%" height="100%" dataset="daiBanProfileInfoDataset"     title="待办任务概要信息列表" notSelectFirstRow="true">
				<next:Columns>
					...
					<next:Column header="待办任务个数" field="TASK_COUNT" width="150" renderer="addLink">
						<next:TextField allowBlank="false"/>
					</next:Column>	
				</next:Columns>
				<next:TopBar>	
				...
				<next:BottomBar>
					<next:PagingToolBar displayMsg="从第{0}条到{1}条，一共{2}条" displayInfo="true" dataset="daiBanProfileInfoDataset"/>
				</next:BottomBar>
			</next:GridPanel>
			</next:AnchorLayout>
			</next:Panel>
...
function query(){
	...
	if(procDefQueryName!="")
	{
		daiBanProfileInfoDataset.setParameter("PROCESS_NAME",procDefQueryName);
	}
	daiBanProfileInfoDataset.load();
}
//通过链接查看待办任务详细信息
function addLink(value,second,record)
{
	var procDefUniqueId=record.get("PROC_DEF_UNIQUE_ID");
	return "<a href=\"javascript:viewDaiBanTaskDetaiInfo('"+procDefUniqueId+"')\">"+value+"</a>";
}
...
function handle()
{
	...
	var rec=selected[0];
	viewDaiBanTaskDetaiInfo(rec.get("PROC_DEF_UNIQUE_ID"));
}

//查看待办任务详细信息
function viewDaiBanTaskDetaiInfo(procDefUniqueId)
{			
	
	var url="jsp/workflow/tasklist/display/procsubject/querydaibandetailinfo.jsp";
	var query="?procDefUniqueId="+procDefUniqueId;
	L5.forward(url+query,"待办任务详细信息");
}
</script>
</html>