...
<html>
	<head><style>a{TEXT-DECORATION:none}</style>
	    ...
	    <% 
		  ...
		  if (procDefUniqueId == null || "null".equals(procDefUniqueId)) {
			  procDefUniqueId = "";
		  }
		  String procDefId=request.getParameter("procDefId");
		  if (procDefId == null || "null".equals(procDefId)) {
			  procDefId = "";
		  }
		  if (procDefUniqueId == null || "".equals(procDefUniqueId))
		  {
			  procDefUniqueId = TaskListUtil.getLatestProcessDefUniqueIdByProcDefId(procDefId);
		  }
		  //任务标题
	      List procSubjectDefList=TaskListJspUtil.getProcDaiBanTaskSubjectDefListByProcDefUniqueId(procDefUniqueId);
	      //任务标题查询条件
	      List procSubjectQueryDefList=TaskListJspUtil.getProcDaiBanTaskSubjectQueryDefListByProcDefUniqueId(procDefUniqueId);
	       //通用帮助查询条件
...
<model:datasets>
	<model:dataset pageSize="10" id="daiBanDataset" cmd="org.loushang.workflow.tasklist.taskinfo.cmd.DaiBanTaskQueryCmd" autoLoad="false" method="queryProcDaiBanTaskTableDetailInfo" global="true">
		<model:record>
			...
			%>
				...
				<model:field name="actRemainTime" type="string" />
				<model:field name="actSetreMainTimeMin" type="string" />
...
<next:ViewPort>
	<next:BorderLayout>
		...
		<next:Center floatable="true" cmargins="0 0 0 0" margins="0 0 0 0" >
			...
			<next:GridPanel id="daiBanTaskGridPanel" name="daiBanTaskGridPanel" anchor="100%" height="100%" dataset="daiBanDataset"     title="待办任务详细信息列表" notSelectFirstRow="true">
				<next:Columns>
					...
					<% if(null != procSubjectDefList && !procSubjectDefList.isEmpty() && procSubjectDefList.size() > 0){
				        ...
				        %>
				   	<next:Column header="<%=procSubjectDef.getSubjectAlias()%>"
						field="<%=procSubjectDef.getSubjectKey()%>"
				   	 	width="<%=procSubjectDef.getSubjectWidth()%>"
						renderer="addLink"
						sortable="true">
						<next:TextField allowBlank="false"/>
					</next:Column>
...
function init()
{
	daiBanDataset.setParameter("procDefUniqueId","<%=request.getParameter("procDefUniqueId")%>");
	daiBanDataset.setParameter("procDefId","<%=request.getParameter("procDefId")%>");
	daiBanDataset.load();
}
...
function query()
{
	...
	<% 
	  ...
	  String subjectFieldTypeQuery;//用于判断查询类型
	  for(int i=0;i<procSubjectQueryDefList.size();i++)
	  {
		  ProcSubjectQueryDef procSubjectQueryDef=(ProcSubjectQueryDef)procSubjectQueryDefList.get(i);
		  if(null == procSubjectQueryDef.getId()){
		  ...
		  subjectFieldTypeQuery=procSubjectQueryDef.getSubjectFieldType();
		  if(null == subjectFieldTypeQuery || "".equals(subjectFieldTypeQuery)){
			subjectFieldTypeQuery = "0";
		  }
	 %>
		 /**
		 ...
		 */
		 ...
		 subjectFieldType = <%=subjectFieldTypeQuery%>;
...
function showState()
{
   ...
   var backUrl="jsp/workflow/tasklist/display/procsubject/querydaibandetailinfo.jsp";
...
{
	var url="jsp/workflow/tasklist/display/procsubject/querydaibanprofileinfo.jsp";
