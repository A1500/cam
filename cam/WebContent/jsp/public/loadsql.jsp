...
<%! 
	...
	public static String excuteFile(File file, String linepara, Statement stmt,
		...
		message.append("<BR><font color=blue>" + res.get("BSPOTHER.READ_FILE") + file.getName() + "</font>");
		try {
			...
			//添加对文件中；的校验
			if(i==0){
				message.append("<BR><font color=red>"+ res.get("BSPOTHER.NOT_SUCCESSFUL")
						+ res.get("BSPOTHER.SURE_SQL_END")  + "</font>");
			}
			is.close();
			is = null;
...
<%
	...
	Statement stmt = null;
	ResultSet set = null;
	out.println("<BR>"+res.get("BSPOTHER.CONECT_DB")+"<BR>");
	...
	if (dbtype.equals("dataSource")) {
		try {// 取默认的数据源对象
			out.println("<BR>"+res.get("BSPOTHER.USE_CONFIG_CONNECT_DB"));
			DataSource dataSource = org.loushang.persistent.jdbc.datasource.DataSourceFactory.defaultFactory
			.getDataSource("dataSource");
			conn = dataSource.getConnection();
	
		} catch (Exception f) {
			message.append(res.get("BSPOTHER.GET_DEFAULT_DATASOURCE_ERROR") + f);
		}
	} else if (dbtype.equals("jndi")) {
		if (jndiType.equals("2"))
			jndiName = "java:comp/env/" + jndiName;
		out.println("<BR>"+res.get("BSPOTHER.CONNECT_TYPE_JNDI")+"jndiName=" + jndiName);
		try {
		...
		try {
			out.println("<BR>"+res.get("BSPOTHER.CONNECT_TYPE_DIRECT_CONNECT")+"url=" + url);
			...
			message.append("<BR>"+res.get("BSPOTHER.DB_DRIVER_ERROR") + e);
			e.printStackTrace();
	...
	if (!message.toString().equals("")) {
		out.println("<BR>"+res.get("BSPOTHER.DB_CONNECT_FAIL")+"<BR>");	
		message.append("<br><A HREF=\"datamain.jsp\">"+res.get("BSPOTHER.BACK")+"</A>");
		out.println(message);
		try {
			if (conn != null)
				conn.close();
...
<head>
<title><%=res.get("BSPOTHER.DEFINE_SQL") %></title>
</head>
<script language="javascript">
	function check(){
		var sql=document.form1.sql.value;
		if(sql==""){
			alert('<%=res.get("BSPOTHER.PLEASE_ENTER_CUSTOM_SQL")%>');
			return false;
			}
		return true;
	}
	...
	function query(){
		...
		if(indexNo != -1) {
			if(indexNo != lastNo) {
				alert('<%=res.get("BSPOTHER.SURE_ONE_SQL")%>');
				return;
			}
		} else {
			alert('<%=res.get("BSPOTHER.SURE_SQL_END")%>');
			return;
		}
		var sqls = sql.split(";");
	...
	function executeSqlFile(){
		...
		var fileType = sqlFile.substring(sqlFile.length-4, sqlFile.length);		if(sqlFile==""){
			alert('<%=res.get("BSPOTHER.SQL_FILE_NULL")%>');
			return false;
		}
		if(fileType!=".sql" && fileType!=".SQL") {
			alert('<%=res.get("BSPOTHER.NOT_SQL_FILE")%>');
			return false;
		}
...
<p align="center"><%=res.get("BSPOTHER.DEFINE_SQL") %></p>
<hr>
...
<table cellpadding=2 cellspacing=0 border=0>
	<tr>
	<td style="border:0"><%=res.get("BSPOTHER.DB_CONNECT_SUCCESSFUL") %></td>
	<td style="border:0"><A HREF="datamain.jsp"><%=res.get("BSPOTHER.RE_CONENCT_NEW_DB") %></A></td>
	</tr>	
	<tr>
	<td style="border:0"><p><%=res.get("BSPOTHER.PLEASE_ENTER_CUSTOM_SQL") %></p></td>
	<td style="border:0"><textarea rows="8" name="sql" cols="89" ><%=sql%></textarea></td>
	<td style="border:0">
	<BUTTON class=btn id=btnSave1 accessKey=S name=btnSave1 type=button  onClick="query()"><%=res.get("BSPOTHER.EXECUTE_QUERY") %></BUTTON>
	<BR>
	<BUTTON class=btn id=btnSave2 accessKey=S name=btnSave2 type=button  onClick="update()"><%=res.get("BSPOTHER.EXECUTE_UPDATE") %></BUTTON>
	<BR>
	<BUTTON class=btn id=btnSave3 accessKey=S name=btnSave3 type=button  onClick="createInsertSql()"><%=res.get("BSPOTHER.GENERATE_INSERT_SQL") %></BUTTON>
	<BUTTON class=btn id=btnSave4 accessKey=S name=btnSave4 type=button  onClick="getInsertSqlFile()"><%=res.get("BSPOTHER.GENERATE_SQL_FILE") %></BUTTON>
	</tr>
</table>
</form>
<form action="loadsql.jsp"  name=form2 method=post enctype="multipart/form-data">
	<table cellpadding=2 cellspacing=0 border=0>
		<tr>
		      <td style="border:0"><%=res.get("BSPOTHER.CHOOSE_SQL_FILE") %></td>
		      <td style="border:0">
		        <input type="file" name="sqlfile" id="sqlfile" size="100" value="" />
		      </td>
		      <td style="border:0">
		        <input type="button" class=btn accessKey=S name="btnSave5" value="<%=res.get("BSPOTHER.EXECUTE_SQL_FILE") %>" onclick="executeSqlFile()">
			  </td>
		</tr>
	</table>
...
<%
	...
	try {
		...
		if(isQuery.equals("update")){
			...
			message.append(res.get("BSPOTHER.EXECUTE_SUCCESSFUL")).append(execCount);
			return;
		}
		...
		if(indexNo != -1) {
			if(indexNo == lastNo) {
				...
				message.append(res.get("BSPOTHER.SURE_ONE_SQL"));
				return;
			}
		} else {
			message.append(res.get("BSPOTHER.SURE_SQL_END"));
			return;
		}
		...
		message.append( "<br>"+res.get("BSPOTHER.IS_ERROR") + e2);
