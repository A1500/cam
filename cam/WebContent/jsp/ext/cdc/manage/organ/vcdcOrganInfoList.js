...
function init(){
	...
	organReportDataSet.setParameter("ORGAN_CODE@=",organCode);
	organReportDataSet.setParameter("REPORT_TYPE@=","N");
...
function query(){
	...
	organReportDataSet.setParameter("REPORT_DATE@=",_$("reportDate"));
	organReportDataSet.setParameter("ORGAN_CODE@=",organCode);
	organReportDataSet.setParameter("REPORT_TYPE@=","N");
	organReportDataSet.setParameter("ORGAN_TYPE@=",'1');
	organReportDataSet.load();	
}
function insert(){
	...
	if(ifInsert){
		...
		var url="jsp/ext/cdc/manage/organ/vcdcOrganInfoEdit.jsp";
		L5.forward(url,'',data);
	}else{
...
function detail(){
	...
	if(organLevel!="SQ"){
		if(records[0].get("ORGANCODE")==organCode){
			...
			data.put("recordId",records[0].get("RECORDID"));
			var url="jsp/ext/cdc/manage/organ/vcdcOrganInfoSum.jsp";
			L5.forward(url,'',data);
		}
	}
	var data=new L5.Map();
	data.put("recordId",records[0].get("recordId"));
	var url="jsp/ext/cdc/manage/organ/vcdcOrganInfoDetail.jsp";
	L5.forward(url,'',data);
}
function report(){
	var grid=L5.getCmp("grid");
