...
function init(){
	...
	fReportDataSet.setParameter("REPORT_TYPE@=","N");
	fReportDataSet.load();
...
function query(){
	...
	fReportDataSet.setParameter("REPORT_DATE@=",_$("reportDate"));
	fReportDataSet.setParameter("ORGAN_CODE@=",organCode);
	fReportDataSet.setParameter("REPORT_TYPE@=","N");
	fReportDataSet.load();	
}
function insert(){
	...
	if(ifInsert){
		...
		var url="jsp/ext/cdc/manage/facilities/vcdcFacilitiesReportEdit.jsp";
		L5.forward(url,'',data);
	}else{
...
function detail(){
	...
	if(organLevel!="SQ"){
		if(records[0].get("ORGANCODE")==organCode){
			...
			data.put("recordId",records[0].get("RECORDID"));
			var url="jsp/ext/cdc/manage/facilities/vcdcFacilitiesReportSum.jsp";
			L5.forward(url,'',data);
		}
	}
	var data=new L5.Map();
	data.put("recordId",records[0].get("recordId"));
	var url="jsp/ext/cdc/manage/facilities/vcdcFacilitiesReportDetail.jsp";
	L5.forward(url,'',data);
}
function report(){
	var grid=L5.getCmp("grid");
