function init(){
	...
	if(method=="insert"){
		serviceReportDataSet.newRecord({"organCode":organCode,"organName":organName,"parentCode":parentCode,"communityNum":"1","reportType":"C"});
...
function deleteService(){
	...
	var delIds = records[0].get("serviceId");
	var serviceInfoDataSet=L5.DatasetMgr.lookup("serviceInfoDataSet");
	var command=new L5.Command("com.inspur.cams.cdc.base.cmd.CdcServiceInfoCmd");
	command.setParameter("delIds",delIds);
	command.execute("delete");
	if(!command.error){
		serviceInfoDataSet.remove(records[0]);
		serviceInfoDataSet.commitChanges();
	}else{
...
function countNum(){
	...
	serviceInfoDataSet.each(function(record,index){
		if(record.get('serviceType')=='1'){
			...
			//获取服务内容的选择项
			var serviceContent = record.get("content");
			if(serviceContent==""){
...
//	}
	...
	serviceInfoDataSet.each(function(record,index){
		if(record.get('serviceType')=='1'){
			...
			if(record.get('serviceArea')==null){
	...
	command.setParameter("serviceInfos",serviceInfoDataSet.getAllChangedRecords());
	if(method=="insert"){
	...
	if (!command.error){
		L5.Msg.alert("提示","保存成功！",function(){
			var url="jsp/ext/cdc/manage/service/cdcServiceInfoList.jsp";
			L5.forward(url);
		});
	}else{
