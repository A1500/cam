...
function init(){
	...
	economyDataSet.setParameter("REPORT_TYPE@=","N");
	economyDataSet.load();
...
function query(){
	var economyDataSet = L5.DatasetMgr.lookup("economyDataSet");
	economyDataSet.setParameter("STATUS@=",_$("status"));
	economyDataSet.setParameter("REPORT_TYPE@=","N");
	economyDataSet.setParameter("REPORT_DATE@=",_$("reportDate"));
	economyDataSet.setParameter("ORGAN_CODE@=",organCode);
	economyDataSet.load();	
}
function insert(){
var insertDataSet = L5.DatasetMgr.lookup("insertDataSet");
	...
	if(ifInsert){
		...
		var url="jsp/ext/cdc/manage/economy/vcdcEconomyInfoEdit.jsp";
		L5.forward(url,'',data);
	}else{
		var data=new L5.Map();
		data.put("method","insert");
...
function detail(){
	...
	var data=new L5.Map();
	data.put("recordId",records[0].get("recordId"));
	var url="jsp/ext/cdc/manage/economy/vcdcEconomyInfoDetail.jsp";
	L5.forward(url,'',data);
}
function report(){
	var grid=L5.getCmp("grid");
...
function sumInsert(){
	var data=new L5.Map();
	data.put("method","insert");
	var url="jsp/ext/cdc/manage/economy/vcdcEconomyInfoSum.jsp";
	L5.forward(url,'',data);
}
function sumRedo(){
	var grid=L5.getCmp("grid");
...
//    dataset.setParameter("dir","desc");
    dataset.load(true);
	L5.dataset2excel(dataset,"/jsp/ext/cdc/manage/economy/vcdcEconomyInfoExcel.jsp?organName="+organName);
}