function init(){
	...
	if(method=="insert"){
		...
		economyDataSet.newRecord({"organCode":organCode,"organName":organName,"parentCode":parentCode,"reportType":"C","communityNum":"1"});
	...
	if(ifNegative<0){
		...
		record.set("ifNegative","0");
			...
			L5.Msg.alert("提示","没有填报历史！",function(){
				economyDataSet.newRecord({"organCode":organCode,"organName":organName,"parentCode":parentCode,"communityNum":"1","reportType":"C"});
			});
		}
	});
...
function save(){
	...
	if (!command.error){
		L5.Msg.alert("提示","保存成功！",function(){
			var url="jsp/ext/cdc/manage/economy/cdcEconomyInfoList.jsp";
			L5.forward(url,'',null);
		});
	}else{
