function init(){
	...
	if(method=="insert"){
		...
		manageReportDataSet.newRecord({"organCode":organCode,"organName":organName,"parentCode":parentCode,"reportType":"C"});
...
function save(){
	...
	if(document.getElementById("ifRule").value==""){
		L5.Msg.alert("提示","是否建立居民自治章程不能为空！");
		return;
	}
	if(document.getElementById("ifPact").value==""){
		L5.Msg.alert("提示","是否建立居民公约不能为空！");
		return;
	}
	var manageDataSet=L5.DatasetMgr.lookup("manageDataSet");
	...
	if (!command.error){ 
			...
			var url="jsp/ext/cdc/manage/manage/cdcManageInfoList.jsp";
			L5.forward(url,'',null);
		});
	}else{
