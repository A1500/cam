...
function init(){
	...
	manageReportDataSet.setParameter("REPORT_TYPE@=","N");
...
function query(){
	var manageReportDataSet = L5.DatasetMgr.lookup("manageReportDataSet");
	manageReportDataSet.setParameter("STATUS@=",_$("status"));
	manageReportDataSet.setParameter("REPORT_TYPE@=","N");
	manageReportDataSet.setParameter("REPORT_DATE@=",_$("reportDate"));
	manageReportDataSet.setParameter("ORGAN_CODE@=",organCode);
	manageReportDataSet.load();	
}
function insert(){
	...
	if(ifInsert){
		...
		var url="jsp/ext/cdc/manage/manage/vcdcManageInfoEdit.jsp";
		L5.forward(url,'',data);
	}else{
...
function detail(){
	...
	var data=new L5.Map();
	data.put("recordId",records[0].get("recordId"));
	var url="jsp/ext/cdc/manage/manage/vcdcManageInfoDetail.jsp";
	L5.forward(url,'',data);
}
function report(){
	var grid=L5.getCmp("grid");
...
function sumInsert(){
	var data=new L5.Map();
	data.put("method","insert");
	var url="jsp/ext/cdc/manage/manage/vcdcManageInfoSum.jsp";
	L5.forward(url,'',data);
}
function sumRedo(){
	var grid=L5.getCmp("grid");
