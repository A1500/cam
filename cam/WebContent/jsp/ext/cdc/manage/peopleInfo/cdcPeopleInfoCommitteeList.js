function init(){
	...
	cdcPeopleInfoDataSet.setParameter("IF_COMMITTEE@=","1");
...
function query(){
	...
	cdcPeopleInfoDataSet.setParameter("DUTY@=",getParam("query_duty"));
	cdcPeopleInfoDataSet.load();
}
...
function insert(){
	...
	var url="jsp/ext/cdc/manage/peopleInfo/cdcPeopleInfoCommitteeEdit.jsp";
	L5.forward(url,'',data);
}
function update(){
...
function detail(){
	...
	var data=new L5.Map();
	data.put("peopleId",records[0].get("peopleId"));
	var url="jsp/ext/cdc/manage/peopleInfo/cdcPeopleInfoCommitteeDetai.jsp";
	L5.forward(url,'',data);
}
function del(){
	var grid=L5.getCmp("grid");
