function init(){
	...
	cdcPeopleInfoDataSet.setParameter("IF_COMMUNITY_WORKER@=","1");
...
function query(){
	...
	cdcPeopleInfoDataSet.setParameter("COMMUNITY_WORKER_TYPE@=",getParam("query_job"));
	cdcPeopleInfoDataSet.load();
}
...
function insert(){
	...
	var url="jsp/ext/cdc/manage/peopleInfo/cdcPeopleInfoWorkerEdit.jsp";
	L5.forward(url,'',data);
}
function update(){
...
function detail(){
	...
	var data=new L5.Map();
	data.put("peopleId",records[0].get("peopleId"));
	var url="jsp/ext/cdc/manage/peopleInfo/cdcPeopleInfoWorkerDetai.jsp";
	L5.forward(url,'',data);
}
function del(){
	var grid=L5.getCmp("grid");
