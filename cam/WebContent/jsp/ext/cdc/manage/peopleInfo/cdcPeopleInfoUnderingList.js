function init(){
	...
	cdcPeopleInfoDataSet.setParameter("IF_COMMITTEE_UNDERLING@=","1");
...
function query(){
	...
	cdcPeopleInfoDataSet.setParameter("JOB_TYPE@=",getParam("query_job"));
	cdcPeopleInfoDataSet.load();
}
...
function insert(){
	...
	var url="jsp/ext/cdc/manage/peopleInfo/cdcPeopleInfoUnderingEdit.jsp";
	L5.forward(url,'',data);
}
function update(){
...
function detail(){
	...
	var data=new L5.Map();
	data.put("peopleId",records[0].get("peopleId"));
	var url="jsp/ext/cdc/manage/peopleInfo/cdcPeopleInfoUnderingDetai.jsp";
	L5.forward(url,'',data);
}
function del(){
	var grid=L5.getCmp("grid");
