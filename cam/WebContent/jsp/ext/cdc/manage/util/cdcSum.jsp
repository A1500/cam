...
function regMethod(){
	...
	listDataSet.on('load',function(){
		cityDataSet.setParameter("organId",parentCodeSearch);
		cityDataSet.load();
	});
	cityDataSet.on('load',function(){
		cityDataSet.each(function(record,index){
			var areaCode="";
			if(organCode.substr("4")=="00000000"){//市级
				areaCode = record.get("userId");
			}else if(organCode.substr("6")=="000000"){//区县
				areaCode = record.get("areaCode").substring(0,9)+"000";
			}else{
				areaCode = record.get("areaCode");
			}
			if(listDataSet.find("organCode",areaCode,0)==-1){
				//有未上报下级单位
				if(parentCodeSearch==organCode){
					reportFlag=false;
				}
				listDataSet.newRecord({"organCode":record.get("userId"),"organName":record.get("userName"),"parentCode":record.get("organId"),"status":"0"});
			}
		});
	});
}
...
function getUp(){
	...
	if(len!=-1&&len!=0){
		...
		organCode= organPath.substr(organPath.lastIndexOf("#")+1,organPath.length);
