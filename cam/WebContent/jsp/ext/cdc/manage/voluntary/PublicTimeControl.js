...
//计算两个时间的间隔
function getTimePoorMin(startTime,endTime){
	//把时间按"_"切成数组
	var sDateArr = startTime.split(" ");
    var startArr=sDateArr[0].split("-");
    var startArr2=sDateArr[1].split(":");
    
	var eDateArr = endTime.split(" ");
    var endArr=eDateArr[0].split("-");
    var endArr2=eDateArr[1].split(":");
	//转为毫秒数
    var btime=new Date(startArr[0],startArr[1]-1,startArr[2],startArr2[0],startArr2[1]-1);
    var etime=new Date(endArr[0],endArr[1]-1,endArr[2],endArr2[0],endArr2[1]-1);
    //计算相差分
    return (etime.getTime()-btime.getTime())/(1000*60);
}
...
// num 相隔天数
// flag 'SUB'向前 'ADD'向后
function getStartDate(date, num, flag) {
	if (flag == 'SUB') {
