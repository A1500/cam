...
function init(){
	...
	naturalReportDataSet.setParameter("REPORT_TYPE@=","N");
...
function query(){
	var naturalReportDataSet = L5.DatasetMgr.lookup("naturalReportDataSet");
	naturalReportDataSet.setParameter("STATUS@=",_$("status"));
	naturalReportDataSet.setParameter("REPORT_DATE@=",_$("reportDate"));
	naturalReportDataSet.setParameter("REPORT_TYPE@=","N");
	naturalReportDataSet.setParameter("ORGAN_CODE@=",organCode);
	naturalReportDataSet.load();	
}
function insert(){
	...
	if(ifInsert){
		...
		var url="jsp/ext/cdc/manage/natural/vcdcNaturalInfoEdit.jsp";
		L5.forward(url,'',data);
	}else{
		var data=new L5.Map();
		data.put("method","insert");
...
function detail(){
	...
	var data=new L5.Map();
	data.put("recordId",records[0].get("recordId"));
	var url="jsp/ext/cdc/manage/natural/vcdcNatualInfoDetail.jsp";
	L5.forward(url,'',data);
}
function report(){
	var grid=L5.getCmp("grid");
	...
	L5.MessageBox.confirm("提示", "是否上报该数据？",function(sta){
		if(sta=="yes"){
			...
			command.setParameter("reportRrecod", records[0]);
...
function sumInsert(){
	var data=new L5.Map();
	data.put("method","insert");
	var url="jsp/ext/cdc/manage/natural/vcdcNatualInfoSum.jsp";
	L5.forward(url,'',data);
}
function sumRedo(){
	var grid=L5.getCmp("grid");
	...
	L5.MessageBox.confirm("提示", "重新汇总将删除原汇总数据，是否继续？",function(sta){
		if(sta=="yes"){
			...
			if (!command.error){
				...
				L5.Msg.alert("提示","删除原汇总数据出错！"+command.error);
			}
		}
	});
}
...
//    dataset.setParameter("dir","desc");
    dataset.load(true);
	L5.dataset2excel(dataset,"/jsp/ext/cdc/manage/natural/cdcNatualInfoExcel.jsp?organName="+organName);
}