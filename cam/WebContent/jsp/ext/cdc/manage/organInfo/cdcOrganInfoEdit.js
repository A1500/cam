function init(){
	...
	if(method=="insert"){
		cdcOrganInfoDataSet.newRecord({"organCode":organCode,"organName":organName,"organType":"1"});
	}else{
...
function save(){
	...
	var record=cdcOrganInfoDataSet.getCurrent();
		...
		L5.Msg.alert("提示","社会组织名称不能为空！");
		return false;
	}
	if(record.get("organKind")==''){
		L5.Msg.alert("提示","社会组织性质不能为空！");
		return false;
	}
	if(record.get("legalPeople")==''){
		L5.Msg.alert("提示","负责人姓名不能为空！");
		return false;
	}
	...
	if(document.getElementById("memberNum").value==''){
		L5.Msg.alert("提示","成员人数不能为空！");
		return false;
	}
	if(document.getElementById("officialNum").value==''){
		L5.Msg.alert("提示","办公场所数量不能为空！");
		return false;
	}
	...
	if(record.get("businessScope")==''){
		L5.Msg.alert("提示","业务范围不能为空！");
		return false;
	}
