...
<body>
<model:datasets>
	<!--经济核对_公积金填报信息的dataset-->
	<model:dataset id="externalHousingFundDataset"
		cmd="com.inspur.cams.drel.esurey.cmd.SamEsureyHousingFundQueryCmd"
		global="true" pageSize="10" method="queryExtHousingFund">
...
<next:GridPanel id="editGridPanel" name="editGridPanel" width="100%"
	stripeRows="true" height="100%" dataset="externalHousingFundDataset" 
	notSelectFirstRow="true" title="公积金填报信息的列表">
	
	<next:Columns>
		<next:RowNumberColumn width="30" />
		
		<next:Column id="name" header="姓名" field="PNAME" width="10%">
		...
		<next:Column id="monthPay" header="月缴纳公积金金额(元)" field="MONTH_PAY" align="right" width="19%" renderer="formatNum" >
			<next:TextField />
		</next:Column>
		<next:Column id="balance" header="公积金余额(元)" field="BALANCE" align="right" width="13%" renderer="formatNum">
			<next:TextField allowBlank="false" />
		</next:Column>
		<next:Column id="collectNum" header="历史提取金额合计(元)" field="COLLECT_NUM" align="right" width="19%" renderer="formatNum">
			<next:TextField allowBlank="false" />
		</next:Column>
	...
	<next:BottomBar>
		<next:PagingToolBar dataset="externalHousingFundDataset" />
	</next:BottomBar>
</next:GridPanel>
