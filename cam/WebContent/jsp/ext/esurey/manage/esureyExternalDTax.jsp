...
<model:datasets>
	<!--经济核对_地税填报信息的dataset-->
	<model:dataset id="externalDTaxDataset"
		cmd="com.inspur.cams.drel.esurey.cmd.SamEsureyTaxQueryCmd"
		global="true" pageSize="10" method="queryDExtTax">
		
	</model:dataset>
	<!-- 纳税类型dataset-->
	<model:dataset id="taxesType" enumName="TAXES.TYPE"
...
<next:GridPanel id="editGridPanel" name="editGridPanel" width="100%"
	stripeRows="true" height="100%" dataset="externalDTaxDataset" 
	notSelectFirstRow="true" title="地税填报信息的列表">
	...
	<next:Columns>
		...
		<next:Column id="taxesPeople" header="纳税人姓名" field="TAXES_PEOPLE" hidden="true" width="10%">
			<next:TextField />
		</next:Column>
		<next:Column id="taxesOrg" header="纳税企业名称" field="TAXES_ORG" width="20%">
			<next:TextField allowBlank="false" />
		</next:Column>
		<next:Column id="turnoverLatestYear" header="最近一年营业额（元）" field="TURNOVER_LATEST_YEAR" align="right" width="17%" renderer="formatNum">
			<next:TextField allowBlank="false" />
		</next:Column>
		<next:Column id="taxesLatestYear" header="最近一年纳税额（元）" field="TAXES_LATEST_YEAR" align="right" width="17%" renderer="formatNum">
			<next:TextField allowBlank="false" />
		</next:Column>
	...
	<next:BottomBar>
		<next:PagingToolBar dataset="externalDTaxDataset" />
	</next:BottomBar>
</next:GridPanel>
