...
<model:datasets>
	...
	<!--社保信息dataset-->
	<model:dataset id="socialDataset"
	...
	<model:dataset id="checkFlgDataset" enumName="CHECK.FLG"
		autoLoad="true" global="true"></model:dataset>
	<!-- 工资类别 -->
	<model:dataset id="salaryTypeDataset" enumName="SALARY_TYPE"
		autoLoad="true" global="true"></model:dataset>
...
<next:GridPanel id="editGridPanel" name="editGridPanel" width="100%"
	...
	<next:Columns>
		...
		<next:Column  header="身份证件号码" field="idCard"  width="20%">
			<next:TextField allowBlank="false" />
		</next:Column>
		...
		<next:Column id="checkFlg" header="状态" dataset="checkFlgDataset1"
			field="checkFlg" width="15%">
			<next:TextField allowBlank="false" />
		</next:Column>
...
<next:Window id="socialWin" height="100" width="550" title="社保信息录入" closeAction="hide">
		<next:TopBar>
			<next:ToolBarItem symbol="->"></next:ToolBarItem>
			<next:ToolBarItem text="提交" iconCls="detail"
				handler="Evt_butcheckin_onclick"></next:ToolBarItem>
		...
		<next:Html>
			<form class="L5form" dataset="socialDataset">
			<table width="100%">
				<tr>
					<td class="FieldLabel" width="15%">工资类别：</td>
					<td class="FieldInput" width="35%"><select  name="工资类别" field="salaryType" style="width:100%">
					<option dataset="salaryTypeDataset"></option>
					</select></td>
					<td class="FieldLabel" width="15%">金额(元)：</td>
					<td class="FieldInput" width="35%"><input type="text" name="金额（元）:" maxlength="16" style="width:100%" align="right" onblur="checkMoney(this.id)"
						field="salary" /></td>
				</tr>
			</table>
			</form>
