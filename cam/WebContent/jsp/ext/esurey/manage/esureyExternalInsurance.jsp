...
<model:datasets>
	<!--经济核对_保监填报信息的dataset-->
	<model:dataset id="externalInsuranceDataset"
		cmd="com.inspur.cams.drel.esurey.cmd.SamEsureyInsuranceQueryCmd"
		global="true" pageSize="10" method="queryExtInsurance">
		
	</model:dataset>
	<model:dataset id="beneType" enumName="BENE_TYPE"
...
<next:GridPanel id="editGridPanel" name="editGridPanel" width="100%"
	stripeRows="true" height="100%" dataset="externalInsuranceDataset" 
	notSelectFirstRow="true" title="保监填报信息的列表">
	...
	<next:Columns>
		...
		<next:Column id="benefitPeople" header="保险受益人" field="BENEFIT_PEOPLE" width="10%">
			<next:TextField />
		</next:Column>

		<next:Column id="benefitDate" header="受益时间" field="BENEFIT_DATE" width="10%">
			<next:TextField />
		</next:Column>
		<next:Column id="benefitType" header="保险受益种类" dataset="beneType" field="BENEFIT_TYPE" width="15%">
			<next:TextField allowBlank="false" />
		</next:Column>
		<next:Column id="benefitNum" header="保险受益金额(元)" field="BENEFIT_NUM" align="right" width="17%" renderer="formatNum">
			<next:TextField allowBlank="false" />
		</next:Column>
	...
	<next:BottomBar>
		<next:PagingToolBar dataset="externalInsuranceDataset" />
	</next:BottomBar>
</next:GridPanel>
