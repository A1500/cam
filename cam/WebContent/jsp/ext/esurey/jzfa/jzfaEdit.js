...
function init() {
	...
	sjtjDataset.setParameter("organType",organType);
	sjtjDataset.load();
...
function query() {
    ...
    sjtjDataset.setParameter("REG_TIMES",lrrqQ);
    
    if (lrrqZ == "") lrrqZ = undefined;
    ds.setParameter("REG_TIMET", lrrqZ);
    sjtjDataset.setParameter("REG_TIMET",lrrqZ);
...
function detail(){
	...
	var planbegin=rec.get("PLAN_BEGIN");
	var planend=rec.get("PLAN_END");
	...
	var url='jsp/ext/esurey/jzfa/jzfamxQuery.jsp';
	var text = '救助方案明细';
		...
		function(sta){
			if(sta=="yes"){
				...
				for(var i=0;i<selected.length;i++){
					jzfaids[i]=selected[i].get("PLAN_ID");
				}
				var command = new L5.Command("com.inspur.sdmz.jzfa.cmd.PlanCmd"); 
