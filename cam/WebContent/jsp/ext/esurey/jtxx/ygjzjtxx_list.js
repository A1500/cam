function init() {
    ...
    var queryFlag = document.getElementById("queryFlag").value;
     ...
     ds.setParameter("organType",organType);
     jtxxNumDs.setParameter("QUERYFLAG",queryFlag);
     jtxxNumDs.setParameter("organCode",organArea);
     jtxxNumDs.setParameter("organType",organType);
    //ds.setParameter("YGJZ_JTXX.JTDZ@!=", '1');//是否更新过救助指数
    ds.load();
    //jtxxNumDs.load();
    L5.QuickTips.init();
};
...
function query() {
    ...
    ds.setParameter("BASEINFO_FAMILY.CHECK_FLAG@=", shbz);
    ds.setParameter("BASEINFO_FAMILY.REG_ORG@=", lrdwId);
    if (lrrqQ == "") lrrqQ = undefined;
    ds.setParameter("BASEINFO_FAMILY.REG_TIME@>=", lrrqQ);
    if(jzlxdm=="") jzlxdm=undefined;
	dataset.setParameter("ASSISTANCE_TYPE",jzlxdm);
    if (lrrqZ == "") lrrqZ = undefined;
    ds.setParameter("BASEINFO_FAMILY.REG_TIME@<=", lrrqZ);
...
function insert() {
    ...
    var url = 'jsp/ext/esurey/jtxx/ygjzjtxx_insert.jsp';
...
function jtxxDetail( jtid,photoid) {
    ...
    var url = 'jsp/ext/esurey/jtxx/ygjzjtxx_forDetail.jsp';
    var text = '审核阳光救助_家庭信息';
    L5.forward(url, text, data);
}
function jtxxExamDetail( jtid ,photoid) {
    ...
    data.put("dataBean", jtid);
    data.put("photoId", photoid);
    var url = 'jsp/ext/esurey/jtxx/ygjzjtxx_exam.jsp';
    var text = '审核阳光救助_家庭信息';
    L5.forward(url, text, data);
}
function jtxxStateDetail( jtid ,photoid) {
    var data = new L5.Map();
    data.put("method", "STATE_EXAM");
    data.put("dataBean", jtid);
    data.put("photoId", photoid);
    var url = 'jsp/ext/esurey/jtxx/ygjzjtxx_exam_detail.jsp';
    var text = '审核阳光救助_家庭信息';
    L5.forward(url, text, data);
}
...
function tochange(){
	...
	{	
    	...
    	data.put("dataBean", selected[0].get('FAMILY_ID'));
    	data.put("photoId", selected[0].get('PHOTO_ID'));
    	var url = 'jsp/ext/esurey/jtxx/ygjzjtxx_change.jsp';
   		var text = '修改阳光救助_家庭信息';
    	L5.forward(url, text, data);
    	}
	}else {
