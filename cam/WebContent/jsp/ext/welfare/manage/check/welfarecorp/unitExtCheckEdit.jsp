...
<script language="javascript">
	...
	function init(){
		...
		if(unitStatusDataSet1.getCount()==0){
			alert("正在进行资格认定申报，暂不能进行网上年检...");
			window.history.go("-1");
		}else{
		...
		if(method=="INSERT"){
			...
			if(wealUnitInfoDataSet.getCount()==0){
				var wealUnitInfoDataSet = L5.DatasetMgr.lookup("wealUnitInfoDataSet");
				wealUnitInfoDataSet.newRecord({"taskCode":taskCode,"applyType":"2"});
			}
			backFill();
		}else{
			var wealUnitInfoDataSet = L5.DatasetMgr.lookup("wealUnitInfoDataSet");
	...
	function save(){
		...
		var infoValidate = wealUnitInfoDataSet.isValidate();
		if(infoValidate != true){
			L5.Msg.alert("提示",infoValidate);
			return "break";
		}
		var unitStatusRecord=wealUnitStatusDataSet.getCurrent();
		
		//维护一下新的员工信息
		var command1=new L5.Command("com.inspur.cams.welfare.base.cmd.WealWorkerCommand");
		command1.setParameter("unitId",unitId);
		command1.setParameter("taskCodeOld",taskCodeOld);
		command1.setParameter("taskCode",taskCode);
		command1.execute("saveNewWorker");
		
		var command = new L5.Command("com.inspur.cams.welfare.welfarecorp.apply.cmd.UnitApplyCmd");
		command.setParameter("unitStatusRecord", unitStatusRecord);
	...
	function backFill(){
		...
		var wealUnitStatusDataSet = L5.DatasetMgr.lookup("wealUnitStatusDataSet");
		wealUnitStatusDataSet.setParameter("SERIAL_NUM@=",serialNum);
			if(serialNum!="")wealUnitStatusDataSet.load();
			wealUnitStatusDataSet.on('load',function(ds){
				if(ds.getCount()>0){
					applyYear = ds.getAt(0).get("applyYear");
					//serialNum = ds.getAt(0).get("serialNum");
					taskCodeOld=ds.getAt(0).get("taskCode");//维护新的员工信息使用
					unitId=ds.getAt(0).get("unitId");//维护新的员工信息使用
					...
					var wealUnitInfoDataSet = L5.DatasetMgr.lookup("wealUnitInfoDataSet");
					wealUnitInfoDataSet.getCurrent().set("companyName",ds.getAt(0).get("companyName"));
					wealUnitInfoDataSet.getCurrent().set("legalPeople",ds.getAt(0).get("legalPeople"));
					wealUnitInfoDataSet.getCurrent().set("regDate",regDate);
...
<model:datasets>
	<!-- 状态表 -->
	<model:dataset id="wealUnitStatusDataSet"
	...
	<!-- 业务信息表 -->
	<model:dataset id="wealUnitInfoDataSet1"
		cmd="com.inspur.cams.welfare.base.cmd.WealUnitInfoQueryCommand">
		<model:record
			fromBean="com.inspur.cams.welfare.base.data.WealUnitInfo">
		</model:record>
					...
					<TR>
						...
						<TABLE cellSpacing=0 cellPadding=0 width='100%' border=0>
							<TBODY>
								...
								<TR>
									<TD width='668' height=34 align=middle class=title1><label id="checkYear" style="font-size:30px;font-weight:400"></label>&nbsp;&nbsp;<STRONG>年度检查表
									</STRONG></TD>
								</TR>
							</TBODY>
						</TABLE>
