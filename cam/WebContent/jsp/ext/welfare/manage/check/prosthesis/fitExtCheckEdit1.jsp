...
<%
	...
	String serialNum="370000-2013-001";
	//获取当前日期
...
<script language="javascript">
	...
	function init(){
			...
			if(fitStatusDataSet.getCount()==0){
				L5.Msg.alert("提示","正在进行资格认定申报，暂不能进行网上年检...");
				return;
			}else{
	...
	function backFill(){
		...
		wealFitStatusDataSet.setParameter("SERIAL_NUM@=",serialNum);
			...
			wealFitStatusDataSet.on('load',function(ds){
				if(ds.getCount()>0){
					var applyYear = ds.getAt(0).get("applyYear");
					...
					document.getElementById("applyYear").innerHTML=ds.getAt(0).get("nextCheck");
						...
						<TABLE cellSpacing=0 cellPadding=0 width='100%' border=0>
							<TBODY>
								...
								<TR>
									...
									<label id='applyYear' field="applyYear"  style='border-bottom:1px solid;font-size: 25.0pt; font-family: 宋体;'></label>&nbsp;年度检查表
									</STRONG></TD>
								</TR>
								<TR>
									<TD height=50></TD>
