...
<script type="text/javascript" src="js/prosthesisGoto.js"></script>
...
<script language="javascript">
	...
	function save(){
		var wealFitInfoDataSet = L5.DatasetMgr.lookup("wealFitInfoDataSet");
		if(method=="INSERT"){
		wealFitInfoDataSet.set("companyName",document.getElementById("companyName").value);
		wealFitInfoDataSet.set("legalPeople",document.getElementById("legalPeople").value);
		wealFitInfoDataSet.set("regDate",document.getElementById("regDate").value);
		}
	...
	function backFill(){
		if(method="INSERT"){
			var wealFitInfoDataSetS = L5.DatasetMgr.lookup("wealFitInfoDataSetS");
			wealFitInfoDataSetS.setParameter("SERIAL_NUM@=",document.getElementById("serialNum").innerHTML);
				wealFitInfoDataSetS.load();
				wealFitInfoDataSetS.on('load',function(ds){
					if(ds.getCount()>0){
					...
					document.getElementById("regDate").value=ds.getAt(0).get("regDate");
					}
				});
		}else{
			var wealFitInfoDataSet = L5.DatasetMgr.lookup("wealFitInfoDataSet");
			wealFitInfoDataSet.setParameter("SERIAL_NUM@=",document.getElementById("serialNum").innerHTML);
				wealFitInfoDataSet.load();
		}
	}
	...
	//第一页返回按钮
	function backPageD(){
		var url='jsp/ext/welfare/manage/check/prosthesis/prosthesischecklist.jsp';
		L5.forward(url);
	}
...
<model:datasets>
	...
	<model:dataset id="wealFitInfoDataSetS" cmd="com.inspur.cams.welfare.base.cmd.WealFitInfoQueryCommand" >
		<model:record fromBean="com.inspur.cams.welfare.base.data.WealFitInfo">
		</model:record>
	</model:dataset>
	<!-- 设施信息表 -->
...
<div id="float" >
	...
	<% if(!"DETAIL".equals(method)){ //说明是补齐补正环节%>
					...
					<TR>
						...
						<TABLE cellSpacing=0 cellPadding=0 width='100%' border=0>
							<TBODY>
								...
								<TR>
									<TD width='668' height=34 align=middle class=title1><STRONG>
									<label field="applyYear" id='applyYear' style='border-bottom:1px solid;font-size:25.0pt; font-family: 宋体;'></label>&nbsp;年度检查表
									</STRONG></TD>
								</TR>
								<TR>
									<TD height=50></TD>
