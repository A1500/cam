...
<script language="javascript">
	...
	function init(){
		...
		wealUnitStatusDataSet.setParameter("COMPANY_NAME@=", username);
		wealUnitStatusDataSet.load(true);
		if(wealUnitStatusDataSet.getCount()!=0){
			alert("已完成资格认定申报，暂不能进行网上资格认定申报...");
			window.history.go("-1");
		}
		if(method=="INSERT"){
			...
			wealUnitInfoDataSet.on('load',function(ds){
				if(ds.getCount()==0){
					var wealUnitInfoDataSet = L5.DatasetMgr.lookup("wealUnitInfoDataSet");
					wealUnitInfoDataSet.newRecord({"taskCode":taskCode,"applyType":"1","companyName":username});
				}
			});
		}else{
			var wealUnitInfoDataSet = L5.DatasetMgr.lookup("wealUnitInfoDataSet");
	...
	function save(){
		...
		command.setParameter("dataSource","ONLINE");//外网保存
...
<div id="float"><input type="button" value="下一页"
	...
	onclick="nextPageW('2')" /> <input type="button" value="保存"
					...
					<TR>
						...
						<TABLE cellSpacing=0 cellPadding=0 width='100%' border=0>
							<TBODY>
								...
								<TR>
									<TD width='668' height=34 align=middle class=title1><STRONG>资格认定申请表
									</STRONG></TD>
								</TR>
							</TBODY>
						</TABLE>
