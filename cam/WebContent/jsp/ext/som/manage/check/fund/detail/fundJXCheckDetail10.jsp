			...
			for(i=1;i<=somJxOrganFundsDataSet.getCount();i++){
				...
				for(j=1;j<=somJxOrganFundsDataSet.getCount();j++){
					if(somJxOrganFundsDataSet.getAt(j-1).get("seqNum") == i){
						...
						if(somJxOrganFundsDataSet.getAt(j-1).get("ifBuildAdministration")=="0"){
							ifBuildAdministration="否";
						}
						if(somJxOrganFundsDataSet.getAt(j-1).get("ifBuildAdministration")=="1"){
						...
						var accountType = "";
						if(somJxOrganFundsDataSet.getAt(j-1).get("accountType")=="1"){
							accountType="基本账户";
						}
						if(somJxOrganFundsDataSet.getAt(j-1).get("accountType")=="2"){
							accountType="一般账户";
						}
						tab.rows[rowLength-1].cells[4].getElementsByTagName("label")[0].innerHTML=accountType;
						tab.rows[rowLength-1].cells[5].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet.getAt(j-1).get("openAccount");
					}
				}
			}
		}
