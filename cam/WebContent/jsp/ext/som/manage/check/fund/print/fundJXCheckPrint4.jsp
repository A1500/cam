...
//初始化
function init4(){
	var somJxFundPeopleDataSet4 = L5.DatasetMgr.lookup("somJxFundPeopleDataSet4");
	somJxFundPeopleDataSet4.setParameter("TASK_CODE@=", taskCode);
	somJxFundPeopleDataSet4.setParameter("PEOPLE_TYPE@in","1,2");
	somJxFundPeopleDataSet4.load();
	somJxFundPeopleDataSet4.on("load",function(ds){
		if(ds.getCount()==0){
			for(i=0;i<26;i++){
				addTr4();
			}
				...
				for(j=0;j<ds.getCount();j++){
					...
					if( record.get('seqNum') == i ){
						if(record.get("peopleType")=="1"){
							++lishiCount;
							var tab1 = document.getElementById("activities4");
							addTr4();
							...
							tab1.rows[rowLength-1].cells[2].getElementsByTagName("label")[0].innerHTML=sex;
							tab1.rows[rowLength-1].cells[3].getElementsByTagName("label")[0].innerHTML=record.get("birthday");
							var dutyName = "";
							if(record.get("sorgDuty")=="00001"){
								dutyName = "理事长";
							}
							if(record.get("sorgDuty")=="00003"){
								dutyName = "副理事长";
							}
							if(record.get("sorgDuty")=="00004"){
								dutyName = "秘书长";
							}
							if(record.get("sorgDuty")=="00005"){
								dutyName = "理事";
							}
							tab1.rows[rowLength-1].cells[4].getElementsByTagName("label")[0].innerHTML=dutyName;
						...
						if(record.get("peopleType")=="2"){
							...
							tab2.rows[rowLength2-1].cells[2].getElementsByTagName("label")[0].innerHTML=sex;
			...
			for(i=0;i<26-lishiCount;i++){
				addTr4();
			}
			for(i=0;i<8-jianshiCount;i++){
				addJianTr4();
			}
		}
		PROGRESS_BAR++;
...
//增加一行方法
function addTr4(){
	 var tab=document.getElementById("activities4");
    //添加行
...
<model:datasets>
	<model:dataset id="somJxFundPeopleDataSet4" cmd="com.inspur.cams.sorg.jxcheck.somjxfundpeople.cmd.SomJxFundPeopleQueryCommand" global="true" method="queryNoPage">
...
<TABLE height=436 cellSpacing=0 cellPadding=0 width=700 align=center
	...
	<TBODY>
		...
		<TR>
			<TD vAlign=top>
			<TABLE class=unnamed1 cellSpacing=0 cellPadding=0 width='100%' id="activities4"
				border=0>
				<TBODY>
					<TR align=middle>
			...
			<TABLE class=font12size1 cellSpacing=0 cellPadding=0 width='100%' id='activitiesJian'
				...
				<TBODY>
					<TR align=middle>
						...
						<TD class=font12size2 width='7%'>性别</TD>
						<TD class=font12size2 width='12%'>出生日期</TD>
