...
function init10(){
	...
	somJxOrganFundsDataSet10.setParameter("TASK_CODE", taskCode);
	somJxOrganFundsDataSet10.setParameter("ORGAN_TYPE","3");
	somJxOrganFundsDataSet10.load();
	somJxOrganFundsDataSet10.on('load',function (){
		if(somJxOrganFundsDataSet10.getCount() == 0){
			for(var i = 0;i<18;i++){
				addFTr10();
			}
		}else{
			var count = 0;
			var tab = document.getElementById("activitiesF10");
			for(i=1;i<=somJxOrganFundsDataSet10.getCount();i++){
				++count;
				addFTr10();
				var rowLength = tab.rows.length;
				for(j=1;j<=somJxOrganFundsDataSet10.getCount();j++){
					if(somJxOrganFundsDataSet10.getAt(j-1).get("seqNum") == i){
						tab.rows[rowLength-1].cells[1].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet10.getAt(j-1).get("organName");
						var ifBuildAdministration = somJxOrganFundsDataSet10.getAt(j-1).get("ifBuildAdministration");
						if(ifBuildAdministration == '0'){
							tab.rows[rowLength-1].cells[2].getElementsByTagName("label")[0].innerHTML="否";
						}
						if(ifBuildAdministration == '1'){
							tab.rows[rowLength-1].cells[2].getElementsByTagName("label")[0].innerHTML="是";
						}
						tab.rows[rowLength-1].cells[3].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet10.getAt(j-1).get("organDate");
						var accountType = somJxOrganFundsDataSet10.getAt(j-1).get("accountType");
						if(accountType == '0'){
							tab.rows[rowLength-1].cells[4].getElementsByTagName("label")[0].innerHTML="否";
						}
						if(accountType == '1'){
							tab.rows[rowLength-1].cells[4].getElementsByTagName("label")[0].innerHTML="基本账户";
						}
						if(accountType == '2'){
							tab.rows[rowLength-1].cells[4].getElementsByTagName("label")[0].innerHTML="一般账户";
						}
						tab.rows[rowLength-1].cells[5].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet10.getAt(j-1).get("openAccount");
					}
				}
			}
...
function save(){
	somJxOrganFundsDataSet10.removeAll();
	var tab1 = document.getElementById("activitiesF10");
	for(i=1;i<tab1.rows.length;i++){
		var record = somJxOrganFundsDataSet10.newRecord();
	...
	command.setParameter("somJxOrganFundsDataSet10",somJxOrganFundsDataSet10.getAllRecords());
...
<model:datasets>
	<model:dataset id="somJxOrganFundsDataSet10" cmd="com.inspur.cams.sorg.jxcheck.somjxorganfunds.cmd.SomJxOrganFundsQueryCommand" method="queryNoPage" global="true" sortField="SEQ_NUM">
...
<TABLE cellSpacing=0 cellPadding=0 width=700 align=center border=0>
	<TBODY>
		...
		<TR>
			<TD height=14>
			<TABLE class=f14pt-b1 cellSpacing=0 cellPadding=0 width='99%' id="activitiesF10"
