...
//初始化
function init14(){
	somCheckDataSet14.setParameter("TASK_CODE@=", taskCode);
	somCheckDataSet14.load();
	somCheckDataSet14.on("load",function(){
		if(taskCode == 'null'){
			somJxContributeDataSet14.newRecord();
			somJxCheckExpenditureDataSet14.newRecord();
			PROGRESS_BAR=PROGRESS_BAR+2;
			return;
		}
		var sorgKind=somCheckDataSet14.get("sorgKind");
		if(sorgKind!="2"){
			somJxContributeDataSet14.newRecord();
			somJxCheckExpenditureDataSet14.newRecord();
			PROGRESS_BAR=PROGRESS_BAR+2;
		}else{
			somJxContributeDataSet14.setParameter("TASK_CODE@=", taskCode);
			somJxContributeDataSet14.setParameter("IF_OPEN@=", "2");
			somJxContributeDataSet14.load();
			somJxContributeDataSet14.on("load",function(ds){
				if(ds.getCount()==0){
					somJxContributeDataSet14.newRecord({"taskCode":taskCode,"ifOpen":"2"});
				}
				setSumMoney_14();
				PROGRESS_BAR++;
			});
			somJxCheckExpenditureDataSet14.setParameter("TASK_CODE@=", taskCode);
			somJxCheckExpenditureDataSet14.load();
			somJxCheckExpenditureDataSet14.on("load",function(ds){
				if(ds.getCount()==0){
					somJxCheckExpenditureDataSet14.newRecord({"taskCode":taskCode});
				}
				setPerMoney_14();
				PROGRESS_BAR++;
			});
...
<model:datasets>
<model:dataset id="somJxContributeDataSet14" cmd="com.inspur.cams.sorg.jxcheck.somjxcontribute.cmd.SomJxContributeQueryCommand" global="true">
	<model:record fromBean="com.inspur.cams.sorg.jxcheck.somjxcontribute.dao.SomJxContribute"></model:record>
</model:dataset>
<model:dataset id="somJxCheckExpenditureDataSet14" cmd="com.inspur.cams.sorg.jxcheck.somjxcheckexpenditure.cmd.SomJxCheckExpenditureQueryCommand" global="true">
	<model:record fromBean="com.inspur.cams.sorg.jxcheck.somjxcheckexpenditure.dao.SomJxCheckExpenditure"></model:record>
</model:dataset>
	<model:dataset id="somCheckDataSet14" cmd="com.inspur.cams.sorg.jxcheck.somjxcheck.cmd.SomJxCheckQueryCommand" global="true">
...
<TABLE height=760 cellSpacing=0 cellPadding=0 width=700 align=center
	border=0>
	<TBODY>
		<TR>
		...
		<TR>
			...
			<TABLE height=645 cellSpacing=0 cellPadding=0 width=700 align=center
				border=0>
				<TBODY>
					<TR vAlign=top>
					...
					<TR>
						<TD height='10' vAlign=bottom align=right>单位：	<span style="color:red">人民币元</span></TD>
					</TR>
					<TR>
						<TD align=middle height=25 valign='top'>
						<form method="post" dataset="somJxContributeDataSet14" onsubmit="return false">
					...
					<TR>
						...
						<form method="post" dataset="somJxCheckExpenditureDataSet14" onsubmit="return false">
						<TABLE cellSpacing=0 cellPadding=0 width='100%' align=center
							...
							<TBODY>
								...
								<TR>
									<TD align=right height=22>单位：	<span style="color:red">人民币元</span></TD>
								</TR>
								<TR>
									<TD>
