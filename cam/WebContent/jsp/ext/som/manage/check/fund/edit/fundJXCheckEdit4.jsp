				...
				for(j=0;j<ds.getCount();j++){
					...
					if( record.get('seqNum') == i ){
						if(record.get("peopleType")=="1"){
							...
							tab1.rows[rowLength-1].cells[8].getElementsByTagName("input")[0].value=record.get("betreffzeile");
						}
						if(record.get("peopleType")=="2"){
							...
							tab2.rows[rowLength2-1].cells[7].getElementsByTagName("input")[0].value=record.get("money");
							tab2.rows[rowLength2-1].cells[8].getElementsByTagName("input")[0].value=record.get("betreffzeile");
						}
					}
...
function save(){
	...
	for(i=1;i<tab1.rows.length;i++){
		...
		record.set("betreffzeile",tab1.rows[i].cells[8].getElementsByTagName("input")[0].value);
		var name=tab1.rows[i].cells[1].getElementsByTagName("input")[0].value;
		var sex=tab1.rows[i].cells[2].getElementsByTagName("select")[0].value;
		var birthday=tab1.rows[i].cells[3].getElementsByTagName("input")[0].value;
		var sorgDuty=tab1.rows[i].cells[4].getElementsByTagName("select")[0].value;
		var remarkDate=tab1.rows[i].cells[5].getElementsByTagName("input")[0].value;
		var meetingTimes=tab1.rows[i].cells[6].getElementsByTagName("input")[0].value;
		var money=tab1.rows[i].cells[7].getElementsByTagName("input")[0].value;
		var betreffzeile=tab1.rows[i].cells[8].getElementsByTagName("input")[0].value;
		if(name==""||sex==""||birthday==""||sorgDuty==""||remarkDate==""||meetingTimes==""||money==""||betreffzeile==""){
			if(i==1){
				L5.Msg.alert("提示","请至少填齐一行理事会成员信息！没有的请填无！");
				return "break";
			}else{
				if(!(name==""&&sex==""&&birthday==""&&sorgDuty==""&&remarkDate==""&&meetingTimes==""&&money==""&&betreffzeile=="")){
					L5.Msg.alert("提示","请至少填齐一行理事会成员信息！");
					return "break";
				}			
			}
		}
	...
	for(i=1;i<tab2.rows.length;i++){
		...
		record.set("money",tab2.rows[i].cells[7].getElementsByTagName("input")[0].value);
		record.set("betreffzeile",tab2.rows[i].cells[8].getElementsByTagName("input")[0].value);
		var name=tab2.rows[i].cells[1].getElementsByTagName("input")[0].value;
		var sex=tab2.rows[i].cells[2].getElementsByTagName("select")[0].value;
		var birthday=tab2.rows[i].cells[3].getElementsByTagName("input")[0].value;
		var remarkDate=tab2.rows[i].cells[4].getElementsByTagName("input")[0].value;
		var meetingTimes=tab2.rows[i].cells[5].getElementsByTagName("input")[0].value;
		var dutyDate=tab2.rows[i].cells[6].getElementsByTagName("input")[0].value;
		var money=tab2.rows[i].cells[7].getElementsByTagName("input")[0].value;
		var betreffzeile=tab2.rows[i].cells[8].getElementsByTagName("input")[0].value;
		if(name==""||sex==""||birthday==""||remarkDate==""||meetingTimes==""||dutyDate==""||money==""||betreffzeile==""){
			if(i==1){
				L5.Msg.alert("提示","请至少填齐一行监事信息！没有的请填无！");
				return "break";
			}else{
				if(!(name==""&&sex==""&&birthday==""&&remarkDate==""&&meetingTimes==""&&dutyDate==""&&money==""&&betreffzeile=="")){
					L5.Msg.alert("提示","请至少填齐一行监事信息！");
					return "break";
				}			
			}
		}
...
function addTr(){
    ...
    newNameTD.style.borderRight="solid windowtext 0.5pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 12.0pt; font-family: 仿宋'><select style='width: 100%;'><option  value=''>请选择...</option><option  value='1'>男</option><option  value='2'>女</option></select></span></p>";
    //添加列:出生日期
    ...
    newNameTD.style.borderRight="solid windowtext 0.5pt";
    //添加列内容
    newNameTD.innerHTML ="<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋'><input type='text' style='width: 100%; ' format='Y-m-d' onclick='LoushangDate(this)' readonly></span></p>";
    //添加列:理事会职务
    ...
    newNameTD.style.borderRight="solid windowtext 0.5pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋'><select name='' style='width: 100%;'><option  value=''>请选择...</option><option  value='00001'>理事长</option><option  value='00003'>副理事长</option><option  value='00004'>秘书长</option><option  value='00005'>理事</option></select></span></p>";
	//添加列:在登记管理机关备案时间
    ...
    newNameTD.style.borderRight="solid windowtext 0.5pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋'><input type='text' style='width: 100%; ' maxlength='6'></span></p>";
...
function addJianTr(){
    ...
    newNameTD.style.borderRight="solid windowtext 0.5pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋'><select name='' style='width: 100%;'><option  value=''>请选择...</option><option  value='1'>男</option><option  value='2'>女</option></select></span></p>";
    //添加列:出生日期
    var newNameTD=newTR.insertCell(3);
    newNameTD.style.padding="0cm 5.4pt 0cm 5.4pt";
    newNameTD.style.borderBottom="solid windowtext 0.5pt";
    ...
    newNameTD.style.borderRight="solid windowtext 0.5pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋'><input type='text' style='width: 100%;' maxlength='10'></span></p>";
	 //添加列:本年度在基金会领取的津贴(人民币元)
    var newNameTD=newTR.insertCell(8);
    newNameTD.style.padding="0cm 5.4pt 0cm 5.4pt";
    newNameTD.style.borderBottom="solid windowtext 0.5pt";
...
<div id="float">
	<input type="button" value="下一页" style="height:25px;width:80px;position:absolute;margin-left:120px;" onclick="nextPageJ('5')"/>
	<input type="button" value="上一页" style="height:25px;width:80px;position:absolute;margin-left:20px;" onclick="lastPageJ('3')"/>
