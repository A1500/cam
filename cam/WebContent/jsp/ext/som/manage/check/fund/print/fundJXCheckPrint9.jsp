...
function init9(){
	...
	somJxOrganFundsDataSet9.setParameter("TASK_CODE", taskCode);
	somJxOrganFundsDataSet9.setParameter("ORGAN_TYPE","2");
	somJxOrganFundsDataSet9.load();
	somJxOrganFundsDataSet9.on('load',function (){
		if(somJxOrganFundsDataSet9.getCount() == 0){
			for(var i = 0;i<20;i++){
				addFTr9();
			}
		}else{
			var count = 0;
			var tab = document.getElementById("activitiesF9");
			for(i=1;i<=somJxOrganFundsDataSet9.getCount();i++){
				++count;
				addFTr9();
				var rowLength = tab.rows.length;
				for(j=1;j<=somJxOrganFundsDataSet9.getCount();j++){
					if(somJxOrganFundsDataSet9.getAt(j-1).get("seqNum") == i){
						tab.rows[rowLength-1].cells[1].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet9.getAt(j-1).get("organName");
						tab.rows[rowLength-1].cells[2].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet9.getAt(j-1).get("peopleName");
						tab.rows[rowLength-1].cells[3].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet9.getAt(j-1).get("organDate");
						tab.rows[rowLength-1].cells[4].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet9.getAt(j-1).get("place");
						var accountType = somJxOrganFundsDataSet9.getAt(j-1).get("accountType");
						if(accountType == '0'){
							tab.rows[rowLength-1].cells[5].getElementsByTagName("label")[0].innerHTML="否";
						}
						if(accountType == '1'){
							tab.rows[rowLength-1].cells[5].getElementsByTagName("label")[0].innerHTML="基本账户";
						}
						if(accountType == '2'){
							tab.rows[rowLength-1].cells[5].getElementsByTagName("label")[0].innerHTML="一般账户";
						}
						tab.rows[rowLength-1].cells[6].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet9.getAt(j-1).get("openAccount");
					}
				}
...
function save(){
	somJxOrganFundsDataSet9.removeAll();
	var tab1 = document.getElementById("activitiesF9");
	for(i=1;i<tab1.rows.length;i++){
		var record = somJxOrganFundsDataSet9.newRecord();
	...
	command.setParameter("somJxOrganFundsDataSet9",somJxOrganFundsDataSet9.getAllRecords());
...
<model:datasets>
	<model:dataset id="somJxOrganFundsDataSet9" cmd="com.inspur.cams.sorg.jxcheck.somjxorganfunds.cmd.SomJxOrganFundsQueryCommand" method="queryNoPage" global="true" sortField="SEQ_NUM">
...
<TABLE cellSpacing=0 cellPadding=0 width=700 align=center border=0>
	<TBODY>
		...
		<TR>
			<TD height=14>
			<table class=f14pt-b1 cellspacing=0 cellpadding=0 width='98%' id="activitiesF9"
				align=center border=0>
				<tbody>
					<tr align=middle>
						<td class=f14pt-boder2 width=5%>序号</td>
						<td class=f14pt-boder2 width=21% height=30>名称</td>
						<td class=f14pt-boder2 width=10%>负责人</td>
						<td class=f14pt-boder2 width=13%>登记时间</td>
