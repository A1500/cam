...
<next:GridPanel id="editGridPanel" name="somchecktaskGrid" width="100%" stripeRows="true" height="200" dataset="taskDs" title="填写年检信息" notSelectFirstRow="true">
	<next:TopBar>
	...
	<next:Columns>
		...
		<next:Column id="sorgType" header="社会类型" field="sorgType" width="90" >
...
<next:GridPanel id="editGridPanel2" name="somcheckGrid" width="100%" stripeRows="true" height="300" dataset="checkDs" title="填写过的年检信息" notSelectFirstRow="true">
	<next:TopBar>
		<next:ToolBarItem symbol="->" ></next:ToolBarItem>
		<next:ToolBarItem text="提交年检材料" iconCls="upload" handler="submit"/>
		<next:ToolBarItem text="修改" iconCls="edit" handler="updateCheck"/>
		<next:ToolBarItem text="查看并打印年检报告" iconCls="detail" handler="detailCheck"/>
	</next:TopBar>
	<next:Columns>
	    <next:RowNumberColumn width="30"/>
	     <next:CheckBoxColumn></next:CheckBoxColumn>
		...
		<next:Column id="cnName" header="社会单位名称" field="cnName" width="250" >
			<next:TextField  />
		</next:Column>
