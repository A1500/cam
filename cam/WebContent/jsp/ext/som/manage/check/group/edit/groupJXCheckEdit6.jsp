...
function showOrHideDesc(value){
	if(value == "1"){
		document.getElementById("noBuildPartyResonSpan").style.display="";
	}else{
		document.getElementById("noBuildPartyResonSpan").style.display="none";
	}
}
function save(){
	...
	if(ifBuildParty=='0'){
		...
		if(noBuildPartyReson == "1" && record.get("noBuildPartyResonDesc") == ""){
			L5.Msg.alert("提示",'"应建未建的未建立党组织原因具体说明"'+"不能为空！");
			return "break";
		}
...
<model:datasets>
	<model:dataset id="somJxCheckPartyDataSet" cmd="com.inspur.cams.sorg.jxcheck.somjxcheckparty.cmd.SomJxCheckPartyQueryCommand" global="true">
		<model:record fromBean="com.inspur.cams.sorg.jxcheck.somjxcheckparty.dao.SomJxCheckParty">
...
<div id="float">	<input type="button" value="下一页" style="height:25px;width:80px;position:absolute;margin-left:120px;" onclick="nextPage('7')"/>
		<input type="button" value="上一页" style="height:25px;width:80px;position:absolute;margin-left:20px;" onclick="lastPage('5')"/>
...
<p class=MsoNormal
	...
	style='font-size: 16.0pt; font-family: 黑体; mso-hansi-font-family: 宋体'>五、党建情况</span></b><b><span
	lang=EN-US
...
<select id="" field="noBuildPartyReson" style='width:200' onchange="showOrHideDesc(this.value)">
