...
function query(){
	...
	ds.setParameter("SOM_JX_CHECK.SORG_TYPE@=","S");
...
function insert() {
	...
	var url='jsp/ext/som/manage/check/group/edit/groupJXCheckEdit1.jsp';
	var text = '增加年检';
...
function detail(){
	...
	data.put("taskCode",selected[0].get('taskCode'));
	var url='jsp/ext/som/manage/check/group/detail/groupJXCheckDetail1.jsp';
	var text = '年检明细';
	L5.forward(url,text,data);
}
function print(){
	...
	if(flag == "Y"){
		...
		data.put("method","DETAIL");
		data.put("taskCode",selected[0].get('taskCode'));
		var url='jsp/ext/som/manage/check/group/print/groupJXCheckPrint1.jsp';
		var text = '年检明细';
		L5.forward(url,text,data);
	}else{
