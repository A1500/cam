...
function checkNumWithKeyCode(obj, ev) {
                      var e = ev || window.event;
                      if (e.which == 8||e.which==13||e.charCode==13) //8为ff退格键 13回车键
                          return true;
                      if (e.keyCode) {
                          if (!(e.keyCode <= 57 && e.keyCode >= 48) && e.keyCode != 46) {
                              return false;
                          }
                          if (!obj.value && (e.keyCode == 48 || e.keyCode == 46))
                              return false;
                          if (obj.value.indexOf(".") > 0 && e.keyCode == 46)
                              return false;
                      } else if (e.which) {
                          if (!(e.which <= 57 && e.which >= 48) && e.which != 46) {
                              return false;
                          }
                          if (!obj.value && (e.which == 48 || e.which == 46))
                              return false;
                          if (obj.value.indexOf(".") > 0 && e.which == 46)
                              return false;
                      }
                      else if (e.charCode) {
                          if (!(e.charCode <= 57 && e.charCode >= 48) && e.charCode != 46) {
                              return false;
                          }
                          if (!obj.value && (e.charCode == 48 || e.charCode == 46))
                              return false;
                          if (obj.value.indexOf(".") > 0 && e.charCode == 46)
                              return false;
                          }
                      else {
                          //alert("浏览器不支持!")
                  ...
                  function TrimLast(obj) {
                      if (obj.value && obj.value.indexOf('.') == obj.value.length - 1) {
...
function setSumMoney(){
	...
	somJxCheckBalanceSheetDataSet.set("faSumS",(setParseFloat("固定资产原价")-setParseFloat("减：累计折旧")+setParseFloat("固定资产净值")+setParseFloat("在建工程")+setParseFloat("文物文化资产")+setParseFloat("固定资产清理")).toFixed(6)*1);
	somJxCheckBalanceSheetDataSet.set("faSumE",(setParseFloat("_固定资产原价")-setParseFloat("_减：累计折旧")+setParseFloat("_固定资产净值")+setParseFloat("_在建工程")+setParseFloat("_文物文化资产")+setParseFloat("_固定资产清理")).toFixed(6)*1);
...
<div id="float">
	<input type="button" value="下一页" style="height:25px;width:80px;position:absolute;margin-left:120px;" onclick="nextPage('9')"/>
		<input type="button" value="上一页" style="height:25px;width:80px;position:absolute;margin-left:20px;" onclick="lastPage('7')"/>
