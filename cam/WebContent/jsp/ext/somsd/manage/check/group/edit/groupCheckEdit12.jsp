...
function save(){
	...
	var specialFundBuildNum=document.getElementById("specialFundBuildNum").value;
	var specialFundBuildedNum=document.getElementById("specialFundBuildedNum").value;
	if(parseInt(specialFundBuildedNum) > (parseInt(specialFundBuildNum))){
		L5.Msg.alert("提示","已办理成立登记手续的专项基金管理机构数不能大于本年度新成立数！");
		return "break";
	}
	var specialFundCancelNum=document.getElementById("specialFundCancelNum").value;
	var specialFundCanceledNum=document.getElementById("specialFundCanceledNum").value;
	...
	for(i=1;i<tab.rows.length;i++){
		...
		var regNumber=document.getElementById("regNumber"+num).value;
		var accounts=document.getElementById("accounts"+num).value;
		var address=document.getElementById("address"+num).value;		
		if(organName==""||manager==""||regTime==""||regNumber==""||accounts==""||address==""){
			if(i==1){
				if(organName!='无'){
					L5.Msg.alert("提示","请至少输入一条专项基金管理机构情况！没有的请填无！");
					return "break";
				}
			}else{
				if(!(organName==""&&manager==""&&regTime==""&&regNumber==""&&accounts==""&&address=="")){
					L5.Msg.alert("提示","请输入完整的专项基金管理机构情况！");
					return "break";
				}			
			}
		}
...
function addTr(){
    ...
    newNameTD.style.borderTop="solid windowtext 1.0pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='line-height: 13.0pt; mso-line-height-rule: exactly'><span lang=EN-US><o:p><input type='text' id='regTime"+xuhao+"' name='登记时间' onClick='WdatePicker()' style='width:98%'/></o:p></span></p>";
    //添加列:登记证号
...
<model:datasets>
	...
	<model:dataset id="somCheckOrgDetailDataSet" cmd="com.inspur.cams.sorg.check.checkorgdetail.cmd.SomCheckOrgDetailQueryCommand" global="true">
		<model:record fromBean="com.inspur.cams.sorg.check.checkorgdetail.data.SomCheckOrgDetail">
			<model:field name="specialFundBuildNum" type="string" rule="require" />
			<model:field name="specialFundBuildedNum" type="string" rule="require" />
			<model:field name="specialFundCancelNum" type="string" rule="require" />			
			<model:field name="specialFundCanceledNum" type="string" rule="require" />
		</model:record>
	</model:dataset>
	<!-- 帐户 -->
...
<input
	...
	style="height: 25px; width: 80px; position: absolute; margin-left: 20px;" onclick="lastPage('11')"/>
<input
	type="button" value="下一页"
	style="height: 25px; width: 80px; position: absolute; margin-left: 120px;" onclick="nextPage('13')"/>
<!--[if lte IE 7]><div></div><![endif]--> <input type="button"
	...
	<span style="height:25px;width:80px;position:absolute;margin-left:420px;">当前页数：14</span>
</div>
<div align=center>
...
<p class=MsoNormal align=left width=800px
	...
	name='专项基金管理机新设数'id='specialFundBuildNum' field="specialFundBuildNum" style="width:50;text-align:center;"
	maxlength="10" onkeyup="value=value.replace(/[^\d]/g,'') "/> </span></span>）个，其中已办理成立登记手续的（<span lang=EN-US><span
	style='mso-spacerun: yes'> <input name='专项基金管理机办理成立登记手续的数'id='specialFundBuildedNum'
	field="specialFundBuildedNum" style="width:50;text-align:center;" maxlength="10" onkeyup="value=value.replace(/[^\d]/g,'') "/> </span></span>）个；<span
...
<p class=MsoNormal align=left width=800px
	...
	lang=EN-US><span style='mso-spacerun: yes'> <input id='specialFundCancelNum'
	name='专项基金管理机注销数' field="specialFundCancelNum" style="width:50;text-align:center;"
	maxlength="10" onkeyup="value=value.replace(/[^\d]/g,'') "/> </span></span>）个，其中已办理注销登记手续的（<span lang=EN-US><span
	style='mso-spacerun: yes'> <input name='专项基金管理机办理注销手续数' id='specialFundCanceledNum'
	field="specialFundCanceledNum" style="width:50;text-align:center;" maxlength="10" onkeyup="value=value.replace(/[^\d]/g,'') "/> </span></span>）个；<span
	lang=EN-US><o:p></o:p></span></span></p>
