...
function save(){
	...
	var delegateBuildNum=document.getElementById("delegateBuildNum").value;
	var delegateBuildedNum=document.getElementById("delegateBuildedNum").value;
	if(parseInt(delegateBuildedNum) > (parseInt(delegateBuildNum))){
		L5.Msg.alert("提示","已办理成立登记手续的代表机构数不能大于本年度新成立数！");
		return "break";
	}
	var delegateCancelNum=document.getElementById("delegateCancelNum").value;
	var delegateCanceledNum=document.getElementById("delegateCanceledNum").value;
	...
	for(i=1;i<tab.rows.length;i++){
		...
		if(organName==""||manager==""||regTime==""||regNumber==""||accounts==""||address==""){
			if(i==1){
				if(organName!='无'){
					L5.Msg.alert("提示","请至少输入一条代表机构情况！没有的请填无！");
					return "break";
				}
			}else{
				if(!(organName==""&&manager==""&&regTime==""&&regNumber==""&&accounts==""&&address=="")){
					L5.Msg.alert("提示","请输入完整的代表机构情况！");
					return "break";
				}			
			}
		}
...
<model:datasets>
	...
	<model:dataset id="somCheckOrgDetailDataSet" cmd="com.inspur.cams.sorg.check.checkorgdetail.cmd.SomCheckOrgDetailQueryCommand" global="true">
		<model:record fromBean="com.inspur.cams.sorg.check.checkorgdetail.data.SomCheckOrgDetail">
			<model:field name="delegateBuildNum" type="string" rule="require" />
...
<input
	...
	style="height: 25px; width: 80px; position: absolute; margin-left: 20px;" onclick="lastPage('9')"/>
<input
	type="button" value="下一页"
	style="height: 25px; width: 80px; position: absolute; margin-left: 120px;" onclick="nextPage('11')"/>
<!--[if lte IE 7]><div></div><![endif]--> <input type="button"
	...
	<span style="height:25px;width:80px;position:absolute;margin-left:420px;">当前页数：12</span>
</div>
<div align=center>
...
<p class=MsoNormal align=left width=800px
	...
	<input name='代表机构新设数'id='delegateBuildNum' field="delegateBuildNum" style="width:50;text-align:center;" maxlength="10"onkeyup="value=value.replace(/[^\d]/g,'') "/>
	）个，其中已办理成立登记手续的（
	<input name='代表机构办理成立登记手续的个数'id='delegateBuildedNum' field="delegateBuildedNum" style="width:50;text-align:center;" maxlength="10"onkeyup="value=value.replace(/[^\d]/g,'') "/>
	）个；<span
...
<p class=MsoNormal align=left width=800px
	...
	<input name='代表机构注销数'id='delegateCancelNum' field="delegateCancelNum" style="width:50;text-align:center;" maxlength="10"onkeyup="value=value.replace(/[^\d]/g,'') "/>
	）个，其中已办理注销登记手续的（
	<input name='代表机构办理注销手续个数'id='delegateCanceledNum' field="delegateCanceledNum" style="width:50;text-align:center;" maxlength="10"onkeyup="value=value.replace(/[^\d]/g,'') "/>
	）个；<span
	lang=EN-US><o:p></o:p></span></span></p>
