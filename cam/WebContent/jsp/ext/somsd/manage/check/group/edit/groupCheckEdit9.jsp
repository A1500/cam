...
function save(){
	...
	var branchBuildNum=document.getElementById("branchBuildNum").value;
	var branchBuildedNum=document.getElementById("branchBuildedNum").value;
	if(parseInt(branchBuildedNum) > (parseInt(branchBuildNum))){
		L5.Msg.alert("提示","已办理成立登记手续的分支机构数不能大于本年度新设立数！");
		return "break";
	}
	var branchCancelNum=document.getElementById("branchCancelNum").value;
	var branchCanceledNum=document.getElementById("branchCanceledNum").value;
	...
	for(i=1;i<tab.rows.length;i++){
		...
		if(organName==""||manager==""||regTime==""||regNumber==""||accounts==""||address==""){
			if(i==1){
				if(organName!='无'){
					L5.Msg.alert("提示","请至少输入一条分支机构情况！没有的请填无！");
					return "break";
				}
			}else{
				if(!(organName==""&&manager==""&&regTime==""&&regNumber==""&&accounts==""&&address=="")){
					L5.Msg.alert("提示","请输入完整的分支机构情况！");
					return "break";
				}			
			}
		}
...
<model:datasets>
	...
	<model:dataset id="somCheckOrgDetailDataSet" cmd="com.inspur.cams.sorg.check.checkorgdetail.cmd.SomCheckOrgDetailQueryCommand" global="true">
		<model:record fromBean="com.inspur.cams.sorg.check.checkorgdetail.data.SomCheckOrgDetail">
			<model:field name="branchBuildNum" type="string" rule="require" />
			<model:field name="branchBuildedNum" type="string" rule="require" />
			<model:field name="branchCancelNum" type="string" rule="require" />			
			<model:field name="branchCanceledNum" type="string" rule="require" />
		</model:record>
	</model:dataset>
	<!-- 帐户 -->
...
<input
	...
	style="height: 25px; width: 80px; position: absolute; margin-left: 20px;" onclick="lastPage('8')"/>
<input
	type="button" value="下一页"
	style="height: 25px; width: 80px; position: absolute; margin-left: 120px;" onclick="nextPage('10')"/>
<!--[if lte IE 7]><div></div><![endif]--> <input type="button"
	...
	<span style="height:25px;width:80px;position:absolute;margin-left:420px;">当前页数：11</span>
</div>
<div align=center>
...
<p class=MsoNormal align=left width=800px
	...
	<input name='分支机构新设数'id='branchBuildNum' field="branchBuildNum" style="width:50;text-align:center;" maxlength="10"onkeyup="value=value.replace(/[^\d]/g,'') "/>
	）个，其中已办理成立登记手续的（
	<input name='分支机构办理成立登记手续的个数'id='branchBuildedNum' field="branchBuildedNum" style="width:50;text-align:center;" maxlength="10"onkeyup="value=value.replace(/[^\d]/g,'') "/>
	）个；<span
	lang=EN-US><o:p></o:p></span></span></p>
...
<p class=MsoNormal align=left width=800px
	...
	<input name='分支机构注销数'id='branchCancelNum' field="branchCancelNum" style="width:50;text-align:center;" maxlength="10"onkeyup="value=value.replace(/[^\d]/g,'') "/>
	）个，其中已办理注销登记手续的（
	<input name='分支机构办理注销手续个数' id='branchCanceledNum' field="branchCanceledNum" style="width:50;text-align:center;" maxlength="10"onkeyup="value=value.replace(/[^\d]/g,'') "/>
	）个；<span
	lang=EN-US><o:p></o:p></span></span></p>
...
<table id="table" class=MsoNormalTable border=1 cellspacing=0 cellpadding=0
	...
	<tr style='height: 19.3pt'>
		...
		<td width="20%"
			style='width: 20.3%; border-top: solid windowtext 1.0pt; border-left: none; border-bottom: solid windowtext 1.0pt; border-right: solid windowtext 1.0pt; mso-border-left-alt: solid windowtext .5pt; mso-border-top-alt: 1.5pt; mso-border-left-alt: .5pt; mso-border-bottom-alt: .5pt; mso-border-right-alt: 1.0pt; mso-border-color-alt: windowtext; mso-border-style-alt: solid; padding: 0cm 5.4pt 0cm 5.4pt; height: 19.3pt'>
		<p class=MsoNormal align=center
			style='text-align: center; line-height: 15.0pt; mso-line-height-rule: exactly'><span
