			...
			for(i=1;i<=somJxOrganFundsDataSet.getCount();i++){
				...
				for(j=1;j<=somJxOrganFundsDataSet.getCount();j++){
					if(somJxOrganFundsDataSet.getAt(j-1).get("sequence") == i){
						...
						tab.rows[rowLength-1].cells[4].getElementsByTagName("textarea")[0].value=somJxOrganFundsDataSet.getAt(j-1).get("address");
						tab.rows[rowLength-1].cells[5].getElementsByTagName("select")[0].value=somJxOrganFundsDataSet.getAt(j-1).get("depositBankAccounts");
						tab.rows[rowLength-1].cells[6].getElementsByTagName("textarea")[0].value=somJxOrganFundsDataSet.getAt(j-1).get("depositBank");
					}
				}
...
function save(){
	...
	for(i=1;i<tab1.rows.length;i++){
		...
		record.set("address",tab1.rows[i].cells[4].getElementsByTagName("textarea")[0].value);
		record.set("depositBankAccounts",tab1.rows[i].cells[5].getElementsByTagName("select")[0].value);
		record.set("depositBank",tab1.rows[i].cells[6].getElementsByTagName("textarea")[0].value);
...
function addFTr(){
    ...
    newNameTD.style.borderRight="solid windowtext 1.0pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋'><input id='manager"+xuhao+"' type='text' style='width: 100%;' maxlength='72'></span></p>";
    //添加列:登记时间
