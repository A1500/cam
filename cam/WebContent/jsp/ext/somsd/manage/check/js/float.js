...
function groupDetailJumpTo(pageIndex){
	...
	var url='jsp/ext/somsd/manage/check/group/detail/groupCheckDetail'+pageIndex+'.jsp';
	var text = '年检列表';
...
function createGroupEditSelect(){
	var floatDiv = document.getElementById("float");
	var sHTML='<select name="导航" onchange="groupEditJumpTo(this.value)" style="height: 25px; width: 150px; position: absolute; margin-left: 520px;margin-top:5px" >';
    floatDiv.insertAdjacentHTML("afterBegin",createGroupContent(sHTML));
    sHTML='<input type="button" value="填写说明" style="height:25px;width:80px;position:absolute;margin-left:720px;" onclick="desc();"/>';
    floatDiv.insertAdjacentHTML("afterBegin",sHTML);
}
function desc(){
	var url='../edit/filldesc.jsp';
	var width = screen.width-200;
	var height = screen.height-200;
	window.open (url, '填写说明', 'height=600, width=800, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes,resizable=no,location=no, status=no');
}
//社会团体修改页面导航跳转
function groupEditJumpTo(pageIndex){
	...
	var data = new L5.Map();
	data.put("taskCode",taskCode);
	var url='jsp/ext/somsd/manage/check/group/edit/groupCheckEdit'+pageIndex+'.jsp';
	var text = '年检列表';
	L5.forward(url,text,data);
}
//社会团体导航内容组装
function createGroupContent(str){
	...
	str+='<option value="01">第0页</option>';
	str+='<option value="02">第1页</option>';
	str+='<option value="03">第2页</option>';
	str+='<option value="1">第3页</option>';
	str+='<option value="2">第4页</option>';
	str+='<option value="3">第5页</option>';
	str+='<option value="4">第6页</option>';
	str+='<option value="5">第7页</option>';
	str+='<option value="6">第8页</option>';
	str+='<option value="7">第9页</option>';
	str+='<option value="8">第10页</option>';
	str+='<option value="9">第11页</option>';
	str+='<option value="10">第12页</option>';
	str+='<option value="11">第13页</option>';
	str+='<option value="12">第14页</option>';
	str+='<option value="13">第15页</option>';
	str+='<option value="14">第16页</option>';
	str+='<option value="15">第17页</option>';
	str+='<option value="16">第18页</option>';
	str+='<option value="17">第19页</option>';
	str+='<option value="18">第20页</option>';
	str+='<option value="19">第21页</option>';
	str+='</select>';
	return str;
}
function back(){
...
function lastPage(page){
	...
	var data = new L5.Map();
	var url='jsp/ext/somsd/manage/check/group/edit/groupCheckEdit'+page+'.jsp';
	data.put("taskCode",taskCode);
	var text = '年检列表';
	L5.forward(url,text,data);
}
...
function lastPage1(page){
	var data = new L5.Map();
	var url='jsp/ext/somsd/manage/check/group/detail/groupCheckDetail'+page+'.jsp';
	data.put("taskCode",taskCode);
	var text = '年检列表';
	L5.forward(url,text,data);
}
...
function ungovDetailJumpTo(pageIndex){
	var data = new L5.Map();
	data.put("taskCode",taskCode);
	var url='jsp/ext/somsd/manage/check/ungov/detail/ungovCheckDetail'+pageIndex+'.jsp';
	var text = '年检列表';
	L5.forward(url,text,data);
}
//修改页面导航创建
function createUngovEditSelect(){
	var floatDiv = document.getElementById("float");
	var sHTML='<select name="导航" onchange="ungovEditJumpTo(this.value)" style="height: 25px; width: 150px; position: absolute; margin-left: 520px;margin-top:5px" >';
    floatDiv.insertAdjacentHTML("afterBegin",createUngovContent(sHTML));
}
//修改页面导航跳转
function ungovEditJumpTo(pageIndex){
	...
	var data = new L5.Map();
	data.put("taskCode",taskCode);
	var url='jsp/ext/somsd/manage/check/ungov/edit/ungovCheckEdit'+pageIndex+'.jsp';
	var text = '年检列表';
	L5.forward(url,text,data);
}
//社会团体导航内容组装
function createUngovContent(str){
	str+='<option value="0">请选择跳转页面</option>';
	str+='<option value="1">第0页</option>';
	str+='<option value="2">第1页</option>';
	str+='<option value="3">第2页</option>';
	str+='<option value="4">第3页</option>';
	str+='<option value="5">第4页</option>';
	str+='<option value="6">第5页</option>';
	str+='<option value="7">第6页</option>';
	str+='<option value="8">第7页</option>';
	str+='<option value="9">第8页</option>';
	str+='<option value="10">第9页</option>';
	str+='</select>';
	return str;
}
function nextPageM(page){
	...
	var data = new L5.Map();
	var url='jsp/ext/somsd/manage/check/ungov/edit/ungovCheckEdit'+page+'.jsp';
	data.put("taskCode",taskCode);
	var text = '年检列表';
	L5.forward(url,text,data);
}
...
function lastPageM1(page){
	var data = new L5.Map();
	var url='jsp/ext/somsd/manage/check/ungov/detail/ungovCheckDetail'+page+'.jsp';
	data.put("taskCode",taskCode);
	var text = '年检列表';
	L5.forward(url,text,data);
}
...
function fundDetailJumpTo(pageIndex){
	var data = new L5.Map();
	data.put("taskCode",taskCode);
	var url='jsp/ext/somsd/manage/check/fund/detail/fundCheckDetail'+pageIndex+'.jsp';
	var text = '年检列表';
	L5.forward(url,text,data);
}
//修改页面导航创建
...
function fundEditJumpTo(pageIndex){
	...
	var data = new L5.Map();
	data.put("taskCode",taskCode);
	var url='jsp/ext/somsd/manage/check/fund/edit/fundCheckEdit'+pageIndex+'.jsp';
	var text = '年检列表';
	L5.forward(url,text,data);
}
//导航内容组装
...
function nextPageJ(page){
	...
	var data = new L5.Map();
	var url='jsp/ext/somsd/manage/check/fund/edit/fundCheckEdit'+page+'.jsp';
	data.put("taskCode",taskCode);
	var text = '年检列表';
	L5.forward(url,text,data);
}
...
function lastPageJ1(page){
	var data = new L5.Map();
	var url='jsp/ext/somsd/manage/check/fund/detail/fundCheckDetail'+page+'.jsp';
	data.put("taskCode",taskCode);
	var text = '年检列表';
	L5.forward(url,text,data);
}
