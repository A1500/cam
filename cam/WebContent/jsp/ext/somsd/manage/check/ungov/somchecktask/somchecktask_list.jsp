...
<next:GridPanel id="editGridPanel2" name="somcheckGrid" width="100%" stripeRows="true" height="100%" dataset="checkDs" title="填写过的年检信息" notSelectFirstRow="true">
	<next:TopBar>
		...
		<next:ToolBarItem text="打印年检报告" iconCls="print" handler="confirmPrint"/>
	</next:TopBar>
	<next:Columns>
	    <next:RowNumberColumn width="30"/>
	     <next:CheckBoxColumn></next:CheckBoxColumn>
		...
		<next:Column id="cnName" header="民办非企业单位名称" field="cnName" width="250" >
			<next:TextField  />
		</next:Column>
		...
		<next:Column id="status" header="年检状态" field="status" width="120" renderer="changeColor">
		</next:Column>
		...
		<next:Column id="correctNotReason" header="原因" field="" editable="false" renderer="clickShowHref" width="80" align="center">
			<next:TextField  />
		</next:Column>
	</next:Columns>
	<next:BottomBar>
