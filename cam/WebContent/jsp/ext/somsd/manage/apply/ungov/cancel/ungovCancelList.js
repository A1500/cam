...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/ungov/cancel/ungovCancelEdit.jsp";
	L5.forward(url,'',data);
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/ungov/cancel/ungovCancelDetail.jsp";
	L5.forward(url,'',data);
}
...
function enter(){
	...
	var url='jsp/ext/somsd/manage/apply/ungov/cancel/ungovCancelEdit.jsp?method=INSERT';
	L5.forward(url);
}
...
function printWord(){
	...
	var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/ungov/jsp/cancel/ungovCancelWord.jsp?taskCode="+taskCode+"&sorgId="+sorgId;
	window.open(url);
}