...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/ungov/rule/ungovRuleEdit.jsp";
	L5.forward(url,'',data);	
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/ungov/rule/ungovRuleDetail.jsp";
	L5.forward(url,'',data);	
}
...
function enter(){
	var url='jsp/ext/somsd/manage/apply/ungov/rule/ungovRuleEdit.jsp?method=INSERT';
	L5.forward(url);
}
