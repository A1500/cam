...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/ungov/buildPartner/ungovBuildPartnerEdit.jsp";
	L5.forward(url,'',data);	
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/ungov/buildPartner/ungovBuildPartnerDetail.jsp";
	L5.forward(url,'',data);	
}
// 受理页面
function forAccept(){
...
function enter(){
	var url='jsp/ext/somsd/manage/apply/ungov/buildPartner/ungovBuildPartnerEdit.jsp?method=INSERT';
	L5.forward(url);
}
...
function printSelect(){
	...
	if(DJSQ==true){
		var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/ungov/jsp/buildPartner/ungovBuildPartnerWord.jsp?taskCode="+taskCode+"&sorgId="+sorgId;
		window.open(url);
	}else if(FZRDJ==true){
		var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/ungov/jsp/buildPartner/ungovPartnerWord.jsp?taskCode="+taskCode+"&sorgId="+sorgId;
		window.open(url);
	}else if(NSJG==true){
