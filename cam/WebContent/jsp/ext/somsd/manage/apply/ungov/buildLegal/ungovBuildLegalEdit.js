...
function init(){
	...
	if(method == "INSERT"){
		somOrganDataSet.newRecord({"cnName":userId,"sorgId":sorgId,"sorgKind":"1","sorgType":"M","ifBranch":'0',"morgName":userId,"morgArea":morgArea});
		somApplyDataSet.newRecord({"cnName":userId,"curActivity":"受理","taskCode":taskCode,"sorgId":sorgId,"applyType":applyType,"dataSource":"ONLINE","sorgType":"M","acceptArea":morgArea,"createOrgan":morgArea,"createPerson":userId});
...
/*
	...
	//法定代表人校验
	if(LegalPeopleDataSet.get('name') == ''){
	...
	*/
	...
	//举办者（自然人）
	var somHeldPeopleRds =  somHeldPeopleDataSet.getAllChangedRecords();
	//举办者（单位）
	var somHeldOrganRds = somHeldOrganDataSet.getAllChangedRecords();
	//个人简历
	var somResumeDataSet=L5.DatasetMgr.lookup("somResumeDataSet");
	...
	somApplyRecord.set("sorgName",organRecord.get("cnName") );
	...
	command.setParameter("somHeldPeopleRds",somHeldPeopleRds);
	command.setParameter("somHeldOrganRds", somHeldOrganRds);
