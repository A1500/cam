...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/fund/rule/fundRuleEdit.jsp";
	L5.forward(url,'',data);	
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/fund/rule/fundRuleDetail.jsp";
	L5.forward(url,'',data);	
}
//打印按钮
function printWord(){
	...
	var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/fund/jsp/rulesApproval/rulesApproval.jsp?taskCode="+taskCode+"&sorgId="+sorgId;
	window.open(url);
}