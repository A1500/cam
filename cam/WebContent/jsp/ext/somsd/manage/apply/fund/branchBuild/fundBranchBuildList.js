...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/fund/branchBuild/fundBranchBuildEdit.jsp";
	L5.forward(url,'',data);
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/fund/branchBuild/fundBranchBuildDetail.jsp";
	L5.forward(url,'',data);
}
...
function enter(){
	var url='jsp/ext/somsd/manage/apply/fund/branchBuild/fundBranchBuildEdit.jsp?method=INSERT';
	L5.forward(url);
}
...
function printWord(){
	...
	if(document.getElementById("SQ").checked==true){
		var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/fund/jsp/branchBuild/branchNameApproval.jsp?taskCode="+taskCode+"&sorgId="+sorgId;
		window.open(url);
	}
...
function printHeld(){
	...
	var dutyId=records[0].get('dutyId');
		...
		var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/fund/jsp/branchBuild/branchLeader.jsp?dutyId="+dutyId+"&sorgId="+sorgId;
		window.open(url);
}