...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/group/delayElection/groupDelayElectionEdit.jsp";
	L5.forward(url,'',data);	
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/group/delayElection/groupDelayElectionDetail.jsp";
	L5.forward(url,'',data);	
}
// 受理页面
function forAccept(){
...
function enter(){
	var url='jsp/ext/somsd/manage/apply/group/delayElection/groupDelayElectionEdit.jsp?method=INSERT';
	L5.forward(url);
}
...
function printWord(){
	...
	var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/group/jsp/delayElection/groupDelayElectionWord.jsp?taskCode="+taskCode+"&sorgId="+sorgId;
	window.open(url);
}