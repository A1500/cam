...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/group/prepare/groupPrepareEdit.jsp";
	L5.forward(url,'',data);
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/group/prepare/groupPrepareDetail.jsp";
	L5.forward(url,'',data);
}
...
function enter(){
	...
	var url='jsp/ext/somsd/manage/apply/group/prepare/groupPrepareEdit.jsp?method=INSERT';
	L5.forward(url);
}
...
function printWord(){
	...
	var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/group/jsp/groupPrepareWord.jsp?taskCode="+taskCode+"&sorgId="+sorgId;
	window.open(url);
}