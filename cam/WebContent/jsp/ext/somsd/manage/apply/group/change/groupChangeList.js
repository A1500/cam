...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/group/change/groupChangeEdit.jsp";
	L5.forward(url,'',data);	
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/group/change/groupChangeDetail.jsp";
	L5.forward(url,'',data);	
}
// 受理页面
function forAccept(){
...
function enter(){
	var url='jsp/ext/somsd/manage/apply/group/change/groupChangeEdit.jsp?method=INSERT';
	L5.forward(url);
}
...
function printWord(){
	...
	var url=L5.webPath+"/jsp/ext/somsd/manage/printWord/group/jsp/groupChangeWord.jsp?taskCode="+taskCode+"&sorgId="+sorgId;
	window.open(url);
}