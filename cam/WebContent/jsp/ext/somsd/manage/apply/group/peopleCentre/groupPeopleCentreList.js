...
function insert(){
	...
	url="jsp/ext/somsd/manage/apply/group/peopleCentre/groupPeopleCentreExt.jsp";
	L5.forward(url,'',data);	
}
function update(){
...
function detail(){
	...
	data.put("taskCode",records[0].get('taskCode'));
	var url='';
	url="jsp/ext/somsd/manage/apply/group/peopleCentre/groupPeopleCentreDetail.jsp";
	L5.forward(url,'',data);	
}

function enter(){
	var sorgType=document.getElementById('sorgType').value;
	url='jsp/ext/somsd/manage/apply/group/peopleCentre/groupPeopleCentreExt.jsp?method=INSERT&sorgType='+sorgType;
	L5.forward(url);
}
