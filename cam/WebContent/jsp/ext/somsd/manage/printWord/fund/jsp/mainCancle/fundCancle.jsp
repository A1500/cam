...
function init(){  
	//取得文件名 和标签    
	var nowFileName = "/jsp/ext/somsd/manage/printWord/fund/doc/fundCancle.doc";
	...
	var somOrganDataSet=L5.DatasetMgr.lookup("somOrganDataSet");
		...
		showMap.put("buildDate",somOrganDataSet.get("buildDate"));
		showMap.put("cancelDate",somOrganDataSet.get("cancelDate"));
		showMap.put("legalPeople",somOrganDataSet.get("legalPeople"));
		showMap.put("cancelReason",somOrganDataSet.get("cancelReason"));
		
		var businessOrganSelect=L5.DatasetMgr.lookup("businessOrganSelect");
		var borgCode=somOrganDataSet.get("borgName");
		businessOrganSelect.setParameter("includeCode", borgCode);
		businessOrganSelect.load();
		businessOrganSelect.on('load', function(){
		showMap.put("borgNameDispaly",businessOrganSelect.get("text"));	
		...
		var records=somLiqpeopleDataSet.getAllRecords();
		for(var i=0;i<records.length;i++){
			var record =records[i];
			name = record.get("name");
			liqDuties = record.get("liqDuties");
			workName = record.get("workName");
			workDuties = record.get("workDuties");
			showMap.put("name"+i,name);
		 	showMap.put("liqDuties"+i,liqDuties);
