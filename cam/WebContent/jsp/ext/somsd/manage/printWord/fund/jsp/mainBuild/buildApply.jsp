...
function init(){  
	//取得文件名 和标签    
	var nowFileName = "/jsp/ext/somsd/manage/printWord/fund/doc/buildApply.doc";
	...
	var showMap=new L5.Map("java.util.HashMap");
		...
		aspForLiShi.setParameter("SORG_DUTIES@=", "00001");
		aspForLiShi.load();
		
		aspForLiShi.on('load', function(){
	
			name = aspForLiShi.get("name");
			workName = aspForLiShi.get("workName");
			workDuties = aspForLiShi.get("workDuties");
			
			
			showMap.put("legalName",name);
			showMap.put("legalWorkName",workName);
			showMap.put("legalWorkDuties","理事长");
