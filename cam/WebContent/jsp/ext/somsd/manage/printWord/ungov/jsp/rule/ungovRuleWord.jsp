...
function init(){  
	//取得文件名 和标签    
	var nowFileName = "/jsp/ext/somsd/manage/printWord/ungov/doc/ungovRuleWord.doc";
	...
	somRuleDataSet.load();
	somRuleDataSet.on('load', loadInfo);
}
function loadInfo(){
	...
	somApplyDataSet.on("load",function(){
		...
		somOrganDataSet.on("load",function(){
			...
			//somRuleDataSet
			var meetingName = somRuleDataSet.get("meetingName");//会议名称
			var meetingDate = somRuleDataSet.get("meetingDate");//时    间
			var shouldNum = somRuleDataSet.get("shouldNum");//应到人数
			var realNum = somRuleDataSet.get("realNum");//实到人数
			var passNum = somRuleDataSet.get("passNum");//赞同人数
			var nopassNum = somRuleDataSet.get("nopassNum");//反对人数
			var giveupNum = somRuleDataSet.get("giveupNum");//弃权人数
			var mainQuestion = somRuleDataSet.get("mainQuestion");//需 要 说 明 的 主 要 问 题
			//somApplyDataSet
			var borgOpinion = somApplyDataSet.get("borgOpinion");//业 务主 管单 位审 查意 见
			...
			showMap.put("realNum",realNum);
			showMap.put("passNum",passNum);
			showMap.put("nopassNum",nopassNum);
			showMap.put("giveupNum",giveupNum);
			showMap.put("mainQuestion",mainQuestion);
			showMap.put("borgOpinion",borgOpinion);
