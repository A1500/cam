...
function init(){  
	//取得文件名 和标签    
	var nowFileName = "/jsp/ext/somsd/manage/printWord/ungov/doc/ungovLegalWord.doc";
...
function loadInfo(){
	...
	if(somOrganDataSet.getCount()>0){
		cnName = somOrganDataSet.get("cnName");//单位名称
	...
	somResumeLegalDataSet.on('load',function(){
		if(somResumeLegalDataSet.getCount()>0){
			...
			for(var i=0;i<length;i++){
				var startDate = records[i].get("startDate");//开始年月
				var endDate = records[i].get("endDate");//结束年月
				var startEndDate = startDate+"至"+endDate;
				showMap.put("startEndDate"+i,startEndDate);//何年月至何年月（开始+至+结束）
				var workOrgan = records[i].get("workOrgan");
				showMap.put("workOrgan"+i,workOrgan);//在何地区何单位
				var duty = records[i].get("duty");
		...
		somLegalPeopleDataSet.on("load",function(){
			//somLegalPeopleDataSet
			if(somLegalPeopleDataSet.getCount()>0){
				var name = somLegalPeopleDataSet.get("name");//姓    名
				var sex = somLegalPeopleDataSet.get("sex")==1?"男":"女";//性别
				var politics = changePolitics(somLegalPeopleDataSet.get("politics"));//政治面貌
				var education = changeEducation(somLegalPeopleDataSet.get("education"));//文化程度
				var birthday = somLegalPeopleDataSet.get("birthday");//出生日期
				var idCard = somLegalPeopleDataSet.get("idCard");//身份证号
				var workDuties = somLegalPeopleDataSet.get("workDuties");//职    务
				var profession = somLegalPeopleDataSet.get("profession");//技术职称
				var populace = somLegalPeopleDataSet.get("populace");//户口所在地
				var adds = somLegalPeopleDataSet.get("adds");//家庭住址
				var postCode = somLegalPeopleDataSet.get("postCode");//邮编
				var phone = somLegalPeopleDataSet.get("phone");//电话
				var personUnit = somLegalPeopleDataSet.get("personUnit");//人事关系所在单位
				var personUnitPhone = somLegalPeopleDataSet.get("personUnitPhone");//电话
				var folkCode = somLegalPeopleDataSet.get("folk");//民族(code)
