...
function init(){  
	//取得文件名 和标签    
	var nowFileName = "/jsp/ext/somsd/manage/printWord/ungov/doc/ungovBuildIndividualWord.doc";
	...
	somApplyDataSet.on("load",function(){
		...
		somHeldOrganDataSet.on("load",function(){
			...
			somHeldPeopleDataSet.on("load",function(){
				...
				somDutyDataSet.setParameter("SORG_DUTIES@null","1");//随便赋值，就能去掉法人（法人是00012，其他为空）
				somDutyDataSet.load();
				somDutyDataSet.on('load',function(){
					...
					somOfficeDataSet.on("load",function(){
						...
						LegalPeopleDataSet.on("load",function(){
							...
							var borgPeople = somApplyDataSet.get("borgPeople");//单位负责人
							var borgPhone = somApplyDataSet.get("borgPhone");//电话
							...
							showMap.put("borgPeople",borgPeople);
							showMap.put("borgPhone",borgPhone);
