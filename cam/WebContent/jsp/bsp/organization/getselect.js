	...
	if( expandAll === 'true') {
	 ...
	 tree.root.expand();
		...
		if(locateOrganId!=null&&locateOrganId!="undefined"){
              ...
              if(!command.error){
            	var rtnLocatePath = command.getReturn("locatePath");
            	var locatePath;
            	if(rtnLocatePath){
            		locatePath="/"+ rtnLocatePath;
            		  var rootIndex=locatePath.indexOf("/"+rootid+"/");
                      if(rootIndex==-1){
         				   rootid=1;
         				}
...
var rootdata = function() {
	...
	var choiceTypes=choiceOrganType.split(',');
	for(var i=0;i<choiceTypes.length;i++){
		var type=choiceTypes[i];
		if(type==checkorganType||(checkorganType.substring(0,1))==type){
			checkorganId=rootRecord.organId;
			break;
		}
	}
	if(i>=choiceTypes.length){
		checkorganId="";
	}
   return {
...
function  confirmValue(){
	...
	if(organId !="" && organName !=""){
	   if(returnValueType=="array"){
	     ...
	     window.returnValue=organId+";"+organName+";"+struId+";"+deptId+";"+deptName+";"+corpId+";"+corpName+";"+organType;
	   }
		window.close();
	}else{
