...
function confirmValue(){
	...
	var operationCodeArr=new Array();
	var operationNameArr=new Array();

	if(functionCode!="" && functionName!=""){
		for(var i=0;i<functionCode.length;i++){
			var command=new L5.Command("org.loushang.bsp.permit.pap.function.cmd.OperationCommand");
			command.setParameter("pub_operations.FUNCTION_CODE@=",functionCode[i]);
			command.execute("getOperationByFunctionId");
			
			var operationRecords=command.getReturn("operationRecords");
			for(var j=operationNameArr.length,k=0;k<operationRecords.length;k++,j++){
				operationCodeArr[j]=operationRecords[k].operationCode;
				operationNameArr[j]=operationRecords[k].operationName;
			}
		}
	}
	...
	if(operationCode!="" && operationName!=""){
		var ocArrlength=operationCodeArr.length
		if(ocArrlength>0){
			arr[0]= operationCodeArr;
			arr[1]=operationNameArr;
		
		for(var i=ocArrlength;i<(ocArrlength+operationCode.length);i++){
			arr[0][i]=operationCode[i-ocArrlength];
			arr[1][i]=operationName[i-ocArrlength];
		}
		}
		else{
			arr[0]=operationCode;
