...
function init() {
	...
	tpDataset.on('load',function(){
		if (method=="INSERT"){
			var command=new L5.Command("org.loushang.bsp.permit.pap.function.cmd.AppCommand");
			command.execute("getSubordinateCount");
			var seq =command.getReturn("seq");
			ds.newRecord({"seq":seq});
...
function testCode(){
	...
	var cmd = new L5.Command('org.loushang.bsp.permit.pap.function.cmd.AppCommand');
