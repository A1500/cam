...
function init() {
	...
	if (method=="INSERT"){
		...
		command.setParameter("parentId",functionCode);
		command.execute("getSubordinateCount");
		var seq =command.getReturn("seq");
		operationDataset.newRecord({"functionCode":functionCode,"functionName":functionName,"seq":seq});
...
function testCode(){
	...
	var cmd = new L5.Command('org.loushang.bsp.permit.pap.function.cmd.OperationCommand');
...
//设置一条url为默认url
function setDefault(){
	var gridPanel = L5.getCmp("urlGridPanel");
	var selects = gridPanel.getSelectionModel();
	var rcd = selects.getSelections();
	if(rcd.length == 0){
		L5.MessageBox.alert("警告","必须选中一条记录");	
		return;
	}
	if(rcd.length > 1){
		L5.MessageBox.alert("警告","只能选择一条记录");	
		return;
	}
	var url = rcd[0].get('urlContent');
	if(! /.jsp$/.test(url)) {
