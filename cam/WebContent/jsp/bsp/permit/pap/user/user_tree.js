...
//	var win = window.frames["appFrame"].window;
    ...
    var url;
	url = "user_edit.jsp?method=UPDATE"
	url+="&primeKey="+encodeURIComponent(encodeURIComponent(record.get("userId")));
	url+="&struId="+encodeURIComponent(encodeURIComponent(parentRecord.get("struId")));
	frame.setAttribute("src", url);
//	//如果之前显示的页面也是维护页面，点击其他用户节点时不再刷新页面只加载用户数据
//	if(frame.src && (frame.src.indexOf("user_edit.jsp?method=UPDATE") != -1)){
//		window.frames["nodeFrame"].window.loadUser(record.get("userId"));
//	} else {
//		frame.setAttribute("src", url);
//	}
}
...
//导出用户
function  exportUser(menuitem){
	// TODO
	alert("导出用户");
}

//导入用户
function  importUser(menuitem){
	// TODO
	alert("导入用户");
}
