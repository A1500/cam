...
function testCode(){
	...
	var cmd = new L5.Command('org.loushang.bsp.eai.pap.function.cmd.ModuleCommand');
	...
	if(hasCode != 'n'){
		page.hasCode = true;
		document.getElementById('moduleHint').innerHTML = '*业务组件编码:'+usingCode+'已被使用';
		return;
	}else{
		page.hasCode = false;
...
function save(){
	...
	if(page.hasCode===true){
		L5.MessageBox.alert(RES_PROMPT,'业务组件编码已经被使用');
		return;
	}
	var valid_name_reg = /[#&\?<>\\{}()*\-+\/]/;
	...
	if(reg_result !== null){
		L5.MessageBox.alert(RES_PROMPT,'业务组件编码包含非法字符:'+reg_result[0]);
		return;
	}
	reg_result = valid_name_reg.exec(rcd.get('moduleName'))
	if(reg_result !== null){
		L5.MessageBox.alert(RES_PROMPT,'业务组件名称包含非法字符:'+reg_result[0]);
		return;
	}
	var command=new L5.Command("org.loushang.bsp.eai.pap.function.cmd.ModuleCommand");
