...
function init() {
	...
	var sysds = L5.DatasetMgr.lookup("sysds");
	sysds.setParameter("SYS_SIGN","1");
	sysds.load(true);
	if(sysds.getCount()==0)
		sysds.newRecord();
	//L5.QuickTips.init();//?
...
function clickSave(){
	 var i=0;	
	 var osSign=document.getElementById("ssSign");
	 var ofSign=document.getElementById("sfSign");
    ...
    if(i!=0){
   	 return false;
		 ...
		 command.setParameter("sysSign","1");	 
