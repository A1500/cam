...
function saveClickAfter() {
	...
	if(!(checkInteger(infoDs.getCurrent().get("approvalRebuiledHouse")))) {
		L5.Msg.alert('提示',"核准重建间数必须为整数！");
		return false;
	}
	if(!(checkInteger(infoDs.getCurrent().get("approvalRepairedHouse")))) {
		L5.Msg.alert('提示',"核准修缮间数必须为整数！");
		return false;
	}
