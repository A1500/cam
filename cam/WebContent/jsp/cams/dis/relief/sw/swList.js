...
function query() {
	infoDs.setParameter("BATCH_DETAIL_ID", batchDetailId);
	infoDs.setParameter("DEAD_NAME@rlike", getParam("qDeadName"));
	infoDs.setParameter("ACCEPT_AREA_CODE", getParam("qAcceptAreaCode"));
	infoDs.load();
}
//进入追加页面
function add() {
	var url = 'swEdit.jsp?method=insert&batchId=' + batchId + '&batchDetailId='
			+ batchDetailId;
	var width = 900;
	var height = 580;
...
//修改档案信息
function edit() {
	var editGrid = L5.getCmp('swGrid');
	var selected = editGrid.getSelectionModel().getSelections();
	if (selected.length != 1) {
		L5.Msg.alert('提示', "请选择一条记录修改!");
		return false;
	}
	var url = 'swEdit.jsp?method=update&batchId=' + batchId + '&batchDetailId='
...
function detail() {
	...
	var url = 'swDetail.jsp?method=detail&batchId=' + batchId + '&batchDetailId='
			+ batchDetailId+'&infoId='+selected[0].get('infoId');
	var width = 900;
	var height = 580;
	var win = window.showModalDialog(url, window,
...
function forExcel(){
	...
	L5.dataset2excel(dataset, "/jsp/cams/dis/relief/sw/swListExcel.jsp");
}
//导入Excel
function forImport(){
