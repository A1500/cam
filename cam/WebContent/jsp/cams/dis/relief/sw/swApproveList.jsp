...
<script type="text/javascript" src="swApproveList.js"></script>
</head>
<body>
<model:datasets>
	<!-- 档案信息 -->
		...
		<next:TabPanel>
			<next:Tabs>
				<next:Panel title="分析汇总" width="100%" height="100%">
					<next:Html>
						<iframe id="swSumList" name="swSumList"
							src='swApproveSumList.jsp?batchId=<%=request.getParameter("batchId")%>&batchDetailId=<%=request.getParameter("batchDetailId")%>&ifApprove=<%=request.getParameter("ifApprove")%>'
			...
			<next:Tabs>
				<next:Panel title="明细信息" width="100%" height="100%">
					<next:Html>
						<iframe id="swDetailList" name="swDetailList"
							src='swList.jsp?batchId=<%=request.getParameter("batchId")%>&batchDetailId=<%=request.getParameter("batchDetailId")%>'
							frameborder="no" border="0" marginwidth="0" marginheight="0"
							scrolling="no" allowtransparency="yes" width="100%" height="100%">
						</iframe>
					</next:Html>
			...
			<next:Tabs>
				<next:Panel title="审批流程" width="100%" height="100%">
					<next:Html>
						<iframe id="swWorkFlowList" name="swWorkFlowList"
							src='swWorkFlow.jsp?batchId=<%=request.getParameter("batchId")%>&batchDetailId=<%=request.getParameter("batchDetailId")%>'
							frameborder="no" border="0" marginwidth="0" marginheight="0"
							scrolling="no" allowtransparency="yes" width="100%" height="100%">
						</iframe>
					</next:Html>
