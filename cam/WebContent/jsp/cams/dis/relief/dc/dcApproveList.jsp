...
<script type="text/javascript" src="dcApproveList.js"></script>
</head>
<body>
<model:datasets>
	<!-- 档案信息 -->
		...
		<next:TabPanel>
				...
				<next:Panel title="分析汇总" width="100%" height="100%">
					<next:Html>
						<iframe id="dcSumList" name="dcSumList"
							src='dcApproveSumList.jsp?batchId=<%=request.getParameter("batchId")%>&batchDetailId=<%=request.getParameter("batchDetailId")%>&ifApprove=<%=request.getParameter("ifApprove")%>'
			...
			<next:Tabs>
				<next:Panel title="明细信息" width="100%" height="100%">
					<next:Html>
						<iframe id="dcDetailList" name="dcDetailList"
							src='dcList.jsp?batchId=<%=request.getParameter("batchId")%>&batchDetailId=<%=request.getParameter("batchDetailId")%>'
							frameborder="no" border="0" marginwidth="0" marginheight="0"
							scrolling="no" allowtransparency="yes" width="100%" height="100%">
						</iframe>
					</next:Html>
			...
			<next:Tabs>
				<next:Panel title="审批流程" width="100%" height="100%">
					<next:Html>
						<iframe id="dcWorkFlowList" name="dcWorkFlowList"
							src='dcWorkFlow.jsp?batchId=<%=request.getParameter("batchId")%>&batchDetailId=<%=request.getParameter("batchDetailId")%>'
							frameborder="no" border="0" marginwidth="0" marginheight="0"
							scrolling="no" allowtransparency="yes" width="100%" height="100%">
						</iframe>
					</next:Html>
