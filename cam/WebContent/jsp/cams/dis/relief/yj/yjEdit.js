...
function saveClickAfter() {
	...
	if(!(checkInteger(infoDs.getCurrent().get("approvalWater")))) {
		L5.Msg.alert('提示',"核准救助饮水必须为整数");
		return false;
	}
...
// 自动添加核准救助饮水
function appWater(item) {
	document.getElementById("approvalWater").value = item.value;
	infoDs.set("approvalWater", item.value);
}
