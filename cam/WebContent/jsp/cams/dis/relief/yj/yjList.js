...
//进入追加页面
function add() {
	var url = 'yjEdit.jsp?method=insert&batchId=' + batchId + '&batchDetailId='
			+ batchDetailId;
	var width = 900;
	var height = 580;
...
function edit() {
	...
	var url = 'yjEdit.jsp?method=update&batchId=' + batchId + '&batchDetailId='
...
function detail() {
	...
	var url = 'yjDetail.jsp?method=detail&batchId=' + batchId + '&batchDetailId='
			+ batchDetailId+'&infoId='+selected[0].get('infoId');
	var width = 900;
	var height = 580;
	var win = window.showModalDialog(url, window,
...
function forExcel(){
	...
	L5.dataset2excel(dataset, "/jsp/cams/dis/relief/yj/yjListExcel.jsp");
}
//导入Excel
function forImport(){
