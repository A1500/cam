...
function query(){
	...
	if(getParam("fillState")){
		...
		batchDetailds.load();
			...
			batchDetailds.setParameter("OR@FILL_STATE@=",'3');	
			batchDetailds.load();
	}
}
...
function deal(){
	...
	var doleflag="0";
	if(selected[0].get("GRANT_TYPE")=="01"&&organType == 13){
		doleflag="1";
	}
	if(selected[0].get("GRANT_TYPE")=="02"&&organType == 14){
		doleflag="1";
	}
	var url="";
	if(selected[0].get("FILL_STATE")=='3'){//3:已审批,
		var url="grant_edit.jsp?reliefType="+selected[0].get("RELIEF_TYPE")+"&batchDetailId="+selected[0].get("BATCH_DETAIL_ID")+"&doleflag="+doleflag;
	}
	var width = 1024;
	var height = 768;
