
<%@ page contentType="text/html; charset=UTF-8" %>
<%@ taglib uri="/tags/next-web" prefix="next"%>
<%@ taglib uri="/tags/next-model" prefix="model"%>
<html>
	<head>
		<title>DIS_FUNDS_ALLOCATION_DETAIL编辑</title>
		<next:ScriptManager/>
		<script>
			var idField='<%=request.getParameter("dataBean")%>';
			var method='<%=request.getParameter("method")%>';
		</script>
		<script type="text/javascript" src="disfundsallocationdetail_edit.js"></script>
	</head>
<body>
<model:datasets>
	<model:dataset id="ds" cmd="com.inspur.cams.dis.base.cmd.DisFundsAllocationDetailQueryCommand" global="true">
		<model:record fromBean="com.inspur.cams.dis.base.dao.DisFundsAllocationDetail"></model:record>
	</model:dataset>
</model:datasets>
<next:ViewPort>
<next:AnchorLayout>
<next:Panel  title="DIS_FUNDS_ALLOCATION_DETAIL">
	<next:TopBar>
		<next:ToolBarItem symbol="->" ></next:ToolBarItem>
		<next:ToolBarItem iconCls="save"  text="保存" handler="save"/>
		<next:ToolBarItem iconCls="undo"  text="返回" handler="back"/>
	</next:TopBar>
	<next:Html>
		<form id="editForm"   dataset="ds" onsubmit="return false" style="padding: 5px;" class="L5form">
		<table border="1"  width="100%" >
		
	   		<tr>
				<td  class="FieldLabel" >ID</td>
				<td class="FieldInput"><input type="text" name="id" field="id"  /><font color="red">*</font> </td>
			
				<td  class="FieldLabel" >DIS_YEAR</td>
				<td class="FieldInput"><input type="text" name="disYear" field="disYear"  /> </td>
			</tr>
	   		<tr>
				<td  class="FieldLabel" >ALLOCATION_DATE</td>
				<td class="FieldInput"><input type="text" name="allocationDate" field="allocationDate"  /> </td>
			
				<td  class="FieldLabel" >ALLOCATION_NUM</td>
				<td class="FieldInput"><input type="text" name="allocationNum" field="allocationNum"  /> </td>
			</tr>
	   		<tr>
				<td  class="FieldLabel" >ALLOCATION_TOTAL</td>
				<td class="FieldInput"><input type="text" name="allocationTotal" field="allocationTotal"  /> </td>
			
				<td  class="FieldLabel" >ALLOCATION_TYPE</td>
				<td class="FieldInput"><input type="text" name="allocationType" field="allocationType"  /> </td>
			</tr>
	   		<tr>
				<td  class="FieldLabel" >ALLOCATION_PURPOSE</td>
				<td class="FieldInput"><input type="text" name="allocationPurpose" field="allocationPurpose"  /> </td>
			
				<td  class="FieldLabel" >ALLOCATE_ORGAN_NAME</td>
				<td class="FieldInput"><input type="text" name="allocateOrganName" field="allocateOrganName"  /> </td>
			</tr>
	   		<tr>
				<td  class="FieldLabel" >ALLOCATE_ORGAN_CODE</td>
				<td class="FieldInput"><input type="text" name="allocateOrganCode" field="allocateOrganCode"  /> </td>
			
				<td  class="FieldLabel" >ALLOCATION_AMOUNT</td>
				<td class="FieldInput"><input type="text" name="allocationAmount" field="allocationAmount"  /> </td>
			</tr>
	   		<tr>
				<td  class="FieldLabel" >NOTE</td>
				<td class="FieldInput"><input type="text" name="note" field="note"  /> </td>
			
				<td  class="FieldLabel" >FILL_ORGAN_CODE</td>
				<td class="FieldInput"><input type="text" name="fillOrganCode" field="fillOrganCode"  /><font color="red">*</font> </td>
			</tr>
	   		<tr>
				<td  class="FieldLabel" >FILL_ORGAN_NAME</td>
				<td class="FieldInput"><input type="text" name="fillOrganName" field="fillOrganName"  /> </td>
			
				<td  class="FieldLabel" >FILL__PEOPLE_NAME</td>
				<td class="FieldInput"><input type="text" name="fillPeopleName" field="fillPeopleName"  /> </td>
			</tr>
	   		<tr>
				<td  class="FieldLabel" >FILL_TIME</td>
				<td class="FieldInput"><input type="text" name="fillTime" field="fillTime"  /> </td>
			
				</tr>				
		</table>
		</form>
   </next:Html>
</next:Panel>
</next:AnchorLayout>
</next:ViewPort>
</body>
</html>
