...
function init() {
	...
	disFundsSourceDepartmentDs.setParameter("sourceType", "0");
...
function computeSum() {
	...
	document.getElementById("amountLblCd").innerHTML = formatNum(sumTotal+"");
	var cd = document.getElementById("amountLblCity").innerHTML;
	if(!isNaN(cd) && cd!='') {
		var at = 0;
		try{
			at = parseFloat(cd);
