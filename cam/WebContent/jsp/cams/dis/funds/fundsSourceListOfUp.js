...
function init() {
	...
	// 如果不是省级用户，不显示“部级发放”
	if (organType!="11") {
		L5.getCmp("addUpbtn").setVisible(false);
		L5.getCmp("editUpbtn").setVisible(false);
		L5.getCmp("deleteUpbtnId").setVisible(false);
	}
...
function queryAllocation() {
	...
	if(organType=="11"){
		allocationDs.setParameter("FILL_ORGAN_CODE", "000000000000");
	}else{
	...
	allocationDs.setParameter("UP_ALLOCATION_NUM@like",document.getElementById("cx_upAllocationNum_up").value);//上级拨款文号(仅上级有)
...
function editUp() {
	...
	if(organType=="11"){
		//加载拨款方向（拨省级、拨青岛）
		if(selected[0].get("sourceType")=="0"&&selected[0].get("receiveOrganCode")==organCode){
			document.getElementById("fundDirect").value = "1";
		}else if(selected[0].get("sourceType")=="0"&&selected[0].get("receiveOrganCode")=="370200000000"){
			document.getElementById("fundDirect").value = "0";
		}
	}
}
//上级，本级，社会公用删除
...
function detailUp() {
	...
	if(organType=="11"){
		//加载拨款方向（拨省级、拨青岛）
		if(selected[0].get("sourceType")=="0"&&selected[0].get("receiveOrganCode")==organCode){
			document.getElementById("fundDirectDetail").innerHTML = "拨付全省（除青岛）";
		}else if(selected[0].get("sourceType")=="0"&&selected[0].get("receiveOrganCode")=="370200000000"){
			document.getElementById("fundDirectDetail").innerHTML = "拨付青岛";
		}
	}
}
// 确定保存
function confirmSave() {
	var singleSourceDs = L5.DatasetMgr.lookup("singleSourceDs");
	var fundDirect=document.getElementById("fundDirect").value;
	// 校验
	...
	command.setParameter("fundDirect", fundDirect);
...
function showDetailWin(_title) {
	var sourceDetailWin;
	if(organType=="11") {
		sourceDetailWin = L5.getCmp("sourceDetailWin");
	}else if(organType=="12") {
		sourceDetailWin = L5.getCmp("sourceDetailWin");
	}else if(organType=="13") {
		sourceDetailWin = L5.getCmp("sourceDetailWinC");
	}else if(organType=="14") {
		sourceDetailWin = L5.getCmp("sourceDetailWinT");
	}
