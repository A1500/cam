...
function initTown(value, cellmeta, record, rowindex, colindex,dataset) {
	...
	var idx=[] ;
	zhiguanNum[0]=0;
	if(lowerAllocationDs.getCount()!=0){
		for(var j=0;j<lowerAllocationDs.getCount();j++){
			idx[j]= lowerAllocationDs.query("upAllocationNum",record.get("upAllocationNum"));
			for(var i=0;i<idx[j].length;i++){
				if(idx[j].items[i]!=""||idx[j].items[i]!=null){
					zhiguanNum[j]+=idx[j].items[i].get("amount");
				}
			}
			return formatMoneyForDis(zhiguanNum[j]);
		}
	}else{
		return formatMoneyForDis(0);
	}
}