...
<script type="text/javascript" src="fundsSourceListOfCon.js"></script>
...
<model:datasets>
	...
	<model:dataset id="contributeDs"
		...
		sortField="DIS_YEAR DESC,ALLOCATION_TIME DESC,FILL_TIME">
...
<!--	社会捐赠		-->
<next:Panel id="contributePanel" name="contributePanel" width="100%"
	height="100%" title="社会捐赠">
		...
		<next:Html>
			...
			<table width="100%">
				<tr>
					...
					<td class="FieldInput"><input type="text" id="cx_disYear_con"
						class="TextEditor" title="年度" maxlength="4" /></td>
					<td class="FieldLabel" nowrap>资金用途：</td>
					<td class="FieldInput"><select id="cx_upAllocationPurpose_con"
				...
				<tr>
					...
					<button onclick="queryCon()">查 询</button>
					&nbsp;
	...
	<next:GridPanel id="contributeGridPanel" dataset="contributeDs"
		autoExpandColumn="upAllocationPurpose" name="contributeGridPanel"
		width="100%" width="99.9%" height="100%" notSelectFirstRow="true"
		hasSum="true">
		<next:TopBar>
			<next:ToolBarItem text="社会捐赠资金管理"></next:ToolBarItem>
			<next:ToolBarItem symbol="->"></next:ToolBarItem>
			<next:ToolBarItem iconCls="add" text="新增" handler="addContribute" />
			<next:ToolBarItem iconCls="edit" text="修改" handler="editContribute" />
			<next:ToolBarItem id="deleteContributeId" iconCls="delete" text="删除"
				handler="deleteComm" />
			<next:ToolBarItem iconCls="detail" text="明细"
				handler="detailContribute" />
			<next:ToolBarItem iconCls="search" text="追踪"
				handler="followContribute" />
		</next:TopBar>
		<next:Columns>
			...
			<next:Column id="amountBalance" header="结余金额(万元)"
				field="amountBalance" align="right" width="90" renderer="formatMoneyForDis"
				summaryType="sum">
				<next:ExtendConfig>
	                    	summaryRenderer : formatMoneyForDis
	                	</next:ExtendConfig>
		...
		<next:BottomBar>
			<next:PagingToolBar dataset="contributeDs" />
		</next:BottomBar>
	</next:GridPanel>
</next:Panel>
</body>
