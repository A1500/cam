 ...
 */
function init(){
	ds.setParameter("reliefType","SW");//因灾死亡
 ...
 */
function detail(){
	var editGrid = L5.getCmp("disasterDeathSolatiumBatchGrid");//获得控制权
	var selected = editGrid.getSelectionModel().getSelections();//获得选中行
	if(selected.length!=1){
		L5.Msg.alert('提示','请选中一条记录查看！');
		return false;
	}
	var url='jsp/cams/dis/assistance/disasterDeathSolatium/disasterDeathSolatiumList.jsp';
	var text='救助批次明细';
	var data=new L5.Map();
