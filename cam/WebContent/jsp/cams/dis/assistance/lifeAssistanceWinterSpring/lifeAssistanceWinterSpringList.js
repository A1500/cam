...
function detail(){
	...
	var cx_infoId = selected[0].get('INFO_ID');
	var cx_recordId = selected[0].get('RECORD_ID');
	var url='lifeAssistanceWinterSpringDetail.jsp?cx_infoId='+cx_infoId+'&cx_recordId='+cx_recordId;
	var width = 900;
	var height = 580;
	var win = window.showModalDialog (url,window,"scroll:yes;status:no;dialogWidth:"+width+"px;dialogHeight:"+height+"px;resizable:0");
}
...
function printPeople(){
	...
	printDs.on("load",function(){
		var printTitle = "";
		printReport(getPrintDataByDataSet(printTitle,printDs),L5.webPath+'/jsp/cams/dis/assistance/dcPrint.fr3');
	});
}
