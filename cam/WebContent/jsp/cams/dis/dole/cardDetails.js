...
function editInfo() {
	...
	if(selected[0].get("doleType")=="2") {
		L5.Msg.alert('提示', "请选择一条资金发放记录进行修改!");
		return false;
	}
...
function deleteInfo() {
	...
	if(selected[0].get("doleType")=="2") {
		L5.Msg.alert('提示', "请选择一条资金发放记录进行删除!");
		return false;
	}
...
// 渲染发放类型
function doleTypeRenderer(value, cellmeta, record, rowindex, colindex, dataset) {
