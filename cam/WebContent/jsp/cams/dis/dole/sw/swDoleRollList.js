...
function hideYh(at) {
	if (at == "0") {
		var swGrid = L5.getCmp("swGrid");
		swGrid.getColumnModel().setHidden(5, true);
		swGrid.getColumnModel().setHidden(6, true);
	}
}
//当标识为明细页面，进行初始化
function initDetail() {
	if (ifDetail == "T") {
		...
		var swGrid = L5.getCmp("swGrid");
		swGrid.getColumnModel().setEditable(8, false);
	}
}
//保存后列表选择效果
function defSelected() {
	var winGrid = L5.getCmp("swGrid");
...
function save() {
	...
	var swGrid = L5.getCmp("swGrid");
	var selected = swGrid.getSelectionModel().getSelections();
...
function onSaveConfirm() {
	...
	var swGrid = L5.getCmp("swGrid");
	var infoSelected = swGrid.getSelectionModel().getSelections();
...
function forExcel(){
	...
	L5.dataset2excel(dataset, "/jsp/cams/dis/dole/sw/swExcel.jsp?allocationType="+allocationType);
}
// 打印救助卡
function printCard() {
}
