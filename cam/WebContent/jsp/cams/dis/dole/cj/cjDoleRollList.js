...
function hideYh(at) {
	if (at == "0") {
		var cjGrid = L5.getCmp("cjGrid");
		cjGrid.getColumnModel().setHidden(5, true);
		cjGrid.getColumnModel().setHidden(6, true);
	}
}
//当标识为明细页面，进行初始化
function initDetail() {
	if (ifDetail == "T") {
		...
		var cjGrid = L5.getCmp("cjGrid");
		cjGrid.getColumnModel().setEditable(10, false);
	}
}
//保存后列表选择效果
function defSelected() {
	var winGrid = L5.getCmp("cjGrid");
...
function save() {
	...
	var cjGrid = L5.getCmp("cjGrid");
	var selected = cjGrid.getSelectionModel().getSelections();
...
function onSaveConfirm() {
	...
	var cjGrid = L5.getCmp("cjGrid");
	var infoSelected = cjGrid.getSelectionModel().getSelections();
...
function forExcel(){
	...
	L5.dataset2excel(dataset, "/jsp/cams/dis/dole/cj/cjExcel.jsp?allocationType="+allocationType);
}
// 打印救助卡
function printCard() {
}
