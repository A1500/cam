...
function hideYh(at) {
	if (at == "0") {
		...
		dcGrid.getColumnModel().setHidden(5, true);
		dcGrid.getColumnModel().setHidden(6, true);
	}
}
//当标识为明细页面，进行初始化
function initDetail() {
	if (ifDetail == "T") {
		...
		var dcGrid = L5.getCmp("dcGrid");
		dcGrid.getColumnModel().setEditable(8, false);
	}
}
//保存后列表选择效果
...
function forExcel(){
	...
	L5.dataset2excel(dataset, "/jsp/cams/dis/dole/dc/dcExcel.jsp?allocationType="+allocationType);
}
// 打印救助卡
function printCard() {
}
