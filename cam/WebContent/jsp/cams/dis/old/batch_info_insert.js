...
function save() {
	...
	if(batchds.getCurrent().get("year")>2013||batchds.getCurrent().get("year")==2013){
		L5.Msg.alert('提示','补录只针对于2013年之前的数据，请慎用！');
		return false;
	}
...
function forOrganSelect() {
	var revalue = window.showModalDialog("../createBatch/dicCity.jsp?organCode=" + organCode
