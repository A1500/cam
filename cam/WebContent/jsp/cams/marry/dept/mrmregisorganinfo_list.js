...
function query1(){
	...
	ds.setParameter("MRM_REGIS_ORGAN_INFO.ORGAN_ID@=",getParam("organId"));		
	ds.setParameter("MRM_REGIS_ORGAN_INFO.TYPES@=",getParam("types"));		
	ds.setParameter("MRM_REGIS_ORGAN_INFO.SSLEVEL@=",getParam("sslevel"));		
	ds.load();
}
...
function insert() {
	...
	var url='jsp/cams/marry/dept/mrmregisorganinfo_edit.jsp';
	var text = '增加登记机关信息表';
...
function update(){
	...
	data.put("dataBean",selected[0].get('organId'));
	var url='jsp/cams/marry/dept/mrmregisorganinfo_edit.jsp';
	var text = '修改登记机关信息表';
	L5.forward(url,text,data);
}
	...
	L5.MessageBox.confirm('确定', '确定要删除选中的记录吗?',function(state){
		if(state=="yes"){
			...
			for(var i=0;i<selected.length;i++){
				delIds[i]=selected[i].get("organId");
			}
...
function queryDetails() {
	...
	//showModelessDialog window
	window.open(
					"mrmregisorganinfo_detail.jsp?method=DETAILS&dataBean=" + organId + "&code=" + deptCode,
					"_blank",
					"scroll:yes;status:no;resizable:no;unadorned:yes;dialogTop:0px;dialogLeft:0px;");
