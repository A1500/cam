...
function init(){  
	//取得文件名 和标签    
	var nowFileName = "mrmQuitPrint.doc";
	//打开文件
	...
	applyDataSet.on('load',function (){
		if( applyDataSet.getCount() > 0 ){
			...
			showMap.put("skinValue",getLevel(record.get('applyLevel')));
			showMap.put("skinValue1",getLevel(record.get('applyLevel')));
			...
			showMap.put("reason",record.get('reason'));
			showMap.put("townOpinion",record.get('townOpinion'));
			showMap.put("townAuditTime",getDayFormat(record.get('townAuditTime')));
			...
			showMap.put("townOrganName",record.get('townOrganName'));
