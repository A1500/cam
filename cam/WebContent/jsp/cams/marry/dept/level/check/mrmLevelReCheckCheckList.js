...
function queryDaiBan(){
	...
	daiBanDataSet.setParameter("APPLY_TYPE@IN",2);
	daiBanDataSet.load();
	daiBanDataSet.on("load",function(ds){
		...
		for(var i=0;i<ds.getCount();i++){
			...
			if(organType=="13"){
				...
				if(townAuditTime>provinceAuditTime){
				...
				if(cityAuditTime>departmentAuditTime){
					record.set("lastOrganName",cityOrganName);
					record.set("lastAuditTime",cityAuditTime);
				}else{
...
function queryYiBan(){
	...
	yiBanDataSet.setParameter("APPLY_TYPE@IN",2);
	yiBanDataSet.load();
...
function queryBanJie(){
	...
	banJieDataSet.setParameter("APPLY_TYPE@IN",2);
	banJieDataSet.load();
}
