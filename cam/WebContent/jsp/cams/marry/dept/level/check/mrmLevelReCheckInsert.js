...
function init() {
    ...
    ds.setParameter("DEPT_CODE@=",deptId);
 	...
 	ds.on("load",function(){
 		...
 		//处理机构名称
 		ds.getAt(0).set("deptName",cityName+ds.getAt(0).get("deptName"));
		...
		if(method=="UPDATE"){
			...
			applyDs.newRecord({"applyType":"2","applyTime":curDate});
			//加载电子档案
	...
	tabpanel.on("activate",function(){
		var skinValue =queryDs.getAt(0).get("skinValue");
		condition(skinValue);
		//writeTable();
	});
...
function checkLevel(){
	var applyLevel=queryDs.getCurrent().get("skinValue");
 	//候登大厅、结婚登记区
 ...
 //	var zfgmSpan=$("zfgmSpan").style.display=="none";//是否政府购买
	...
	standardDs.on("load",function(){
				...
				var con2=parseInt(record.get("con2"));
 				if(ds.getAt(0).get("foreign")=="0"){//办理内地居民婚姻登记的机关
			 		if(deptNumRy>=con1 && deptNumRy<con2 && parseInt(organCode.substring(4,6))<20){//判断市辖区
				 		...
				 		var r=deptNumRy;
				 		if(!eval(famula)){
						 	var div='<font color="green">'+"(注:"+'</font>'+"目前情况：辖区户籍人口"+deptNumRy+"万人，婚姻登记员"+djyNum+"名。";
				 			$("djyDiva").innerHTML=div;
				 			message+=div+"<br>";
			 			}
			 		}
			...
			if(i==28){
				...
				if(!n){
			  	...
			  	if(!eval(famula)){	
				 	...
				 	if(blsj){
				 		div="(注:"+note+")；"+'</font>'+"目前情况：婚姻登记历史数据已补录至"+blsj+"。";
				 	}
	    ...
	    for (var i = 0; i < message3aArr.length-1; i++){
 		...
 		if(message!=""){
 			wShow(ds.getAt(0).get("deptName")+"申请"+getDicText(evaluLevelDataSet,applyLevel)+"婚姻登记机关<br>不符合标准项如下：<br>",message3aText,
 			myDate.toLocaleDateString());
 			return false;
 		}else{
 			alert("符合"+getDicText(evaluLevelDataSet,applyLevel)+"婚姻登记机关评定标准，可打印申报表！");
 		}
 	});		 	
}
function printWord(){
...
function save_win(){	
	...
	if (!command.error) {
		...
		ds.getAt(0).get("deptName")+"申请"+getDicText(evaluLevelDataSet,applyLevel)+"婚姻登记机关"))
...
function setApplyRecord(){
	...
	// 申请等级
	applyRecord.set("applyLevel",applyLevel);
	// 对外人工或语音咨询电话
	...
	// 历史数据补录至年份
	applyRecord.set("hisDataDate",getDateLen(record.get("blsj"),4));
	// 数据量
	...
	applyRecord.set("townOrganName",cityName+organName);		
...
/*function basePerson(){
	...
	basePersonDs.setParameter("MRM_BASE_PERSON.NAME@not like","%婚姻%");
	basePersonDs.load();
	basePersonDs.on('load',function(){
		for(var i=0;i<basePersonDs.getTotalCount();i++){
			var bpRecord=basePersonDs.getCurrent();
			var personId=bpRecord.get("personId");
			url="../../../person/mrm_baseperson_print.jsp?personId="+personId;
			window.showModalDialog(url,window);
			var activeDocument=request.getParameter("activeDocument");
			window.close();
		    activeDocument.saveAs("E:\a.doc");
		}
	});
}*/
var flag=true;
//判定婚姻登记机关等级评定标准是否符合
function condition(skinValue){
	if(method=="UPDATE"){
		compareDs.setParameter("APPLY_ID@=",applyId);
		if(compareDs.getCount()>0){
	 		return;
	 	}
		compareDs.load();
		...
		//var queryDs=L5.DatasetMgr.lookup("queryDs");
		//var applyLevel=queryDs.getAt(0).get("skinValue");
		var applyLevel= skinValue;
...
function writeTable(){
	...
	var r = t.insertRow();
	 ...
	 t2.align="center";
	 var t3=r.insertCell();
	 t3.innerHTML= $("skinValue").innerText.replace(/(^\s*)|(\s*$)/g,"")+"标准要求";
	 t3.align="left";
	 t3.width="150";
	 var t4=r.insertCell();
	 t4.innerHTML= "实际完成情况";
	...
	for(var i=1;i<=compareDs.getCount();i++){	
		var r = t.insertRow();
		 ...
		 r.insertCell().innerHTML= '<textarea rows="3"cols="40" style="font-size:17px;width:100%;overflow:hidden;border:0" onblur="addActoral(this)">'+
...
function wordcontorl(){ 
	...
	with (myTable.Cell(1,3).Range) { 
		font.Size = 12; 
		InsertAfter($("skinValue").innerText.replace(/(^\s*)|(\s*$)/g,"")+"标准要求"); 
	} 
