...
function handle(){
	...
	data.put("deptId",records[0].get("organCode"));
	...
	if(daiBanDataSet.getCurrent().get("applyType")=='1'){
...
function renderOrganName(value,c,rec){
	return '<a href="javascript:detail(\''+rec.get("applyId")+'\',\''+rec.get('organCode')+'\',\''+rec.get('applyType')+'\',\''+rec.get('curOrganName')+'\')">'+value+'</a>';
}

function detail(applyId,deptId,applyType,curOrganName){
	var data=new L5.Map();
	...
	data.put("organCode",organCode);
	data.put("organName",encodeURIComponent(encodeURIComponent(curOrganName)));
	data.put("organType",organType);
	var url='jsp/cams/marry/dept/level/apply/mrmLevelApplyDetail.jsp';	
	L5.forward(url,"",data);		
}
...
function queryDaiBan(){
	...
	daiBanDataSet.setParameter("APPLY_TYPE@IN",1);
	daiBanDataSet.load();
...
function queryYiBan(){
	...
	yiBanDataSet.setParameter("APPLY_TYPE@IN",1);
	yiBanDataSet.load();
...
function queryBanJie(){
	...
	banJieDataSet.setParameter("APPLY_TYPE@IN",1);
	banJieDataSet.load();
}
