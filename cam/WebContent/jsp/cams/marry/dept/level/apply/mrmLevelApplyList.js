...
function newApply(){	
	...
	applyQueryDataSet.setParameter("APPLY_TYPE",1);
 	...
 	applyQueryDataSet.on("load",function(ds){
		...
		var url='jsp/cams/marry/dept/level/apply/mrmLevelApplyInsert.jsp';
		L5.forward(url,"",data);	
	});	
}
//办理						
...
function detail(applyId,deptId,applyType){
	...
	data.put("organCode",deptId);
	oName = encodeURIComponent(encodeURIComponent(oName));
	data.put("organName",oName);
...
function query(){
	...
	applyDataSet.setParameter("APPLY_TYPE",1);
	applyDataSet.load();
}
