function init(){
	...
	var inHTML = '';
	$("photoDiv").innerHTML="&nbsp;<img id=\"img\" name=\"RELATIONER_PHOTOC_IMG\" src="+src+" align=\"top\""+
...
function del(){
	if(organCode.substring(0,6) != organCodeOld.substring(0,6)){
		L5.Msg.alert("提示","删除失败,不是本婚姻登记处图片！");
		return false;
	}
	L5.MessageBox.confirm('确定', '确定要删除吗?', function(state) {
		if(state == 'yes'){
			...
			command.setParameter("delIds",id);
