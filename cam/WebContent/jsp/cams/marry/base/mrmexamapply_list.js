function init() {
	...
	ndYearDataset.baseParams["filterSql"]=" ks_sign = '0' and ks_yn = '01'";//1:登记员
	ndYearDataset.load();
...
//	apds.load();
	apds.on('load',function(){
		...
		//	document.getElementById("ksAddress1").innerHTML=(records[0].get("ksAdd"));
		//	document.getElementById("ksAddress2").innerHTML=(records[1].get("ksAdd"));
		}
	...
	ksqsDataset.setParameter('filterSql', " ND_YEAR = '" + ndYear + "' and KS_SIGN = '0'  and ks_yn = '01' ");
//	ksqsDataset.load();
	var ksTimeDataset = L5.DatasetMgr.lookup("ksTimeDataset");
	ksTimeDataset.setParameter('filterSql', " ND_YEAR = '" + ndYear + "' and KS_SIGN = '0' and ks_yn = '01'");
	ksTimeDataset.load();
	...
	Personds.setParameter("bzyCode","01");//登记员
	Personds.load();
	//判断人员状态
...
function insert() {
	...
	L5.MessageBox.confirm('确定', applyString+'确定要报名吗?', function(state) {
		if (state == "yes") {
			...
			command.setParameter("ksSign","0");//登记员
			command.execute("mrmexamapply"); //alert(command.getReturn("qstj"));
			...
			if (!command.error) {
				...
				L5.forward("jsp/cams/marry/base/mrmexamapply_list.jsp");
			} else {
...
function commonExamapply() {
	...
	document.getElementById("docpath").value = "jsp/cams/marry/words/mrmexamapply.doc";
	...
	document.getElementById("ksSign").value = "0";
