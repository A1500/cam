...
//	L5.DatasetMgr.lookup("ndYearDataset").load();
	...
	ndYearDataset.baseParams["filterSql"]=" ks_sign = '1'";//1:颁证员
	ndYearDataset.load();
	L5.QuickTips.init();
};
...
function query(){
	...
	ds.setParameter("MRM_EXAM_SCHEDULE.KS_SIGN@=",1);	
	ds.load();
}
...
function insert() {
	...
	var url='jsp/cams/marry/person/mrmexamschedulebzy_edit.jsp';
...
function detail(){
	...
	data.put("method","UPDATE");
	data.put("dataBean",selected[0].get('ksapId'));
	var url='jsp/cams/marry/person/mrmexamschedulebzy_detail.jsp';
	var text = '修改登记员资格考试安排表';
	L5.forward(url,text,data);
}
