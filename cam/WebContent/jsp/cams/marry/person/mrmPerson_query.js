...
function query() {
	...
	if(paramDeptId == null || paramDeptId == "") {
		...
		ds.setParameter("deptId", paramDeptId);
	}
	
	ds.setParameter("sex", getParam("sex"));
	ds.setParameter("culture", getParam("culture"));
	ds.setParameter("politics", getParam("politics"));
	ds.setParameter("kind", getParam("kind"));
	...
	if (queryAge != null && queryAge != "") {
		...
		if (queryAge.match(reg) == null) {
			...
			ds.setParameter("queryAge", queryAge);
		}
	}

	ds.setParameter("queryWorkState", getParam("queryWorkState"));
	ds.load();
}
...
function detailHref(value, cellmeta, record, rowindex, colindex, dataset) {
	return '<a href="javascript:detail(\'' + record.get("PERSON_ID") + '\')">'
			+ value + '</a>';
}
...
function forExcel() {
	...
	L5.dataset2excel(ds, "/jsp/cams/marry/person/exportPerson_query.jsp");
