	...
	if (organType == 11) {
		ds.on("load", function() {
					...
					for (var i = 0; i < records.length; i++) {
						if (records[i].get("KSJG") != "01"
								&& records[i].get("KSJG") != "02") {
							records[i].set("KSJG", "");
						}
						if (records[i].get("ZGZZT") != "01"
								&& records[i].get("ZGZZT") != "02") {
...
function query() {
	...
	ds.setParameter("certificateId", getParam("certificateId"));
	...
	dsExcel.setParameter("certificateId", getParam("certificateId"));
...
function detail() {
	...
	ksqsDataset.setParameter('filterSql', " ND_YEAR = '" + ndYear
					+ "' and KS_SIGN = '0'  and ks_yn = '01'");
	ksqsDataset.load();
	var ksTimeDataset = L5.DatasetMgr.lookup("ksTimeDataset");
	ksTimeDataset.setParameter('filterSql', " ND_YEAR = '" + ndYear
					+ "' and KS_SIGN = '0'  and ks_yn = '01'");
	ksTimeDataset.load();
...
//						var records = ds.getAllChangedRecords();
						...
						// var ds = L5.DatasetMgr.lookup("ds");
//						 ds. commitChanges();
//						var r = ds.query("ksxxid", selected[i].get("ksxxid"));
//
//						for (var i = 0; i++; i < r.size) {
//
//							ds.query("ksxxid", selected[i].get("ksxxid"))[i]
//									.set("ksgl", "01");
//						}
//						ds.commitChanges();
					...
					/*
					 * //var command=new
					 * L5.Command("com.inspur.cams.marry.base.cmd.MrmQualificationExamCommand");
					 * command.setParameter("delIds", delIds);
					 * command.execute("plkshg"); if (!command.error) {
					 * 
					 * ds.commitChanges(); }else{
					 * L5.Msg.alert('提示',"设置时出现错误！"+command.error); }
					 */
				} else {
					return false;
				}
...
function commonQuery() {
	...
	var flag = selected[0].get('KSJG');
	if (flag != "01") {
		L5.Msg.alert('提示', "该登记员考试不合格!");
		return false;
	}

	document.getElementById("docpath").value = "jsp/cams/marry/person/print/mrmqualificationexam.doc";
...
function tochange() {
	...
	var ksqs = document.getElementById('ksqs1').value;// alert(ndYear+"
														// "+ksqs);
	var apds = L5.DatasetMgr.lookup("apds");
	apds.baseParams["ND_YEAR"] = ndYear;
	apds.baseParams["KS_SIGN"] = "0";
...
function emportExcel() {
	...
	dsExcel.baseParams["ksSign"] = "0";// 登记员
	dsExcel.load();
	dsExcel.on("load", function() {
				L5
						.dataset2excel(dsExcel,
								"/jsp/cams/marry/person/excel/mrmqualificationExcel.jsp");
			});
}