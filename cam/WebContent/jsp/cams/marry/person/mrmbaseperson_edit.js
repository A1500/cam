...
function init() {
	...
	if (method == null || method == "") {
		...
		record.set('djfwtype', '01');// 默认为内地居民
		document.getElementById("deptId").value = deptId;
		document.getElementById("deptName").value = deptName;
		// document.getElementById().value = '01';
...
function updateAll() {
	if (organCode.substring(2, 12) == "0000000000") {
		...
		document.getElementById("djyccfzsj").disabled = true;
...
function loadPhoto(photoId) {
	var src = rootPath + "download?table=COM_PHOTO_" + photoId.substring(0, 6)
			+ "&column=PHOTO_CONTENT&pk=PHOTO_ID&PHOTO_ID='" + photoId + "'";
	$("photoDiv").innerHTML = "";
	$("photoDiv").innerHTML = "<img id=\"img\" name=\"RELATIONER_PHOTOC_IMG\" width=82.5 height=110 src="
			+ src
			+ " align=\"top\" ondblclick=\"editPhoto(1,\'photo\',\'RELATIONER_PHOTOC_IMG\');\"	onerror=\"javascript:this.src=\'"
			+ rootPath
			+ "/jsp/cams/marry/image/default.jpg\'\" alt=\"暂无图片\" /><input type=\"hidden\" id=\"photo\" name=\"photo\"/>";
}
// 加载签名
function loadQm(photoId) {
	var src = rootPath + "download?table=COM_PHOTO_" + photoId.substring(0, 6)
			+ "&column=PHOTO_CONTENT&pk=PHOTO_ID&PHOTO_ID='" + photoId + "'";
	$("qmDiv").innerHTML = "";
	$("qmDiv").innerHTML = "<img id=\"img1\" name=\"RELATIONER_PHOTOC_IMG1\" width=144 height=44 src="
			+ src
			+ " align=\"top\" ondblclick=\"editPhoto(4,\'qmName\',\'RELATIONER_PHOTOC_IMG1\');\"	onerror=\"javascript:this.src=\'"
			+ rootPath
			+ "/jsp/cams/marry/image/qianming.jpg\'\" alt=\"暂无图片\" /><input type=\"hidden\" id=\"qmName\" name=\"qmName\"/>";
}
function savePerson() {
	//校验资格证号和第一次发证时间
	var fzsj=document.getElementById("djyccfzsj").value;
	var zgzbh=document.getElementById("certificateId").value;
	
	if (fzsj == '' && zgzbh!='') {
		L5.Msg.alert('提示', "初次发证时间不能为空!");
		return false;
	}
	if (fzsj != '' && zgzbh=='') {
		L5.Msg.alert('提示', "资格证编号不能为空!");
		return false;
	}
	
var records = ds.getAllChangedRecords();
	var photoInfo = document.getElementById("photo").value;
	var qmN = document.getElementById("qmName").value;
	// 照片判断，照片点击修改隐藏域发生变化时进行DATASET修改
	if (qmN == '' && photoInfo == '' && records.length < 1) {
...
//资格证号和第一次发证时间
//function zgzhgl(value){
// if(value){
//  if(!document.getElementById("djyccfzsj").value){
//   L5.Msg.alert("提示","请填写初次发证时间");
//  }
// }
//}
// 删除职务信息
...
function savePosition() {
	...
	/*
	 * if(isValidate != true){ L5.Msg.alert('提示',"校验未通过,不能保存!"+isValidate);
	 * return false; }
	 */
	var personId = document.getElementById("personId").value;
	var command = new L5.Command("com.inspur.cams.marry.base.cmd.MrmPositionCommand");
...
function back() {
	 ...
	 * 
	 * 
	 * if(flag=='1'){ var url='jsp/cams/marry/person/mrmbaseperson_list.jsp';
	 * var text = '人员基本信息表'; L5.forward(url,text); }else if(flag=='2'){ var
	 * url='jsp/cams/marry/person/mrmPerson_query.jsp'; var text = '婚姻登记员名册';
	 * L5.forward(url,text); }
	 */
}
