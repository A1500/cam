...
function back() {
	if(pageflag=='1'){
		...
		var url = "jsp/cams/marry/query/marry/status/queryStatus.jsp";
...
function queryNextLevel(value){
	...
	data.put("xzqu", xzqh);
	data.put("pageflag","2");
	var url = "jsp/cams/marry/query/marry/status/statusNextLevel.jsp";
	L5.forward(url, null, data);
}

function forExcel(){
	var statisticsTime = document.getElementById("statisticsTime").innerText;
	L5.dataset2excel(ds, "/jsp/cams/marry/query/marry/status/exportStatusExcel.jsp?statisticsTime="+statisticsTime);
}