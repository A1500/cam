...
function back() {
	if(pageflag=='1'){
		...
		data.put("cxsjq",cxsjq);
		data.put("cxsjz",cxsjz);
		var url = "jsp/cams/marry/query/marry/marryPeopleType/marryPeopleTypeQuery.jsp";
		L5.forward(url, null, data);
	}else{
		history.go(-1);
	}
...
function forExcel(){
	...
	L5.dataset2excel(dsNext, "/jsp/cams/marry/query/marry/marryPeopleType/exportPeopleTypeExcel.jsp?statisticsTime="+statisticsTime);
}