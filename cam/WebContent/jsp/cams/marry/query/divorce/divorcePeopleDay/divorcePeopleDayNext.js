function init() {
	...
	if (cxsjq != null && cxsjqm !=null) { 
		dsNext.setParameter("sumDateYear", cxsjq);
		dsNext.setParameter("sumDateMonth", cxsjqm);
		dsNext.load();
	}
...
function setStatisticsTime(){
		document.getElementById("statisticsTime").innerText = cxsjq + "年" + cxsjqm + "月";
} 
...
function back() {
	if(pageflag=='1'){
		var data = new L5.Map();
		data.put("cxsjq",cxsjq);
		data.put("cxsjqm",cxsjqm);
		var url = "jsp/cams/marry/query/divorce/divorcePeopleDay/divorcePeopleDay.jsp";
		L5.forward(url, null, data);
	}else{
		history.go(-1);
	}
...
function forExcel(){
	...
	L5.dataset2excel(dsNext, "/jsp/cams/marry/query/divorce/divorcePeopleDay/exportDivorcePeopleDayExcel.jsp?statisticsTime="+statisticsTime);
}