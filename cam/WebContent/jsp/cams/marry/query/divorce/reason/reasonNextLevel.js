...
function back() {
	if(pageflag=='1'){
		...
		var url = "jsp/cams/marry/query/divorce/reason/queryReason.jsp";
...
function queryNextLevel(value){
	...
	data.put("xzqu", xzqh);
	data.put("pageflag","2");
	var url = "jsp/cams/marry/query/divorce/reason/reasonNextLevel.jsp";
	L5.forward(url, null, data);
}

function forExcel(){
	var statisticsTime = document.getElementById("statisticsTime").innerText;
	L5.dataset2excel(ds, "/jsp/cams/marry/query/divorce/reason/exportreasonExcel.jsp?statisticsTime="+statisticsTime);
}