...
function queryNextLevel(value){
	...
	data.put("xzqu", xzqh);
	data.put("pageflag", "2");
	var url = "jsp/cams/marry/query/divorce/ageGap/ageGapNextLevel.jsp";
...
function back() {
	if(pageflag=='1'){
		...
		data.put("cxsjq",cxsjq);
		data.put("cxsjz",cxsjz);
		var url = "jsp/cams/marry/query/divorce/ageGap/queryageGap.jsp";
		L5.forward(url, null, data);
	}else{
		history.go(-1);
	}
...
function forExcel(){
	...
	L5.dataset2excel(ds, "/jsp/cams/marry/query/divorce/ageGap/exportageGapExcel.jsp?statisticsTime="+statisticsTime);
}