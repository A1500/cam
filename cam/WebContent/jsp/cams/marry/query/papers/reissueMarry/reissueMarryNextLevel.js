...
function back() {
	if(pageflag=='1'){
		...
		var url = "jsp/cams/marry/query/papers/reissueMarry/queryreissueMarry.jsp";
...
function queryNextLevel(value){
	...
	data.put("xzqu", xzqh);
	data.put("pageflag","2");
	var url = "jsp/cams/marry/query/papers/reissueMarry/reissueMarryNextLevel.jsp";
	L5.forward(url, null, data);
}

function forExcel(){
	var statisticsTime = document.getElementById("statisticsTime").innerText;
	L5.dataset2excel(ds, "/jsp/cams/marry/query/papers/reissueMarry/exportreissueMarryExcel.jsp?statisticsTime="+statisticsTime);
}