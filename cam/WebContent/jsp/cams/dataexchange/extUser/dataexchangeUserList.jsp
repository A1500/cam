...
<html>
	<head>
		<title>数据共享平台用户维护列表</title>
		<next:ScriptManager/>
		<script type="text/javascript" src="dataexchangeUserList.js"></script>
...
<model:datasets>
	<model:dataset id="ds" cmd="com.inspur.cams.comm.extuser.cmd.ComExtUserQueryCmd" global="true">
		...
		<model:params>
			<model:param name="ORGAN_TYPE" value="EX"></model:param>
			<model:param name="IN_USE" value="1"></model:param>
			<model:param name="AREA_CODE@LIKE" value='<%=BspUtil.getOrganCode()%>'></model:param>
		</model:params>
	</model:dataset>
...
<next:GridPanel id="editGridPanel" name="comextuserGrid" width="100%" stripeRows="true" height="100%" dataset="ds" title="数据共享平台用户维护列表" notSelectFirstRow="true">
	<next:TopBar>
	...
	<next:Columns>
		...
		<next:Column id="userId" header="用户登录名" field="userId" width="250" >
			<next:TextField allowBlank="false" />
		</next:Column>
	
		<next:Column id="userName" header="用户名称" field="userName" width="250" >
			<next:TextField  />
		</next:Column>
		
		<next:Column id="adds" header="地址" field="adds" width="90" >
			<next:TextField  />
		</next:Column>
