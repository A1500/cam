...
//检查国内电话号码
function checkTelCreate(domId) {
	if (document.getElementById(domId).value) {
		if (!isTelphone(document.getElementById(domId).value)) {
			document.getElementById(domId).value = "";
			L5.Msg.alert("提示", "请输入正确的国内固定电话");
			return;
		}
	}
}
...
//检查网址
function checkWebSite(domId) {
	if (document.getElementById(domId).value) {
		if (!isWebSite(document.getElementById(domId).value)) {
			document.getElementById(domId).value = "";
			L5.Msg.alert("提示", "请输入正确的网址信息");
			return;
		}
	}
}
...
/*function selectAreaCreate(flag) {
	var userInfo;
	var command = new L5.Command("com.inspur.cams.fis.util.InformSessionCmd");
	command.execute("getUserInfo");
	if (!command.error) {
	...
	var cantCode = parentOrganCode;
	var str = parentOrganName;
	var cantName = str.substr(0, str.indexOf("民政"));
	var cantType = parentOrganType;

	var url = L5.webPath
			+ "/jsp/cams/fis/comm/approvalAreaTree.jsp?cantCode=" + cantCode
			+ "&cantName=" + escape(encodeURIComponent(cantName))
			+ "&cantType=" + cantType + "&selectedType=13,";
	var win = showModalDialog(url, null,
			"scroll:no;status:no;dialogWidth:500px;dialogHeight:550px");
...
//	alert(win);
	...
	var record = fisCemeInfoDataSet.getCurrent();
	record.set("deptName", name);
	
}*/
function selectAreaCreate(flag){
	var fisOrganCode="";
	if(organType==13||organType==12){
		fisOrganCode = organCode.substring(0,4) + "00000000";
	}else if (organType==14){
		fisOrganCode = organCode.substring(0,6) + "000000";
	}
	var revalue = window
			.showModalDialog(
					L5.webPath
							+ "/jsp/cams/fis/comm/fisDicCityExt.jsp?radioMaxlevel=2&level=3&isRootSelect=0&organCode="
	...
	if(revalue!=undefined&&revalue!=""){
		...
		document.getElementById("deptName").value = list[4];
		var fisCemeInfoDataSet=L5.DatasetMgr.lookup("fisCemeInfoDataSet");
		var record = fisCemeInfoDataSet.getCurrent();
...
function download(){
	 window.open(L5.webPath+"/jsp/cams/fis/wf/download.jsp");
}