...
function init(){
	...
	if(method=="insert"){
		...
		accessoriesDataSet.setParameter("endCode","Y10");
...
function save(){
	...
	var historyRecord=historyDataSet.getCurrent();
		historyRecord.set("firstOpinion",applyInfoRecord.get("zeroOpinion"));
		historyRecord.set("firstResult",applyInfoRecord.get("zeroResult"));
		historyRecord.set("firstTime",time);
	var fisCemeDataSet=L5.DatasetMgr.lookup("fisCemeDataSet");
...
//		alert("待开发!");
		...
		if(validCemeExpend!=true){
			L5.Msg.alert("提示","校验未通过:  "+validCemeExpend+222);
			return false;
		}
		var recordExpend=fisCemeExpendInfoDataSet.getCurrent();
		
//		recordExpend.set("recordId",recordIdNew);
		historyRecord.set("recordId",recordExpend.get("recordId"));
		var cmd = new L5.Command("com.inspur.cams.fis.base.cmd.FisCemeApplyInfoCmd");
	...
	//var url="jsp/cams/fis/wf/cemeAcceptance/fisCemeInfoList.jsp";
	//L5.forward(url);
}
function returnBack(){
	history.go(-1);
