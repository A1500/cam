...
<script type="text/javascript" src="fisCemeChangeRecordDetail.js"></script> 
<script type="text/javascript" src='<%=SkinUtils.getJS(request,"cams.js")%>'></script>
<script type="text/javascript">
	var recordId='<%=request.getParameter("recordId")%>';changeId
	var cemeApplyId='<%=request.getParameter("cemeApplyId")%>';
	var changeId='<%=request.getParameter("changeId")%>';
	var method='<%=request.getParameter("method")%>';
	var actDefName = '<%=request.getParameter("actDefName")%>';
		...
		<next:EditGridPanel id="uploadGrid" width="99.9%"
			...
			<next:TopBar>
				...
				<next:ToolBarItem iconCls="add" text="附件查看"
					handler="clickDetailFile"></next:ToolBarItem>
			</next:TopBar>
			<next:Columns>
				<next:RowNumberColumn />
				<next:RadioBoxColumn></next:RadioBoxColumn>
...
<next:Window id="downloadList" closeAction="hide" title="附件查看"
