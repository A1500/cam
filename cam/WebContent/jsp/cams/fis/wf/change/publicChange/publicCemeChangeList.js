function init(){
	...
	daiBanDataSet.setParameter("PROP","1");
	daiBanDataSet.load();
...
function query(){
	...
	daiBanDataSet.setParameter("FILL_ORGAN_CODE",organCode);
	daiBanDataSet.setParameter("APPLY_FLAG","C");
	daiBanDataSet.setParameter("CORP","C");
...
function edit(){
	...
	if(selected[0].get("IS_REPORT")!="5"){
		L5.Msg.alert('提示',"当前业务已在流程中，不可修改");
		return false;
	}
	var data = new L5.Map();
...
function del(){
	...
	if(records[0].get("IS_REPORT")!="5"){
		L5.Msg.alert('提示',"当前业务已在流程中，不可删除");
		return false;
	}
...
function submit(){
	...
	if(selected[0].get("IS_REPORT")!="5"){
		L5.Msg.alert('提示',"当前业务已在流程中，不可重复提交");
		return false;
	}
...
function showState(){
	...
	if(records[0].get("IS_REPORT")=="5"){
		L5.Msg.alert("提示","该业务未上报，无法查看流程");
		return;
	}
	var workflowDs = L5.DatasetMgr.lookup("workflowDs");
	workflowDs.setParameter("subject",organName+"经营性公墓变更");
	...
	workflowDs.setParameter("curOrgan",records[0].get("CUR_ORGAN"));
