...
function add(){
	...
	var url='jsp/cams/fis/wf/change/publicChange/publicCemeChangeBKEdit.jsp';
	var text = '';
...
function edit(){
	...
	//if(selected[0].get("IS_REPORT")!="11"){
...
function detail(){
	...
	data.put("recordId",selected[0].get('RECORD_ID'));
	data.put("cemeApplyId",selected[0].get('CEME_APPLY_ID'));
	var url='jsp/cams/fis/wf/change/publicChange/publicCemeChangBKDetail.jsp';
	var text = '明细查看';
	L5.forward(url,text,data);
}
function submit(){
