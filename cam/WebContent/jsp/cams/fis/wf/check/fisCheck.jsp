...
<script type="text/javascript" src="fisCheck.js"></script>
<script language="javascript">
		var NJYear = '<%=DateUtil.getYear()%>';
		var createTime = '<%=DateUtil.getTime()%>';
		var organCode = '<%=BspUtil.getOrganCode()%>';
		var organType = '<%=BspUtil.getOrganType()%>';
		var organName = '<%=BspUtil.getOrganName()%>';
		//新的业务编号
		var newCemeApplyId = '<%=IdHelp.getUUID32()%>';
		//新的打分ID
		...
		var cemeApplyId='<%=request.getParameter("taskCode")%>';
...
<model:datasets>
	...
	<model:dataset id="xJResultDs" enumName="FIS.CEME.RESULT"
		autoLoad="true" global="true"></model:dataset>
	<!-- 业务表 加载年检业务-->
...
<next:ViewPort>
	<next:Panel width="100%" height="100%">
		<next:TopBar>
			...
			<next:ToolBarItem iconCls="save" text="保存" handler="saveClick" id="save"/>
				...
				<next:Html>
					...
					<table width="100%">
						<tr>
							...
							<td class="FieldInput" colspan="3" width="69%"><input  style="text-align:right"
								type="text" name="总得分" field="firstTotalScore" readonly="readonly" id="firstTotalScore"/></td>
						</tr>
						<tr>
							...
							<td class="FieldInput"><input type="text" name="检查时间"
								...
								format="Y-m-d" onclick="LoushangDate(this)" /><font color="red">*</font></td>
							<td class="FieldLabel">县级检查结果</td>
							<td class="FieldInput"><select  name="检查结果"
								field="firstResult">
								<option dataset="cJDs"></option></select><font color="red">*</font></td>
						</tr>
						<tr>
							<td class="FieldLabel" width="8%">县级检查意见</td>
							<td class="FieldInput" colspan="3"><textarea  onpropertychange="if(value.length>1200) value=value.substr(0,1000)"   rows="30"
								name="检查意见" id="firstOpinion" field="firstOpinion" maxlength="1500"
								style="width: 80%" readonly="readonly"> </textarea><font color="red">*可输入1200字</font></td>
						</tr>
					</table>
					</fieldset>
					</div>
					...
					<table width="100%">
						<tr>
							<td class="FieldLabel" width="31%" colspan="2">总得分:</td>
							<td class="FieldInput" colspan="3" width="69%"><input  style="text-align:right"
								type="text" name="总得分" field="secondTotalScore" readonly="readonly"/></td>
						</tr>
						<tr>
							...
							<td class="FieldInput"><select  name="年检结果"
								field="secondResult">
								<option dataset="xJResultDs"></option></select></td>
						</tr>
						<tr>
