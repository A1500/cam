...
function insert(){
	...
	var url="jsp/cams/fis/wf/createCemeOperation/fisCemeInfoEdit.jsp";
	L5.forward(url,'',data);
}
function update(){
	...
	{
		L5.Msg.alert("提示","当前新建申请正在流转，已不具备修改权限。",function(){
			return 0;
		});
	}else{
		var data=new L5.Map();
...
function submit(){
	...
	if (records[0].get("IS_REPORT")!=4)
	{
		L5.Msg.alert("提示","当前新建申请正在流转，已不具备提交权限。",function(){
			return 0;
		});
	}else{
				...
				var waitConfig = {
						   ...
						   fn:function(){
							   var command=new L5.Command("com.inspur.cams.fis.base.cmd.FisCemeApplyInfoCmd");
								...
								command.execute("startCreateOperation");
