function init() {
	...
	accessoriesDataSet.setParameter("startCode","B71");
	accessoriesDataSet.setParameter("endCode","B92");
			...
			if(result==2){
				L5.MessageBox.confirm('确认','取消该新建申请后流程将走向完结，是否继续操作?',function(state){
					if (state=="yes"){
						...
						commandSubmit.execute("sendOfCreateAll");
						
						
						if (!commandSubmit.error){
							L5.Msg.alert("提示","公墓申建申请已审批通过！",function(){
								returnBack();
							});
						}else{
...
//	historyBackup.setParameter("CEME_APPLY_ID@=",cemeApplyId);
...
function clickManage() {
	...
	if(organType=="11"&&record.get("code")!="B91"){
...
function click_upload() {
		var editGrid = L5.getCmp('uploadGrid');2
		var selected = editGrid.getSelectionModel().getSelections();
		var record = selected[0];
