...
function query(){
	...
	if(organizer!=""){
		allConditon+="&F.ORGANIZER@LIKE:%"+organizer+"%";
	}
	if(domination!=""){
		allConditon+="&F.DOMINATION@LIKE:%"+domination+"%";
	}
	if(applyTime!=""){
		allConditon+="&E.APPLY_TIME@LIKE:%"+applyTime+"%";
	}

	fisCemeApplyInfoDataSet.setParameter("mysetwhere","FIRST_UNIT_ID@=:"+organCode+allConditon);
	fisCemeApplyInfoDataSet.load(true);
}
function insert(){
	var data=new L5.Map();
	data.put("method","insert");
	var url="jsp/cams/fis/wf/expendCemeOperation/fisCemeInfoEdit.jsp";
	L5.forward(url,'',data);
}
function update(){
	...
	if (records[0].get("IS_REPORT")!=4)
	{
		L5.Msg.alert("提示","当前扩建申请正在流转，已不具备修改权限。",function(){
			return 0;
		});
	}else{
		var data=new L5.Map();
...
function submit(){
	...
	if (records[0].get("IS_REPORT")!=4)
	{
		L5.Msg.alert("提示","当前扩建申请正在流转，已不具备提交权限。",function(){
			return 0;
		});
	}else{
				...
				var waitConfig = {
						   ...
						   fn:function(){
							   var command=new L5.Command("com.inspur.cams.fis.base.cmd.FisCemeApplyInfoCmd");
								...
								command.execute("startCreateExpendOp");
...
function detail(){
	...
	var data=new L5.Map();
	data.put("cemeApplyId",records[0].get("CEME_APPLY_ID"));
	var url="jsp/cams/fis/wf/expendCemeOperation/fisCemeInfoDetail.jsp";
	//alert("到这了！！")
