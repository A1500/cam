function init() {
	...
	if (actDefName=="省级审批"){
		      ...
		      document.getElementById("businessCemeCertificateId1").disabled=true;
		      document.getElementById("rectifyDeadline1").disabled=true;
				...
				if(result==2){
					L5.MessageBox.confirm('确认','取消该验收申请后流程将走向完结，是否继续操作?',function(state){
						if (state=="yes"){
							...
							commandSubmit.execute("sendCemeAcceptanceCity");
...
function boHui(){
	...
	L5.MessageBox.confirm("提示", "是否确定驳回到下级单位？",function(sta){
		if(sta=="yes"){	
			...
			if (!commandSubmit.error){
				L5.Msg.alert("提示","已经驳回到市级！",function(){
					returnBack();
				});
			}else{
...
function show(){
   ...
   document.getElementById("businessCemeCertificateId1").disabled=false;
   document.getElementById("rectifyDeadline1").disabled=true;
