...
function query() {
	...
	ds.on("load", function() {
				...
				if (ds.getCount() > 0)
					cemeTitle.setText("需要进行检查的经营性公墓(" + ds.getTotalCount()
							+ "条)");
				else
					cemeTitle.setText("需要进行检查的经营性公墓");
			});
	ds.load();
}
...
function fullChange() {
	...
	L5.Msg.confirm('确认整改？', '是否要对公墓进行整改？', function(isOk) {
		if (isOk == 'yes') {
			...
			cmd.execute('fullChangeNoProfitCheck');
			if (!cmd.error) {
...
function report() {
	...
	if (selected[0].get('CHECK_NUM') == "0"
			|| selected[0].get('CHECK_NUM') == "2") {
		L5.Msg.alert('提示', "公墓未检查，不可上报!");
		return false;
	} else if (selected[0].get('CHECK_NUM') == "4") {
		L5.Msg.alert('提示', "公墓本年度年检已过两次，不可上报!");
		return false;
	}

	L5.Msg.confirm('确认上报？', '公墓检查结果上报后不可修改！', function(isOk) {
		if (isOk == 'yes') {
			var cmd = new L5.Command('com.inspur.cams.fis.ep.base.cmd.FisCemeCheckCmd');
			cmd.setParameter('WF_ID', selected[0].get('WF_ID'));
			cmd.execute('reportProfitCheck');
			if (!cmd.error) {
