...
function query() {
	...
	ds.on("load", function() {
				...
				if (ds.getCount() > 0)
					cemeTitle.setText("需要进行年检的公益性公墓(" + ds.getTotalCount()
							+ "条)");
				else
					cemeTitle.setText("需要进行年检的公益性公墓");
			});
	ds.load();
}
...
function report() {
	...
	L5.Msg.confirm('确认上报？', '公墓年检结果上报后不可修改！', function(isOk) {
		if (isOk == 'yes') {
			var cmd = new L5.Command('com.inspur.cams.fis.ep.base.cmd.FisCemeCheckCmd');
			cmd.setParameter('WF_ID', selected[0].get('WF_ID'));
			cmd.execute('reportNoProfitCheck');
			if (!cmd.error) {
...
function print() {
	...
	var url = "../../comm/print/cemeNoprofitCheckPrint.jsp?docPath=cemeNoprofitCheckPrint.doc";
	url += "&cemeId=" + cemeId;
	url += "&applyId=" + applyId;
	var text = '公益性公墓年检报告打印';
	var width = 1024;
	var height = 768;
