...
<script type="text/javascript">
	...
	var recordId = '<%=IdHelp.getUUID32()%>';
	var userName = '<%=BspUtil.getEmpOrgan().getShortName()%>';
	var date = '<%= DateUtil.getTime()%>'
</script>
		<script type="text/javascript" src="fisFuneralDeadInfoList.js"></script>
...
<model:datasets>
	...
	<!-- 火化记录查询导出 -->
	<model:dataset id="ds" cmd="com.inspur.cams.fis.base.cmd.FisCremationExportBackupQueryCmd" pageSize="200">
...
<next:GridPanel id="editDeadPanel" dataset="fisFuneralDeadInfoDS"   width="100%" stripeRows="true" height="100%" >
	<next:TopBar>
		...
		<next:ToolBarItem text="导出记录查看" iconCls="detail" handler="exportShow" id="exportShow"/>
		<next:ToolBarItem text="明细" iconCls="detail" handler="deadInfo_detail" />
		<next:ToolBarItem text="导出" iconCls="export" handler="exportCremation"/>
	</next:TopBar>
	<next:Columns>					    
		...
		<next:Column id="RECEIVE_TIME" header="入馆时间" field="RECEIVE_TIME" width="80" align="center" >
			<next:TextField allowBlank="false" />
		</next:Column>
		
		<next:Column id="DEATH_DATE" header="去世时间" field="DEATH_DATE" width="80" align="center" >
			<next:TextField allowBlank="false" />
		</next:Column>
