...
<title>逝者信息</title>
<next:ScriptManager />
<script type="text/javascript" src="fisCremationDeadInfoList.js"></script>
<script type="text/javascript">
	var organCode='<%=request.getParameter("organCode")%>';
	var unitId='<%=request.getParameter("unitId")%>';
	var qStartTime='<%=request.getParameter("qStartTime")%>';
	var qEndTime='<%=request.getParameter("qEndTime")%>';
	var flag='<%=request.getParameter("flag")%>';
	var organName='<%=java.net.URLDecoder.decode(request.getParameter("organName"), "UTF-8")%>';
</script>
</head>
<body>
<model:datasets>
	...
	<model:dataset id="fisFuneralDeadInfoDS"
		...
		method="queyCremationDeadInfoByArea" pageSize="25">
	</model:dataset>
	<!-- 骨灰去向 -->
	<model:dataset id="ashesGoDS" enumName="FIS.ASHES.GO" autoLoad="true"
		global="true">
	</model:dataset>
	...
	<!-- 优惠类型 -->
	<model:dataset id="fisReducationTypeDic"
		cmd="com.inspur.cams.fis.base.cmd.FisFuneralDeadInfoQueryCmd"
		method="queryByReductionDic" autoLoad="true">
		<model:record>
			<model:field name="value" mapping="PREFERENTIAL_TYPE_NAME"
				type="string" />
			<model:field name="text" mapping="PREFERENTIAL_TYPE_NAME"
				type="string" />
		</model:record>
	</model:dataset>
	<!-- 教育状况 -->
	...
	<model:dataset id="fisDeathCauseDS" enumName="FIS.DEATH.CAUSE"
		autoLoad="true" global="true">
	</model:dataset>
</model:datasets>
<next:GridPanel id="editDeadPanel" dataset="fisFuneralDeadInfoDS"
	width="100%" stripeRows="true" height="100%">
	<next:TopBar>
		<next:ToolBarItem id="qrToolbar"></next:ToolBarItem>
		<next:ToolBarItem symbol="->"></next:ToolBarItem>
		<next:ToolBarItem text="查看详情" iconCls="detail"
			handler="deadInfo_detail" />
	...
	<next:Columns>
		...
		<next:Column id="FULL_NAME" header="户口所在地" field="FULL_NAME"
			width="270" align="left">
			<next:TextField allowBlank="false" />
		</next:Column>

		<next:Column id="UNIT_NAME" header="殡仪馆名称" field="UNIT_NAME"
			width="100" align="center">
			<next:TextField allowBlank="false" />
		</next:Column>

		<next:Column id="NAME" header="逝者姓名" field="NAME" width="90"
			align="center">
			<next:TextField />
		</next:Column>

		<next:Column id="SEX" header="性别" field="SEX" width="50"
			dataset="commSexDS" align="center">
			<next:TextField allowBlank="false" />
		</next:Column>
		...
		<next:Column id="ID_CARD" header="证件号码" field="ID_CARD" width="140"
			align="center">
			<next:TextField allowBlank="false" />
		</next:Column>

		<next:Column id="REDUCATION_TYPE_NAME" header="享受惠民政策"
			field="REDUCATION_TYPE_NAME" width="125" align="center">
			<next:TextField allowBlank="false" />
		</next:Column>

		<next:Column id="DEATH_CAUSE" header="死亡原因" field="DEATH_CAUSE"
			width="90" align="center" dataset="fisDeathCauseDS">
			<next:TextField allowBlank="false" />
		</next:Column>

		<next:Column id="RECEIVE_TIME" header="入馆时间" field="RECEIVE_TIME"
			width="125" align="center">
			<next:TextField allowBlank="false" />
		</next:Column>

		<next:Column id="START_TIME" header="火化时间" field="START_TIME"
			width="125" align="center">
			<next:TextField allowBlank="false" />
		</next:Column>

		<next:Column id="ASHES_GO" header="骨灰去向" field="ASHES_GO" width="150" dataset="ashesGoDS"
			align="center">
			<next:TextField allowBlank="false" />
		</next:Column>
	</next:Columns>
...
<next:Window id="deathDetailWin" height="500" width="790" modal="true"
	autoScroll="true" closable="close">
	<next:TopBar>
	...
	<next:Panel autoScroll="true">
		<next:Html>
			<form id="dathEditForm" onsubmit="return false" class="L5form"
				dataset="fisFuneralDeadInfoDS">
			<table border="1" width="100%">
				<tr>
					<td class="FieldLabel" width="15%"><label>姓名:</label></td>
					<td class="FieldInput" width="17%"><label maxlength="25"
						name="逝者姓名" field="NAME" /></td>
					...
					<td class="FieldInput"><label name="逝者证件类型" field="CARD_TYPE"
						dataset="cardTypeDS" /></td>
				</tr>
				<tr>
					<td class="FieldLabel"><label>证件号码:</label></td>
					<td class="FieldInput"><label name="逝者证件号码" maxlength="18"
						style="width: 95%" field="ID_CARD" /></td>
					<td class="FieldLabel">出生日期:</td>
					<td class="FieldInput"><label name="出生日期" id="winBirthDay"
						field="BIRTHDAY" /></td>
					<td class="FieldLabel">去世时间:</td>
					<td class="FieldInput"><label name="去世时间" id="winDeathDate"
						field="DEATH_DATE" /></td>
				</tr>
				<tr>
					<td class="FieldLabel">年龄:</td>
					<td class="FieldInput"><label name="年龄" maxlength="3"
						field="AGE" /></td>
					...
					<td class="FieldInput"><label name="国籍" field="NATION"
						dataset="nationDS" /></td>
				</tr>
				<tr>
					...
					<td class="FieldInput"><label name="文化程度" field="EDUCATION"
						dataset="DmJyzkDataSet" /></td>
					<td class="FieldLabel">职业:</td>
					<td class="FieldInput"><label name="职业" field="PROFESSION"
				...
				<tr>
					...
					<td class="FieldInput"><label name="宗教信仰" field="RELIGION"
						dataset="religionDS" /></td>
				...
				<tr>
					...
					<td class="FieldInput"><label field="ASHES_GO"
						dataset="ashesGoDS" /></td>
				...
				<tr>
					...
					<td style="display: none;"><label field="POPULACE" /></td>
					<td class="FieldLabel">生前住址:</td>
					<td class="FieldInput" colspan="3"><label maxlength="50"
						name="生前住址" style="width: 83%" field="FAMILY_ADD" /></td>
				</tr>
				<tr>
					<td class="FieldLabel">是否要灰:</td>
					<td class="FieldInput"><label maxlength="50" name="是否要灰"
						style="width: 83%" field=IF_TODUST dataset="yesOrNoDs" /></td>
					<td class="FieldLabel">原工作单位:</td>
					<td class="FieldInput" colspan="5"><label maxlength="50"
						name="工作单位" style="width: 83%" field="WORK_NAME" /></td>
				</tr>
				<tr>
					<td class="FieldLabel">去世原因:</td>
					<td class="FieldInput"><label name="去世原因" field="DEATH_CAUSE"
						dataset="fisDeathCauseDS" /></td>
					<td class="FieldLabel">去世地点:</td>
					<td class="FieldInput" colspan="3"><label maxlength="50"
						style="width: 83%" name="去世地点" field="DEATH_PLACE" /></td>
				</tr>
				<tr>
					<td class="FieldLabel">病种:</td>
					<td class="FieldInput"><label name="病种" field="DEATH_DISEASE"
						dataset="diseaseDataset" /></td>
					<td class="FieldLabel">随身遗物:</td>
					<td class="FieldInput" colspan="3"><textarea rows="2"
						readonly="readonly" cols="15" name="随身遗物" style="width: 83%"
						field="RELICS" lang="100"></textarea></td>
				</tr>
				<tr>
					...
					<td class="FieldInput" colspan="3"><textarea rows="2"
						readonly="readonly" cols="15" style="width: 83%" name="遗体状态备注"
						field="CORPSE_STATUS_NOTE" lang="100"></textarea></td>
				</tr>
				<tr>
					<td class="FieldLabel">是否无名尸:</td>
					<td class="FieldInput"><label field="IF_NAMELESS" name="是否无名尸"
						dataset="yesOrNoDs" /></td>
				...
				<tr>
					...
					<td class="FieldInput" colspan="5"><label lang="100" name="备注"
						style="width: 75%" field="NOTE" /></td>
				</tr>
			</table>
			</form>
		</next:Html>
