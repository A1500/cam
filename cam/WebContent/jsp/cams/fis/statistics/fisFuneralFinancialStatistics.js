...
// 查询
function query() {
	var STime = document.getElementById("SCremationTime").value;
	var ETime = document.getElementById("ECremationTime").value;
	if(STime==""){
		L5.Msg.alert("提示","请输入开始统计的年份");
		return false;
	}
	if(ETime==""){
		ETime = year;
	}
	if(STime>year||STime>ETime){
		L5.Msg.alert("提示","您所输入的起始年份超过了截止年份,请重新输入");
		return false;
	}
	...
	ds.setParameter("qStartTime", STime);
	ds.setParameter("qEndTime", ETime);
	ds.load();
}
...
function exportExcel() {
	...
	var queryCondition = STime+"年   至   "+ETime+"年";
	var ds = L5.DatasetMgr.lookup("ds");
	...
	var url = L5.webPath
			+ "/jsp/cams/fis/statistics/fisFuneralFinancialStatisticsExcel.jsp";

	L5.dataset2excel(ds, "/jsp/cams/fis/statistics/fisFuneralFinancialStatisticsExcel.jsp?queryCondition="+queryCondition);
