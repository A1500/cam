...
function init() {
	var fisFuneralOrganManageDS = L5.DatasetMgr.lookup("fisFuneralOrganManageDS");
	fisFuneralOrganManageDS.on("load", function() {
				document.getElementById("reDiv").style.display = "";
				L5.Msg.hide();
				
				var records = fisFuneralOrganManageDS.getAllRecords();
					for (var i = 0; i < records.length; i++) {
						if (records[i].get("TOTALAREA") != '0' && records[i].get("TOTALAREA") != 0) {
							records[i].set("TOTALAREA", toDecimal(records[i].get("TOTALAREA")));
						}
					}
			});
}
...
function query() {
	...
	getStatisticsTime();
	var fisFuneralOrganManageDS = L5.DatasetMgr.lookup("fisFuneralOrganManageDS");
	fisFuneralOrganManageDS.setParameter("qPopulace", currentOrganCode);
	fisFuneralOrganManageDS.load();
}
...
function forDetail(obj, flag) {
	...
	var fisFuneralOrganManageDS = L5.DatasetMgr.lookup("fisFuneralOrganManageDS");
	var record = fisFuneralOrganManageDS.getAt(rowIndex);
...
function exportExcel() {
	var ds = L5.DatasetMgr.lookup("fisFuneralOrganManageDS");
	...
	var url = L5.webPath
			+ "/jsp/cams/fis/statistics/fisFuneralOrganStatisticsExcel.jsp";

	L5.dataset2excel(ds, "/jsp/cams/fis/statistics/fisFuneralOrganStatisticsExcel.jsp");
