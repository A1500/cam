...
var linkmanName=decodeURIComponent('<%=request.getParameter("linkmanName")%>');
var linkmanPhone='<%=request.getParameter("linkmanPhone")%>';
var deptName='<%=BspUtil.getDeptOrgan().getOrganName()%>';
</script>
<script type="text/javascript">
var webObj;
var activeDocument;
...
function init(){  
	//取得文件名 和标签    
	var nowFileName = "partyBuildInform.doc";
	...
	if(deptName.lastIndexOf('区')>0){
		deptName=deptName.substring(0,deptName.lastIndexOf('区')+1);
	}else if(deptName.lastIndexOf('县')>0){
		deptName=deptName.substring(0,deptName.lastIndexOf('县')+1);
	}else if(deptName.lastIndexOf('市')>0){
		deptName=deptName.substring(0,deptName.lastIndexOf('市')+1);
	}else if(deptName.lastIndexOf('省')>0){
		deptName=deptName.substring(0,deptName.lastIndexOf('省')+1);
	}
	deptName+='社会组织党工委办公室';
	...
	showMap.put("linkmanName",linkmanName);
	showMap.put("linkmanPhone",linkmanPhone);
	showMap.put("deptName",deptName);
	showMap.put("partyNum",partyNum);
	getMarks();
	writeDoc(showMap);
}
