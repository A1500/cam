...
function init(){
	...
	var nowFileName="";
	if(certType == '0'){
		var nowFileName = "groupBuild.doc";
	}else{
		var nowFileName = "groupBuildC.doc";
	}
	...
	somGroupCertPrintDataset.setParameter("organCode",organCode);
	...
	somGroupCertPrintDataset.on("load",function(ds){
		...
		//由于没法选择证书图片不打印，此处采用变通方式，第一次初始化添加证书图片，点打印按钮进行不添加图片的初始化
		if(tag == 0){
			if(certType == '0'){
				var nowFileName = "groupBuild.doc";
				//webObj.AddPicFromUrl(L5.webPath + "/jsp/cams/sorg/comm/print/certPic/group/"+"groupO.jpg",true,0,0,1,100,0);
			}else{
				var nowFileName = "groupBuildC.doc";
				//webObj.AddPicFromUrl(L5.webPath + "/jsp/cams/sorg/comm/print/certPic/group/"+"groupC.png",true,0,0,1,100,0);
			}
		}
		tag="1";
	});
