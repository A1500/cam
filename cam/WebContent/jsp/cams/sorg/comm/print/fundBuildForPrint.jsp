...
<script type="text/javascript">
	<%String id=request.getParameter("id");
		...
		String legalPeople=request.getParameter("legalPeople");
...
function init(){  
	...
	//取得文件名 和标签    
	if(certType == '0'){
		nowFileName = "fundBuildO.doc";
	}else{
		nowFileName = "fundBuildC.doc";
	}
	...
	somUngovCertPrintDataset.setParameter("legalPeople",legalPeople);
	...
	somUngovCertPrintDataset.setParameter("checkResult",checkResult);
	...
	somUngovCertPrintDataset.on("load",function(ds){
		...
		showMap.put("checkResult",checkResult);
		if(ds.get("sorgKind")=="1"){
			showMap.put("sorgKind","公募基金会");
		}
		if(ds.get("sorgKind")=="2"){
			showMap.put("sorgKind","非公募基金会");
		}
		getMarks();
		writeDoc(showMap);
		//设置文档为只读
