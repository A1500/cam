...
<script type="text/javascript">
	<%String id=request.getParameter("id");
		...
		String actArea=request.getParameter("actArea");
		if(actArea==null){
			actArea="";
		}
		if(business==null){
...
function init(){  
	...
	//取得文件名 和标签    
	if(certType == '0'){
		nowFileName = "groupBranchBuildO.doc";
	}else{
		nowFileName = "groupBranchBuildC.doc";
	}
	...
	somGroupCertPrintDataset.setParameter("business",business);
	...
	somGroupCertPrintDataset.on("load",function(ds){
		...
		if(certType == '0'){
			...
			showMap.put("regDateY",ds.get("BuildYear"));
			showMap.put("regDateM",ds.get("BuildMonth"));
			showMap.put("regDateD",ds.get("BuildDay"));
		}else{
			showMap.put("signDate",ds.get("FYear")+"年"+ds.get("FMonth")+"月"+ds.get("FDay")+"日");
			showMap.put("regDate",ds.get("BuildYear")+"年"+ds.get("BuildMonth")+"月"+ds.get("BuildDay")+"日");
		}
		getMarks();
		writeDoc(showMap);
		//设置文档为只读
...
<body >
<model:datasets>
	<model:dataset id="somGroupCertPrintDataset" cmd="com.inspur.cams.sorg.base.cmd.SomCertQueryCmd" global="true" method="createCert">
		<model:record fromBean="com.inspur.cams.sorg.base.data.SomCert">
			...
			<model:field name="BuildYear" />
			<model:field name="BuildMonth" />
			<model:field name="BuildDay" />
		</model:record>
	</model:dataset>
</model:datasets>
<next:ViewPort>
