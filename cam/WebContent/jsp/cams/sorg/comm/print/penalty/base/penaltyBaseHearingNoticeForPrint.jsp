...
var hearingAddDetail=decodeURIComponent('<%=request.getParameter("hearingAddDetail")%>');
var hearingDelayEndDate='<%=request.getParameter("hearingDelayEndDate")%>';
var hearingPresenterAvoidEndDate='<%=request.getParameter("hearingPresenterAvoidEndDate")%>';
</script>
<script type="text/javascript">
var webObj;
var activeDocument;
...
function init(){  
	//取得文件名 和标签    
	var nowFileName = "penaltyBaseHearingNotice.doc";
	...
	ds.on("load",function(ds){
		...
		var hearingApplyDate=ds.get("hearingApplyDate").split("-");	
		var hearingApplyDateY = hearingApplyDate[0];
		var hearingApplyDateM = hearingApplyDate[1];
		var hearingApplyDateD = hearingApplyDate[2];
		showMap.put("hearingApplyDateY",hearingApplyDateY);
		showMap.put("hearingApplyDateM",hearingApplyDateM);
		showMap.put("hearingApplyDateD",hearingApplyDateD);	
		if(ds.get("hearingApplyType")=="1"){
			showMap.put("hearingApplyType","口头");
		}else{
			showMap.put("hearingApplyType","书面");		
		}
		var hearingDate=ds.get("hearingDate").split("-");	
		var hearingDateY=hearingDate[0];
		var hearingDateM=hearingDate[1];
		var hearingDateD=hearingDate[2];
		showMap.put("hearingDateY",hearingDateY);	
		showMap.put("hearingDateM",hearingDateM);
		showMap.put("hearingDateD",hearingDateD);
		showMap.put("hearingAdd",ds.get("hearingAdd"));
		showMap.put("hearingAddDetail",hearingAddDetail);
		showMap.put("invInfact",ds.get("invInfact"));
		if(ds.get("hearingType")=="1"){
			showMap.put("hearingType","公开");
		}else{
			showMap.put("hearingType","不公开");		
		}
		hearingDelayEndDate=hearingDelayEndDate.split("-");
		var hearingDelayEndDateY=hearingDelayEndDate[0];
		var hearingDelayEndDateM=hearingDelayEndDate[1];
		var hearingDelayEndDateD=hearingDelayEndDate[2];
		showMap.put("hearingDelayEndDateY",hearingDelayEndDateY);	
		showMap.put("hearingDelayEndDateM",hearingDelayEndDateM);
		showMap.put("hearingDelayEndDateD",hearingDelayEndDateD);
		hearingPresenterAvoidEndDate=hearingPresenterAvoidEndDate.split("-");
		var hearingPresenterAvoidEndDateY=hearingPresenterAvoidEndDate[0];
		var hearingPresenterAvoidEndDateM=hearingPresenterAvoidEndDate[1];
		var hearingPresenterAvoidEndDateD=hearingPresenterAvoidEndDate[2];
		showMap.put("hearingPresenterAvoidEndDateY",hearingPresenterAvoidEndDateY);	
		showMap.put("hearingPresenterAvoidEndDateM",hearingPresenterAvoidEndDateM);
		showMap.put("hearingPresenterAvoidEndDateD",hearingPresenterAvoidEndDateD);
		showMap.put("hearingPresenter",ds.get("hearingPresenter"));		
		showMap.put("hearingPresenterOrgan",ds.get("hearingPresenterOrgan"));	
		showMap.put("hearingPresenterDuties",ds.get("hearingPresenterDuties"));	
		showMap.put("hearingRecorder",ds.get("hearingRecorder"));	
		showMap.put("hearingRecorderOrgan",ds.get("hearingRecorderOrgan"));	
		showMap.put("hearingRecorderDuties",ds.get("hearingRecorderDuties"));	
