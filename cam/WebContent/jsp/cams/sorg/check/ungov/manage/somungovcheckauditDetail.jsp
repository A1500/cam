	...
	<head>
		...
		<script language="javascript">
			...
			function forCheckClose(){
				...
				flowDsRecord.set("auditOrganId",'<%=organId%>');
				flowDsRecord.set("auditDeptId",'<%=deptId%>');
				flowDsRecord.set("auditPeopleId",'<%=empId%>');
				flowDsRecord.set("auditDate",'<%=date%>');
				flowDsRecord.set("auditPeopleName",'<%=empName%>');
				flowDsRecord.set("auditOpinionId","1");
			...
			function init(){
				...
				flowDs.on('load',function(){
					if(flowDs.getCount()==0){
						flowDs.newRecord({'auditOpinionId':'1'});
					}
				});
				flowHisDs.load();
				flowHisDs.on('load',function(){
			...
			function backFlow(){
				...
				if (!command.error) {
					L5.Msg.alert("提示","退回处长审核成功！",function(){
						var win=L5.getCmp("forCheck");
						win.hide();
					});		
				}else{
					L5.Msg.alert('提示',"退回处长审核时出现错误！"+command.error);
				}
			}
			function returnList(){
				var data = new L5.Map();
				var url='jsp/cams/sorg/check/ungov/manage/somungovcheckaudit_list.jsp';
				var text = '年检列表';
...
<model:datasets>
	...
	<model:dataset id="flowHisDs" cmd="com.inspur.cams.sorg.check.somcheckflow.cmd.SomCheckFlowQueryCommand" global="true">
		<model:record fromBean="com.inspur.cams.sorg.check.somcheckflow.data.SomCheckFlow"></model:record>
		<model:params>
			<model:param name="TASK_CODE" value='<%= taskCode%>'></model:param>
		</model:params>
	</model:dataset>
...
<iframe id="frame" marginwidth=0 marginheight=0 width=100% height=93% src="../detail/ungovCheckDetail1.jsp?taskCode=<%= taskCode%>&taskFlag=<%= taskFlag%>" frameborder=0></iframe>
...
<next:Window id="forCheck" closeAction="hide" title="填写审核意见" width="500"  autoScroll="true">
	<next:TopBar>
		...
		<next:ToolBarItem text="审核通过" iconCls="detail" handler="forCheckClose"></next:ToolBarItem>
		<next:ToolBarItem text="退回到处长审核" iconCls="detail" handler="backFlow"></next:ToolBarItem>
		...
		<form method="post" onsubmit="return false" class="L5form" dataset="flowDs">
			<table width="100%">
				<tr>
					...
					<td class="FieldInput" colspan="5">
					<select field="auditOpinionId" title="处理意见" name='"处理意见"' style="width:150px" default="0">
				...
				<tr>
					...
					<td class="FieldInput" colspan="5">
						<textarea rows="10" name='"处理补充意见"' field="auditOpinion" style="width:98%"></textarea>
					</td>
				</tr>
			</table>
		</form>
