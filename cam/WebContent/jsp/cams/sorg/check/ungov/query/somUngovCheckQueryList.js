...
function detail(){
	...
	data.put("taskFlag","Query");
	data.put("taskCode",selected[0].get('taskCode'));
	var url='jsp/cams/sorg/check/ungov/detail/ungovCheckDetail1.jsp';
	var text = '年检明细';
...
function print(){
	...
	data.put("method","DETAIL");
	data.put("taskCode",selected[0].get('taskCode'));
	var url='jsp/cams/sorg/check/ungov/print/ungovCheckPrint1.jsp';
	var text = '年检明细';
	L5.forward(url,text,data);
}
//点击导出Excel
function ToExcel(){
	...
	L5.dataset2excel(dataset,"/jsp/cams/sorg/check/ungov/query/somUngovQueryToExcel.jsp");
}
