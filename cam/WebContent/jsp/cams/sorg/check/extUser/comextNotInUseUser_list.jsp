...
<html>
	<head>
		<title>社会组织外网冻结用户维护列表</title>
		<next:ScriptManager/>
		<script type="text/javascript" src="comextNotInUseUser_list.js"></script>
...
<model:datasets>
	<model:dataset id="ds" cmd="com.inspur.cams.comm.extuser.cmd.ComExtUserQueryCmd" global="true">
		...
		<model:params>
			<model:param name="ORGAN_TYPE@in" value="ST,MF,JJ"></model:param>
			<model:param name="IN_USE" value="1"></model:param>
			<model:param name="IF_FREEZE" value="1"></model:param>
		</model:params>
	</model:dataset>
...
<next:GridPanel id="editGridPanel" name="comextuserGrid" width="100%" stripeRows="true" height="100%" dataset="ds" title="社会组织外网用户维护列表" notSelectFirstRow="true">
	<next:TopBar>
		...
		<next:ToolBarItem text="解除冻结" iconCls="edit" handler="changeStatus"/>
	</next:TopBar>
	<next:Columns>
		...
		<next:Column id="userName" header="社会组织名称" field="userName" width="250" >
			<next:TextField  />
		</next:Column>
		
		<next:Column id="organType" header="社会组织类型" field="organType" width="90"  renderer="detailHref">
			<next:TextField allowBlank="false" />
		</next:Column>
