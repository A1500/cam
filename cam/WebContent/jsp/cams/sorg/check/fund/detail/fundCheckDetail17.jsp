	...
	somCheckPublicActivitiesDataSet.on('load',function(ds){
		...
		if(count!=0){
			for(i=1;i<=count;i++){
				...
				document.getElementById("JNAME"+i).innerHTML=ds.getAt(i-1).get("activityName");
				document.getElementById("income"+i).innerHTML=ds.getAt(i-1).get("income");
				document.getElementById("expend"+i).innerHTML=ds.getAt(i-1).get("expenditure");
				document.getElementById("year"+i).innerHTML=ds.getAt(i-1).get("itemYear");
				document.getElementById("yearEnd"+i).innerHTML=ds.getAt(i-1).get("endYear");
				document.getElementById("itemContent"+i).innerHTML=ds.getAt(i-1).get("socialBenefits");
			}
		}
	});
	//创建页面导航下拉框
...
function getSomCheckPublicActivitiesDataSet(){
	...
	for(var i = 1;i<=5;i++){
		...
		record.set("activityName",document.getElementById("JNAME"+i).innerHTML);
...
<div id="float" class="noprint">
	<input type="button" value="下一页" style="height: 25px; width: 80px; position: absolute; margin-left: 120px;" onclick="nextPageJ1('18')" />
	<input type="button" value="上一页" style="height: 25px; width: 80px; position: absolute; margin-left: 20px;" onclick="nextPageJ1('16')" />
...
<TABLE cellSpacing=0 cellPadding=0 width=660 align=center border=0>
	<TBODY>
		...
		<TR>
			...
			id  = "times" style='text-align:center;border-bottom: solid 1px;width:30;font-size: 16.0pt; font-family: 仿宋_GB2312'></label> &nbsp;）项公益活动，具体情况如下：
			</TD>
		</TR>
		<TR>
			<TD>
