...
function loadsomSession(ds){
	...
	somMeetingDataSet.on('load',loadaspForLiShi);
...
function loadaspForLiShi(ds){
	...
	aspForLiShi.setParameter("SORG_DUTIES@in", "00001,00003,00004,00005");
	aspForLiShi.load();
...
function loadElectronic(ds){
	...
	somElectronicDataSet_load();
	fillOrganInfo(document.getElementById('sorgName').value);
}
//点击上传附件按钮
...
function loadOrganInfo(name){
	somOrganDataSetQuery.removeAll();
	somOrganDataSetQuery.setParameter("CN_NAME@=",name);
	somOrganDataSetQuery.setParameter("SORG_STATUS@in","9,23");
	somOrganDataSetQuery.setParameter("SORG_TYPE@=","J");
	somOrganDataSetQuery.load();
	somOrganDataSetQuery.on('load',checkOrgan);
}
//回填相应信息
function checkOrgan(ds){
	ds.un('load',checkOrgan);
	var name=document.getElementById("sorgName").value;
	if(somOrganDataSetQuery.getCount() == 0){
		L5.Msg.alert("提示","该基金会名称尚未成功核准或已经设立，无法进行设立登记！");
		document.getElementById("sorgName").value = '';
		cnName='';
		return;
	}else{
		...
		somOrganDataSet.set('cnName',name);
		cnName=name;
		changeSorgId(sorgId);//更新sorgId
	}
	somApplyDataSet.getCurrent().set("curOpinion","");
}
//更新sorgId
function changeSorgId(sorgId){
	somApplyDataSet.set("sorgId",sorgId);//业务
	//社会组织
	somOrganDataSet.getCurrent().set("sorgId",sorgId);
	somOrganDataSet.getCurrent().set("sorgStatus","9");
	//拟任法定代表人
	aspChiefDataset.set("sorgId",sorgId);
	aspChiefDataset.set('id','');
	//发起人
	var somHeldPeoples=somHeldPeopleDataSet.getAllRecords();
	for(var i=0;i<somHeldPeoples.length;i++){
		somHeldPeoples[i].state = L5.model.Record.STATE_NEW;
		somHeldPeoples[i].set("sorgId",sorgId);
	}
	//发起单位
	var somHeldOrgans=somHeldOrganDataSet.getAllRecords();
	for(var i=0;i<somHeldOrgans.length;i++){
		somHeldOrgans[i].state = L5.model.Record.STATE_NEW;
		somHeldOrgans[i].set("sorgId",sorgId);
	}
	//届次
	...
	//理事
	var aspForLiShis=aspForLiShi.getAllRecords();
	for(var i=0;i<aspForLiShis.length;i++){
		aspForLiShis[i].state = L5.model.Record.STATE_NEW;
		aspForLiShis[i].set("sorgId",sorgId);
		aspForLiShis[i].set("id","");
	}
	//监事
	var aspForJianShis=aspForJianShi.getAllRecords();
	for(var i=0;i<aspForJianShis.length;i++){
		aspForJianShis[i].state = L5.model.Record.STATE_NEW;
		aspForJianShis[i].set("sorgId",sorgId);
		aspForJianShis[i].set("id","");
	}
...
function confirmWin(){
	...
	if(checkSorgName(newName)){
		...
		win.hide();
		nameChangeFlag="1";
		document.getElementById('sorgName').value=newName;
