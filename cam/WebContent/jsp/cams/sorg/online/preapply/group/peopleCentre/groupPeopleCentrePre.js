		...
		somOrganDataSet.on("load",function(organDs){
			...
			somOrganDataSetQuery.on("load",function(){
				if(somOrganDataSetQuery.getCount() == 0){
					...
					somSessionQueryDataSet.setParameter("SORG_ID",sorgId);
					somSessionQueryDataSet.load();

					somSessionQueryDataSet.on('load',loadTrueResume);
				}
			});
		});
	});
...
function loadTrueResume(ds){
	...
	if(ds.getCount() > 0){
		...
		somResumeDataSet.on('load', loadTrueDuty);
	}
}
//加载届中的人员
function loadTrueDuty(ds){
	ds.un("load",loadTrueDuty);
	var somDutyQueryDataSet=L5.DatasetMgr.lookup("somDutyQueryDataSet");
	somDutyQueryDataSet.setParameter("SESSION_ID", sessionId);
	somDutyQueryDataSet.load();
	somDutyQueryDataSet.on('load', loadDuty);
}
//加载这次申请提交的人员
function loadDuty(ds){
	...
	somDutyDataSet.load();
	somDutyDataSet.on('load', loadResume);
}
//加载这次申请提交的人员简历
...
function addTogether(ds){
	...
	somDutyDataSet.add(somDutyQueryDataSet.getAllRecords());
	var somResumeDataSet=L5.DatasetMgr.lookup("somResumeDataSet");
...
function save(){
	...
	var somSessionRecord=somSessionQueryDataSet.getCurrent();
	var somDutyDataSet=L5.DatasetMgr.lookup("somDutyDataSet");
...
function setEndDate(){
	var somSessionQueryDataSet=L5.DatasetMgr.lookup("somSessionQueryDataSet");
	var record=somSessionQueryDataSet.getCurrent();
	var tenure=_$("tenure");
