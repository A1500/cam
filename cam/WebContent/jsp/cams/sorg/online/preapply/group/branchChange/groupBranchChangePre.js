...
function funAdd(somApplyDataSet){
	...
	somOrganDataSet.on('load',function(){
		if(somOrganDataSet.getCount()==0){
			L5.Msg.alert("提示","根据外网填报的分支（代表）机构名称在系统中没有查到对应的社会组织！");
			return ;
		}
...
function forShow(somChangeList){
	...
	for(var i=0;i<records.length;i++){
		...
		if(changeItem==0){
			...
			somDutyQueryDataSet.setParameter("NAME",records[i].get('changeAfter'));
			somDutyQueryDataSet.setParameter("SORG_ID",sorgIdExist);
			somDutyQueryDataSet.setParameter("IF_SERVE","1");
			somDutyQueryDataSet.setParameter("SORG_STATUS","00002");
			somDutyQueryDataSet.load();//法定代表人
			somDutyQueryDataSet.on('load',function(){
				if(somDutyQueryDataSet.getCount() >= 1){
					...
					for(var j=0;j<somDutyRecords.length;j++){
						...
						legalDutyId= somDutyRecords[j].get("dutyId");
					}
				}else{
					ifPeopleExist = '0';
...
function forChangeItem(num){
	...
	if(num=='0'){
		...
		somLegalPeopleDataSet.setParameter("REG_STATUS","3");//批准中的，必须先备案才能进行负责人的变更
		...
		somLegalPeopleDataSet.on('load',function(){
			if(somLegalPeopleDataSet.getCount() > 0){
				...
				var win=L5.getCmp('AddChange1'+num);
				win.show();
				document.getElementById('leaPeopleNameAfter').value=somLegalPeopleDataSet.get('name');
