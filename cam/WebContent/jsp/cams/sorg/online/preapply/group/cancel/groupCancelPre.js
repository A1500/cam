...
function funAdd(somOrganDataset){
	...
	if(num==0){
		...
		L5.Msg.alert("提示","登记证号不存在或不是社会团体的登记证号！");
	}else if(num>1){
		somOrganDataSet.newRecord();
		document.forms["cbForm"].reset();
		...
		somOrganDataSet.set("cancelReasonDesc",somOrganQueryDataSet.get("cancelReasonDesc"));
		somOrganDataSet.set("liqResult",somOrganQueryDataSet.get("liqResult"));
		//查看社会组织类型
		if(sorgType!="S"){
			somOrganDataSet.newRecord();
			document.forms["cbForm"].reset();
			L5.Msg.alert("提示","该登记证号不是社会团体的登记证号！");		
			return;
		}
		//查看社会组织状态
		if(sorgStatus!="22"){
			somOrganDataSet.newRecord();
			document.forms["cbForm"].reset();
			L5.Msg.alert("提示","社会团体不是已成立状态，不能进行注销操作！");
			return;
		}
...
function save(){
	...
	for(var i=0;i<somLiqpeopleRds.length;i++){
			...
			somLiqpeopleRds[i].set("sorgId",sorgId);
	}
	var somApplyRd = somApplyDataSet.getCurrent();
	somApplyRd.set("sorgId",sorgId);
	...
	var command = new L5.Command("com.inspur.cams.sorg.apply.group.cmd.GroupCancelCmd");
	//查看社会组织状态
	...
	command = new L5.Command("com.inspur.cams.sorg.apply.group.cmd.GroupCancelCmd");
	//提交保存
	...
	if(!command.error){	
		...
		var url='jsp/cams/sorg/online/preapply/group/cancel/groupCancelPreList.jsp';
