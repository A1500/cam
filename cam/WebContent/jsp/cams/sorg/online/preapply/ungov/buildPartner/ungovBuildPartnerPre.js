...
function init(){
	...
	L5.getCmp("sorgTab").setActiveTab("6");
...
function save(){
	...
	for(var i=0;i<somHeldPeopleRecords.length;i++){
		...
		somHeldPeopleRecords[i].set("sorgId",sorgIdExist);
		/**
		 * 处理网上申请的sorgId结束
		 */
	 ...
	 * 处理网上申请的sorgId
	 */
	LegalPeopleRecord.set("sorgId",sorgIdExist);
	organRecord.set("sorgId",sorgIdExist);
	somSessionRecord.set("sorgId",sorgIdExist);
	...
	for(var i=0;i<somDutyRecords.length;i++){
		...
		somDutyRecords[i].set("sorgId",sorgIdExist);
		somDutyRecords[i].set("peopleId","");
	}
	var somOfficeDataSetRds=somOfficeDataSet.getAllRecords();
	for(var i=0;i<somOfficeDataSetRds.length;i++){
		somOfficeDataSetRds[i].state = L5.model.Record.STATE_NEW;
		somOfficeDataSetRds[i].set("sorgId",sorgIdExist);
	}
	...
	for(var i=0;i<somHeldOrganRecords.length;i++){
		...
		somHeldOrganRecords[i].set("sorgId",sorgIdExist);
	}
	...
	for(var i=0;i<somPartyMemberRecords.length;i++){
		...
		somPartyMemberRecords[i].set("sorgId",sorgIdExist);
	}
	var somPartyPraiseDataSet=L5.DatasetMgr.lookup("somPartyPraiseDataSet");
	...
	for(var i=0;i<somPartyPraiseRecords.length;i++){
		...
		somPartyPraiseRecords[i].set("sorgId",sorgIdExist);
	}
	/**
	 * 处理网上申请的sorgId结束
	 */
	...
	//举办者
	command.setParameter("somHeldPeopleRds", somHeldPeopleRecords);
	//举办单位
	command.setParameter("somHeldOrganRds", somHeldOrganRecords);
	...
	command.setParameter("somPartyMemberRecords",somPartyMemberRecords);
	var somPartyPraiseDataSet=L5.DatasetMgr.lookup("somPartyPraiseDataSet");
...
function chooseLegal(){
	...
	if(records&&records.length==1){
		...
		L5.MessageBox.confirm('确定', '选定[ '+records[0].get("name")+' ]为法定代表人(负责人)？',function(sta){
			if(sta=="yes"){
				//如果法定代表人已经存在
				if(LegalPeopleDataSet.getCount() == 1 && LegalPeopleDataSet.getCurrent().get("name") != ""){
					L5.MessageBox.confirm('确定', '法定代表人(负责人)已经存在，是否覆盖？',function(yes){
						if(yes=="yes"){
...
function loadOrganInfo(name){
	...
	//somOrganDataSet.setParameter("SORG_KIND","1");//法人
	...
	somOrganDataSetQuery.on('load',function(){
		if(somOrganDataSetQuery.getCount() == 0){
			...
			//somOrganDataSet.set("sorgKind",'1');
		}
