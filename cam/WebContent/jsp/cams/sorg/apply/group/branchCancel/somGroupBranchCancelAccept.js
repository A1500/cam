...
function funAdd(somOrganDataset){
	...
	if(num==0){
		somOrganDataSet.newRecord();
		L5.Msg.alert("提示","登记证号不存在或不是社会团体分支（代表）机构的登记证号！");
	}else if(num>1){
		somOrganDataSet.newRecord();
		...
		if(sorgType!="S"){
			somOrganDataSet.newRecord();
			L5.Msg.alert("提示","该登记证号不是社会团体分支（代表）机构的登记证号！");		
			return;
		}
		//查看社会组织状态
		if(sorgStatus!="22"){
			somOrganDataSet.newRecord();
			L5.Msg.alert("提示","社会团体分支（代表）机构不是已成立状态，不能进行注销操作！");
			return;
		}	
		//查看社会组织主体状态
		var command = new L5.Command("com.inspur.cams.sorg.apply.group.cmd.GroupBranchCancelCmd");
		...
		if(mainSorgStatus!="22"){
			somOrganDataSet.newRecord();
			L5.Msg.alert("提示","社会团体主体机构不是已成立状态，不能进行注销操作！");
			return;
		}			

		document.getElementById("cancelReason").value="";
		document.getElementById("cancelReasonDesc").value="";
