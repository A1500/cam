...
<html>
<head>
<title>社会团体注销登记处理</title>
<next:ScriptManager></next:ScriptManager>
<script type="text/javascript" src="somGroupCancel.js"></script>
...
<model:datasets>
	...
	<model:dataset id="somOrganDataSet" cmd="com.inspur.cams.sorg.base.cmd.SomOrganQueryCmd" method="queryHis" global="true">
		...
		<%
			//从后台获取环节按钮权限，显示要显示按钮
						...
						for (int i = 0; i < list.size(); i++) {
							...
							String buttonIcon = "";
							if ("printYesCancel()".equals(buttonFunName)) {
								buttonIcon = "print";
							}
							if ("submit()".equals(buttonFunName)) {
								buttonIcon = "select";
							}
					...
					<legend>社会团体法人注销登记信息</legend>
						...
						<table width="100%">
							<tr>
								...
								<td class="FieldInput" style="width:30%"><label id="sorgCode" field="sorgCode" name="'登记证号'"/></td>
								<td class="FieldLabel" style="width:20%"><label>社团代码：</label></td>
								<td class="FieldInput" style="width:30%"><label field="organCode" name='"社团代码"'/></td>
							</tr>
							<tr>
								<td class="FieldLabel"><label>社团名称：</label></td>
								<td class="FieldInput"><label field="cnName" name="'社团名称'"/></td>
							...
							<tr>
								...
								<td class="FieldInput"><label id="sorgKind" field="sorgKind" name='"社团类别"' dataset="sorgKindSelect" /></td>
								<td class="FieldLabel"><label>所属行（事）业：</label></td>
								<td class="FieldInput"><label id="busScope" field="busScope" name='"所属行（事）业"' dataset="busScopeSelect" /></td>	
							</tr>
							<tr>							
							...
							<tr>
								...
								<td class="FieldInput"><label id="borgName" field="borgName" name='"业务主管单位"' dataset="businessOrganSelect" /></td>
							</tr>	
							<tr>
								<td class="FieldLabel"><label>注销社团原因：</label></td>
								<td class="FieldInput">
									<select id="cancelReason" field="cancelReason" name='"注销社团原因"' style="width:80%" disabled="disabled">
							...
							<tr id="cancelReasonDescTr" >
								<td class="FieldLabel"><label>注销原因说明：</label></td>
								<td class="FieldInput" colspan="3"><textarea rows="3" id="cancelReasonDesc" field="cancelReasonDesc" readonly="readonly" name='"注销原因说明"' style="width:93%"style="width:93%;background-color:#F5FAFA;border:0;overflow:auto;"></textarea></td>
							</tr>							
							<tr>
								<td class="FieldLabel"><label>清算结论：</label></td>
								<td class="FieldInput" colspan="3"><textarea rows="3" id="liqResult" field="liqResult" readonly="readonly" name="'清算结论'" style="width:93%"style="width:93%;background-color:#F5FAFA;border:0;overflow:auto;"></textarea></td>
							</tr>
						</table>
						</form>
						</fieldset>
				...
				<next:Panel title="处理意见" width="100%" height="100%">
					<next:Html>
						<form method="post" dataset="somApplyDataSet" onsubmit="return false" class="L5form">						
							<table width="100%">
								<tr>
									<td class="FieldLabel" style="width:15%"><label>处理意见：</label></td>
									<td class="FieldInput" style="width:85%">
										<select id='curOpinionId'field="curOpinionId" name="'处理意见'">
			...
			<table width="100%">
				<tr>
					...
					<td class="FieldInput" width="25%">
						<label type="text" name="'清算组织负责人'" field="ifChief" dataset="yesornoDataSet" style="width:90%"/>
					</td>
				</tr>
				<tr>
...
<next:Window id="winSomFlow" title="查看流程历史意见" width="650" height="100%" autoHeight="true" resizable="false" closeAction="hide" modal="true">
	<next:TopBar>
		<next:ToolBarItem symbol="->"></next:ToolBarItem>
		<next:ToolBarItem iconCls="return" text="关闭" handler="winCloseSomFlow"></next:ToolBarItem>
	</next:TopBar>
	<next:Panel id="panelWinFlow">
		<next:Html>
			<form id="formWinFlow" method="post" dataset="winSomFlowDataSet" onsubmit="return false" class="L5form">
			<table width="100%">
				<tr>
					<td class="FieldLabel" width="25%"><label>处理环节：</label></td>
					<td class="FieldInput" width="25%"><label type="text" name="'处理环节'" field="activity" style="width:90%"/></td>
					<td class="FieldLabel" width="25%"><label>处理单位：</label></td>
					<td class="FieldInput" width="25%"><label type="text" name="'处理单位'" field="organId" dataset="pubOrganDataSet" style="width:90%"/></td>
				</tr>
				<tr>
					<td class="FieldLabel"><label>处理人：</label></td>
					<td class="FieldInput"><label type="text" name="'处理人'" field="actorId" dataset="pubOrganDataSet" style="width:90%"/></td>
					<td class="FieldLabel"><label>处理部门：</label></td>
					<td class="FieldInput"><label type="text" name="'处理部门'" field="deptId" dataset="pubOrganDataSet" style="width:90%"/></td>
				</tr>
				<tr>
					<td class="FieldLabel"><label>意见：</label></td>
					<td class="FieldInput"><label type="text" name="'意见'" field="opinionId" dataset="opinionSelect2" style="width:90%"/></td>
					<td class="FieldLabel"><label>处理时间：</label></td>
					<td class="FieldInput"><label type="text" name="'处理时间'" field="commitTime" style="width:90%"/></td>
				</tr>				
				<tr>	
					<td class="FieldLabel"><label>处理补充意见：</label></td>
					<td class="FieldInput" colspan="3"><label type="text" name="'处理补充意见'" field="opinion" style="width:97%"/></td>
				</tr>
			</table>
			</form>
		</next:Html>
