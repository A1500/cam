...
function init(){
	...
	//过滤tab页
	if(actname == 'null' || actname=='supply'){
		L5.getCmp("tab").remove(8);
	}
	//党员奖罚情况中，授予称号过滤
...
//加载页面所有信息
function loadInfo(){
	somApplyDataSet.un("load",loadInfo);
...
function sumDutyOnLoad(){
	...
	LegalPeopleDataSet.insert(0,legalRecord.copy());
...
function updateInfo(){
	...
	LegalPeopleRecord.set("photoText",_$("photoId"));
...
//名称变更
function changeName(){
	var cnName=document.getElementById('cnName').innerHTML;
	if(cnName==''){
...
function confirmWin(){
	...
	if(checkSorgName(newName)){
		...
		win.hide();
		nameChangeFlag="1";
		document.getElementById('cnName').innerText=newName;
