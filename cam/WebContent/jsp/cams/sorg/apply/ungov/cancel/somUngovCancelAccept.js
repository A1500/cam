...
function loadInfo(){
	...
	somOrganDataSet.on("load",function(){
		...
		if(sorgType!="M"){
			somOrganDataSet.newRecord();
			L5.Msg.alert("提示","该登记证号不是民办非企业单位的登记证号！",function(){
				goBack();
			});		
			return;
		}
...
function funAdd(somOrganDataset){
	...
	if(num==0){
		somOrganDataSet.newRecord();
		L5.Msg.alert("提示","登记证号不存在或不是民办非企业单位的登记证号！");
	}else if(num>1){
		somOrganDataSet.newRecord();
		...
		if(sorgType!="M"){
			somOrganDataSet.newRecord();
			L5.Msg.alert("提示","该登记证号不是民办非企业单位的登记证号！");		
			return;
		}
		//查看社会组织状态
		if(sorgStatus!="22"){
			somOrganDataSet.newRecord();
			L5.Msg.alert("提示","民办非企业单位不是已成立状态，不能进行注销操作！");
			return;
		}			
