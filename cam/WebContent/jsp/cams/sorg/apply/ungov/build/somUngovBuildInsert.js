...
function init(){
	...
	if(method == 'INSERT'){
		...
		//LegalPeopleDataSet.newRecord({"taskCode":taskCode});
...
function fillHeldPeople(value){
	...
	somHeldPeopleDataSet.each(function(record, index){
		if(record.get('name') == value){
			var aspRecord =somDutyInsertDataSet.getCurrent();
			...
			aspRecord.set('personUnit',record.get('personUnit'));
...
function save(){
	...
	//举办者
	command.setParameter("somHeldPeopleRds", somHeldPeopleDataSet.getAllRecords());
	//举办单位
	command.setParameter("somHeldOrganRds", somHeldOrganDataSet.getAllRecords());
...
function chooseLegal(){
	...
	if(records&&records.length==1){
		...
		L5.MessageBox.confirm('确定', '选定[ '+records[0].get("name")+' ]为法定代表人(负责人)？',function(sta){
			if(sta=="yes"){
				//如果法定代表人已经存在
				if(LegalPeopleDataSet.getCount() == 1 && LegalPeopleDataSet.get("name" != '' )){
