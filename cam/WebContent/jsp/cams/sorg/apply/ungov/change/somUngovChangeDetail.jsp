...
function forCertType(){
	...
	if(num!='' && endDate!='' && beginDate!='' && PrintDate!=''){
		...
		var url=L5.webPath+"/jsp/cams/sorg/comm/print/ungovBuildForPrint.jsp?id="+sorgId+"&PrintDate="+PrintDate+"&beginDate="+beginDate+"&endDate="+endDate+"&taskCode="+taskCode+"&certType="+num+"&business="+encodeURIComponent(business);
		PostNewWin(url);
	}else{
		...
		<next:Tabs>
			<next:Panel title="基本信息"  width="100%" height="100%" autoScroll="true">
				<next:Html>
					...
					<next:EditGridPanel id="somResumeLegalGrid" dataset="somLegalPeopleResumeDataSet" stripeRows="true" title="本人简历" width="100%" height="90%" autoScroll="true">
						<next:TopBar>
							<next:ToolBarItem symbol="->"></next:ToolBarItem>
						</next:TopBar>
						<next:Columns>
					...
					<next:EditGridPanel id="uploadLegalPeopleGrid1" title="法律要件" stripeRows="true" dataset="somLegalPeopleElectronicDataSet" width="100%" height="100%" autoScroll="true">
						<next:TopBar>
							<next:ToolBarItem symbol="附件列表"></next:ToolBarItem>
							<next:ToolBarItem symbol="->"></next:ToolBarItem>
						</next:TopBar>
