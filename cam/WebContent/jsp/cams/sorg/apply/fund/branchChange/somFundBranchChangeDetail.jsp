...
function forCertType(){
	...
	if(num!='' && endDate!='' && beginDate!='' && PrintDate!=''&& actArea!=''){
		...
		var url=L5.webPath+"/jsp/cams/sorg/comm/print/fundBranchBuildForPrint.jsp?id="+sorgId+"&PrintDate="+PrintDate+"&beginDate="+beginDate+"&endDate="+endDate+"&taskCode="+taskCode+"&certType="+num+"&sorgCode="+sorgCode+"&business="+encodeURIComponent(business)+"&actArea="+encodeURIComponent(actArea);
		PostNewWin(url);
	}else{
				...
				<next:Panel>
					<next:Html>
						...
						<form method="post" dataset="somApplyDataSet" onsubmit="return false" class="L5form" >
								<table width="100%">
									<tr>
										<td class="FieldInput"colspan="3"><textarea name="基金会内部履行程序" field="process" readonly="readonly" cols="65" rows="4" style="width:95%"style="width:95%;background-color:#F5FAFA;border:0;overflow:auto;"></textarea></td>
									</tr>
								</table>
								</form>
							</fieldset>
