...
function forCertType(){
	...
	business=_$('business');
	actArea=_$('actArea');
	if(certType!='' && endDate!='' && beginDate!='' && PrintDate!='' && sorgCode!=''){
		if(endDate<beginDate){
			L5.Msg.alert('提示',"有效期限止不得早于有效期限起！");
			return;
		}
		var openUrl=url+"/jsp/cams/sorg/comm/print/fundBranchBuildForPrint.jsp?id="+sorgId+"&PrintDate="+PrintDate+"&beginDate="+beginDate+"&endDate="+endDate+"&taskCode="+taskCode+"&certType="+certType;
