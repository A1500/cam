...
function cnNameUK(){
	if(_$("cnName").trim() != ""){
		...
		//command.setParameter("mainSorgId",_$("mainSorgId"));
...
function ifEdit(){
	if(method=="UPDATE"){alert($("mainSorgCode"));
		$("mainSorgCode").disable=true;
	}
}
...
function printAccept(){
	if(cnName ==  ''){
		L5.Msg.alert("提示","请先录入基金会分支（代表）机构名称！");
		return ;
	}
