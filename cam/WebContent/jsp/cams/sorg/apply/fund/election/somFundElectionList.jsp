...
<html>
<head>
<script type="text/javascript" src="somFundElectionSession.js"></script>
<title>任务查询</title>
...
<script language="javascript">
	var procTypeId="somFundElection";
	var act='<%=act%>';
			...
			<next:Panel title='完结任务' width='100%' height='100%'>
				<next:Panel >
					<next:Html>
						...
						<table width="100%">
							<tr>
								<td class="FieldLabel" style="width:20%"><label>基金会名称：</label></td>
...
function handleyiban(){
	...
	var url='jsp/cams/sorg/apply/fund/election/somFundElectionDetail.jsp?taskCode='+rec.get("TASK_CODE")+"&method=yiban&act="+act+'&processId='+processId+"&actDefUniqueId="+actDefUniqueId+"&assignmentId="+assignmentId;
	L5.forward(url);
}
//查看已办流程图
...
function forDeal(){
	...
	var url;
	if(act == 'supply'){
		var url='jsp/cams/sorg/apply/fund/election/somFundElectionEdit.jsp?method=UPDATE&act='+act+'&taskCode='+rec.get("TASK_CODE")+'&processId='+processId+"&actDefUniqueId="+actDefUniqueId+"&assignmentId="+assignmentId;
	}else{
		var url='jsp/cams/sorg/apply/fund/election/somFundElectionDetail.jsp?taskCode='+rec.get("TASK_CODE")+"&method=daiban&act="+act+'&processId='+processId+"&actDefUniqueId="+actDefUniqueId+"&assignmentId="+assignmentId;
	}
	L5.forward(url);
}
