	...
	if (!command.error) {
		if(actname=='issue' || end){//是发证环节流程结束
			...
			L5.Msg.alert("提示","成功提交至下一环节!",function(){
				if(actname == 'audit'){
					if(newRecord.get("curOpinionId")=="1"){ //同意-批准通过-根据taskCode更改外网业务状态
						updateOnlineStatusPass(taskCode);
					}
					if(newRecord.get("curOpinionId")=="0"){ //不同意-批准不通过-根据taskCode更改外网业务状态
						updateOnlineStatusNoPass(taskCode);
					}
				}
				window.history.go(-1);
			});
...
function forChangeShow(ds){
	...
	if(changeItem=='0'){
		...
		document.getElementById('nameAfter').innerHTML=record.get('changeAfter');

	}else if(changeItem=='1'){
		document.getElementById('houseNameBefore').innerHTML=record.get('changeBefore');
		document.getElementById('houseNameAfter').innerHTML=record.get('changeAfter');
	}else if(changeItem=='2'){
		//document.getElementById('leaPeopleNameBefore').innerHTML = record2.get('changeBefore');
...
function forCertType(){
	...
	checkResult = encodeURIComponent(checkResult);
	if(business=='' ){
		L5.Msg.alert('提示',"请输入业务范围！");
		return;
	}
	if(checkResult=='' ){
		L5.Msg.alert('提示',"请输入年检记录！");
		return;
	}
	...
	if(certType!='' && endDate!='' && beginDate!='' && PrintDate!=''){
		...
		var url=L5.webPath+"/jsp/cams/sorg/comm/print/fundBuildForPrint.jsp?id="+sorgId+"&PrintDate="+PrintDate+"&beginDate="+beginDate+"&endDate="+endDate+"&taskCode="+taskCode+"&certType="+certType+"&business="+encodeURIComponent(business)+"&checkResult="+checkResult;
		PostNewWin(url);
	}else{
		...
		<next:Tabs>
			<next:Panel title="基金会变更登记申请表" width="100%" height="100%" autoScroll="true">
				...
				<next:Panel>
					<next:Html>
						<fieldset>
							<legend>基金会内部履行程序</legend>
					<form method="post" dataset="somApplyDataSet" onsubmit="return false" class="L5form" >
								<table width="100%">
									<tr>
										<td class="FieldInput"colspan="3"><textarea name="基金会内部履行程序" field="process" readonly="readonly" cols="65" rows="4" style="width:98%;background-color:#F5FAFA;border:0;overflow:auto;"></textarea></td>
									</tr>
								</table>
								</form>
								</fieldset>
							...
							<form method="post" dataset="somApplyDataSet" onsubmit="return false" class="L5form" >
								...
								<tr>
									...
									<td class="FieldInput" colspan="5"><textarea rows="3" field="examinOpinion" style="width:98%;background-color:#F5FAFA;border:0;overflow:auto;" readonly="readonly" ></textarea>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td class="FieldInput" colspan="5"><textarea rows="3" name='审核补充意见' field="checkOpinion" style="width:98%;background-color:#F5FAFA;border:0;overflow:auto;" readonly="readonly"></textarea>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td class="FieldInput" colspan="5"><textarea rows="3" name='批准补充意见' field="auditOpinion" style="width:98%;background-color:#F5FAFA;border:0;overflow:auto;" readonly="readonly"></textarea>
									</td>
								</tr>
							</table>
					</form>
			...
			<next:Panel title="流程历史意见"id="historyPanel">
				...
				<next:Columns>
					...
					<next:Column header="处理意见" field="opinionId"
						 width="10%"editable="false"  dataset="hisOpinionDataSet">
						<next:TextField/>
					</next:Column>
						...
						<table width="100%">
							...
							<tr>
								...
								<tr>
									...
									<td class="FieldInput" colspan="5"><label type="text"  field="sorgCheckDate" format="Y-m-d" onclick="WdatePicker()" style="width:22%"/></td>
							</tr>
							<tr>
...
<table width="100%">
			...
			<tr>
				...
				<td class="FieldInput"><label  id="nameAfter" style="color:blue"/></td>
			</tr>
			<tr>
				<td class="FieldLabel"><label style="color:blue">变更程序：</label></td>
				<td class="FieldInput"><textArea cols="75" rows="4" style="width: 100%;color:blue" field='changeDesc' readonly="readonly"></textArea></td>
			</tr>
			<tr>
...
<!-- 法定代表人变更 -->
<next:Window id="AddChange2" modal="true" closeAction="hide" title="法定代表人变更" width="600"  autoScroll="true">
	<next:TopBar>
		<next:ToolBarItem text='' id="displayItem" />
		<next:ToolBarItem symbol="->"></next:ToolBarItem>
...
<table width="100%">
			<tr>
				...
				<td class="FieldInput"style="width:25%"><label  id="leaPeopleNameBefore" field="changeBefore"/></td>
...
<table width="100%">
			...
			<tr>
				...
				<td class="FieldInput"><label id="houseNameAfter"  style="color:blue"/></td>
			</tr>
			<tr>
				<td class="FieldLabel"><label style="color:blue">变更程序：</label></td>
				<td class="FieldInput"><textArea cols="75" rows="4" readonly="readonly" style="width: 100%;color:blue" field="changeDesc" ></textArea></td>
			</tr>
			<tr>
				<td class="FieldLabel"><label style="color:blue">变更理由：</label></td>
...
<!-- 变更原始基金数额 -->
<next:Window id="AddChange4" modal="true" closeAction="hide" title="原始基金数额变更" width="500"  autoScroll="true">
	<next:TopBar>
		<next:ToolBarItem text='' id="displayItem" />
		<next:ToolBarItem symbol="->"></next:ToolBarItem>
		<next:ToolBarItem text="关闭" iconCls="return" handler="closeChangeWin"></next:ToolBarItem>
...
<table width="100%">
			...
			<tr>
				...
				<td class="FieldInput"><label id="moneyAfter" style="color:blue"></label>万元</td>
			</tr>
			<tr>
				<td class="FieldLabel"><label style="color:blue">基金来源变更为：</label></td>
				<td class="FieldInput"><label id="moneySourceAfter" style="color:blue"/></td>
			</tr>
			<tr>
				<td class="FieldLabel"><label style="color:blue">变更程序：</label></td>
...
<next:Window id="AddChange5" modal="true" closeAction="hide" title="业务主管单位变更" width="600"  autoScroll="true">
	...
	<table width="100%">
			<tr>
				...
				<select
					id="borgNameBefore" field="changeBefore" disabled="disabled"style="width:80%">
			...
			<tr>
				...
				<td class="FieldInput"style="width:25%"><select
					field="changeAfter"style="width:80%" disabled="disabled">
					<option dataset="businessOrganSelect"></option>
				</select></td>
			</tr>
			<tr>
...
<table width="100%">
			...
			<tr>
				...
				<td class="FieldInput" style="width:15%" colspan="3"><textarea  rows="8" id="business"  style="width:100%" ></textarea></td>
			</tr>
			<tr>
				<td class="FieldLabel" style="width:15%"><label>年检记录：</label></td>
				<td class="FieldInput" style="width:15%" colspan="3"><input type="text" id="checkResult"  style="width:100%" ></td>
			</tr>
			<tr>
...
<!-- 基金会类型变更 -->
<next:Window id="AddChange6" closeAction="hide" title="基金会类型变更" width="600"  autoScroll="true" modal="true">
	<next:TopBar>
		<next:ToolBarItem text='' id="displayItem" />
		<next:ToolBarItem symbol="->"></next:ToolBarItem>
		<next:ToolBarItem text="关闭" iconCls="return" handler="closeChangeWin"></next:ToolBarItem>
