			...
			for(i=1;i<=somJxOrganFundsDataSet10.getCount();i++){
				...
				for(j=1;j<=somJxOrganFundsDataSet10.getCount();j++){
					if(somJxOrganFundsDataSet10.getAt(j-1).get("seqNum") == i){
						...
						var ifBuildAdministration = "";
						if(somJxOrganFundsDataSet10.getAt(j-1).get("ifBuildAdministration")=="0"){
							ifBuildAdministration="否";
						}
						if(somJxOrganFundsDataSet10.getAt(j-1).get("ifBuildAdministration")=="1"){
							ifBuildAdministration="是";
						}
						tab.rows[rowLength-1].cells[2].getElementsByTagName("label")[0].innerHTML=ifBuildAdministration;
						tab.rows[rowLength-1].cells[3].getElementsByTagName("label")[0].innerHTML=somJxOrganFundsDataSet10.getAt(j-1).get("organDate");
						var accountType = "";
						if(somJxOrganFundsDataSet10.getAt(j-1).get("accountType")=="1"){
							accountType="基本账户";
						}
						if(somJxOrganFundsDataSet10.getAt(j-1).get("accountType")=="2"){
							accountType="一般账户";
						}
						tab.rows[rowLength-1].cells[4].getElementsByTagName("label")[0].innerHTML=accountType;
