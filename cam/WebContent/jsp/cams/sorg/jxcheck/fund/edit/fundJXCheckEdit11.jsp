			...
			for(i=1;i<=somJxOrganFundsDataSet.getCount();i++){
				...
				for(j=1;j<=somJxOrganFundsDataSet.getCount();j++){
					if(somJxOrganFundsDataSet.getAt(j-1).get("seqNum") == i){
						...
						tab.rows[rowLength-1].cells[4].getElementsByTagName("input")[0].value=somJxOrganFundsDataSet.getAt(j-1).get("place");
					}
				}
...
function addFTr(){
    ...
    newNameTD.style.borderRight="solid windowtext 1.0pt";
    //添加列内容
    newNameTD.innerHTML ="<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋_GB2312'><input type='text'  maxlength='100' style='width:100%' ></span></p>"
    //添加列:负责人
