			...
			for(i=1;i<=somJxOrganFundsDataSet.getCount();i++){
				...
				for(j=1;j<=somJxOrganFundsDataSet.getCount();j++){
					if(somJxOrganFundsDataSet.getAt(j-1).get("seqNum") == i){
						...
						tab.rows[rowLength-1].cells[4].getElementsByTagName("textarea")[0].value=somJxOrganFundsDataSet.getAt(j-1).get("place");
						tab.rows[rowLength-1].cells[5].getElementsByTagName("select")[0].value=somJxOrganFundsDataSet.getAt(j-1).get("accountType");
						tab.rows[rowLength-1].cells[6].getElementsByTagName("textarea")[0].value=somJxOrganFundsDataSet.getAt(j-1).get("openAccount");
					}
				}
...
function addFTr(){
    ...
    newNameTD.style.borderRight="solid windowtext 1.0pt";
    //添加列内容
    newNameTD.innerHTML ="<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋_GB2312'><textarea id='organName"+xuhao+"' onkeyup='this.value = this.value.slice(0,100)' ROWS='3' COLS='30'></textarea></span></p>"
    //添加列:负责人
    ...
    newNameTD.style.borderRight="solid windowtext 1.0pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 14.0pt; font-family: 仿宋_GB2312'><input id='peopleName"+xuhao+"' type='text' style='width: 100%;' maxlength='72'></span></p>";
    //添加列:登记时间
    ...
    newNameTD.style.borderBottom="solid windowtext 1.0pt";
    //添加列内容
    newNameTD.innerHTML = "<p class=MsoNormal style='mso-line-height-alt: 12.0pt'><span lang=EN-US style='font-size: 12.0pt; font-family: 仿宋_GB2312'><textarea  ROWS='3' COLS='30' onkeyup='this.value = this.value.slice(0,100)' ></textarea></span></p>";
...
<div id="float">
	<input type="button" value="下一页" style="height: 25px; width: 80px; position: absolute; margin-left: 120px;" onclick="nextPageJ('10')" />
	<input type="button" value="上一页" style="height: 25px; width: 80px; position: absolute; margin-left: 20px;" onclick="nextPageJ('8')" />
