...
function init() {
	...
	if(method=="INSERT"){
		...
		somOrganDataSet.newRecord({"sorgId":sorgId,"sorgStatus":'12',"sorgType":"S","ifBranch":'0'});
...
function goBack(){
	if(method=="UPDATE"){
		var data=new L5.Map();
		data.put("searchCnName",encodeURIComponent(encodeURIComponent(cnName)));
		data.put("searchSorgCode",sorgCode);	
		url="jsp/cams/sorg/history/group/prepare/groupHistoryPrepareList.jsp";
		L5.forward(url,'',data);
	}else{
		history.go(-1);
	}
