...
function init(){
	...
	if(method=="INSERT"){
		...
		//流程意见
		somApplyDataSet.newRecord({"taskCode":taskCode,"sorgId":sorgId,"applyType":applyType,"dataSource":"MEND","sorgType":"S","acceptOpinionId":"1","examinOpinionId":"1","checkOpinionId":"1","auditOpinionId":"1"});
	}else{
		forUPdateLoad();
	}
...
function fillOrganInfo(){
	if(method=="INSERT"){
		if(""!=_$("cnName")){
			...
			somOrganDataSet.on("load",fillOrganInfoOrganOnload);
		}
	}
}
function fillOrganInfoOrganOnload(){
				if(somOrganDataSet.getCount()!=0){
					changeResideSource();
					if(somOrganDataSet.get("sorgStatus")!="12"){
						L5.Msg.alert("提示","该社会团体名称已经存在！");
						somOrganDataSet.newRecord();
					}else{
						sorgId=somOrganDataSet.get("sorgId");
					}
				}else{
					sorgId=currentSorgId;
					somOrganDataSet.newRecord({"cnName":_$("cnName"),"sorgId":sorgId});
				}
				somOrganDataSet.un('load',fillOrganInfoOrganOnload);
}
...
function goBack(){
	if(method=="UPDATE"){
		...
		url="jsp/cams/sorg/history/group/mainBuild/groupHistoryBuildList.jsp";
		L5.forward(url,'',data);
	}else{
		history.go(-1);
	}
...
function confirm(){
	...
	record.set("promiseOrgan",document.getElementById('promiseOrgan').value);
	record.set("promiseCode",document.getElementById('promiseCode').value);
	if(updateRecord != ""){
