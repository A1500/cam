...
function init() {
	if(method=="INSERT"){
		...
		somApplyDataSet.newRecord({"taskCode":taskCode,"dataSource":"MEND","sorgType":"J","acceptOpinionId":"1","examinOpinionId":"1","checkOpinionId":"1","auditOpinionId":"1"});
	}else{
...
function save(){
	...
	somOrganQueryDataSet.on("load",function(ds){
		...
		if(method=="INSERT"){
			...
			if(sorgStatus!="32"){
				L5.Msg.alert("提示","基金会分支/代表机构不是已注销状态，不能进行修改操作！",function(){
					goBack();
				});
				return;
			}
...
function goBack(){
	if(method=="UPDATE"){
		...
		url="jsp/cams/sorg/history/fund/branchCancel/fundBranchHistoryCancelList.jsp";
		L5.forward(url,'',data);
	}else{
		history.go(-1);
	}
