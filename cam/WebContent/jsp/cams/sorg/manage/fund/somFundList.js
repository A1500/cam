...
function insert(){
	var url = "jsp/cams/sorg/manage/fund/somFundInsert.jsp";
	var text = "主体录入";
	L5.forward(url,text);
}

function update(){
	var url = "jsp/cams/sorg/manage/fund/somFundUpdate.jsp";
	var text = "主体修改";
...
function cancel(){
	var url = "jsp/cams/sorg/manage/fund/somFundCancel.jsp";
	var text = "主体注销";
	var grid = L5.getCmp("grid");
	var sm = grid.getSelectionModel();
	var record = sm.getSelected();
	...
	if(record.get("sorgStatus")=="52"){
		...
		if(num != 0){
			L5.Msg.alert("提示","此基金会还有"+num+"个未被注销的分支/代表机构，请先将其注销！");
			return;
		}
	}
...
function repeal(){
	...
	if(record.get("sorgStatus")=="32" || record.get("sorgStatus")=="62"){
		L5.Msg.alert("提示","选中的基金会已注销，不能进行撤销操作!");
		return;
	}
	if(record.get("sorgStatus")=="52"){
		L5.Msg.alert("提示","选中的基金会已被撤销!");
		return;
	}
	var id = record.get("sorgId");
	//查询基金会的分支是否已被撤销
	...
	var num = command.getReturn("num");
	if(num != 0){
		L5.Msg.alert("提示","此基金会还有"+num+"个未被撤销的分支/代表机构，请先将其注销！");
		return;
	}
...
function branch(){
	...
	url = "jsp/cams/sorg/manage/fund/somFundBranchList.jsp";
	var map = new L5.Map();
...
function detail(){
	...
	if(sorgStatus=="22" || sorgStatus=="52"){
		url = "jsp/cams/sorg/manage/fund/somFund.jsp";
	}
	if(sorgStatus=="32" || sorgStatus=="62"){
		url = "jsp/cams/sorg/manage/fund/somFund2.jsp";
	}
	var id = record.get("sorgId");
	var map = new L5.Map();
	map.put("sorgId",id);
