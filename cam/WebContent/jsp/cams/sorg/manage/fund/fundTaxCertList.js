function init(){
	...
	somTaxCertDataSet.setParameter("MORG_AREA@=",morgArea);
	somTaxCertDataSet.setParameter("CREATE_PEOPLE@=",peopleName);
	somTaxCertDataSet.setParameter("TASK_CODE@LIKE",'J%');
	somTaxCertDataSet.load();
	somTaxCertDataSet.on("load",somTaxCertOnLoad);
	L5.QuickTips.init();
}

function somTaxCertOnLoad(ds){
...
function insert(){
	...
	var url="jsp/cams/sorg/manage/fund/fundTaxCertEdit.jsp";
	L5.forward(url,'',data);
}
function detail(){
	var grid=L5.getCmp("grid");
...
function detailHis(){
	...
	data.put("sorgId",records[0].get("sorgId"));
	data.put("cnName",encodeURIComponent(encodeURIComponent(records[0].get("certNo"))));
	data.put("sorgCode",records[0].get("regPeople"));
	data.put("id",records[0].get("id"));
	data.put("taskCode",records[0].get("taskCode"));
	var url="jsp/cams/sorg/manage/fund/fundTaxCertDetail.jsp";
	L5.forward(url,'',data);
}