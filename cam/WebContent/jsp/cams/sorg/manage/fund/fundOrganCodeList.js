function init(){
	...
	somOrganCodeDataSet.setParameter("MORG_AREA@=",morgArea);
	somOrganCodeDataSet.setParameter("CREATE_PEOPLE@=",peopleName);
	somOrganCodeDataSet.setParameter("TASK_CODE@LIKE",'J%');
	somOrganCodeDataSet.load();
	somOrganCodeDataSet.on("load",somOrganCodeOnLoad);
	L5.QuickTips.init();
}

function somOrganCodeOnLoad(ds){
	ds.each(function(record, id){
		...
		record.set('organCode',cnName);
		record.set('regPeople',sorgCode);
	});
	ds.commitChanges();
}
function queryItem(){
...
function insert(){
	...
	var url="jsp/cams/sorg/manage/fund/fundOrganCodeEdit.jsp";
	L5.forward(url,'',data);
}
function detail(){
	var grid=L5.getCmp("grid");
...
function detailHis(){
	...
	data.put("sorgId",records[0].get("sorgId"));
	data.put("cnName",encodeURIComponent(encodeURIComponent(records[0].get("organCode"))));
	data.put("sorgCode",records[0].get("regPeople"));
	data.put("id",records[0].get("id"));
	data.put("taskCode",records[0].get("taskCode"));
	var url="jsp/cams/sorg/manage/fund/fundOrganCodeDetail.jsp";
	L5.forward(url,'',data);
}