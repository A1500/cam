...
var borgName;
 function init(){
	...
	if(sorgCode!='null'){
		$("sorgCode").value=sorgCode;
		querySorgOrganMess();	
	}
	somPartyMemberDS.newRecord({"memberId":memberId});
	...
	var peopleType=_$('peopleType');
 	...
 	//根据列表传过来的参数判断是否是整建制转入
	showFullSystemInDiv();
	//根据列表传过来的参数判断是否是转入，转入显示转入相关数据项，准建制转入项灰色不能修改
	showInDiv();
...
function queryOrgan(organDataSet){
	if(organDataSet.getCount()==0){
		...
		var somPartyQueryDataSet=L5.DatasetMgr.lookup("somPartyQueryDataSet");
		somPartyQueryDataSet.setParameter("SORG_ID@=",sorgId);
		...
		somPartyQueryDataSet.on('load',function(){
			if(somPartyQueryDataSet.getCount()>0){
				if(somPartyQueryDataSet.get('partyStatus')=='1'){
					L5.Msg.alert("提示",'所属党组织已撤销，不能进行新增操作！');
					$("sorgCode").value="";
					sorgId='';
					return;
				}
 ...
 function savePartyMember(){
		...
		somPartyMember.set("sorgId",sorgId);
		...
		command.execute("insertPartyMember");
		if (!command.error) {
			L5.MessageBox.confirm('保存成功', '是否需要继续添加党员？',function(state){
			if(state=="yes"){
			var data=new L5.Map();
...
function checkSorgCodeIfNull(){
	if($("sorgCode").value == ""){
		L5.Msg.alert("提示","请先填写社会组织登记证号!");
		$("sorgCode").focus();
		return false;
	}
	return true;
 ...
 * 如是显示整建制转入相关数据项
 */
function showFullSystemInDiv(){
	if(ifFullSystemIn == "1"){
		var somPartyMemberDS=L5.DatasetMgr.lookup("somPartyMemberDS");
		somPartyMemberDS.set('inType',"1");
