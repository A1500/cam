...
/*function printcert(){
	var text="";
	var url="jsp/cams/sorg/manage/ungov/somUngovCertPrint.jsp";
	L5.forward(url,text);
}*/
//点击增加证书信息按钮跳转到增加页面
function insert(){
	var text="";
	var url="jsp/cams/sorg/manage/ungov/somUngovCertInsert.jsp";
	L5.forward(url,text);
}
...
function updateCert(){
		...
		if(records&&records.length==1){
			...
			var url="jsp/cams/sorg/manage/ungov/somUngovCertUpdate.jsp?id="+id;
			var text = '修改证书信息';
			L5.forward(url,text);
		}else{
...
function changeCert(){
		...
		if(records&&records.length==1){
			...
			var id=records[0].get("id");
			var url="jsp/cams/sorg/manage/ungov/somUngovCertChange.jsp?id="+id;
			var text = '换发证书信息';
			L5.forward(url,text);
		}else{
			L5.Msg.alert("提示","请选择一条记录！");
...
function jtxxDetailHref(value,cellmeta,record,rowindex,colindex,dataset){
	var ds=L5.DatasetMgr.lookup("somUngovCertDataset");
...
function inPrintDemo(){
	...
	var text='';
	if(num==0){
		url="jsp/cams/sorg/manage/ungov/somUngovCertPrintView.jsp?id=ss";	
	}else if(num==1){
		url="jsp/cams/sorg/manage/ungov/somUngovCertPrintView.jsp?id=ss";
	}else if(num==2){
		url="jsp/cams/sorg/manage/ungov/somUngovCertPrintView.jsp?id=ss";
