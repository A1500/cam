function querySorgUngovMess(){
	$("sorgId").value="";
	$("cnName").innerHTML="";
	var sorgCode=_$("sorgCode").trim();
	if(sorgCode!=""){
	var organDataSet=L5.DatasetMgr.lookup("organDataSet");
	organDataSet.setParameter("SOM_ORGAN.SORG_TYPE@=","M");
	organDataSet.setParameter("SOM_ORGAN.SORG_CODE@=",sorgCode);
	organDataSet.load();
	organDataSet.on('load',queryUngov);
	}

}	
function queryUngov(organDataSet){
	if(organDataSet.getCount()==0){
		...
		L5.Msg.alert("提示","此民办非企业单位已注销，不可以进行备案！");
		return;
	}else if("52"==organDataSet.getCurrent().get("sorgStatus")){
		L5.Msg.alert("提示","此民办非企业单位已撤销，不可以进行备案！");
		return;
		...
		L5.Msg.alert("提示","此民办非企业单位未正式成立，不可以进行备案！");
		return;
	}
}