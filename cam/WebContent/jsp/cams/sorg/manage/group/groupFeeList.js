function init(){
	...
	somFeeDataSet.setParameter("MORG_AREA@=",morgArea);
	somFeeDataSet.setParameter("CREATE_PEOPLE@=",peopleName);
	somFeeDataSet.setParameter("TASK_CODE@LIKE",'S%');
	somFeeDataSet.load();
	somFeeDataSet.on("load",somFeeOnLoad);
	L5.QuickTips.init();
}

function somFeeOnLoad(ds){
	ds.each(function(record, id){
		...
		record.set('fee',cnName);
		record.set('regDate',sorgCode);
	});
	ds.commitChanges();
}
...
function insert(){
	...
	var url="jsp/cams/sorg/manage/group/groupFeeEdit.jsp";
	L5.forward(url,'',data);
}
function detail(){
	var grid=L5.getCmp("grid");
...
function detailHis(){
	...
	data.put("cnName",encodeURIComponent(encodeURIComponent(records[0].get("fee"))));
	data.put("sorgCode",records[0].get("regDate"));
	data.put("sorgId",records[0].get("sorgId"));
