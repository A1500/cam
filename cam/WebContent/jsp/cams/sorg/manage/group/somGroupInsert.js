...
//---------------------------------社团基本信息校验-----------------------------------------------
	...
	//社团名称/中文
	if(cnNameNum != 0){
		L5.Msg.alert("提示","校验未通过："+"输入的社团名称/中文已存在");
		L5.getCmp("tab").setActiveTab("0");
		return;
	}
	...
	if(_isPostCode(_$("sorgPost")) != true){
		L5.Msg.alert("提示","校验未通过："+"社团邮编格式不正确");
		L5.getCmp("tab").setActiveTab("0");
		return;
	}
	//社团电话号码
	...
	if(isFTel(_$("sorgFax")) !=true && _$("sorgFax") != ""){
		L5.Msg.alert("提示","校验未通过："+"社团传真格式不正确");
		L5.getCmp("tab").setActiveTab("0");
		return;
	}
	//社团网址
	if(IsURL(_$("sorgWeb")) !=true && _$("sorgWeb") != ""){
		L5.Msg.alert("提示","校验未通过："+"社团网址格式不正确");
		L5.getCmp("tab").setActiveTab("0");
		return;
	}
	//单位会员数和个人会员数
	if(_$("unitsNum").trim() == "" && _$("peopleNum").trim() == ""){
		L5.Msg.alert("提示","校验未通过："+"单位会员数或个人会员数不能为空");
		L5.getCmp("tab").setActiveTab("0");
		return;
	}
	if(_$("unitsNum").trim() != "" && _$("peopleNum").trim() == ""){
...
function editItem(){
	...
	if(record.get("isLegal")=="1"){
		L5.Msg.alert("提示","此人已在其他社会团体或基金会担任法定代表人，不能选用!");
		return ;
	}
