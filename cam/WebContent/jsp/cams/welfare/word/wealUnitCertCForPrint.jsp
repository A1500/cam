...
function init(){  
	...
	//取得文件名 和标签    
	if(pageNum=="2"){
		nowFileName = "unitCertC2.doc";
	}else{
		nowFileName = "unitCertC1.doc";
	}
	...
	welfCertDataSet.on("load",function(ds){
		...
		showMap.put("serialNum",serialNum);
		...
		showMap.put("startDateY",ds.get("startDate").split("-")[0]);
		showMap.put("startDateM",ds.get("startDate").split("-")[1]);
		showMap.put("endDateY",ds.get("endDate").split("-")[0]);
		showMap.put("endDateM",ds.get("endDate").split("-")[1]);
		...
		//打开副本第二页打印
		if(pageNum!="2"){
			url=L5.webPath+"/jsp/cams/welfare/word/wealUnitCertCForPrint.jsp?taskCode="+taskCode+"&pageNum=2";
			window.open(url);
		}
	});
}
