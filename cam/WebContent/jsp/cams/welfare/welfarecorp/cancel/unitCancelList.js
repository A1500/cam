function init() {
	...
	//根据企业类别加载数据
	ds.setParameter("CORPORATION_TYPE@=","2");
	ds.setParameter("WEAL_FIT_LOGOFF.MORG_AREA@LIKE",organCode+"%");
...
function insert() {
	...
	var url='jsp/cams/welfare/welfarecorp/cancel/unitCancelEdit.jsp';
	var text = '增加注销';
...
function detail(){
	...
	data.put("taskCode",selected[0].get('taskCode'));
	var url='jsp/cams/welfare/welfarecorp/cancel/unitCancelDetail.jsp';
	var text = '查看注销';
	L5.forward(url,text,data);
}
