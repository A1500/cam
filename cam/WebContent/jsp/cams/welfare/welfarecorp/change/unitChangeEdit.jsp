...
<script language="javascript">
	...
	function init(){
		if(method=="INSERT"){
			...
			ds.on('load',function(ds){
				if(ds.getCount()==0){
					ds.newRecord({"changId":changId,"taskCode":taskCode,"corporationType":"2","morgArea":organArea,"morgName":organName});
				}
			});
		}else{
			ds.setParameter("TASK_CODE@=", taskCode);
	...
	function save(){
		...
		command.execute("saveUnitChange");
		if (!command.error) {
			method="UPDATE";
...
<table border=0 cellpadding=0 cellspacing=0 width=678 class=unnamed1 style='border-collapse:
 ...
 <tr height=52 style='mso-height-source:userset;height:39.0pt' dataset="ds">
  ...
  border-left:none;width:125pt'><div align="left"><input id="companyName" field="companyName" title="企业名称" style="width:98%;" onclick="queryUnitList()"/></div>　</td>
 ...
 <input id="firstScope" field="firstScope" style="display:none;" />
...
<next:Window id="unitWin" title="选择要变更的企业" width="650" height="400" autoHeight="true" resizable="false" closeAction="hide" modal="true">
	...
	<next:GridPanel id="editGridPanel" name="wealUnitstatusGrid" width="98%" stripeRows="true" height="300" dataset="dsWin" autoScroll="true" title="企业列表">
		<next:TopBar>
