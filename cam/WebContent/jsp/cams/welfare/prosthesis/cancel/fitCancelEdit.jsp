...
<script language="javascript">
	...
	function init(){
		if(method=="INSERT"){
			...
			ds.on('load',function(ds){
				if(ds.getCount()==0){
					ds.newRecord({"taskCode":taskCode,"corporationType":"1"});
				}
			});
		}else{
			ds.setParameter("TASK_CODE@=", taskCode);
	...
	function save(){
		...
		record.set("regDate",'<%=DateUtil.getTime()%>')
	...
	function fillInfo(){
		...
		wealFitStatusDataSet.setParameter("SERIAL_NUM@=",_$("serialNum"));
		if(_$("serialNum")!=""){
			wealFitStatusDataSet.load();
		}
	...
	function back(){
		var data = new L5.Map();
		var url='jsp/cams/welfare/prosthesis/cancel/fitCancelList.jsp';
		L5.forward(url);
	}
	//************************************弹出窗口方法开始
	//点击企业名称弹出企业列表
...
<next:Window id="fitWin" title="选择要注销的企业" width="650" height="400" autoHeight="true" resizable="false" closeAction="hide" modal="true">
