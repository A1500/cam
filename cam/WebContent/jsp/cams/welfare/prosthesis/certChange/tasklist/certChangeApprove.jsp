...
<iframe id="applyframe" marginwidth=0 marginheight=0 width=100% height=93% src="../info/fitCertChangeDetail1.jsp?method=DETAIL&act=act&taskCode=<%=request.getParameter("primaryKey") %>" frameborder=0></iframe>
...
function init() {
	...
	if(actDefName=='补齐补正'){
		document.getElementById("applyframe").src="../info/fitCertChangeEdit1.jsp?method=DETAIL&taskCode="+taskCode;
	}
...
function send() {
	...
	if(validateNotNull(fitApplyRecord,"")){
		...
		command.execute("sendCertChange");
		if(!command.error){
...
function goBack(item) {
	//history.go(-1);
	var url='jsp/cams/welfare/prosthesis/certChange/tasklist/certChangeApproveList.jsp?actId='+getActId(actDefName);
	L5.forward(url);
}
//校验非空的域是否为空
