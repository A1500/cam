...
<script language="javascript">
	...
	function save(){
		...
		command.setParameter("applyType","3");//调用年检同样的方法，此参数传递至后台判断是年检还是换证
		...
		wealFitStatusDataSet.setParameter("SERIAL_NUM@=",serialNum);
			...
			wealFitStatusDataSet.on('load',function(ds){
				if(ds.getCount()>0){
					...
					var wealFitInfoDataSet = L5.DatasetMgr.lookup("wealFitInfoDataSet1");
					wealFitInfoDataSet.setParameter("SERIAL_NUM@=",serialNum);
					wealFitInfoDataSet.setParameter("APPLY_YEAR@=",applyYear);
					wealFitInfoDataSet.load();
					wealFitInfoDataSet.on('load',function(ds){
