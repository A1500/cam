...
<script type="text/javascript">
 	...
 	function init(){
		...
		wealFitInfoDataSet.on("load",function(ds){
			if(ds.getCount==0){
				...
				document.getElementById("totalNum").value=totalNum==null?"0":totalNum;
				document.getElementById("disNum").value=disNum==null?"0":disNum;
				document.getElementById("noProNum").value=noProNum==null?"0":noProNum;
				document.getElementById("proNum").value=proNum==null?"0":proNum;
								...
								var otr=tjb1.insertRow();//插入一个tr 
						 		 ...
						 		 ocel1.innerHTML="<div align='center'>"
						 		 						+ds.getAt(i).get('deviceName')
											     			+"<input type='hidden' id='deviceCode"+index+"' vlaue='"+ds.getAt(i).get('deviceCode')+"'></input>"
											     			+"<input type='hidden' id='deviceId"+index+"' vlaue='"+ds.getAt(i).get('deviceId')+"'></input>"
											     			+"<input type='hidden' id='deviceName"+index+"' vlaue='"+ds.getAt(i).get('deviceName')+"'></input></div>";
								document.getElementById("deviceCode"+index).value=	ds.getAt(i).get("deviceCode");		
								document.getElementById("deviceName"+index).value=	ds.getAt(i).get("deviceName");
								document.getElementById("deviceId"+index).value=	ds.getAt(i).get("deviceId");
						 		 var otrr=tjb2.insertRow();
						 		 var ocel2=otrr.insertCell(0);//插入一个td 
						 		 ocel2.innerHTML="<div align='center'><input type='text' name='数量' id='deviceNum"+index+"' /></div>"; 
						 		document.getElementById("deviceNum"+index).value=	ds.getAt(i).get("deviceNum");	
						 		 if(ds.getCount()!=i+1){
							 			ocel1.className="td";
							 			ocel2.className="td";
							 		 }
		...
		wealFitProductivepowerDataSet.on('load',function(ds){
			...
			document.getElementById("productNumT").innerHTML=productNumTotle;
			document.getElementById("productValueT").innerHTML=productValueTotle;
		});
 	}
 	...
 	function productCondion(){
 	  ...
 	  function addcp(){ 
 	  	 var otr=pntable.insertRow();//插入一个tr 
 		 var ocel1=otr.insertCell(0);//插入一个td   
 		 ocel1.innerHTML="<div align='center'><input type='text'  id='productName"+cpIndex+"' /><input type='hidden'  id='productId"+cpIndex+"' /></div>";  
 		 var otrr=pnnumtable.insertRow();
 		 var ocel2=otrr.insertCell(0);//插入一个td 
 		 ocel2.innerHTML="<div align='center'><input type='text'  id='productNum"+cpIndex+"' name='productNum' /></div>"; 
 		 var otrrr=pvtable.insertRow();
 		 var ocel3=otrrr.insertCell(0);
 		 ocel3.innerHTML="<div align='center'><input type='text'  id='productValue"+cpIndex+"' /></div>";
 		 cpIndex++;
 	  
 	  } 
 	   function addjb(){ 
 		  	var dicFitdeviceDs = L5.DatasetMgr.lookup("dicFitdeviceDs");
 		  	index--;
 		  	dicFitdeviceDs.load();
 		  	dicFitdeviceDs.on('load',function(ds){
				if(ds.getCount()>0){
					for(var i=0;i<ds.getCount();i++){
							var otr=tjb1.insertRow();//插入一个tr 
					 		 var ocel1=otr.insertCell(0);//插入一个td  
					 		//ocel1.className="unnamed6";
					 		 ocel1.innerHTML="<div align='center'>"
					 								+ds.getAt(i).get('name')
										     			+'<input type="hidden" id="deviceCode'+index+'"></input><input type="hidden" id="deviceName'+index+'"></input></div>';
							document.getElementById("deviceCode"+index).value=	ds.getAt(i).get("code");
							document.getElementById("deviceName"+index).value=	ds.getAt(i).get("name");
					 		 var otrr=tjb2.insertRow();
					 		 var ocel2=otrr.insertCell(0);//插入一个td 
					 		//ocel2.className="unnamed6";
					 		 ocel2.innerHTML="<div align='center'><input type='text' name='数量' id='deviceNum"+index+"' /></div>"; 
					 		 if(ds.getCount()!=i+1){
						 			ocel1.className="td";
						 			ocel2.className="td";
 	  ...
 	  function save(){
		...
		/************基础设施信息*******************/
		var deviceRecords=new Array();
		for(var i=0;i<index;i++){
			if(document.getElementById("deviceCode"+i).value!=""){
				var wealFitDeviceDataSet = L5.DatasetMgr.lookup("wealFitDeviceDataSet");
				var deviceRecord=wealFitDeviceDataSet.newRecord();
				 deviceRecord.set("taskCode",taskCode);
				 deviceRecord.set("deviceCode",document.getElementById("deviceCode"+i).value);
				 deviceRecord.set("deviceName",document.getElementById("deviceName"+i).value);
				 deviceRecord.set("deviceNum",document.getElementById("deviceNum"+i).value);
				 if(document.getElementById("deviceId"+i)){
					 deviceRecord.set("deviceId",document.getElementById("deviceId"+i).value);
				 }
				 deviceRecords[i]=deviceRecord;
			}
		}
		/************生产能力信息*******************/
		/* var productRecords=new Array();
		for(var i=0;i<cpIndex;i++){
			if(document.getElementById("productName"+i).value!=""){
				var wealFitProductivepowerDataSet = L5.DatasetMgr.lookup("wealFitProductivepowerDataSet");
				var productRecord=wealFitProductivepowerDataSet.newRecord();
				 productRecord.set("taskCode",taskCode);
				 productRecord.set("productName",document.getElementById("productName"+i).value);
				 productRecord.set("produceId",document.getElementById("productId"+i).value);
				 productRecord.set("productCode",i);
				 productRecord.set("productNum",document.getElementById("productNum"+i).value);
				 productRecord.set("productValue",document.getElementById("productValue"+i).value);
				 productRecords[i]=productRecord;
			 }
		}
		 */
		...
		command.setParameter("deviceRecords", deviceRecords);
		//command.setParameter("productRecords", productRecords);
	...
	function validateInfo(){
		...
		var wealWorkerDataSet = L5.DatasetMgr.lookup("wealWorkerDataSet");
		if(wealWorkerDataSet.getCount()<1){
...
<div id="float" >
	<input field="" type="button" value="下一页" style="height:25px;width:80px;position:absolute;margin-left:20px;" onclick="nextPageJ('3')"/>
	...
	<input field="" type="button" value="返回" style="height:25px;width:80px;position:absolute;margin-left:320px;" onclick="backPageJ();"/>
				...
				<tbody>
					<tr>
					    ...
					    <td colspan="8" class=unnamed5><div align="left">
					      <input type="text" vname="企业名称" id="companyName" field="companyName" size=70/><font color="red">*</font>
					    </div></td>
					  </tr>
					  <tr>
					    <td class=unnamed6 height="30" colspan="3"><div align="center" >注册地址</div></td>
					    <td colspan="4" class=unnamed2><div align="left">
					      <input type="text" vname="注册地址" id="address" field="address" size=70/><font color="red">*</font>
					    	</div>
					    </td>
					  ...
					  <tr>
					  	...
					  	<td colspan="3" class=unnamed2>
					  		<input type="text" vname="法人代表" id="legalPeople" field="legalPeople"/><font color="red">*</font>
					  		
					  	</td>
					  	<td colspan="2" class=unnamed3 height="30"><div align="center">身份证号</div></td>
					  	<td colspan="3" class=unnamed6>
					  		<input type="text" vname="身份证号" id="legalIdCard" field="legalIdCard"size="40"/><font color="red">*</font>
					  	</td>
					  </tr>
					  <!-- 固定电话 手机 -->
					   <tr>
					  	...
					  	<td colspan="3" class=unnamed2>
					  		<input type="text" vname="固定电话" id="legalPhone" field="legalPhone"/><font color="red">*</font>
					  ...
					  <tr>
					   ...
					   <!-- 经办人   身份证号  固定电话 手机-->
					     <tr>
					  	...
					  	<td colspan="3" class=unnamed2>
					  		<input type="text" vname="经办人" id="agentPeople" field="agentPeople"/><font color="red">*</font>
					  		
					  	</td>
					  	<td colspan="2" class=unnamed3 height="30"><div align="center">身份证号</div></td>
					  	<td colspan="3" class=unnamed6>
					  		<input type="text" vname="身份证号" id="agentIdCard" field="agentIdCard" size="40"/><font color="red">*</font>
					  	</td>
					  </tr>
					  <!-- 固定电话 手机 -->
					   <tr>
					  	<td colspan="3" class=unnamed6 height="30"><div align="center">固定电话</div></td>
					  	<td colspan="3" class=unnamed2>
					  		<input type="text" vname="固定电话" id="agentPhone" field="agentPhone"/><font color="red">*</font>
					 ...
					 <!-- 网站地址 -->
					  ...
					  <tr>
					    ...
					    <td colspan="3" class=unnamed2>
					     <select id="economicNatureCode" field="economicNatureCode" vname="经济性质" >
					     	<option dataset="ecoDataSet" ></option>
					     </select><font color="red">*</font>
					    </td>
					    <td class=unnamed3 height="30" colspan="2"><div align="center">组织机构代码</div></td>
					    <td colspan="3" class=unnamed6>
					      <input type="text" name="组织机构代码" id="organCode" field="organCode" size=50/>
					    </td>
					    
					    <tr>
					    <td class=unnamed6 colspan="3" height="30" ><div align="center">营业执照编号</div></td>
					    <td colspan="3" class=unnamed2>
					      <input type="text" name="营业执照编号" id="licenseCode" field="licenseCode" />
					   </td>
					    
					    <td colspan="2" class=unnamed3 height="30" ><div align="center">税务登记编号</div></td>
					    <td colspan="3" class=unnamed6><input type="text" name="税务登记编号" id="registrationCode" field="registrationCode" /></td>
					  </tr>
					  <tr>
					  	<td colspan="3" class=unnamed6 height="30" ><div align="center">注册资金(万元)</div></td>
					    <td colspan="8" class=unnamed2><input type="text" vname="注册资金" id="regFund" field="regFund" /><font color="red">*</font></td>
					  </tr>
						
					 <tr>
						  	<td colspan="3" class=unnamed6 height="30" ><div align="center">总经营面积（m2）</div></td>
						  	<td colspan="3" class=unnamed2>
						  		<input type="text" vname="总经营面积" id="operateArea" field="operateArea"/><font color="red">*</font>
						  		
						  	</td>
						  	<td colspan="2" class=unnamed3 height="30" ><div align="center">制作间面积（m2）</div></td>
						  	<td colspan="3" class=unnamed6>
						   ...
						   <tr>
						  	<td colspan="3" class=unnamed6 height="30" ><div align="center">康复室面积（m2）</div></td>
						  	<td colspan="3" class=unnamed2>
						  		<input type="text" vname="康复室面积" id="recoveryArea" field="recoveryArea"/><font color="red">*</font>
						  		
						  	</td>
						  	<td colspan="2" class=unnamed3 height="30" ><div align="center">接待室面积（m2）</div></td>
						  	<td colspan="3" class=unnamed6>
						   ...
						   <tr>
						  	<td colspan="3" class=unnamed6 height="30" ><div align="center">训练厅面积（m2）</div></td>
						  	<td colspan="3" class=unnamed2>
						  		<input type="text" vname="训练厅面积" id="trainArea" field="trainArea"/><font color="red">*</font>
						  		
						  	</td>
						  	<td colspan="2" class=unnamed3 height="30" ><div align="center">理疗室面积（m2）</div></td>
						  	<td colspan="3" class=unnamed6>
						  		<input type="text" vname="理疗室面积" id="treatmentArea" field="treatmentArea"/><font color="red">*</font>
						  	</td>
						  </tr> 
						   <tr>
						  	<td colspan="3" class=unnamed6 height="30" ><div align="center">办公面积（m2）</div></td>
						  	<td colspan="3" class=unnamed2>
						  		<input type="text" vname="办公面积" id="workArea" field="workArea"/><font color="red">*</font>
						  		
						  	</td>
						  	<td colspan="2" class=unnamed3  ><div align="center">休闲区面积（m2）</div></td>
						  	<td colspan="3" class=unnamed6>
						  		<input type="text" vname="休闲区面积" id="leisureArea" field="leisureArea"/><font color="red">*</font>
						  	</td>
						  </tr>				
						   <tr>
						  	<td class=unnamed6 height="30"  colspan="3"><div align="center">其&nbsp;它（m2）</div></td>
						    <td colspan="8" class=unnamed2><div align="left">
							 ...
							 <tr >
							    ...
							    <td colspan="2" class=unnamed2 height="30"><div align="center">合计</div></td>
							    <td colspan="3" class=unnamed3 height="30"><div align="center">非生产人员</div></td>
							    <td colspan="2" class=unnamed6 height="30"><div align="center">生产人员</div></td>
							    <td colspan="2" class=unnamed2 height="30"><div align="center">残疾职工</div></td>
							  </tr>
							  <tr>
							    <td colspan="2" class=unnamed2 height="30"><div align="center">
							      <input type="text" name="合计" id="totalNum" />
							    </div></td>
							    <td colspan="3" class=unnamed3 align="center" height="30"><input type="text" name="非生产人员" id="noProNum" /></td>
							    <td colspan="2" class=unnamed6><input type="text" name="生产人员" id="proNum" /></td>
							    <td colspan="2" class=unnamed2 align="center"><input type="text" name="残疾职工" id="disNum" /></td>
							  </tr>
							  <tr>
					<!--假肢矫形器职业资格证书执有情况  -->
							  <tr>
							 ...
							 <!--  <tr id="tbl21">
							    <td colspan="2" class=unnamed6 height="30"><div align="center">
							      <input type="text" name="姓名1" id="name0" />
							    </div></td>
							    <td colspan="3" class=unnamed2 align="center"><input type="text" name="身份证号1" id="idCard0" /></td>
							    <td colspan="3" class=unnamed2 align="center"><input type="text" name="证书编号1" id="certCode0" /></td>
							    <td colspan="3" class=unnamed2 align="center"><input type="text" name="发证时间1" id="certDate0" format="Y-m-d" onclick="LoushangDate(this)" readonly="readonly" style="text-align: center"/></td>
							  </tr>
							 添加按钮 -->
							 <tr>
					    ...
					    <td colspan="6" class=unnamed2 height="30"><div align="center">年生产能力</div>
					    	<tr>
					    		...
					    		<td class=unnamed2 colspan="3"><div align="center">产值（万元）<input type="button" value="增加" onclick="showProductWin()" /></div></td>
					    	</tr>
					    </td>
					</tr>
					<!-- <tr>
					...
					<tr>
					  	<td colspan="4" class="unnamed7" ><div align="right">(合计)</div></td>
					  	<td colspan="2" class="unnamed2"><div align="center"><label name="数量" id="productNumT"/></div></td>
