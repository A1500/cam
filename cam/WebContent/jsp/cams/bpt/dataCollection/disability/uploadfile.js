...
function init() {
	...
	//bptElectronicList.setParameter("DOMICILE_CODE",organCode);
		...
		if(records.length>=1){
			...
			for(var i=0;i<electCount;i++){
				Id+=records[i].get("electronicId")+":";
				 stypes+=records[i].get("serviceType")+":";
			}
		}
//		}else{
//			insertPeopleid();
...
function selectContent(){
	var url = "../../edoc/contentSelect.jsp?peopleId='"+peopleId+"'&serviceType='"+serviceType+"'&organCode='"+organCode+"'";
...
function getContent(){
	...
	if(revalue==";;;;;;;"){ 
		...
		document.getElementById("ARCHIVE_NO").innerHTML="";
...
function lookDetail(value,cellmeta,record,rowindex,colindex,dataset){
 	...
 	for(var i=0;i<record.length;i++){*/
		...
		//	var returnfile="<a href='#' style='color:blue;text-decoration:none' onclick='editfiles(\""+electronicId+"\")'>编辑</a>";
  ...
  function checkElectronic() {
  	...
  	var url=L5.webPath+"/jsp/cams/bpt/comm/checkFileview.jsp?Id="+Id+"&serviceType="+stypes+"&peopleId="+peopleId;
  	var electronic = param.window.open(url);
  }
function savefile(){
		var contentNumName=  document.getElementById("contentNumName").value;
		if(contentNumName==""){
			L5.Msg.alert('提示',"请选择电子档案目录！");
			return;
		}
