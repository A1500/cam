...
function query(){
	...
	var soldierStateCode = getParam("soldierStateCode");
...
function reset(){
	...
	document.getElementById("soldierStateCode").value = "";
}


function insert() {
    
	var url="cbptcountretiredsoldierAdd.jsp?method=INSERT";
    ...
    var height = screen.height-150;
 	...
 	if(returnValue!=""&returnValue!=undefined){
 		...
 		var peopleId = list[0];
 		var familyId = list[1];
 		var url2="retiredsoldierUpdateManage.jsp?familyId="+familyId+"&peopleId="+peopleId;
...
function update(){
	...
	var url="retiredsoldierUpdateManage.jsp?familyId="+familyId+"&peopleId="+peopleId;
	var width = screen.width-200;
    var height = screen.height-150;
...
function detail(){
	...
	var peopleId = selected[0].get('PEOPLE_ID');
	var familyId = selected[0].get('FAMILY_ID');
	var url='bptcountretiredsoldierDetail.jsp?peopleId='+peopleId+'&familyId='+familyId;
	var width = screen.width-200;
    var height = screen.height-150;
	var win = window.showModalDialog (url,window,"scroll:no;status:no;dialogWidth:"+width+"px;dialogHeight:"+height+"px;resizable:0");
}
...
function  ett(){
   var url=L5.webPath+"/jsp/cams/bpt/excelToTable/countretiredsoldier/ErrMessage.jsp";
