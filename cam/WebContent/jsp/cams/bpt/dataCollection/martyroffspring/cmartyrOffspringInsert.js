...
function saveClick(){ //  保存添加信息
   ...
   if(!validateDateFormat(sacrificeDate)){
   ...
   if(!validateDateFormat(lezswApproveDate)){
		L5.Msg.alert('提示','批准烈士（平反）时间格式不正确！');
	   return false;
	}
   //批准日期
   var approvalDate = MartyrOffspringDataSet.get("approvalDate");
   ...
   var idCard = BaseinfoPeopleDataSet.get("idCard");
	if(!writeAble(idCard,"B1",false)){
		L5.Msg.alert('提示', '该人员已经是老烈子女人员！');
		return false;
	}
   
   BaseinfoPeopleRecord.set("martyrOffspringStateCode",MartyrOffspringRecord.get("martyrOffspringStateCode"));
   BaseinfoPeopleRecord.set("martyrOffspringNo",MartyrOffspringRecord.get("martyrOffspringNo"));
