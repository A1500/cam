function init() {
	...
	if (method == null || method == "") {
		...
		PeopleDemobilizedDataset.set("demobilizedStateCode",2);
...
function saveClick() {
	...
	var domicileType = document.getElementById("domicileType");
	if(domicileType==''){
	...
	var approvalDate = PeopleDemobilizedRecord.get("approvalDate");
	...
	//校验开户银行
	var bank = PeopleDemobilizedRecord.get("bank");
	if(bank == ''){
		L5.Msg.alert('提示','开户银行不能为空！');
		return false;
	}
	//校验开户名称
	var accountName = PeopleDemobilizedRecord.get("accountName");
	if(accountName == ''){
		L5.Msg.alert('提示','开户名称不能为空！');
		return false;
	}
	//校验银行账号
	var accountCode = PeopleDemobilizedRecord.get("accountCode");
	if(accountCode == ''){
	...
	if(!writeAble(idCard,"41",false)){
		L5.Msg.alert('提示', '该人员已经是在乡人员！');
		return false;
	}
