function init() {
	if (method == null || method == "") {
		...
		DemobilizedIllnessDataset.set("demobilizedStateCode",2);
...
function saveClick(){
	...
	if(L5.getDom("demobilizedNo")==null||L5.getDom("demobilizedNo")=="")
	{
	...
	var approvalDate =DemobilizedIllnessDataset.get("approvalDate");
	if(approvalDate==''){
	...
	var decruitmentDate =DemobilizedIllnessDataset.get("decruitmentDate");
	...
	//校验开户银行
	var bank = DemobilizedIllnessRecord.get("bank");
	if(bank == ''){
		L5.Msg.alert('提示','开户银行不能为空！');
		return false;
	}
	//校验开户名称
	var accountName = DemobilizedIllnessRecord.get("accountName");
	if(accountName == ''){
		L5.Msg.alert('提示','开户名称不能为空！');
		return false;
	}
	//校验银行账号
	var accountCode = DemobilizedIllnessRecord.get("accountCode");
	if(accountCode == ''){
