...
<html>
	<head>
		<title>档案目录选择</title>
		<next:ScriptManager/>
		<script type="text/javascript" src="baseinforContentSelect.js"></script>
		<script>
			var applyId = '<%=request.getParameter("applyId")==null?"":request.getParameter("applyId")%>';
			var peopleId= <%=request.getParameter("peopleId")%>;
			var serviceType= <%=request.getParameter("serviceType")%>;
			var organCode = <%=request.getParameter("organCode")%>;
			var onlyinsert = <%=request.getParameter("onlyinsert")%>;
			var vbptElectronicRecords;
		</script>
	</head>
<body >
<model:datasets>
	<model:dataset id="ElectronicDataset" cmd="com.inspur.cams.bpt.manage.cmd.BptElectronicQueryCommand" global="true" pageSize="30" method="queryBaseInforSelectedContents">
...
<next:GridPanel id="CatalogueArchivesSelect"  width="100%" notSelectFirstRow="true" stripeRows="true" height="100%" dataset="CatalogueArchivesDataset">
	...
	<next:Columns>
	    ...
	    <next:Column id="serviceType" header="业务类型" field="serviceType" width="220" dataset="ServiceTypeDataset"></next:Column>
