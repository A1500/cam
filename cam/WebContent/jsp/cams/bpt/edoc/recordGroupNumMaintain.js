...
function saveArchivalFonds(){
	...
	if(!_isNum(archivalFondsName)){
		L5.Msg.alert('提示',"输入内容必须是数字！");
		return false;
	}
	
	if(archivalFondsName.length>3){
		L5.Msg.alert('提示',"长度必须小于三位！");
		return false;
	}
	...
	if (!command.error) {
		ArchivalFondsDataset.getCurrent().state = L5.model.Record.STATE_MODIFIED;
		ArchivalFondsDataset.setParameter("DOMICILE_CODE@=",organCode);
		ArchivalFondsDataset.load();
