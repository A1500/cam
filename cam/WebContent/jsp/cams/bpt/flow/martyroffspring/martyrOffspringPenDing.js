function init(){
	...
	MartyrOffspringDataSet.setParameter("organCode",organCode);
...
function insert(){
	var url=L5.webPath + "/jsp/cams/bpt/flow/martyroffspring/martyrOffspringInsert.jsp";  
    ...
    var returnValue = window.showModalDialog(url,window,"scroll:yes;status:no;dialogWidth:"+width+"px;dialogHeight:"+height+"px;resizable:1");
 	if(returnValue!=""&returnValue!=undefined){
 		...
 		var familyId = list[1];
 		var applyId = list[2];
 		var url2="martyrOffspringUpdateManage.jsp?familyId="+familyId+"&peopleId="+peopleId+"&applyId="+applyId;
...
function update(){
		...
		var url=L5.webPath+'/jsp/cams/bpt/flow/martyroffspring/martyrOffspringUpdateManage.jsp?peopleId='+peopleId + '&familyId=' + familyId+'&applyId='+applyId+'&photoId='+photoId;
		var width = screen.width-100;
    	var height = screen.height-140;
...
function detail(){
	...
	var width = screen.width-100;
    var height = screen.height-140;
	var url=L5.webPath+'/jsp/cams/bpt/flow/martyroffspring/martyrOffspringDetailManage.jsp?familyId='+familyId+'&peopleId='+peopleId+'&photoId='+photoId+'&applyId='+applyId;
...
function del(){
	...
	L5.MessageBox.confirm('确定', '确定要删除选中的记录吗?',function(state){
		if(state=="yes"){
			...
			command.execute("deleteMartyrOffspring");
			if (!command.error) {
				for(var i=0;i<selected.length;i++){
			    	MartyrOffspringDataSet.remove(selected[i]);
				}
				MartyrOffspringDataSet.commitChanges();
			}else{
