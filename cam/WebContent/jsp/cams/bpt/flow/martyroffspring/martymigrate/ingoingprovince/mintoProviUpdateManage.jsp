...
<html>
<head>
<title>外省部分烈士（错杀被平反人员）子女定补关系跨省迁入我省修改</title>
<next:ScriptManager />
		<script>
...
<next:Panel width="100%">
	<next:TabPanel>
		...
		<next:Panel title="类别信息" width="100%" height="100%" id="intoProviTabPanel">
				<next:Html>
				    <iframe id="intoProvi" src="mintoProviUpdate.jsp?applyId=<%=request.getParameter("applyId")%>&peopleId=<%=request.getParameter("peopleId")%>"
						frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" width="100%" height="100%">
	   				</iframe>
				</next:Html>
		</next:Panel>
