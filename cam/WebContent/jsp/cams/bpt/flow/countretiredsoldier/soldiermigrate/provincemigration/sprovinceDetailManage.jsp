...
<html>
<head>
<title>年满60周岁农村籍退役士兵定补关系省内迁移信息查看</title>
<next:ScriptManager />
		<script>
...
<next:Panel width="100%">
	<next:TabPanel>
		...
		<next:Panel title="迁移信息" width="100%" height="100%" id="emigrationTabPanel">
				<next:Html>
				  <iframe id="emigration" src="sprovinceMigrationDetail.jsp?applyId=<%=request.getParameter("applyId")%>&peopleId=<%=request.getParameter("peopleId")%>"
						frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" width="100%" height="100%">
	   				</iframe>
				</next:Html>
		</next:Panel>
