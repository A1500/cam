...
function save(){
	...
	if(!writeAble(idCard,"81",true)){
		L5.Msg.alert('提示', '该人员已经是60周岁退役士兵！');
		return false;
	}
	RetiredSoldierRecord.set("serviceType","82");
...
function getCodeAndName(){
		...
		var list = revalue.split(";");
			 RetiredSoldierDataSet.getCurrent().set("ingoingCode",list[0]);
			 RetiredSoldierDataSet.getCurrent().set("ingoingAddress",list[1]);
		}
}
function back(){	
  	history.go(-1);	 
