...
<html>
<head>
<title>年满60周岁农村籍退役士兵定补关系跨省迁入我省信息查看</title>
<next:ScriptManager />
		<script>
...
<next:Panel width="100%">
	<next:TabPanel>
		...
		<next:Panel title="类别信息" width="100%" height="100%" id="intoProviTabPanel">
				<next:Html>
				    <iframe id="intoProvi" src="sintoProviDetail.jsp?peopleId=<%=request.getParameter("peopleId")%>&applyId=<%=request.getParameter("applyId")%>"
						frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" width="100%" height="100%">
	   				</iframe>
				</next:Html>
		</next:Panel>
