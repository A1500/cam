...
<% 	String changeContent = request.getParameter("changeContent");
	if(changeContent!=null&&!"".equals(changeContent)){
		changeContent = java.net.URLDecoder.decode(request.getParameter("changeContent"), "UTF-8");
	} %>
</head>
<next:ScriptManager />
<script>
function init() {
	...
	if(changeContent!="null"&&changeContent!=""){
			document.getElementById("change").value = changeContent;
	}
}
function change(){
var serviceType;
...
var printtype = document.getElementById("printtype").value;
 if(printtype==""||printtype=="1") { 
 	...
 	var command = new L5.Command("com.inspur.cams.bpt.manage.cmd.BptBaseinfoPeopleCommand");
		...
		command.execute("getMsgForNOPrint");
		var msg = command.getReturn("msg");
		document.getElementById("change").value=msg;
	}
}
</script>
<body bgcolor="DFE8F7">
<table align="center" bgcolor="DFE8F7">
	...
	<tr>
		...
		<tr>
		<td colspan="3" id="changeContent"><span class="STYLE7">变更内容：</span>		  <textarea id="change" name="change" style="height: 180;width: 300"> </textarea></td>
	</tr>
	<tr>
