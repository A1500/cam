...
function save(){
	...
	//校验需变更原因及内容是否为空
	var changeReasonContent = _$("exchangeReason");
	if(changeReasonContent==''){
		L5.Msg.alert('提示',"证件变更原因及内容不能为空!");
		return false;
	}
	...
	command.execute("addChangeCertiDisa");
...
function getIdCard(){
	...
	var revalue =  window.showModalDialog(url,window,"dialogHeight:"+height+"px;dialogWidth:"+width+"px;scroll:yes;status:no;resizable:1");
