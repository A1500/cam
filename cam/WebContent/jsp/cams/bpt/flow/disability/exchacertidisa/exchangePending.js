function init() {
	...
	DisabilityPeopleDataSet.setParameter("serviceType", '13');
	DisabilityPeopleDataSet.setParameter("serviceType1", '18');
...
function enableButtons() {
	...
	L5.getCmp("endButton").setDisabled(false);
...
function disableButtons() {
	...
	L5.getCmp("endButton").setDisabled(true);
}
function del(){
	disableButtons();
	...
	L5.MessageBox.confirm('确定', '确定要删除选中的记录吗?',function(state){
		if(state=="yes"){
			...
			command.setParameter("disType", "exchange");
...
function insert() {
//	var url = "jsp/cams/bpt/flow/disability/exchacertidisa/exchaCertiDisaInsert.jsp";
//	var text = '添加换证补证人员信息';
//	L5.forward(url, text);
	var width = screen.width-100;
    var height = screen.height-140;
	var url = 'exchaCertiDisaInsert.jsp';
	...
	if(returnValue!=""&returnValue!=undefined){
 		...
 		var familyId = list[1];
 		var applyId = list[2];
 		var url2="exchaCertiDisaUpdate.jsp?familyId="+familyId+"&peopleId="+peopleId+"&applyId="+applyId;
...
//	var url = 'jsp/cams/bpt/flow/disability/exchacertidisa/exchaCertiDisaUpdate.jsp';
//	var text = '修改人员换证补证信息修改';
//	L5.forward(url, text, data);
	var width = screen.width-100;
    var height = screen.height-140;
	var url = 'exchaCertiDisaUpdate.jsp?familyId=' + familyId + '&peopleId='+ peopleId + '&applyId=' + applyId;
...
function detail() {
	...
	var width = screen.width-100;
    var height = screen.height-140;
	var url = 'exchaCertiDisaDetail.jsp?familyId=' + familyId + '&peopleId='
...
function createAndSend() {
	disableButtons();
	operation("pass","submitButton");
		enableButtons();
}

function operation(value,btnId) {
	var editGrid = L5.getCmp('editGridPanel');
	var selected = editGrid.getSelectionModel().getSelections();
	if (selected.length != 1) {
	...
	L5.getCmp(btnId).setDisabled(true);
	...
	if(!command.error){
		   ...
		   L5.forward(url,"已办任务页面");
	     	...
	     	L5.getCmp(btnId).setDisabled(false);
	      alert(command.error);
	     }
}
...
function batchSend() {
	...
	for(var i=0;i<selected.length;i++){
		...
		if(townAduitIncharge!=null&&townAduitIncharge!="1"){
			...
			command1.setParameter("record", DisabilityPeopleRecord);
			command1.setParameter("condition","pass");
			command1.execute("createAndSend");
			if(!command1.error){
			   //跳转到已办任务页面
		...
		enableButtons();
				...
				L5.getCmp("bthSmtBtn").setDisabled(false);
				alert("【"+name+"】批量发起失败！"+command1.error);
		enableButtons();
				return false;
			}
		}else{
			L5.getCmp("bthSmtBtn").setDisabled(false);
			L5.Msg.alert("提示","请先保存【"+name+"】的县级审批信息!");
		enableButtons();
			return false;
		}
	}
...
function operationById(value,applyId,peopleId) {
	...
	command.setParameter("serviceType","13");  
	command.execute("createAndSend");
	if(!command.error){
		   alert("流程发起并发送成功!");
		   //跳转到已办任务页面
