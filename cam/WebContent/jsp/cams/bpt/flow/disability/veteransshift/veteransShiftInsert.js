function init() {
		if (method==null||method==""){
			...
			BptPeopleDisabilityDataSet.set("disabilityTypeCode","11");
...
function saveClick(){
	...
	if(BaseinfoPeopleRecord.get("peopleId")==""&&BaseinfoPeopleRecord.get("familyId")==""){
	  ...
	  // peopleId = BaseinfoPeopleRecord.get("peopleId");
	   ...
	   //BptPeopleDisabilityRecord.set("applyId",applyId);
	   //applyId = BptPeopleDisabilityRecord.get("applyId");
	}
	...
	var BptPeopleDisabilityForm =  L5.getDom("BptPeopleDisabilityForm");
	if(BptPeopleDisabilityForm == null || BptPeopleDisabilityForm == "") {
		L5.Msg.alert("提示","请保存类别信息");
		return false;
	}
	...
	command.execute("addVeteransShift");
	if (!command.error) {
