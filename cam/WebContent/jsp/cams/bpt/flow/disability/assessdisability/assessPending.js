...
function del() {
	...
	L5.MessageBox.confirm('确定', '确定要删除选中的记录吗?', function(state) {
		if (state == "yes") {
			...
			command.setParameter("disType", "assess");
...
function insert() {
	// var
	// url="jsp/cams/bpt/flow/disability/assessdisability/disabilityAssessInsert.jsp";
	// var text='添加伤残人员信息';
	// var data = new L5.Map();
	// data.put("method","INSERT");
	// L5.forward(url,text,data);
	var url = "disabilityAssessInsert.jsp?method=INSERT";
	...
	if (returnValue != "" & returnValue != undefined) {
		...
		var familyId = list[1];
		var applyId = list[2];
		var url2 = "disabilityUpdateManage.jsp?familyId=" + familyId
...
function update() {
	...
	var serviceType = '11';
	var url = "disabilityUpdateManage.jsp?familyId=" + familyId + "&peopleId="
			+ peopleId + '&applyId=' + applyId + '&serviceType=' + serviceType;
	var width = screen.width - 100;
	var height = screen.height - 140;
	...
	// var
	// url='jsp/cams/bpt/flow/disability/assessdisability/disabilityUpdateManage.jsp';
	// var text = '伤残人员信息修改';
	// L5.forward(url,text,data);
}
...
function detail() {
	...
	var width = screen.width - 100;
	var height = screen.height - 140;
	var url = 'disabilityDetailManage.jsp?familyId=' + familyId + '&peopleId='
...
// 插入信息之后直接发起
function casByapplyId(applyId, peopleId) {
	var command = new L5.Command("com.inspur.cams.bpt.manage.cmd.BptApplyDisabilityCommand");
	command.setParameter("APPLY_ID", applyId);
	command.execute("queryApplyInfo");
	var townAduitIncharge = command.getReturn("townAduitIncharge");
