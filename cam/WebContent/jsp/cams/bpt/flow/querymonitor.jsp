...
<model:datasets>
	<model:dataset  id="MonitorDataset" cmd="com.inspur.cams.bpt.manage.cmd.WfProcessBusinessQueryCommand" method="queryMonitor"  global="true">
		<model:record>
...
<next:ViewPort>
	<next:BorderLayout>
		...
		<next:Center floatable="true" cmargins="0 0 0 0" margins="0 0 0 0" >
			...
			<next:GridPanel id="MonitorGridPanel" name="MonitorGridPanel" anchor="100%" clickToSelectedForChkSM="true" notSelectFirstRow="true"  height="100%" dataset="MonitorDataset"     title="流程监控列表" >
				<next:TopBar>
				...
				<next:Columns>
					...
					<next:Column id="procCreateTime" header="流程结束时间" field="procEndTime" width="100" align="center" >
						<next:TextField allowBlank="false"/>
					</next:Column>
					<next:Column id="activeActDefNames" header="当前活动环节" field="activeActDefNames" width="100" >
						<next:TextField allowBlank="false"/>
					</next:Column>
				...
				<next:BottomBar>
					<next:PagingToolBar  dataset="MonitorDataset"/>
				</next:BottomBar>
			</next:GridPanel>
			</next:AnchorLayout>
			</next:Panel>
...
function init(){
	...
	var daiBanDataset = L5.DatasetMgr.lookup("MonitorDataset");
	//要查询的流程类型或要过滤掉的流程类型
	daiBanDataset.setParameter("domicode",organCode);
	daiBanDataset.load();
	L5.QuickTips.init();
}
...
function handle()
{
	var userGrid=L5.getCmp('MonitorGridPanel');
	...
	if(status == "结束") {
		url="command/dispatcher/org.loushang.workflow.tasklist.forward.TaskListDispatcherCmd/endTaskForward";
...
function showState()
{
   var daiBanTaskGridPanel=L5.getCmp("MonitorGridPanel");
