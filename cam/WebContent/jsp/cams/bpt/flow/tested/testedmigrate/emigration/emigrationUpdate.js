function init(){
	var TestedEmigratDataset = L5.DatasetMgr.lookup("TestedEmigratDataset");
 	TestedEmigratDataset.setParameter("APPLY_ID",applyId);
...
function save(){
	var TestedEmigratDataset = L5.DatasetMgr.lookup("TestedEmigratDataset");
	var record=TestedEmigratDataset.getCurrent();
	//校验迁入地民政局是否为空
...
function func_Close(){
   ...
   function getCodeAndName(){
		...
		var list = revalue.split(";");
			 TestedEmigratDataset.getCurrent().set("ingoingCode",list[0]);
			 TestedEmigratDataset.getCurrent().set("ingoingAddress",list[1]);
		}
}
