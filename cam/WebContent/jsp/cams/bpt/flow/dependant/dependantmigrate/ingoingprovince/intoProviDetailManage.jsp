...
<html>
<head>
<title>外省三属人员抚恤关系迁入我省信息查看</title>
<next:ScriptManager />
		<script>
...
<next:Panel width="100%">
	<next:TabPanel>
		...
		<next:Panel title="电子档案" width="100%" height="100%"
				id="uploadTabPanel">
				<next:Html>
				  <iframe id="upload" src="../../../../comm/uploadfileDetail.jsp?peopleId=<%=request.getParameter("peopleId")%>&serviceType=22&applyId=<%=request.getParameter("applyId")%>"
						frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" width="100%" height="100%">
	   				</iframe>
				</next:Html>
		</next:Panel>
		<next:Panel title="迁入信息" width="100%" height="100%" id="intoPrintPanel">
				<next:Html>
				  <iframe id="intoPrint" src="intoProviPrint.jsp?peopleId=<%=request.getParameter("peopleId")%>&applyId=<%=request.getParameter("applyId")%>"
						frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" width="100%" height="100%">
	   				</iframe>
				</next:Html>
		</next:Panel>
...
<script type="text/javascript">
	function init(){
		//迁入信息重复打开
		L5.getCmp("intoPrintPanel").on("activate",dependtInReOpen);
	}
	//迁入信息重复打开
	function dependtInReOpen(){
