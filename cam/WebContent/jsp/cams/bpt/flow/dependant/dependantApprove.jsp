...
<next:Panel width="100%" id="typeTabPanel" title="类别信息" height="100%">
				<next:Html>
					<iframe id="instance" src="../../../jsp/cams/bpt/flow/dependant/comm/dependantTypeDetail.jsp?processId=<%=(String)request.getParameter("processId")%>&assignmentId=<%= (String)request.getParameter("assignmentId")%>"
						frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="yes" allowtransparency="yes" width="100%" height="100%">
	   				</iframe>
				</next:Html>
</next:Panel>
...
<next:Panel title="电子档案" width="100%" height="100%"
				id="uploadTabPanel">
				<next:Html>
				  <iframe id="upload" src="../../../jsp/cams/bpt/comm/uploadfile.jsp?peopleId=<%=peopleId%>&serviceType=21&applyId=<%=applyId%>"
		...
		<table border="0"  width="99%" >
			<div>
				<table width="99%">
			   		...
			   		<tr>
						<td  class="FieldLabel" colspan="2">优抚对象类别</td>
						<td class="FieldInput" colspan="3"><label type="text" name="dependant_type_code" field="DEPENDANT_TYPE_CODE" dataset="DependantTypeDataset"  /></td>
					</tr>
				</table>
			</div>
				<br>
...
function updateInfo() { // 若流程为第一环节则可修改
	...
	var url = L5.webPath+"/jsp/cams/bpt/flow/dependant/dependantUpdateManage.jsp?peopleId=" + peopleId + "&familyId=" + familyId;
