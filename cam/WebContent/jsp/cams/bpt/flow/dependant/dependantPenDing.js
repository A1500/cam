function init() {
	var DisabilityPeopleDataSet = L5.DatasetMgr
			.lookup("BptPeopleDependantDataSet");
...
function insert() {
	var url = L5.webPath + "/jsp/cams/bpt/flow/dependant/dependantInsert.jsp";
	...
	if (returnValue != "" & returnValue != undefined) {
		...
		var familyId = list[1];
		var applyId = list[2];
		var url2 = "dependantUpdateManage.jsp?familyId=" + familyId
...
function update() {
	...
	var url = L5.webPath
			+ '/jsp/cams/bpt/flow/dependant/dependantUpdateManage.jsp?peopleId='
...
function del() {
	...
	L5.MessageBox.confirm('确定', '确定要删除选中的记录吗?', function(state) {
		if (state == "yes") {
			...
			command.execute("deleteDependant");
			if (!command.error) {
				for (var i = 0; i < selected.length; i++) {
					BptPeopleDependantDataSet.remove(selected[i]);
				}
				BptPeopleDependantDataSet.commitChanges();
			} else {
...
function detail() {
	...
	var width = screen.width - 100;
	var height = screen.height - 140;
	var url = 'dependantDetailManage.jsp?familyId=' + familyId + '&peopleId='
			+ peopleId + '&photoId=' + photoId + '&applyId=' + applyId;
	var win = window.showModalDialog(url, window,
			"scroll:yes;status:no;dialogWidth:" + width + "px;dialogHeight:"
					+ height + "px;resizable:1");
	if (win == null) {
...
/*
 * //发起单个流程 function createAndSend(){ var editGrid=L5.getCmp('editGridPanel');
 * var selected=editGrid.getSelectionModel().getSelections();
 * if(selected.length!=1){ L5.Msg.alert("提示","请选择一个人员进行提交！"); return false; }
 * var dependantRecord = selected[0]; var peopleId =
 * dependantRecord.get("PEOPLE_ID"); var applyId =
 * dependantRecord.get("APPLY_ID"); var command=new
 * L5.Command("com.inspur.cams.bpt.manage.cmd.BptPeopleDependantCommand");
 * command.setParameter("applyId", applyId);
