...
function insert() {
	...
	var url=L5.webPath+'/jsp/cams/bpt/flow/demobilizedillness/illmigrate/illMigrationInsert.jsp';
...
function detail() {
	...
	var width = screen.width-100;
    var height = screen.height-120;
	var url = 'illMigrationDetailManage.jsp?idCard=' + idCard + '&peopleId='
			+ peopleId + '&applyId=' + applyId + '&familyId=' + familyId;
	var win = window.showModalDialog(url, window,
			"scroll:yes;status:no;dialogWidth:" + width + "px;dialogHeight:"
					+ height + "px;resizable:1");
...
function createAndSend() {
	...
	var IllnessEmigrateRecord = selected[0];
	DisabilityEmigrateRecord.set("SERVICE_TYPE","4B");
	var command = new L5.Command("com.inspur.cams.bpt.manage.cmd.BptApplyEmigrationCommand");
	command.setParameter("record", IllnessEmigrateRecord);
