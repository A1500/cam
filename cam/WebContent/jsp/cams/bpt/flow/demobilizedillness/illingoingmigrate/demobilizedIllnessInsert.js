...
function saveClick(){
	...
	//var allowanceMonth = DemobilizedIllnessRecord.get("allowanceMonth")+'-12';
		// DemobilizedIllnessRecord.set("allowanceMonth",allowanceMonth);
	//新增人员信息
	...
	DemobilizedIllnessRecord.set("serviceType","46");
	BaseinfoPeopleRecord.set("demobilizedNo",DemobilizedIllnessRecord.get("oldPaperNo"));
   ...
   //校验属地行政规划
	...
	command.execute("insertIllIngoingProvince");
	
	if (!command.error) {
		var applyId = command.getReturn("applyId");
		peopleId = command.getReturn("peopleId");
		familyId =command.getReturn("familyId");
		/*var url="demobilizedIllnessUpdateManage.jsp?applyId="  + applyId+"&peopleId="+peopleId+"&familyId="+familyId;
		...
		*/
		window.returnValue=applyId+";"+peopleId+";"+familyId+";46";
		window.close();		
	}else{
		L5.Msg.alert('提示',"保存时出现错误！"+command.error);
	}
