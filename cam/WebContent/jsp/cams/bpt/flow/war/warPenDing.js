function init() {
	var WarinfoDataSet = L5.DatasetMgr.lookup("WarinfoDataSet");
	WarinfoDataSet.setParameter("organCode", organCode);
	WarinfoDataSet.load();
	L5.QuickTips.init();
...
// 新增参战业务受理信息
function insert() {
	var url = L5.webPath + "/jsp/cams/bpt/flow/war/bptwarAdd.jsp";
	...
	if (returnValue != "" & returnValue != undefined) {
		...
		var familyId = list[1];
		var applyId = list[2];
		var url2 = "bptwar_edit.jsp?familyId=" + familyId + "&peopleId="
		...
		if (state == "yes") {
			...
			if (!command.error) {
				for (var i = 0; i < selected.length; i++) {
					WarinfoDataSet.remove(selected[i]);
				}
				WarinfoDataSet.commitChanges();
			} else {
...
function update() {
	...
	var url = L5.webPath + "/jsp/cams/bpt/flow/war/bptwar_edit.jsp?peopleId="
...
function sure() {// 认定人员信息
	...
	var isApprove = command.getReturn("isApprove");
	var isSure = command.getReturn("isSure");
	if (save) {
		if(isApprove=="1"){
 ...
 * L5.Command("com.inspur.cams.bpt.manage.cmd.BptPeopleWarCommand");
 * command.setParameter("applyId",applyId);
...
function detailWar() {
	...
	var width = screen.width - 100;
	var height = screen.height - 140;
	var url = 'bptwarDetailMain.jsp?familyId=' + familyId + '&peopleId='
			+ peopleId + '&applyId=' + applyId;
	var win = window.showModalDialog(url, window,
			"scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:"
					+ height + "px;resizable:1");
