...
function insert() {
	...
	var url='exchaCertiDemoInsert.jsp?method=INSERT';
...
function update(){
	...
	var width = screen.width-300;
    var height = screen.height-400;
	var url = 'exchaCertiDemoUpdate.jsp?familyId=' + familyId + '&peopleId='
...
function detail() {
	...
	var url='exchaCertiDemoDetail.jsp?familyId=' + familyId + '&peopleId='
			+ peopleId + '&applyId=' + applyId;
	var width = screen.width-300;
    var height = screen.height-400;
	var win = window.showModalDialog (url,window,"scroll:no;status:no;dialogWidth:"+width+"px;dialogHeight:"+height+"px;resizable:1");
