...
function query(){
	...
	ds.setParameter("registerType","81");		
	ds.load();
...
function detail(){
	...
	var url = "soldierDetail.jsp?registerId="+registerId;
...
function insert() {
	var width = screen.width-200;
    var height = screen.height-200;
	var url = "soldierAdd.jsp?method=INSERT";
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
...
function update(){
    ...
    var height = screen.height-200;
    var registerId = selected[0].get('registerId');
	var url = "soldierEdit.jsp?method=UPDATE&registerId="+registerId;
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
...
function expExcel(){
	...
	DemobilizedIllExportDataset.on("load",function(){
		L5.dataset2excel(DemobilizedIllExportDataset,"/jsp/cams/bpt/capital/writeregister/countretiredsoldiergrant/expSoldierGrantToExcel.jsp?pageIndex="+ds.pageInfo.pageIndex);
	});
}