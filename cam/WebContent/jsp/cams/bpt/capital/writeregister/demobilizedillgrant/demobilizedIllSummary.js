...
function query(){
	...
	ds.setParameter("registerType","42");		
	ds.load();
...
function detail(){
	...
	var url = "demobilizedIllDetail.jsp?registerId="+registerId;
...
function insert() {
	var width = screen.width-200;
    var height = screen.height-200;
	var url = "demobilizedIllAdd.jsp?method=INSERT";
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
...
function update(){
    ...
    var height = screen.height-200;
    var registerId = selected[0].get('registerId');
	var url = "demobilizedIllEdit.jsp?method=UPDATE&registerId="+registerId;
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
 ...
 */
function impExcel() {
	var url=L5.webPath+"/jsp/cams/bpt/capital/excel/imp_disablity.jsp?registerType=42" ;
...
//导出Excel
function expExcel(){
	var DemobilizedIllExportDataset = L5.DatasetMgr.lookup("DemobilizedIllExportDataset");
	var editGrid=L5.getCmp('editGridPanel');
	var selected=editGrid.getSelectionModel().getSelections();
	if(selected.length!=1){
		L5.Msg.alert('提示',"请选择一条记录修改!");
	...
	var registerId = selected[0].get('registerId'); 
	DemobilizedIllExportDataset.setParameter("registerId",registerId);
	DemobilizedIllExportDataset.load();
	DemobilizedIllExportDataset.on("load",function(){
		L5.dataset2excel(DemobilizedIllExportDataset,"/jsp/cams/bpt/capital/writeregister/demobilizedillgrant/expDemoIllGrantToExcel.jsp?pageIndex="+ds.pageInfo.pageIndex);
	});
}