...
function query(){
	...
	ds.setParameter("registerType","51");
	ds.load();
...
function detail(){
	...
	var url = "livingAllowanceGrantDetail.jsp?registerId="+registerId;
...
function changeDetail(){
    ...
    var height = screen.height-200;
    var registerId = selected[0].get('registerId');
	var url = "livingAllowGrantChangeDetail.jsp?registerId="+registerId;
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
...
function insert() {
	var width = screen.width-200;
    var height = screen.height-200;
	var url = "livingAllowanceGrantAdd.jsp?method=INSERT";
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
...
function update(){
	...
	var url = "livingAllowanceGrantEdit.jsp?method=UPDATE&registerId="+registerId+"&respectiveYears="+respectiveYears;
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
 ...
 */
function impExcel() {
	var url=L5.webPath+"/jsp/cams/bpt/capital/excel/imp_disablity.jsp?registerType=51" ;
...
//导出Excel
function expExcel(){
	var LivingAllowanceExportDataset = L5.DatasetMgr.lookup("LivingAllowanceExportDataset");
	var editGrid=L5.getCmp('editGridPanel');
	var selected=editGrid.getSelectionModel().getSelections();
	if(selected.length!=1){
		L5.Msg.alert('提示',"请选择一条记录修改!");
	...
	var registerId = selected[0].get('registerId'); 
	LivingAllowanceExportDataset.setParameter("registerId",registerId);
	LivingAllowanceExportDataset.load();
	LivingAllowanceExportDataset.on("load",function(){
		L5.dataset2excel(LivingAllowanceExportDataset,"/jsp/cams/bpt/capital/writeregister/livingallowancegrant/expLivingGrantToExcel.jsp?pageIndex="+ds.pageInfo.pageIndex);
	});
}
