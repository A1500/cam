...
function query(){
	...
	ds.setParameter("registerType","21");//伤残抚恤金		
	ds.load();
	noReclick(ds,"search");
}
function detail(){
	...
	var url = "depandentGrantDetail.jsp?registerId="+registerId;
...
function changeDetail(){
    ...
    var height = screen.height-200;
    var registerId = selected[0].get('registerId');
	var url = "depandentGrantChangeDetail.jsp?registerId="+registerId;
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
...
function insert() {
	var width = screen.width-200;
    var height = screen.height-200;
	var url = "depandentGrantAdd.jsp?method=INSERT";
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
...
function update(){
    ...
    var registerId = selected[0].get('registerId');
    var respectiveYears = selected[0].get('respectiveYears');
	var url = "depandentGrantEdit.jsp?method=UPDATE&registerId="+registerId+"&respectiveYears="+respectiveYears;
	var win = window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	init();
}
 ...
 */
function impExcel() {
	var url=L5.webPath+"/jsp/cams/bpt/capital/excel/imp_disablity.jsp?registerType=21" ;
...
//导出Excel
function expExcel(){
	var DepandentExportDataset = L5.DatasetMgr.lookup("DepandentExportDataset");
	var editGrid=L5.getCmp('editGridPanel');
	var selected=editGrid.getSelectionModel().getSelections();
	if(selected.length!=1){
		L5.Msg.alert('提示',"请选择一条记录修改!");
	...
	var registerId = selected[0].get('registerId'); 
	DepandentExportDataset.setParameter("registerId",registerId);
	DepandentExportDataset.load();
	DepandentExportDataset.on("load",function(){
		L5.dataset2excel(DepandentExportDataset,"/jsp/cams/bpt/capital/writeregister/depandentgrant/expDenptGrantToExcel.jsp?pageIndex="+ds.pageInfo.pageIndex);
	});
}