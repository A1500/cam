...
<script type="text/javascript" src="depandentSummary.js"></script>
</head>
<body>
<model:datasets>
	<model:dataset id="DepandentExportDataset" cmd="com.inspur.cams.bpt.manage.cmd.BptCapitalDetailQueryCommand" global="true" method="queryExportData">
		...
		<table border="1" width="100%">
			<tr>
				...
				<td class="FieldInput"><input type="text" id="startDate"
					...
					id="endDate" class="TextEditor" title="所属年月" style="width: 80"
...
<next:GridPanel id="editGridPanel" name="bptcapitalsummaryGrid" clickToSelectedForChkSM="true"
	...
	<next:TopBar>
		...
		<next:ToolBarItem text="修改发放名册" iconCls="edit" handler="update"
			id="update" />
		<next:ToolBarItem text="删除发放名册" iconCls="remove" handler="del"
			id="delete" />
		<next:ToolBarItem text="确认发放" iconCls="select" handler="grantConfirm"
			id="grantConfirm" />
		<next:ToolBarItem text="查看变更明细" iconCls="detail"
			handler="changeDetail" id="changeDetail" />
		<next:ToolBarItem text="查看发放明细" iconCls="view" handler="detail"
			id="detail" />
	...
	<next:Columns>
		...
		<next:Column id="registerName" header="名册名称" field="registerName"
			width="170">
		</next:Column>
		<next:Column id="registerType" header="名册类别" field="registerType"
			width="120" dataset="dsRegType" hidden="true">
		</next:Column>
		<next:Column id="organName" header="发放单位" field="organName" width="90">
		</next:Column>
		<next:Column id="respectiveYears" header="发放年度"
			field="respectiveYears" width="70" align="left" hidden="true">
		</next:Column>
		...
		<next:Column id="grantFlag" header="发放状态" field="grantFlag" width="60"
			dataset="dsGrantFlag">
		</next:Column>
