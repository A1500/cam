...
function query(){
	ds.setParameter("grantUnit",getParam("domicileCode"));
	ds.setParameter("organCode",organCode);
	ds.setParameter("respectiveYears",getParam("respectiveYears"));
	ds.load();
	ds.on("load",function(){
...
function insert() {
	var url="nonCompulsoryGrantAdd.jsp";
...
function update(){
	...
	var url="nonCompulsoryGrantEdit.jsp?registerId="+registerId;
	var width = screen.width-100;
    var height = screen.height-140;
 	var returnValue = window.showModalDialog(url,window,"scroll:yes;status:no;dialogWidth:"+width+"px;dialogHeight:"+height+"px;resizable:1");
	query();
...
function detail(){
	...
	var registerId = selected[0].get("registerId");
	var url="nonCompulsoryGrantDetail.jsp?registerId="+registerId;
	var width = screen.width-100;
    var height = screen.height-140;
 	var returnValue = window.showModalDialog(url,window,"scroll:yes;status:no;dialogWidth:"+width+"px;dialogHeight:"+height+"px;resizable:1");
}
function del(){
	...
	L5.MessageBox.confirm('确定', '确定要删除选中的记录吗?',function(state){
		if(state=="yes"){
			...
			var command=new L5.Command("com.inspur.cams.bpt.manage.cmd.BptCompulsorySummaryCommand");
 ...
 */
function impExcel() {
	var url=L5.webPath+"/jsp/cams/bpt/capital/excel/imp_disablity.jsp?registerType=00" ;
	var width = screen.width-100;
   	var height = screen.height-140;
...
//导出Excel
function expExcel(){
	var CompulSoryDataset = L5.DatasetMgr.lookup("CompulSoryDataset");
	var editGrid=L5.getCmp('editGridPanel');
	var selected=editGrid.getSelectionModel().getSelections();
	if(selected.length!=1){
	...
	var registerId = selected[0].get('registerId'); 
	CompulSoryDataset.setParameter("registerId",registerId);
	CompulSoryDataset.load();
	CompulSoryDataset.on("load",function(){
		L5.dataset2excel(CompulSoryDataset,"/jsp/cams/bpt/capital/writeregister/noncompulsorygrant/expCompulsoryGrantToExcel.jsp?pageIndex="+ds.pageInfo.pageIndex);
	});
}