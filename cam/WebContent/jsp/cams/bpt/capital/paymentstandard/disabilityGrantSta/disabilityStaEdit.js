function init() {
	...
	editGridPanel.on("afteredit",function(){
		...
		var subTotal = parseFloat((record.get("increaseMechanism")=='')?'0':record.get("increaseMechanism"))+
					   ...
					   parseFloat((record.get("coalSubsidyMon")=='')?'0':record.get("coalSubsidyMon"))+
					   parseFloat((record.get("oldLonelyMon")=='')?'0':record.get("oldLonelyMon"))+
					   parseFloat((record.get("antiJapaneseMon")=='')?'0':record.get("antiJapaneseMon"))+
		...
		var yearTotal = parseFloat((record.get("provinceStandard")=='')?'0':record.get("provinceStandard"))+
					    parseFloat((record.get("nursingCost")=='')?'0':record.get("nursingCost"))+parseFloat(subTotal);
		record.set("yearTotal",yearTotal);
	});
	if(method=="insert"){
		ds.setParameter("STANDARDS_COUNTIES",'370000000000');
		ds.setParameter("TYPE","11");
			...
			if(ds.getCount()>0){
				...
				for(var j=0;j<records.length;j++){
					...
					subTotal[j] =  parseFloat((records[j].get("increaseMechanism")=='')?'0':records[j].get("increaseMechanism"))+
								   ...
								   parseFloat((records[j].get("coalSubsidyMon")=='')?'0':records[j].get("coalSubsidyMon"))+
								   parseFloat((records[j].get("oldLonelyMon")=='')?'0':records[j].get("oldLonelyMon"))+
								   parseFloat((records[j].get("antiJapaneseMon")=='')?'0':records[j].get("antiJapaneseMon"))+
					...
					yearTotal[j] = parseFloat((records[j].get("provinceStandard")=='')?'0':records[j].get("provinceStandard"))+
								    parseFloat((records[j].get("nursingCost")=='')?'0':records[j].get("nursingCost"))+parseFloat(subTotal[j]);
...
function save() {
    ...
    for(var i=0;i<records.length;i++){
		if(records[i].get("disabilityLevelCode")==''){
			L5.Msg.alert('提示',"伤残等级不能为空！");
			return false;
		}
		if(records[i].get("disabilityNature")==''){
