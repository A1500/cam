...
function setReportTitle() {
	...
	if (year == '') {
		reportTitleSpan.innerHTML = "临时救助统计情况";
	} else {
		reportTitleSpan.innerHTML ="临时救助统计情况";
	}
...
function test(value) {
	...
	data.put("xzqu", xzqu);
	data.put("pageflag","2");
	var url = "/jsp/cams/bpt/query/temporary_help/query_temporary_help.jsp";
	...
	var url = L5.webPath+"/jsp/cams/bpt/query/temporary_help/query_temporary_help.jsp"+parm;
	window.open(url);
}
 function back(){
	if(pageflag=='1'){
		...
		var url = "jsp/cams/bpt/query/temporary_help/page_temporary_help.jsp";
		L5.forward(url, null, data);
	}else{
		history.go(-1);
	}
...
function printBill(){
	...
	var url = L5.webPath+'/jsp/cams/bpt/query/print/page_temporary_help1.fr3'; 
...
function redirect(obj, type) {
	...
	var	url = L5.webPath+"/jsp/cams/bpt/query/bpt_people/temporaryHelpQuery.jsp?autoQuery=true&query_organCode="
