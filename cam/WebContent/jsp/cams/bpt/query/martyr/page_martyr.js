...
function setReportTitle() {
	...
	if (year1 ==year2) {
		reportTitleSpan.innerHTML = getChnNumber(year1) + "年山东省参战统计表";
	}else{
		reportTitleSpan.innerHTML =getChnNumber(year1)+"--"+getChnNumber(year2)+ "山东省参战统计表";
	}
}
...
var type = document.getElementById("timeType").value;
	...
	var url = L5.webPath+"/jsp/cams/bpt/query/martyr/query_martyr.jsp"+parm;
	window.open(url);
}
 //打印预览调用方法
function printBill(){
	var printDataset=L5.DatasetMgr.lookup("ds");
	var url = L5.webPath+'/jsp/cams/bpt/query/print/querymartyr.fr3'; 
	var reportName="山东省老烈子女统计表";
...
function redirectQuery(obj,statusType,domicileType) {
	var  urlPage="query_martyroffList.jsp";
	...
	"&domicileType=" + domicileType + "&statusType="+statusType+
