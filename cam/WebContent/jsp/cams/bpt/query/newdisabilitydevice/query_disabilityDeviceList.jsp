...
<script language="javascript">
	...
	var startTime = '<%=request.getParameter("cxsjq") == null?"":request.getParameter("cxsjq")%>';
	var endTime="<%=request.getParameter("cxsjz")%>";
	var query_timeType='<%=request.getParameter("timeType")%>';
	var assDevicesType = '<%=request.getParameter("assDevicesType")%>';
	var configuration = '<%=request.getParameter("configuration")%>';
		 	 //查询条件打开合并函数
		...
		<script type="text/javascript" src="query_disabilityDeviceList.js"></script>
...
<model:datasets>
	...
	<model:dataset id="BptPeopleDisabilityDataSet"
		...
		method="queryAssDevices" global="true" pageSize="100">
	</model:dataset>
	<!-- 行政区划 -->
	...
	<model:dataset id="assDevicesDataset"
		...
		<model:params>
			...
			<model:param name="filterSql" value='IS_EFFICIENT=1 '></model:param>
			<model:param name="sort" value='APPLY_APPARATUS_ID'></model:param>
		</model:params>
	</model:dataset>
		...
		<table border="1" width="100%">
			...
			<tr>
				...
				<td class="FieldInput"><select id="configuration"
					...
					<option value='0'>新申请或更换</option>
					<option value='1'>维修</option>
				</select></td>
				<td align="center"  colspan="2" ><button id="queryButton" onclick="query()">查 询</button></td>
			</tr>
			<tr>
				<td class="FieldLabel" nowrap="nowrap">批准日期：</td>
					<td  colspan="3" nowrap="nowrap" style="background-color: #f5fafa;text-align: left;font: normal 12px tahoma, arial, helvetica, sans-serif;"><select id ="timeType" style="width:80px;" onchange="showTime()">
