...
function showDiv() {
	...
	if (length == 0) {
		ds.newRecord({
					...
					"demobilized_iliness_num" : 0,
					"demobilized_iliness_mon" : 0,
					"demobilized_iliness_avg_mon" : 0,
					"war_num" : 0,
					"war_town_mon" : 0,
					"war_town_avg_mon" : 0,
					"tested_num" : 0,
					"tested_town_mon" : 0,
					"tested_town_avg_mon" : 0,
					"soldier_num" : 0,
					"soldier_town_mon" : 0,
...
function test(value) {
	...
	var url = L5.webPath+"/jsp/cams/bpt/query/livingSubsidyPayment/query_LivingSubsidyPayment.jsp?xzqu="+xzqu+"&pageflag=1"+sj;
	window.open(url);
}
 //打印预览调用方法
function printBill(){
