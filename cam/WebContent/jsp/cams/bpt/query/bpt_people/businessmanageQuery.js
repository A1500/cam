...
function query() {
	...
	var ds = L5.DatasetMgr.lookup("BptBusinessManage");
	...
	ds.setParameter("serviceType",type);
	ds.load();
	noReclick(ds,"queryButton");
}

function format(value) {
	var records = ApplyDataset.getAllRecords();
	for(var i = 0;i<records.length;i++) {
		if(records[i].get("value") == value) {
			return records[i].get("text");
		}  
	}
	return value;
}
...
function emport() {
	if(BptBusinessManage.getCount() == 0) {
		L5.Msg.alert("提示","没有要导出的数据");
		return false;
	}
	L5.dataset2excel(BptBusinessManage,"/jsp/cams/bpt/query/excel_export/businessmanageExcel.jsp");
}