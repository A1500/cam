...
function query(){
	CountretiredsoldierErrDataset.setParameter("organCode",user);
	CountretiredsoldierErrDataset.setParameter("makeTable","BPT_IMP_SOLDIER_MAKE");
	CountretiredsoldierErrDataset.load();
}

//60岁老兵错误导出
function errExp(){
	var pringDs = L5.DatasetMgr.lookup("CountretiredsoldierErrDataset");
	pringDs.setParameter("organCode",user);
	pringDs.setParameter("makeTable","BPT_IMP_SOLDIER_MAKE");
...
function into(){
	var xmlPath="com/inspur/cams/bpt/excelToTable/bathXml/countretiredsoldier.xml" ;
	 ...
	 if(msg!=null){
	 	...
	 	if(m[2]=="0"){
	 		if(confirm("验证成功！是否写入数据库?")){
	 			var command = new L5.Command("com.inspur.cams.bpt.excelToTable.command.SoldierCommand");
...
function downTemplate(){
	var filename = '60zhousuimoban.xls';
	var path = 'jsp/cams/bpt/excelToTable/excel/countretiredsoldier.xls';
