...
function update(){
	...
	var url=L5.webPath+"/jsp/cams/bpt/excelToTable/demobilized/demobilizedErrUpdate.jsp?id=" + id  ;
...
function query(){
	DemobilizedErrDataset.setParameter("organCode",user);
	DemobilizedErrDataset.setParameter("makeTable","bpt_imp_demobilized_make");
	DemobilizedErrDataset.load();
}
/**
 * 所属机构窗口
 */
...
function into(){
	var xmlPath="com/inspur/cams/bpt/excelToTable/bathXml/demobilized.xml" ;
	...
	if(msg!=null){
	 	...
	 	if(m[2]=="0"){
	 		if(confirm("验证成功！是否写入数据库?")){
	 			var command = new L5.Command("com.inspur.cams.bpt.excelToTable.command.DemobilizedCommand");
...
function downTemplate(){
	var filename = 'laofuyuanmoban.xls';
	var path = 'jsp/cams/bpt/excelToTable/excel/demobilized.xls';
	window.location=L5.webPath+"/fileDownload?filePath="+path + "&fileName="+filename + "&project=1";
}

//乡复员军人错误导出
function errExp(){
	var pringDs = L5.DatasetMgr.lookup("DemobilizedErrDataset");
	pringDs.setParameter("organCode",user);
	pringDs.setParameter("makeTable","bpt_imp_demobilized_make");
