...
function update(){
	...
	var url=L5.webPath+"/jsp/cams/bpt/excelToTable/tested/testedErrUpdate.jsp?id=" + id  ;
...
function query(){
	testErrDataset.setParameter("organCode",user);
	testErrDataset.setParameter("makeTable","bpt_imp_tested_make");
	testErrDataset.load();
}

//参试错误导出
function errExp(){
	var pringDs = L5.DatasetMgr.lookup("testErrDataset");
	pringDs.setParameter("organCode",user);
	pringDs.setParameter("makeTable","bpt_imp_tested_make");
...
function into(){
	var xmlPath="com/inspur/cams/bpt/excelToTable/bathXml/tested.xml" ;
	 ...
	 if(msg!=null){
	 	...
	 	if(m[2]=="0"){
	 		if(confirm("验证成功！是否写入数据库?")){
	 			var command = new L5.Command("com.inspur.cams.bpt.excelToTable.command.TestedCommand");
...
function save(){
	...
	if (!command.error) {
		...
		query();
		/*var disabiltyErrDataset=L5.DatasetMgr.lookup("disabiltyErrDataset");
	    disabiltyErrDataset.setParameter("organCode",organCode);
		disabiltyErrDataset.setParameter("makeTable","bpt_imp_tested_make");
		disabiltyErrDataset.setParameter("isVailidate","0");
...
function downTemplate(){
	var filename = 'canshimoban.xls';
	var path = 'jsp/cams/bpt/excelToTable/excel/tested.xls';
