...
function update(){
	...
	var url=L5.webPath+"/jsp/cams/bpt/excelToTable/demobilizedIllness/demobilizedIllnessErrUpdate.jsp?id=" + id  ;
...
/*disabiltyErrDataset.setParameter("domicileCode",getParam("domicileCode"));
	...
	demoIllErrDataset.setParameter("organCode",user);
	demoIllErrDataset.setParameter("makeTable","bpt_imp_demillness_make");
	demoIllErrDataset.load();
}
/**
 * 所属机构窗口
 */
...
function into(){
	var xmlPath="com/inspur/cams/bpt/excelToTable/bathXml/demobilizedIllness.xml" ;
	 ...
	 if(msg!=null){
	 	...
	 	if(m[2]=="0"){
	 		if(confirm("验证成功！是否写入数据库?")){
	 			var command = new L5.Command("com.inspur.cams.bpt.excelToTable.command.DemobilizedIllnessCommand");
...
function del(){
	...
	command.setParameter("makeTable", "bpt_imp_demillness_make");
...
function downTemplate(){
	var filename = 'bingtuimoban.xls';
	var path = 'jsp/cams/bpt/excelToTable/excel/demobilizedIllness.xls';
	window.location=L5.webPath+"/fileDownload?filePath="+path + "&fileName="+filename + "&project=1";
}

//三属错误导出
function errExp(){
	var pringDs = L5.DatasetMgr.lookup("demoIllErrDataset");
	pringDs.setParameter("organCode",user);
	pringDs.setParameter("makeTable","bpt_imp_demillness_make");
