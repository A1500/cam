...
function update(){
	...
	var url=L5.webPath+"/jsp/cams/bpt/excelToTable/disability/disabilityErrUpdate.jsp?id=" + id  ;
...
function query(){
	/*disabiltyErrDataset.setParameter("domicileCode",getParam("domicileCode"));
	disabiltyErrDataset.setParameter("name",getParam("name"));
	disabiltyErrDataset.setParameter("idCard",getParam("idCard"));
	disabiltyErrDataset.setParameter("isInDB",getParam("isInDB"));	
	disabiltyErrDataset.setParameter("isVailidate",getParam("isVailidate"));	
	disabiltyErrDataset.setParameter("isRepeat",getParam("isRepeat"));	*/
	disabiltyErrDataset.setParameter("organCode",user);
	disabiltyErrDataset.setParameter("makeTable","bpt_imp_people_disability_make");
	disabiltyErrDataset.load();
}
/**
 * 所属机构窗口
 */
...
function into(){
	var xmlPath="com/inspur/cams/bpt/excelToTable/bathXml/disabilityBath.xml" ;
...
function downTemplate(){
	var filename = 'shangcanmoban.xls';
	var path = 'jsp/cams/bpt/excelToTable/excel/disability.xls';
	window.location=L5.webPath+"/fileDownload?filePath="+path + "&fileName="+filename + "&project=1";
}
function excel_click(){//导出Excel
var ds=L5.DatasetMgr.lookup("disabiltyErrDataset1");
		ds.setParameter("organCode",user);
		ds.setParameter("makeTable","bpt_imp_people_disability_make");
	...
	ds.baseParams["limitExcel"]=total;  //导出记录数，如导出类型为导出符合查询条件所有记录时，其值为ds.getTotalCount()
		L5.dataset2excel(ds,"/jsp/cams/bpt/excelToTable/excel/errExcel.jsp");
	ds.baseParams["excelType"]="reset"; //重置导出类型
}

//伤残错误导出
function errExp(){
	var pringDs = L5.DatasetMgr.lookup("disabiltyErrDataset");
	pringDs.setParameter("organCode",user);
	pringDs.setParameter("makeTable","bpt_imp_people_disability_make");
