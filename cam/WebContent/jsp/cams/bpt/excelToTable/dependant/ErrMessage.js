...
function update(){
	...
	var url=L5.webPath+"/jsp/cams/bpt/excelToTable/dependant/dependantErrUpdate.jsp?id=" + id  ;
...
function query(){
	...
	DependantErrDataset.setParameter("organCode",user);
	DependantErrDataset.setParameter("makeTable","bpt_imp_dependant_make");
	DependantErrDataset.load();
}
/**
 * 所属机构窗口
 */
...
function into(){
	var xmlPath="com/inspur/cams/bpt/excelToTable/bathXml/dependant.xml" ;
	...
	if(msg!=null){
	 	...
	 	if(m[2]=="0"){
	 		if(confirm("验证成功！是否写入数据库?")){
	 			var command = new L5.Command("com.inspur.cams.bpt.excelToTable.command.DependantCommand");
...
function downTemplate(){
	var filename = 'sanshumoban.xls';
	var path = 'jsp/cams/bpt/excelToTable/excel/dependant.xls';
	window.location=L5.webPath+"/fileDownload?filePath="+path + "&fileName="+filename + "&project=1";
}

//三属错误导出
function errExp(){
	var pringDs = L5.DatasetMgr.lookup("DependantErrDataset");
	pringDs.setParameter("organCode",user);
	pringDs.setParameter("makeTable","bpt_imp_dependant_make");
