...
function query(){
	MartyrOffspringErrDataset.setParameter("organCode",user);
	MartyrOffspringErrDataset.setParameter("makeTable","BPT_IMP_MARTYROFFSPRING_MAKE");
	MartyrOffspringErrDataset.load();
}

//老烈子女错误导出
function errExp(){
	var pringDs = L5.DatasetMgr.lookup("MartyrOffspringErrDataset");
	pringDs.setParameter("organCode",user);
	pringDs.setParameter("makeTable","BPT_IMP_MARTYROFFSPRING_MAKE");
...
function into(){
	var xmlPath="com/inspur/cams/bpt/excelToTable/bathXml/martyroffspring.xml" ;
	 ...
	 if(msg!=null){
	 	...
	 	if(m[2]=="0"){
	 		if(confirm("验证成功！是否写入数据库?")){
	 			var command = new L5.Command("com.inspur.cams.bpt.excelToTable.command.MartyrCommand");
...
function downTemplate(){
	var filename = 'laoliezinvmoban.xls';
	var path = 'jsp/cams/bpt/excelToTable/excel/martyroffspring.xls';
