...
function query(){
	ds.setParameter("BPT_MARTYR_BUILDINGS.DOMICILE_CODE@like",getParam("domicileCode"));
	ds.setParameter("BPT_MARTYR_BUILDINGS.UNIT_FULLNAME@=",getParam("unitFullname"));		
	ds.setParameter("BPT_MARTYR_BUILDINGS.PERSON_SECTION@=",getParam("personSection"));		
	ds.setParameter("BPT_MARTYR_BUILDINGS.PROTECTION_UNIT@=",getParam("protectionUnit"));		
	ds.setParameter("REG_ORGAN_CODE@like@String",getSubStrOrganCode());
	ds.load();
...
function insert() {
	...
	var url='jsp/cams/bpt/institution/martyrbuildings/bptmartyrbuildings_edit.jsp';
	var text = '增加烈士纪念建筑物保护单位信息';
...
function update(){
	...
	data.put("dataBean",selected[0].get('mbuildingId'));
	var url='jsp/cams/bpt/institution/martyrbuildings/bptmartyrbuildings_edit.jsp';
	var text = '修改烈士纪念建筑物保护单位信息';
	L5.forward(url,text,data);
}
	...
	L5.MessageBox.confirm('确定', '确定要删除选中的记录吗?',function(state){
		if(state=="yes"){
			...
			for(var i=0;i<selected.length;i++){
				delIds[i]=selected[i].get("mbuildingId");
			}
...
function reset(){
	...
	document.getElementById("protectionUnit").value = "";
}
...
function Evt_butdetail_click() {
	...
	data.put("method","UPDATE");
	data.put("dataBean",selected[0].get('mbuildingId'));
	var url='jsp/cams/bpt/institution/martyrbuildings/bptmartyrbuildings_detail.jsp';
	var text = '优抚医院信息';
	L5.forward(url,text,data);
}
