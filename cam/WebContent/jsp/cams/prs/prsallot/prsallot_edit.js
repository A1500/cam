...
function nameNew(){
	var retiredSoldierDataset = L5.DatasetMgr.lookup("retiredSoldierDataset");
	var name = retiredSoldierDataset.get("name");
	name = name.replace(/\s/ig,'');//去掉空格符，制表符，换行符等一系列空白符
	retiredSoldierDataset.set("name",name);//异步，存dataset
	document.getElementById("name").value = name;//修改显示页面。如果没有此行，虽然在方法中dataset修改了新的name，由于异步调用，退出方法后前台界面依旧显示旧的name值，然后dataset又在前台读了一遍name，把方法内修改好的name又覆盖了。
}
/*
