...
function getPrintDataByDataSet(reportName,printDataset){
	...
	var dqsj=new Date();
	var cxsjq = planYear+"-01-01";
	var cxsjz = planYear+"-12-31";
	var xmlStr="<?xml version='1.0' encoding='GBK' standalone='no'?>";
	xmlStr+="<Report ReportName='' Creater='"+dqsj+"' CreateOrgan='"+organName+"' ReportNo='"+planYear+"' ReportType='"+planYear+"'>";
...
<model:datasets>
	...
	<model:dataset id="retiredSoldierDataset"
		...
		method="queryDisabilityPlacement" pageSize="20">
		<model:record>
			...
			<model:field name="FILENUM" type="string" />
			<model:field name="RECEIVETIME" type="string" />
			<model:field name="NORECEIVERESON" type="string" />
		</model:record>
	</model:dataset>
	<model:dataset id="printDataset"
		cmd="com.inspur.cams.prs.prsretiredsoldiers.cmd.PrsRetiredSoldiersQueryCommand"
		method="queryDisabilityPlacement" pageSize="15">
		<model:record>
			<model:field name="REGION" type="string" />
			<model:field name="NAME" type="string" />
			<model:field name="SPEC" type="string" />
...
<next:GridPanel id="editGridPanel" name="prsretiredsoldiersGrid"
	...
	<next:Columns>
		...
		<next:Column id="SPEC" header="未接受原因" field="NORECEIVERESON"
			width="90">
