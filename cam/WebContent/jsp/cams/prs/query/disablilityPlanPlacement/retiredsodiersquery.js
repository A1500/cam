...
function init() {
	...
	retiredSoldierDataset.setParameter("planYear", planYear);
	retiredSoldierDataset.setParameter("type", type);
...
function printBill(){
	...
	printDataset.setParameter("planYear", planYear);
	printDataset.setParameter("type", type);
	printDataset.load();
}
// 打印方法传递的record
...
function exportCSV(){
	...
	if(type=='0'){
		var fileName = planYear+"年度"+orgName+"计划内移交伤病残移交退役士兵未接收退役士兵信息";
		L5.dataset2excel(ds, "/jsp/cams/prs/query/disablilityPlanPlacement/unReceivedsoldiersexport.jsp?fileName="+fileName);
	}else if(type =='1'){
		var fileName = planYear+"年度"+orgName+"计划内移交伤病残移交退役士兵已接收退役士兵信息";
		L5.dataset2excel(ds, "/jsp/cams/prs/query/disablilityPlanPlacement/receivedsoldiersexport.jsp?fileName="+fileName);
	}
}
