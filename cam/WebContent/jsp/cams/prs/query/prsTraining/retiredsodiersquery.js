...
function init() {
	...
	retiredSoldierDataset.setParameter("trainingflag", trainingflag);
	if(training_type!="null"&&training_type!=null&&training_type!=""){
		retiredSoldierDataset.setParameter("training_type", training_type);
	}
	...
	printDataset.setParameter("trainingflag", trainingflag);
	if(training_type!="null"&&training_type!=null&&training_type!=""){
		printDataset.setParameter("training_type", training_type);
	}
...
function exportCSV(){
	...
	L5.dataset2excel(ds, "/jsp/cams/prs/query/soldiersexport.jsp?fileName="+fileName);
}
...
function printBill2(){
    ...
    var printTile = "退役士兵培训统计表";
    printReport(getPrintDataByDataSet(printTitle,printDataset),L5.webPath+'/jsp/cams/prs/query/retiredSoldiersTrainTongji5.fr3');
}
/**
 * 培训类型枚举值转换
 */
function dicTrainingType(){
	var printDataset = L5.DatasetMgr.lookup("printDataset");
	var records=printDataset.getAllRecords();
	var n=records.length;
	if(records.length==0){return;}
	for(i=0;i<n;i++){
		var TRAINING_TYPE = records[i].get('TRAINING_TYPE');
		switch (TRAINING_TYPE) {
		case  "0":
			records[i].set("TRAINING_TYPE","职业教育");
			break;
		case  "1":
			records[i].set("TRAINING_TYPE","技能培训");
			break;
		}
	}
}
