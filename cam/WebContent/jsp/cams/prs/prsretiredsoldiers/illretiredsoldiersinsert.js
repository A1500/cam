...
function init() {
	...
	recordSoldier.set("retiredSoldierType", 4);
	recordSoldier.set("sex", "1");
	recordSoldier.set("naton", "01");
	recordSoldier.set("disabilityFlag", "1");
	document.getElementById("disabilityFlag").disabled = true;
	...
	prssoldiersArmyDataset.set("mentalGrade","0");
...
function approvalandsave() {
	...
	var receiveTime = document.getElementById("receiveTime").value;
	if(!checkBirthday(receiveTime)&&(receiveTime!="")){
	    L5.Msg.alert("提示", "接收时间格式或者内容不正确!");
	    return false;
	}
	...
	if (!command.error) {
		L5.Msg.alert("提示","保存成功！",function(){
			...
			var text = '计划移交伤病残士兵接收管理';
		  L5.forward(url, text, data);
		});
	} else {
		L5.Msg.alert("错误", "保存失败"+command.error);
	}
...
function back() {
	var url = 'jsp/cams/prs/prsretiredsoldiers/illretiredsodierslist.jsp';
...
function ifReceiveChanges(){
	...
	if(ifReceive=="0"){
		 ...
		 document.getElementById("receiveTimeDiv").style.display="none";
		...
		document.getElementById("receiveTimeL").style.display="";
		 document.getElementById("receiveTimeDiv").style.display="";
