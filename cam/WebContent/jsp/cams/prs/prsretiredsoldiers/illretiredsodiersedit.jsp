...
<script type="text/javascript" src="illretiredsodiersedit.js"></script>
</head>
<body>
	...
	<next:TabPanel>
		<next:Tabs>
			<next:Panel title="伤病残退役士兵基本信息">
				<next:Html>
					<iframe id="instanceDisability"
						src="../prsretiredsoldiers/illprsedit.jsp?dataBean=<%=request.getParameter("idField")%>&method=<%=request.getParameter("method")%>&type=<%=request.getParameter("type")%>"
			...
			<next:Panel title="伤病残退役士兵部队信息" id="armyInfo">
				<next:Html>
					<iframe id="instanceDisability"
						src="../prssoldiersarmy/illarmyedit.jsp?dataBean=<%=request.getParameter("idField")%>&method=<%=request.getParameter("method")%>&type=<%=request.getParameter("type")%>"
						frameborder="no" border="0" marginwidth="0" marginheight="0"
