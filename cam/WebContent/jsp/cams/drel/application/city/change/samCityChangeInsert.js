...
function loadApplyInfo(ds){
	...
	if(ds.getCount() == 1){
		...
		setChangeItems(ds.get('changeItem'));
...
function loadFilesListDs(ds){
   ds.un("load",loadFilesListDs);
    if(applyId != ''){
    	ArchCatalogDS.setParameter("applyType","011");
...
function save(){
	...
	//把家庭信息的地区维护成当前登录单位的地区
	//familyRecord.set("domicileCode",organArea);
	//familyRecord.set("domicileName",organName);
...
function scanFiles(){
    ...
    var url="../../../../../comm/arch/archUploadfile.jsp?applyId="+applyId+"&sysType=sam&applyType=011&hrefFlg="+getCurrent();
