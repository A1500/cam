...
function init(){
    ...
    //城市低保
    asisstanceClassDataset.setParameter("filterSql", "ASSISTANCE_TYPE='01' AND IN_USE='1' AND AREA_CODE= '" + organArea.substring(0, 6) + "000000'");
...
function submitApplyCheck(){
	...
	if(applyName=="check"){
		...
		if(applyDataSet.get("auditOpinionId") != '3'){
			var assMoney = familyDataSet.get('assistanceMoney');
			if( assMoney == '' && assMoney!=0 ){
...
function detailexpand(){
		...
		var url='../apply/samCityExpandDetail.jsp?familyId='+familyId+'&applyId='+applyId+"&hrefFlg="+getCurrent();
