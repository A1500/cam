...
<script type="text/javascript">
	...
	var assistanceTypeName="农村居民最低生活保障";
	...
	function newApply(){
		...
		var url='samCountryChangeInsert.jsp?method=insert&applyId=&name='+nameValue+"&hrefFlg="+getCurrent();
	...
	function handle(){
		...
		if(records&&records.length==1){
		    ...
		    if(nameValue=="accept" || ( records[0].get('ACCEPT_DEPT_ID') == records[0].get('CHECK_DEPT_ID')&& flag=='1' ) ){
				var url='samCountryChangeInsert.jsp?method=update&name='+nameValue+'&applyId='+records[0].get('APPLY_ID')+'&assignmentId='+records[0].get('ID')+"&hrefFlg="+getCurrent();
		    }else{
		  	  var url='samCountryChangeDetail.jsp?method=handle&name='+nameValue+'&applyId='+records[0].get('APPLY_ID')+'&assignmentId='+records[0].get('ID')+"&hrefFlg="+getCurrent();
		    }
			window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
...
<model:datasets>
	<!-- 业务信息 -->
	<model:dataset id="applyDataSet" cmd="com.inspur.cams.drel.sam.cmd.SamApplyInfoQueryCmd" method="queryDaiBan" global="true" sortField="APPLY_DATE" pageSize="20">
		<model:params>
			<model:param name="processType" value='samCountryChange'></model:param>
    ...
    <model:dataset id="samChangeItemDs" enumName="SAM_CHANGE_ITEM" autoLoad="true" global="true"></model:dataset>
			...
			<next:Panel width="100%" height="100%">
				<next:Panel title="待办业务查询" width="100%" autoHeight="true" titleCollapse="true" collapsible="true"  >	
				 ...
				 <next:GridPanel  clickToSelectedForChkSM="true"id="daiBanGridPanel" enableHdMenu="true"  title="待办业务列表" dataset="applyDataSet" width="100%" height="100%" stripeRows="true" notSelectFirstRow="true">
					<next:TopBar>		
					...
					<next:Columns>
						...
						<next:Column  header="社区（村）" field="DOMICILE_NAME"  width="200" sortable="true"  ></next:Column>							
				        <next:Column header="户主姓名" field="FAMILY_NAME" width="150" sortable="true" ></next:Column>
			        	<next:Column header="身份证号码" field="FAMILY_CARD_NO" width="150" sortable="true" ></next:Column>
			        	<next:Column header="变更事项" field="CHANGE_ITEM" width="150" dataset="samChangeItemDs" sortable="true" tooltip="点击排序"> </next:Column>
						<next:Column header="登记日期" field="APPLY_DATE" width="150" sortable="true" tooltip="点击排序" ></next:Column>
						<next:Column header="当前环节" field="ACT_DEF_NAME" width="140" sortable="true" ></next:Column>
					</next:Columns>
					<next:BottomBar>
