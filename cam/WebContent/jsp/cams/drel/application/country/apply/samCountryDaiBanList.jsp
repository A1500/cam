...
<script type="text/javascript">
	...
	function newApply(){
		...
		var url='samCountryInsert.jsp?method=insert&applyId=&name='+nameValue+"&hrefFlg="+getCurrent();
	...
	function handle(){
		...
		if(records&&records.length==1){
		    ...
		    if(nameValue=="accept" || ( records[0].get('ACCEPT_DEPT_ID') == records[0].get('CHECK_DEPT_ID')&& flag=='1' ) ){
				var url='samCountryInsert.jsp?method=update&name='+nameValue+'&applyId='+records[0].get('APPLY_ID')+'&assignmentId='+records[0].get('ID')+"&hrefFlg="+getCurrent();
		    }else{
		  	  var url='samCountryDetail.jsp?method=handle&name='+nameValue+'&applyId='+records[0].get('APPLY_ID')+'&assignmentId='+records[0].get('ID')+"&hrefFlg="+getCurrent();
		    }
			window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	...
	function query(){
	   var lrdwId=document.getElementById("lrdwId").value;
		...
		var mysetwhere="FAMILY_NAME:"+familyName+"&FAMILY_CARD_NO:"+idCard+"&DOMICILE_CODE:"+
		                lrdwId+"&APPLY_DATE:"+regDateStart+"-"+regDateEnd;
...
<model:datasets>
	...
	<model:dataset id="applyDataSet" cmd="com.inspur.cams.drel.sam.cmd.SamApplyInfoQueryCmd" method="queryDaiBan" global="true" pageSize="20" >
		<model:params>
			<model:param name="processType" value='samCountry'></model:param>
				...
				<next:Panel title="待办业务查询" width="100%" autoHeight="true" titleCollapse="true" collapsible="true"  >	
				 ...
				 <next:GridPanel  clickToSelectedForChkSM="true"id="daiBanGridPanel" title="待办业务列表" dataset="applyDataSet" width="100%" height="100%" stripeRows="true" notSelectFirstRow="true">
					<next:TopBar>		
						...
						<next:ToolBarItem iconCls="print"  text="打印公示书" handler="printCountryDaiban"/> 	
					</next:TopBar>
					<next:Columns>
						...
						<next:Column header="核对状态" field="checkFlg" width="140" renderer="getCheckFlg"></next:Column>
