...
function gotoPeople(){
	...
	var url='/jsp/cams/drel/application/cityLow/apply/cityLowFamilyAreaCount.jsp';
	var text = '城市低收入区域家庭数统计';
...
<model:datasets>
	<!-- 家庭基本信息 -->
	<model:dataset id="lowFamilyNoCountDataSet" cmd="com.inspur.cams.drel.application.cityLow.cmd.SamCityLowConfirmQueryCmd" global="true" pageSize="10" method="lowFamilyWealthCount">
		<model:record >
			<model:field name="apanageName" mapping="APANAGE"/>
			<model:field name="avgIncome" mapping="FAMINCOME"/>
			<model:field name="avgProperty" mapping="FAMILYPROPERTY"/>
		</model:record>
	</model:dataset>
</model:datasets>
				...
				<next:Panel title="低收入家庭查询" width="100%" autoHeight="true" titleCollapse="true" collapsible="true"  >	
				 ...
				 <next:GridPanel  clickToSelectedForChkSM="true" id="daiBanGridPanel" title="低收入家庭列表" dataset="lowFamilyNoCountDataSet" width="100%" height="100%" stripeRows="true" notSelectFirstRow="true">
					...
					<next:Columns>
				        ...
				        <next:Column header="平均收入" field="avgIncome" width="150" ></next:Column>
				         <next:Column header="平均资产" field="avgProperty" width="150" ></next:Column>				       
					</next:Columns>
					<next:BottomBar>
