...
<script type="text/javascript">
	...
	function newApply(){
		...
		var url='samFiveInsert.jsp?method=insert&applyId=&name='+nameValue+"&hrefFlg="+getCurrent();
	...
	function handle(){
		...
		if(records&&records.length==1){
		    ...
		    if(nameValue=="accept" || ( records[0].get('ACCEPT_DEPT_ID') == records[0].get('CHECK_DEPT_ID')&& flag=='1' ) ){
				var url='samFiveInsert.jsp?method=update&name='+nameValue+'&applyId='+records[0].get('APPLY_ID')+'&assignmentId='+records[0].get('ID')+"&hrefFlg="+getCurrent();
		    }else{
		  	  var url='samFiveDetail.jsp?method=handle&name='+nameValue+'&applyId='+records[0].get('APPLY_ID')+'&assignmentId='+records[0].get('ID')+"&hrefFlg="+getCurrent();
		    }
			window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
				...
				<next:Panel title="待办业务查询" width="100%" autoHeight="true" titleCollapse="true" collapsible="true"  >	
				 ...
				 <next:GridPanel  clickToSelectedForChkSM="true"id="daiBanGridPanel" title="待办业务列表" dataset="applyDataSet" width="100%" height="100%" stripeRows="true" notSelectFirstRow="true">
					<next:TopBar>		
						...
						%>	
						<next:ToolBarItem iconCls="print"  text="打印公示书" handler="printFiveDaiBan"/> 	
					</next:TopBar>
					<next:Columns>
