...
function prepareForInsert(){
	...
	if(ActivityName == "check"){
		familyDataSet.newRecord({"assistanceMode":"21","applyDate":nowTime,"address":address,"regId":regId,"regOrg":regOrg,"regOrgName":regOrgName,"regPeople":regPeople,"regTime":regTime,"regOrgType":regOrgType,"apanageName":organName,"apanageCode":organArea});//一档
		checkNoticeDataset.newRecord({"activityId":"check"});
		checkSurveyDataset.newRecord({"activityId":"check"});
	}else{
		familyDataSet.newRecord({"domicileName":organName,"domicileCode":organArea,"assistanceMode":"21","applyDate":nowTime,"address":address,"regId":regId,"regOrg":regOrg,"regOrgName":regOrgName,"regPeople":regPeople,"regTime":regTime,"regOrgType":regOrgType,"apanageName":organName,"apanageCode":organArea});//一档
...
function loadAsisstanceClass(){
	//加载供养机构字典
	var supportOrgDataset = L5.DatasetMgr.lookup("supportOrgDataset");
	supportOrgDataset.setParameter("filterSql", "SERVE_TOWNS_ID LIKE '%" + organArea + "%' AND IF_CURRENT='1'");
	supportOrgDataset.load();
...
function loadFilesListDs(ds){
    ...
    if(applyId != ''){
    	ArchCatalogDS.setParameter("applyType","030");
...
function save(){
	...
	if (!command.error) {
		...
		if(!hasSelef){
			...
			var width = 1000;
		    var height = 433;
			var url="samBasePeopleInsert.jsp?familyId="+familyId+"&flag=1"+"&hrefFlg="+getCurrent()+"&applyId="+applyId;
...
function addPeople(){
	...
	if(!hasSelef){
		...
		var width = 1000;
	    var height = 433;
		var url="samBasePeopleInsert.jsp?familyId="+familyId+"&hrefFlg="+getCurrent()+"&applyId="+applyId;
		window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	}
}
//修改人员信息
function changePeople(){
	...
	if(records&&records.length==1){
		var width = 1000;
	    var height = 433;
		var url="samBasePeopleInsert.jsp?familyId="+familyId+"&peopleId="+records[0].get('PEOPLE_ID')+"&hrefFlg="+getCurrent()+"&applyId="+applyId;
		window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	}else{
...
function addSupport(){
	...
	var width = 850;
   		 var height = 200;
	var url="../../../comm/supportEdit.jsp?familyId="+familyId+"&assistanceType=03"+"&applyId="+applyId+"&hrefFlg="+getCurrent();
	window.showModalDialog(url, window, "scroll:0;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
}
//修改赡扶抚养义务人员
function changeSupport(){
	...
	if(records&&records.length==1){
		var width = 850;
   		 var height = 200;
		var url="../../../comm/supportEdit.jsp?familyId="+familyId+"&supportId="+records[0].get('supportId')+"&assistanceType=03"+"&applyId="+applyId+"&hrefFlg="+getCurrent();
		window.showModalDialog(url, window, "scroll:0;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	}else{
		L5.Msg.alert("提示","请选择一条记录！");
	}
...
function scanFiles(){
    ...
    var url="../../../../../comm/arch/archUploadfile.jsp?applyId="+applyId+"&sysType=sam&applyType=030&hrefFlg="+getCurrent();
