...
function loadFamilyQuery(ds){
	...
	if(ds.getCount()<1){
		...
		var supportWay=ds.get('SUPPORT_WAY');
		$('supportWay').value=ds.get('SUPPORT_WAY');//供养方式
		if(supportWay=='03'){//集中供养
			supportOrg=ds.get('SUPPORT_ORG');
...
function confirmFamilyQueryWin(){	
	...
	if(!record){
		...
		var supportWay=record.get('SUPPORT_WAY');
		$('supportWay').value=record.get('SUPPORT_WAY');//供养方式
		if(supportWay=='03'){//集中供养
			supportOrg=record.get('SUPPORT_ORG');
			$('supportOrg').disabled=false;
		}
		//加载分类施保类别字典
		loadAsisstanceClass();
...
function loadFilesListDs(ds){
    ...
    if(applyId != ''){
    	ArchCatalogDS.setParameter("applyType","031");
...
function submitApplyCheck(){
	...
	var checkPersonStatusVal = checkPersonStatus();
...
function addPeople(){
	...
	if(!hasSelef){
		var width = 1000;
	    var height = 433;
		var url="../apply/samBasePeopleInsert.jsp?familyId="+familyId+"&flag=1"+"&hrefFlg="+getCurrent()+"&applyId="+applyId;
		window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	}else{		
		var width = 1000;
	    var height = 433;
		var url="../apply/samBasePeopleInsert.jsp?familyId="+familyId+"&hrefFlg="+getCurrent()+"&applyId="+applyId;
		window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	}
}
//修改人员信息
function changePeople(){
	...
	if(records&&records.length==1){
		...
		var width = 1000;
	    var height = 433;
		var url="../apply/samBasePeopleInsert.jsp?familyId="+familyId+"&peopleId="+records[0].get('PEOPLE_ID')+"&hrefFlg="+getCurrent()+"&applyId="+applyId+"&isChangeUpdate=1";
		window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	}else{
		L5.Msg.alert("提示","请选择一条记录！");
	}
...
function scanFiles(){
    ...
    var url="../../../../../comm/arch/archUploadfile.jsp?applyId="+applyId+"&sysType=sam&applyType=031&hrefFlg="+getCurrent();
