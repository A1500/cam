...
<script type="text/javascript">
	...
	function newApply(){
		...
		var url='samFiveChangeInsert.jsp?method=insert&applyId=&name='+nameValue+"&hrefFlg="+getCurrent();
	...
	function handle(){
		...
		if(records&&records.length==1){
		    ...
		    if(nameValue=="accept" || ( records[0].get('ACCEPT_DEPT_ID') == records[0].get('CHECK_DEPT_ID')&& flag=='1' ) ){
				var url='samFiveChangeInsert.jsp?method=update&name='+nameValue+'&applyId='+records[0].get('APPLY_ID')+'&assignmentId='+records[0].get('ID')+"&hrefFlg="+getCurrent();
		    }else{
		  	  var url='samFiveChangeDetail.jsp?method=handle&name='+nameValue+'&applyId='+records[0].get('APPLY_ID')+'&assignmentId='+records[0].get('ID')+"&hrefFlg="+getCurrent();
		    }
			window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
...
<model:datasets>
	...
	<model:dataset id="applyDataSet" cmd="com.inspur.cams.drel.sam.cmd.SamApplyInfoQueryCmd" method="queryDaiBan" global="true" pageSize="20" >
		<model:params>
			<model:param name="processType" value='samFiveChange'></model:param>
