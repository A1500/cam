...
function init(){
	...
	familyDataSet.on('load',function(){
		if(familyDataSet.getCount() == 1){
			familyDataSet.set('applyDate',nowTime);
...
function loadFamilyDs(){
    familyDataSet.un("load",loadFamilyDs);
...
function detailexpand(){
	...
	var url='../apply/samFiveExpandDetail.jsp?familyId='+familyId+'&applyId='+applyId+"&hrefFlg="+getCurrent()+"&ifFile=0";
