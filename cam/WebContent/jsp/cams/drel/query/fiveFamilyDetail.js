...
function loadBank(ds){
	...
	if(familyTreatmentDataSet.getCurrent()){
		ds.set("supportWay",familyTreatmentDataSet.get('supportWay'));
		ds.set("supportOrg",familyTreatmentDataSet.get('supportOrg'));
	}
	ds.un("load",loadBank);
...
function peopleDetail(peopleId){
	var width = 1000;
    var height = 423;
	var url="../comm/samRuralAreasPeopleDetail.jsp?peopleId="+peopleId+'&applyId='+applyId;
...
function detailSupport(supportId){
	var width = 850;
    var height = 187;
	var url="../comm/supportDetail.jsp?familyId="+familyId+"&supportId="+supportId+"&assistanceType=03";
