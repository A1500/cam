...
<title>低保人员查询</title>
<next:ScriptManager />
	<script type="text/javascript" src="queryAllPeopleList.js"></script>
...
<model:datasets>
	...
	<!-- 分类施保类别 -->
	 ...
	 <!-- 属别 -->
	<model:dataset id="samAssistanceType" enumName="SAM_ASSISTANCETYPE.ASSISTANCETYPE" autoLoad="true" global="true">
	</model:dataset>
</model:datasets>
		...
		<table width="100%">
			...
			<tr>
			   ...
			   <td class="FieldLabel" nowrap="nowrap">享受年月</td>
                ...
                <td class="FieldInput" ><select id="relationshipType" style="width: 180px" name="与户主关系"  title="与户主关系"  > <option dataset="relationshipTypeDataset"></option> </select></td>
			
			</tr>
			<tr> 
				<td class="FieldLabel" nowrap="nowrap">保障类别</td>
                <td class="FieldInput" width="25%">
                	<select id="assistanceTypeId" name="保障类别" title="保障类别" style="width: 180px">
						<option dataset="samAssistanceType"></option>
					</select>
                </td>
				<td class="FieldLabel" width="8%">人员类型</td>
				<td class="FieldInput" >			    	
					<input type="text" id='peopleTypeName' style="width:83%" readOnly="true" />
					<img src="<%=SkinUtils.getImage(request, "l5/help.gif")%>" style="cursor:hand" onclick="openTypeWin()"/>
					<input type="hidden" id='peopleType'>
				</td>
				<td class="FieldButton" nowrap="nowrap" colspan="2">
...
<next:GridPanel id="infoQueryGrid" name="baseinfoPeopleResource"
	...
	<next:Columns>
   		...
   		<next:Column header="保障类别" field="ASSISTANCE_TYPE" width="80" align="center"  sortable="true" dataset="samAssistanceType"/>
		...
		<next:Column header="低（五）保证号" field="CARD_NO" width="130" align="center">
					<next:TextField />
				</next:Column>
