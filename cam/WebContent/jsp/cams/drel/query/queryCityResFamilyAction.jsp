...
<next:ScriptManager />
<script type="text/javascript" src="queryCityResFamilyAction.js"></script>
...
<model:datasets>
	...
	<!-- 分类施保类别 -->
	<model:dataset id="samAssistanceItemList"
	...
	<!-- 分类施保类别 -->
	<model:dataset id="asisstanceClassDataset"
		cmd="com.inspur.cams.comm.dicm.DicQueryCmd" global="true">
		<model:record fromBean="com.inspur.cams.comm.dicm.DicBean"></model:record>
		<model:params>
		...
		<table width="100%">
			...
			<tr>
				...
				<td class="FieldInput"><select name="assistanceClass"
					...
					<option
						dataset="<%if("00000000".equals(organArea.substring(4, 12))){
				...
				<td class="FieldInput" width="25%"><input type="text"
					...
					onclick="setmonth(this)" style="width: 180px"
					style="padding-right:20px;background:url('<%= SkinUtils.getImage(request, "default/rl.gif ") %> ') no-repeat scroll right center transparent; " /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="FieldInput"><input type="text"
					id='assistanceTypeName' style="width: 85%" readOnly="true" /> <img
					src="<%=SkinUtils.getImage(request, "l5/help.gif")%>"
					style="cursor: hand" onclick="openTypeWin()" /> <input
					type="hidden" id='assistanceType'></td>
...
<next:GridPanel id="infoQueryGrid" name="baseinfoFamilyResource"
	...
	<next:Columns>
		...
		<next:Column header="主要致贫原因" field="POVERTY_CAUSE" width="100"
			align="center" dataset="provertyCause">
			<next:TextField />
		</next:Column>
		...
		<next:Column header="开户银行" field="BANK" width="150" align="center"
			dataset="sambankDS">
			<next:TextField />
		</next:Column>
