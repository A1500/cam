function init() {
	...
	document.getElementById("assistanceType_treatment").value="02";
    if(organArea.substring(4,12) == '00000000'){//是市级或者省级
	    ...
	    asisstanceClassDataset.setParameter("filterSql", "ASSISTANCE_TYPE='02' AND IN_USE='1' AND AREA_CODE= '" + organArea.substring(0, 6) + "000000'");//城市低保
...
function query(){
	...
	baseinfoFamilyDS.setParameter("assistanceType","02");
	...
	baseinfoFamilyDS.on("load", function(){
    	...
    	if(baseinfoFamilyDS.getCount()>0) 
			qrToolbar.setText("农村低保家庭查询信息(" + baseinfoFamilyDS.getTotalCount() + "条)");
		else
			qrToolbar.setText("农村低保家庭查询信息");
    });
