function init() {
	ds.setParameter("sort","ASSITANCE_TYPE,VALID_BEGIN_DATE");
	ds.setParameter("SAM_LOW_STANDARDS.ASSITANCE_TYPE@in","D2,D1");
	//ds.setParameter("SAM_LOW_STANDARDS.ASSITANCE_TYPE@=","D1");
	ds.load();
	L5.QuickTips.init();
};
...
function query(){	
	ds.setParameter("SAM_LOW_STANDARDS.APANAGE_NAME@=",getParam("apanageName"));		
...
function save(){
	...
	if(!isFloat(records.get("lowStandards"))){
		L5.Msg.alert('提示',"校验未通过,不能保存!"+'低收入标准不是数字!');
		return false;
	}
...
function checkLowStandards(){
	...
	queryDs.on('load',function(ds){
		if(ds.getCount()>0){
			L5.Msg.alert("提示","低收入标准已存在，请在原标准基础上修改!");
