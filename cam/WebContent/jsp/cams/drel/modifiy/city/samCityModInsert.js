...
function changePeople(){
	...
	if(records&&records.length==1){
		var width = 1000;
	    var height = 600;
		var url="../../application/city/apply/samBasePeopleInsert.jsp?familyId="+familyId+"&peopleId="+records[0].get('PEOPLE_ID')+"&hrefFlg="+getCurrent()+"&applyId="+applyId+"&isMod=1"+"&isChangeUpdate=1";
		window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	}else{
...
function changeSupport(){
	...
	if(records&&records.length==1){
		var width = 850;
	    var height = 200;
		var url="../../comm/supportEdit.jsp?familyId="+familyId+"&supportId="+records[0].get('supportId')+"&assistanceType=01"+"&hrefFlg="+getCurrent()+"&isMod=1";
		window.showModalDialog(url, window, "scroll:0;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
	}else{
		L5.Msg.alert("提示","请选择一条记录！");
	}
