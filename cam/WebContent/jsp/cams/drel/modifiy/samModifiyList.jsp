...
<script type="text/javascript">
	...
	function insert(){
		...
		if(assistanceType=='01'){
			url='city/samCityModInsert.jsp';
		}else if(assistanceType=='02'){
			url='country/samCountryModInsert.jsp';
		}else if(assistanceType=='03'){
			url='five/samFiveModInsert.jsp';
		}
	...
	function update(){
		...
		if(records&&records.length==1){
			...
			url+='?method=update&familyId='+records[0].get('FAMILY_ID')+'&applyId='+records[0].get('APPLY_ID')+"&hrefFlg="+getCurrent();
			window.showModalDialog(url, window, "scroll:no;status:no;dialogWidth:" + width + "px;dialogHeight:" + height + "px;resizable:1");
		}else{
	...
	function query(){
	 ...
	 /*   
	    if(_$("cardNo")=='' &&  _$("familyName") == '' && _$("familyCardNo") == '' && lrdwId==organArea){
	    	L5.Msg.alert('提示',"请输入特定人员查询条件！");
	    	return;
	    }
	   */ 
...
<model:datasets>
	...
	<model:dataset id="familyList" cmd="com.inspur.cams.drel.sam.cmd.BaseinfoFamilyQueryCmd" method="querySamFamilyList" global="true"  >
		<model:params>
			...
			<model:param name="dataSource" value='his,saas,apply'></model:param>
		</model:params>
	</model:dataset>
...
<next:Panel width="100%" height="100%">
	...
	<next:GridPanel id="daiBanGridPanel" title="数据修改列表" dataset="familyList" width="100%" height="100%" stripeRows="true" notSelectFirstRow="true">
		<next:TopBar>		
