...
<script type="text/javascript"
	src="queryRuralResFamilyActionProvince.js"></script>
<script type="text/javascript"
	src='<%=SkinUtils.getJS(request,"DatePicker.js")%>'></script>
...
<model:datasets>
	...
	<model:dataset id="baseinfoFamilyDS"
		...
		method="querySamFamilyListProvince" pageSize="15">
	</model:dataset>
	<!-- 救助对象类型 -->
	...
	<!-- 分类施保类别 -->
	<model:dataset id="asisstanceClassDataset1"
		cmd="com.inspur.cams.comm.dicm.DicQueryCmd" autoLoad="true">
		<model:record fromBean="com.inspur.cams.comm.dicm.DicBean"></model:record>
		<model:params>
	...
	<model:dataset id="belongingDataSet" enumName="SAM_BELONGING"
		autoLoad="true" global="true"></model:dataset>
	<!-- 供养方式 -->
	<model:dataset id="supportWayDataset" enumName="SAM.SUPPORT_WAY"
		autoLoad="true" global="true"></model:dataset>
</model:datasets>
	...
	<next:Html>
		...
		<table width="100%">
			...
			<tr>
				...
				<td class="FieldInput" width="25%"><input type="text"
					name="familyCardNo" id="familyCardNo" style="width: 180px"
					onchange="checkIDCard(this)" /></td>
			...
			<tr>
				...
				<td class="FieldInput" width="25%"><input type="text"
					id="assistanceMoneyStart" name="assistanceMoneyStart"
					style="width: 52px" /> &nbsp;～&nbsp; <input type="text"
					id="assistanceMoneyEnd" name="assistanceMoneyEnd"
					style="width: 52px" /></td>
				<td class="FieldLabel" nowrap="nowrap" width="8%">保障人口数</td>
				<td class="FieldInput" width="25%"><input type="text"
					id="sAssitancePeopleNum" name="保障人口数起" style="width: 52px" />
				&nbsp;～&nbsp; <input type="text" id="eAssitancePeopleNum"
					name="保障人口数止" style="width: 52px" /></td>
				<td class="FieldLabel" nowrap="nowrap" width="8%">享受年月</td>
				<td class="FieldInput" width="25%"><input type="text"
					id="assistanceBegin" name="assistanceBegin" readonly="readonly"
					onclick="setmonth(this)"
					style='padding-right: 20px; background: url('
					<%= SkinUtils.getImage(request, "default/rl.gif ") %> ') no-repeat
					scroll right center transparent;'/></td>
			</tr>
			<tr>
				...
				<td class="FieldLabel" width="8%">属别</td>
				<td class="FieldInput"><select name="belonging" id="belonging"
					style="width: 180px">
					<option dataset="belongingDataSet"></option>
				</select></td>
				<% }else if("03".equals(assistanceType)){ %>
				<td class="FieldLabel">供养方式</td>
				<td class="FieldInput"><select id='supportWay' name='供养方式'
					style="width: 180px">
...
<next:GridPanel id="infoQueryGrid" name="baseinfoFamilyResource"
	...
	<next:TopBar>
		<next:ToolBarItem id="qrToolbar" text="家庭查询信息"></next:ToolBarItem>
	...
	<next:Columns>
		...
		<next:Column header="低保（五保）证号" field="CARD_NO" width="130"
			align="center">
			<next:TextField />
		</next:Column>
		...
		<next:Column header="分类施保类别" field="ASSISTANCE_CLASS" width="100"
			dataset="asisstanceClassDataset1" align="center">
			<next:TextField />
		</next:Column>
		<next:Column header="分类施保金（元）" field="ASSISTANCE_CLASS_MONEY"
			width="100" align="right" renderer="formatNum">
			<next:TextField />
		</next:Column>
		<next:Column header="属别" field="BELONGING" dataset="belongingDataSet"
			width="100" align="center">
			<next:TextField />
		</next:Column>
		<% }else if("03".equals(assistanceType)){ %>
		<next:Column header="供养方式" field="SUPPORT_WAY"
			dataset="supportWayDataset" width="100" align="right">
			<next:TextField />
		</next:Column>
		<% }else {%>
		<next:Column header="救助金计算方式" field="ASSISTANCE_MODE" width="120"
		...
		<next:Column header="家庭住址" field="ADDRESS" width="300" align="center">
			<next:TextField />
		</next:Column>
