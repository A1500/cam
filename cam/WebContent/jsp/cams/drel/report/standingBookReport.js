...
function exportExcel(){
	...
	var filename="城市低保台账统计";
	//var class_path = "org.loushang.next.web.cmd.OutExcelCommand";
...
/*
	...
	if(domicileCode){
		if(domicileCode.substring(2,12) == "0000000000"){//ç370000000000
			domicileCode= domicileCode.substring(0,2);
		} else if (domicileCode.substring(4,12) == "00000000"){//å¸370100000000
			domicileCode= domicileCode.substring(0,4);
		} else if (domicileCode.substring(6,12) == "000000"){//å¿370104000000
			domicileCode= domicileCode.substring(0,6);
		} else if (domicileCode.substring(9,12) == "000"){//ä¹¡370104003000
			domicileCode= domicileCode.substring(0,9);
		}
		ds.setParameter("domicile_Code@like",domicileCode+"%");//370104002001槐荫区
	}else{
		if(organCode.substring(2,12) == "0000000000"){//ç370000000000
			organCode= organCode.substring(0,2);
		} else if (organCode.substring(4,12) == "00000000"){//å¸370100000000
			organCode= organCode.substring(0,4);
		} else if (organCode.substring(6,12) == "000000"){//å¿370104000000
			organCode= organCode.substring(0,6);
		} else if (organCode.substring(9,12) == "000"){//ä¹¡370104003000
			organCode= organCode.substring(0,9);
		}
		ds.setParameter("domicile_Code@like",organCode);
	}
	dsprint.setParameter("ASSISTANCE_TYPE","01");
	dsprint.load();
	noReclick(dsprint,"exportExcel");
	
	dsprint.on("load",function(ds){
		L5.dataset2excel(ds,"/jsp/cams/drel/report/standingBookExcel.jsp?domicileName="+document.getElementById("domicileName").value);
	});
		*/
}
function query(){
	...
	ds.setParameter("assistanceType","01");
	ds.load();
}
