...
<title>选择敬老院</title>
<script type="text/javascript" src="geroTree.js"></script>
</head>
<body>
<model:datasets>
	<model:dataset id="dicCity_detaildataset" global="true">
		...
		<next:Panel title="敬老院"  anchor="100% 100%" border="false" height="100%" width="100%" autoScroll="false" >
			<next:TopBar>
			...
			<next:Tree name="dicCitytree" id="dicCitytree" collapsible="true" rootExpanded="true" autoScroll="true" height="100%" lines="true">
				<next:TreeDataSet dataset="dicCityDataSet" root="rootdata">
					...
					<next:TreeLoader cmd="com.inspur.cams.drel.samu.cmd.SamGerocomiumInfoQueryCmd" method="queryGero" trigger="getOrganManage">
						<next:treeRecord name="geroTreeRecord" idField="RECORD_ID">
							<model:field name="RECORD_ID" type="string"/>
		   					<model:field name="GEROCOMIUM_ID" type="string"/>	
		   					<model:field name="GEROCOMIUM_NAME" type="string"/>	
						</next:treeRecord>
						<next:TreeBaseparam name="areaLevelId" value="getParentOrgan"/>
					</next:TreeLoader>
				</next:TreeDataSet>
				...
				<next:TreeNodemodel recordType="geroTreeRecord">
					<next:TreeNodeAttribute name="text" mapping="GEROCOMIUM_NAME"></next:TreeNodeAttribute>
	   				<next:TreeNodeAttribute name="showType" handler="getChoiceBox"></next:TreeNodeAttribute>
				</next:TreeNodemodel>
			</next:Tree>
		</next:Panel>
