function init() {
	...
	pocketApplyDetailDs.setParameter("clothDate",clothDate);
...
function conf(){
	var pocketApplyDetailDs=L5.DatasetMgr.lookup("pocketApplyDetailDs");
		...
		command.execute("saveCheckCloth");
		if (!command.error){
...
function saveInsert(){
	...
	if(records.length==0){
		...
		for(var i=0;i<records.length;i++){
			...
			record.set("clothDate",clothDate);
			record.set("clothStatus","0");
			record.set("clothType",clothType);
			record.set("clothNum",clothNum);
