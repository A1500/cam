...
<html>
	<head>
		<title>服装发放</title>
		<next:ScriptManager/>
		<script type="text/javascript" src="clothApplyList.js"></script>
...
<model:datasets>
     <!-- 敬老院业务主表 -->
	<model:dataset id="geroApplyDs" cmd="com.inspur.cams.drel.samu.cmd.SamGeroApplyQueryCmd" method="queryClothList" global="true" pageSize="15">
				...
				<div>
					<form style="width: 100%; height: 100%;" class="L5form" id="queryForm">
						<table  border="1" width="100%" >
							<tr >
							    <td class="FieldLabel" style="width:12%">敬老院名称</td>
								<td class="FieldInput" style="width:15%"><input type="text" id="gerocomiumNameQuery" style="width:80%"/><img
									src="<%=SkinUtils.getImage(request, "l5/help.gif")%>"
									style="cursor: hand" onclick="func_ForGeroSelect()" /> <input
									type="text" id="gerocomiumIdQuery" name="gerocomiumIdQuery" 
									style="display: none" /></td>
								<td class="FieldLabel" style="width: 12%">服装类型:</td>
								<td class="FieldInput" style="width: 15%"><select type="text"  id="clothTypeQuery"  style="width:85%" readonly="readonly"  />
								<option dataset="clothTypeDS"></option>
								</select>
								</td>	
								<td class="FieldLabel" style="width: 12%">发放日期:</td>
								<td class="FieldInput" style="width: 15%"><input type="text"  id="clothDateQuery" format="Y-m-d"
								onclick="LoushangDate(this)"  style="width:85%" readonly="readonly"  />
								
								</td>	
								<td class="FieldButton" style="width: 15%" ><button onclick="query()" id="queryBtn">查 询</button>&nbsp;&nbsp;<button onclick="reset()">重 置</button> </td>
							</tr>
						</table>
					</form>
				</div>
...
<next:GridPanel id="editGridPanel" name="samreleasesummaryGrid" title="服装发放名册列表" width="100%" stripeRows="true"  
	...
	<next:Columns>
		...
		<next:Column  header="发放时间" field="CLOTH_DATE" width="10%" align="center" renderer="detailRender">
			<next:TextField allowBlank="false" />
		</next:Column>
		<next:Column id="applyPeopleNum" header="发放总人数" field="TOTAL_NUM" width="100" align="right">
            <next:NumberField  />
        </next:Column>
		<next:Column  header="发放总套数" field="APPLY_CLOTHNUM" width="10%"    align="right">
			<next:NumberField allowBlank="false" />
		</next:Column>
		<next:Column id="applyStatus" header="发放确认状态" field="CLOTH_STATUS" width="10%" dataset="clothStatusDS"  align="center">
			<next:TextField allowBlank="false" />
		</next:Column>
		<next:Column id="applyType" header="发放服装类型" field="CLOTH_TYPE" width="10%" dataset="clothTypeDS"  align="center">
			<next:TextField allowBlank="false" />
		</next:Column>
		</next:Columns>
...
<next:Window id="applyWin" title="登记名册" closeAction="hide"  width="540" modal="true">
    ...
    <next:Html>
        <form   onsubmit="return false"  class="L5form">
            <table width='100%'>
                <tr>
            <td class="FieldLabel" id="valueLabel" style="width:15%;">敬老院</td>
                    <td class="FieldInput" style="width:18%"><select
					id="gerocomiumName" name="敬老院" title="敬老院" onchange="testCheck()">
					<option dataset="samGeroDataSet"></option>
					</select></td>
            
                    <td class="FieldLabel" style="width:15%">发放日期</td>
                    <td class="FieldInput"style="width:18%" >
                        <input type="text" name="发放日期"  id="clothDate" style="width:80%" format="Y-m-d"
                         onclick="LoushangDate(this)" readonly/>
                        <font color="red">*</font>
                    </td>
                </tr>
                <tr>
                    <td class="FieldLabel" style="width:15%">服装类型</td>
                    <td class="FieldInput"style="width:18%">
                        <select id="clothType" name="服装类型">
                            <option dataset="clothTypeDS"/>
                        </select>
                    </td>
                    <td class="FieldLabel" style="width:15%;">发放套数（套）</td>
                    <td class="FieldInput" style="width:18%"  >
                        <input type="text" name="发放套数"  id="clothNum" style="width:80%" />
                        <font color="red">*</font>
                    </td>
                </tr>
