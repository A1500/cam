...
<next:Panel  name="form" width="98%" border="0" bodyStyle="padding-bottom:10px;padding-top:12px;" autoHeight="true" >
	<next:Html>
		<fieldset style="overflow: visible;" class="GroupBox"><legend
				...
				<div>
					<form name="frm" style="width: 95%; height: 100%;" class="L5form" action="../../../../../ReportServer" target="Rpt_CountryMedicalAssistanceQuery">
...
<iframe id="Rpt_CountryMedicalAssistanceQuery" name="Rpt_CountryMedicalAssistanceQuery" width="100%" height="100%">
...
<script type="text/javascript">
  var struId = '<%=BspUtil.getStruId()%>';
	...
	function query() {
		...
		command.setParameter("RPT_CountryMEDICALASSISTANCE.REPORTCORP@=", getParam("reportCorpOrganName"));
   		command.afterExecute = function(){
   			var msg = command.getReturn("message", false);
	   		if (msg==null || msg=="") {//
	   			if(provinceFlag == "000" && reportOrganCode!='371000') {//省级用户
					document.forms[0].reportlet.value = "/Rpt_CountryMedicalAssistanceProvinceUpdate.cpt";
					document.forms[0].id.value = command.getReturn("id", false);
				}else if(cityFlag == "00") {//市级用户修改
					document.forms[0].reportlet.value = "/Rpt_CountryMedicalAssistanceCityUpdate.cpt";
					document.forms[0].id.value = command.getReturn("id", false);
				}else {//其他
		...
		command.execute("queryCountrymedicalassistanceForupdate");
	}
	//选择单位
	function selectDept(){
