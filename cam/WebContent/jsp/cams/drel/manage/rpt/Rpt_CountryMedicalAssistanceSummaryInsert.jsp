...
<script type="text/javascript">
	...
	if(provinceFlag == "000" && reportOrganCode!='371000') {//省级汇总 汇总市级已上报的数据
		document.getElementById("Rpt_CountryMedicalAssistanceReport").src = "../../../../../ReportServer?reportlet=Rpt_CountryMedicalAssistanceProvinceSummary.cpt&op=write&reportCorpOrganId=<%=reportCorpOrganId %>&reporterOrganId=<%=reporterOrganId %>&reportTime=<%=reportTime %>&reportOrganCode=<%=reportOrganCode%>&reportyear=<%=request.getParameter("year")%>&reportquarter=<%=request.getParameter("quarter")%>&reported=1&id=<%=request.getParameter("id")%>";
	}else if(cityFlag == "00") {//市级汇总 汇总区县级已上报的数据
		document.getElementById("Rpt_CountryMedicalAssistanceReport").src = "../../../../../ReportServer?reportlet=Rpt_CountryMedicalAssistanceCitySummary.cpt&op=write&reportCorpOrganId=<%=reportCorpOrganId %>&reporterOrganId=<%=reporterOrganId %>&reportTime=<%=reportTime %>&reportOrganCode=<%=reportOrganCode%>&reportyear=<%=request.getParameter("year")%>&reportquarter=<%=request.getParameter("quarter")%>&reported=0&id=<%=request.getParameter("id")%>";
	}else {//其他
