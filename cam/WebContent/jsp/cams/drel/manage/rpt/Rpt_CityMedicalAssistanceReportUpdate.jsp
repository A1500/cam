...
<%
	...
	String Flag = reportOrganCode.substring(reportOrganCode.length()-3);//省标志
...
<script type="text/javascript">
	...
	var Flag = '<%=Flag %>';
	if(cityFlag == "00") {//市级用户
		//document.getElementById("Rpt_CityMedicalAssistanceReport").src = "../../../../../ReportServer?reportlet=Rpt_CityMedicalAssistanceReport2Update.cpt&op=write&reportCorpOrganId=<%=reportCorpOrganId %>&reporterOrganId=<%=reporterOrganId %>&reportTime=<%=reportTime %>&reportOrganCode=<%=reportOrganCode%>&year=<%=request.getParameter("year")%>&quarter=<%=request.getParameter("quarter")%>&reported=0&id=<%=request.getParameter("id")%>";
		L5.Msg.alert("提示", "市级用户没有填报业务！",function(){window.history.go(-1);});
	}else {//区县级用户
		document.getElementById("Rpt_CityMedicalAssistanceReport").src = "../../../../../ReportServer?reportlet=Rpt_CityMedicalAssistanceReportUpdate.cpt&op=write&id=<%=request.getParameter("id")%>";
	}
	//设置默认值
	function setDefault() {
		...
		if(state=="yes"){
			...
			//必须使用异步方式，然后转向必须在afterExecute回调函数中。
	   		...
	   		if (msg==null || msg=="") {
				...
				if(msg == "上报成功！") {
					var url = "jsp/cams/drel/manage/rpt/Rpt_CityMedicalAssistanceReport.jsp";
