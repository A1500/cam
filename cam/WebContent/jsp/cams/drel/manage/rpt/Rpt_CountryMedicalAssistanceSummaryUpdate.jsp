...
<script type="text/javascript">
	...
	if(provinceFlag == "000" && reportOrganCode!='371000') {//省级汇总 汇总市级已上报的数据
		document.getElementById("Rpt_CountryMedicalAssistanceReport").src = "../../../../../ReportServer?reportlet=Rpt_CountryMedicalAssistanceProvinceSummaryUpdate.cpt&op=write&reportCorpOrganId=<%=reportCorpOrganId %>&reporterOrganId=<%=reporterOrganId %>&reportTime=<%=reportTime %>&reportOrganCode=<%=reportOrganCode%>&reportyear=<%=request.getParameter("year")%>&reportquarter=<%=request.getParameter("quarter")%>&reported=1&id=<%=request.getParameter("id")%>";
	}else if(cityFlag == "00") {//市级用户
		document.getElementById("Rpt_CountryMedicalAssistanceReport").src = "../../../../../ReportServer?reportlet=Rpt_CountryMedicalAssistanceCitySummaryUpdate.cpt&op=write&reportCorpOrganId=<%=reportCorpOrganId %>&reporterOrganId=<%=reporterOrganId %>&reportTime=<%=reportTime %>&reportOrganCode=<%=reportOrganCode%>&reportyear=<%=request.getParameter("year")%>&reportquarter=<%=request.getParameter("quarter")%>&reported=0&id=<%=request.getParameter("id")%>";
	}else {//区县级用户无汇总业务
		L5.Msg.alert("提示", "区县级用户无汇总业务！",function(){window.history.go(-1);});
	}
	//设置默认值
	function setDefault() {
		...
		if(state=="yes"){
			...
			command.setParameter("RPT_CountryMEDICALASSISTANCE.REPORTCORP@=", getParam("reportcorp"));
	   		...
	   		if (msg==null || msg=="") {
				...
				if(msg == "上报成功！") {
					var url = "jsp/cams/drel/manage/rpt/Rpt_CountryMedicalAssistanceSummary.jsp";
	       			var text = "农村医疗救助工作情况填报";
					L5.Msg.alert("提示", msg,function(){L5.forward(url, text);});
	        		
				}else if(msg == "已经上报的数据不能再进行上报！") {
					L5.Msg.alert("提示", "已经上报的数据不能再进行上报！");
				}
			}
			}
