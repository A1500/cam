...
function init(){
	...
	applyDataSet.on("load",function(){
		if(applyDataSet.getCount() == 1){
			...
			cardNo=applyDataSet.get('cardNo');
	...
	familyDataSet.on('load',function(){
		if(familyDataSet.getCount() == 1){
			...
			familyDataSet.set('cardNo',cardNo);
...
function loadAsisstanceClass(){
    ...
    asisstanceClassDataset.setParameter("filterSql", "ASSISTANCE_TYPE='02' AND IN_USE='1' AND AREA_CODE= '" + organArea.substring(0, 6) + "000000'");
...
function peopleDetail(peopleId){
	var width = 1000;
    var height = 600;
	var url="samRuralResidentsPeopleDetail.jsp?peopleId="+peopleId+'&applyId='+applyId;;
...
function detailSupport(supportId){
	var width = 850;
    var height = 187;
	var url="supportDetail.jsp?familyId="+familyId+"&supportId="+supportId+"&assistanceType=02";
