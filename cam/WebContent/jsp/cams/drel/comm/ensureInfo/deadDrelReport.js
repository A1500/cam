...
function initMarryYearChart(){
	var chart;
	 chart = new Highcharts.Chart({
         ...
         tooltip: {
             formatter: function() {
                 return '已死亡'+ this.x +'低保五保对象共: '+ Highcharts.numberFormat(this.y, 0) +
                     ' 人';
             }
         },
         plotOptions: { 
...
*/	
	ops = {
			...
			loader : dcLoader,
			dim : organCode,
			url : '/command/ajax/com.inspur.cams.drel.report.cmd.SamBaseCaseReportQueryCmd/queryDeadDrelObjectByOrgan',
			remoteSort : false,
			//idField : 'ORGAN_CODE',
			//frozenColumns : [[]],	
			columns : [[{
						...
						formatter:function (value,record){
							return renshu(value,record.ORGAN_CODE,"''");
						}
					}, {
						field : 'CITY_NUM',
						title : '城市低保',
						...
						align : 'right',
						formatter:function (value,record){
							return renshu(value,record.ORGAN_CODE,"'01'");
						}
					}, {
						field : 'COUNTRY_NUM',
						title : '农村低保',
						...
						align : 'right',
						formatter:function (value,record){
							return renshu(value,record.ORGAN_CODE,"'02'");
						}
					}, {
						field : 'FIVE_NUM',
						title : '农村五保',
						...
						align : 'right',
						formatter:function (value,record){
							return renshu(value,record.ORGAN_CODE,"'03'");
						}
					}]],
			pagination : false,
			rownumbers : true,
...
function bptObjectDetailOrgan(organCode,objectType){
	var url=L5.webPath+'/jsp/cams/drel/report/deadQuery.jsp?organCode='+organCode+'&objectType='+objectType+'&queryType=organCode';
...
function bptObjectDetailMonth(time){
	var url=L5.webPath+'/jsp/cams/drel/report/deadQuery.jsp?time='+time+'&queryType=month&organCode='+organCode;
	var width = screen.width-60;
	var height = 580;
	var win = window.showModalDialog (url,window,"scroll:yes;status:no;dialogWidth:"+width+"px;dialogHeight:"+height+"px;resizable:0");
	if (win==null) {
