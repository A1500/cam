...
function selectedNode(record){
	...
	if(record.recordType=="organRecord"){
    	var url="comExtOrganEdit.jsp?method=update&organId="+record.get("organId");
		frame.setAttribute("src", url);
	}else if(record.recordType=="userRecord"){
		var url="comExtUserEdit.jsp?method=update&userId="+record.get("userId");
		frame.setAttribute("src", url);
	}else{
		return;
	}
...
function addUser(menuitem){
	var record=menuitem.parentMenu.record;
	var frame = document.getElementById("nodeFrame");
	var url="comExtUserEdit.jsp?method=insert&organId="+record.get("organId")
