...
<html>
<head>
<title>外部单位用户管理</title>
<script type="text/javascript" src="comExtOrganTree.js"></script>
</head>
<body>
<next:ViewPort>
<next:BorderLayout>
	<next:Left split="true" cmargins="5 0 0 0" margins="5 0 0 0">
		<next:Panel title="外部用户管理" collapsible="true" autoScroll="true" width="20%" anchor="100% 100%">
			<next:Html>
				<next:Tree id="usertree" width="100%" border="false" lines="true">
					<next:TreeDataSet dataset="stru" root="rootdata">
						...
						<next:TreeLoader cmd="com.inspur.cams.comm.extuser.cmd.ComExtOrganQueryCmd" method="queryOrgan" trigger="organTrigger">
							<next:treeRecord name="organRecord" idField="organId" fromBean="com.inspur.cams.comm.extuser.data.ComExtOrgan"/>
							<next:TreeBaseparam name="organId" value="getOrganId"></next:TreeBaseparam>
							<next:TreeBaseparam name="organName" value="getOrganName"></next:TreeBaseparam>
							<next:TreeBaseparam name="organType" value="getOrganType"></next:TreeBaseparam>
							<next:TreeBaseparam name="id" value="getId"></next:TreeBaseparam>
						</next:TreeLoader>
					...
					<next:TreeNodemodel recordType="organRecord">
						...
						<next:RightmouseMenu menuId="organMenu">
							<next:MenuItem text="增加用户" handler="addUser"></next:MenuItem>
