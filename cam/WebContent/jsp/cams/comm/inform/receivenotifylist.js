...
function click_view() {
	...
	window.location = "viewinform.jsp?queryid=" + queryid + "&page=receivepage";
}
...
function receiveRenderer(value, cellmeta, record, rowindex,
	...
	for (var i = 0; i < organDS.getCount(); i++) {
		if (value.indexOf(organDS.getAt(i).get("value")) != -1) {
			str += organDS.getAt(i).get("text") + ",";
		}
	}
