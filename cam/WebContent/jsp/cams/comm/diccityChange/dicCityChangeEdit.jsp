...
<model:datasets>
	<model:dataset id="dicCityChangeDataSet" cmd="com.inspur.cams.comm.diccityChange.cmd.DicCityChangeQueryCmd" pageSize="200">
		<model:record fromBean="com.inspur.cams.comm.diccityChange.data.DicCityChange">
			...
			<model:field name="preUpsCode" type="string" rule="require" />
			<model:field name="preUpsName" type="string" rule="require" />
			<model:field name="postChangeName" type="string" rule="require" />
			<model:field name="postChangeCode" type="string" rule="require" />
			<model:field name="postUpsCode" type="string" rule="require" />
			<model:field name="postUpsName" type="string" rule="require" />
			<model:field name="changeType" type="string" rule="require" />
			<model:field name="serialNum" type="string" rule="require" />
			<model:field name="remark" type="string" rule="require" />
		</model:record>
	</model:dataset>
	<!-- 变更类型 -->
	<model:dataset id="changeTypeDataset" enumName="COMM.CHANGE.UPDATE.TYPE" autoLoad="true" global="true"></model:dataset>
	<!-- 代码级别 (区县)-->
...
<next:BorderLayout>
	<next:Left split="true" cmargins="5 0 0 0" margins="5 0 0 0">
		<next:Panel title="现有区划代码列表" collapsible="true" autoScroll="true" width="20%" anchor="100% 100%">
			<next:Html>
	...
	<next:Center floatable="true">
		...
		<fieldset>
			<%if(organCode.endsWith("00000000")){ %>
				<legend>县及以上行政区划变更</legend>
			<%}else{ %>
				<legend>县以下行政区划变更</legend>
			<%} %>
			<table width="100%">
				<tr>
					<td class="FieldLabel" style="width:18%">变更类型：</td>
					<td class="FieldInput" style="width:36%" colspan="2">
					<select id="changeType" field="changeType"  name="变更类型" style="width:90%">
		                 <option dataset="changeTypeDataset"></option>
		            </select></td>
				</tr>
				<tr >
				...
				<tr >
					...
					<td class="FieldInput" style="width:36%"><input type="text" name="变更前上级区划名称" id="preUpsName" field="preUpsName" maxlength="100" style="width:90%" readonly/></td>
				</tr>
				<tr id="dmjb5">
				<td class="FieldLabel" rowspan="4" align="center" style="width:18%">变更后:</td>
					<td class="FieldLabel" style="width:10%">上级区划代码：</td>
					<td class="FieldInput" style="width:36%"><input type="text" name="变更后上级区划代码" id="postUpsCode" field="postUpsCode" maxlength="12" style="width:90%"/>
					<img id="img" src="<%=SkinUtils.getImage(request,"l5/help.gif")%>"  style="cursor:hand" onclick="func_ForDomicileSelect1()" /></td>
				</tr>
				<tr >
					<td class="FieldLabel" style="width:10%">上级区划名称：</td>
					<td class="FieldInput" style="width:36%"><input type="text" name="变更后上级区划名称" id="postUpsName" field="postUpsName" maxlength="100" style="width:90%"/></td>
				</tr>
				<tr id="dmjb3">
					<td class="FieldLabel" style="width:10%">区划名称：</td>
					<td class="FieldInput" style="width:36%"><input type="text" name="变更后区划名称" id="postChangeName" field="postChangeName" maxlength="50" style="width:90%"/></td>
				</tr>
				<tr id="dmjb4">
					<td class="FieldLabel" style="width:10%">区划代码：</td>
					<td class="FieldInput" style="width:36%"><input type="text" name="变更后区划代码" id="postChangeCode" field="postChangeCode" maxlength="12" onblur="changeSelect(this.value)" style="width:90%"/></td>
				</tr>
				
				<tr id="dmjb1"> 
					<td class="FieldLabel" style="width:18%">类别：</td>
					<td class="FieldInput" style="width:36%" colspan="2">
					<select id="codeLeve" name="代码类别" style="width:90%">
				...
				<tr id="dmjb2">
					<td class="FieldLabel" style="width:18%">类别：</td>
					<td class="FieldInput" style="width:36%" colspan="2">
					<select id="codeLeve2" name="代码类别" style="width:90%">
				...
				<tr id="dmjb7">
					<td class="FieldLabel" style="width:18%">类别：</td>
					<td class="FieldInput" style="width:36%" colspan="2">
					<select id="codeLeve3" name="代码类别" style="width:90%">
		                 <option dataset="codeLeveDatasetCity"></option>
		            </select></td>
				</tr>
				<tr>
					<td class="FieldLabel" style="width:18%">序号：</td>
					<td class="FieldInput" style="width:36%" colspan="2"><input type="text" onblur="_isNum(this.value)" name="序号" id="serialNum" field="serialNum" maxlength="4" /><font color="red">序号为1~9999(序号大小决定调整执行的先后顺序!!!)</font></td>
				</tr>
				<tr id="dmjb6">
					<td class="FieldLabel" style="width:18%">变更说明：</td>
					<td class="FieldInput" style="width:36%" colspan="2"><textarea  onpropertychange="if(value.length>100) value=value.substr(0,100)" name="变更说明" id="remark" field="remark"  style="width:90%"></textarea></td>
				</tr>
				<%if(organCode.endsWith("00000000")){ %>
					<tr>
