...
bean[name]=data[name];}
bean.state=this.state;return bean;},toJsonString:function(){var obj=new Object();obj.javaClass="Record";obj.id=this.id;obj.state=this.state;obj.data=this.data;return L5.encode(obj);},set:function(name,value,srcElement){var result=true;var field=this.fields.get(name);if(field==null){L5.Msg.alert("提示",name+" is undefine in model");return;}
...
if(ifall){for(var i=0;i<this.fields.items.length;i++){var v=this.msgIndex[i]?this.msgIndex[i]:lastItems.pop();if(this.validate[v]===false){if(this.msgLable[v]==null){this.msgLable[v]=v;}
if(ifdes){if(this.cellLable&&this.cellLable[v])
allmsgs+=this.cellLable[v]+this.msg[v]+"!\n";else{allmsgs+=this.msgLable[v]+this.msg[v]+"!\n";}}
else
...
if(ifdes){if(this.cellLable&&this.cellLable[v]){return this.cellLable[v]+this.msg[v]+"!";}
else{return this.msgLable[v]+this.msg[v]+"!";}}
else
...
e=e.statusText;L5.Msg.alert(e.title?e.title:(L5.loadDateError?L5.loadDateError:"load date error:"),e);});}},unregister:function(){for(var i=0,s;s=arguments[i];i++){this.remove(this.lookup(s));}},lookup:function(id){var dataset=typeof id=="object"?id:this.get(id);if(dataset==null){alert(L5.notfind+id);}
return dataset;},getKey:function(o){return o.ds||o.id;}});

L5.model.Dataset=function(config){this.data=new L5.util.MixedCollection(false);this.data.getKey=function(o){return o.id;};this.pos=0;this.pageInfo={"pageSize":-1,"pageIndex":1,"startCursor":0};if(config&&config.pageSize&&config.pageSize>0){this.pageInfo.pageSize=config.pageSize;delete config.pageSize;}
this.method;this.baseParams={};this.loadParams={};this.paramNames={"start":"start","limit":"limit","defaultSort":"defaultSort","sort":"sort","dir":"dir"};this.defaultSortInfo=new L5.List();if(config.reader){var meta=config.reader.meta;if(meta){var fields=meta.fields;if(fields&&fields.length>0){for(var i=0;i<fields.length;i++){var sortDir=fields[i].sortDir;if(sortDir){var sortObj=new L5.Map();sortObj.put("field",fields[i].name);sortObj.put("dir",sortDir);this.defaultSortInfo.add(sortObj);}}}}}
...
this.proxy.load(p,this.reader,this.loadRecords,this,options);return true;}else{return false;}},reload:function(options){this.loadPage(L5.applyIf(options||{},this.lastOptions));},getCustomData:function(key){if(this.customDatas){var obj=this.customDatas[key];if(obj.javaClass=="DataSet")
return obj;else
...
this.lastOptions={};var r=this.reader.readRecords(o);this.loadRecords(r,{add:append},true);},getCount:function(){return this.data.length||0;},getTotalCount:function(){return this.totalLength||0;},turnPage:function(pageIndex){var pageTotal=this.getTotalPage();if(pageIndex<1){L5.Msg.alert("提示",L5.turnPageFirst?L5.turnPageFirst:"page num too small");return false;}
if(pageIndex>pageTotal){L5.Msg.alert("提示",L5.turnPageLast?L5.turnPageLast:"page num too large");return false;}
...
this.proxy.load(p,this.reader,this.loadRecords,this,options);return true;}else{return false;}},getCount:function(){return Math.min(this.getTotalCount()-this.pageInfo.startCursor,this.pageInfo.pageSize);},getTotalCount:function(){return this.data.length;},turnPage:function(pageIndex){if(pageIndex<1){L5.Msg.alert("提示",L5.turnPageFirst?L5.turnPageFirst:"page num too small");return false;}
if(pageIndex>this.getTotalPage()){L5.Msg.alert("提示",L5.turnPageLast?L5.turnPageLast:"page num too large");return false;}
...
if(v.javaClass&&v.time){var o=new Date(v.time);if(dateFormat)o.formatstr=dateFormat;return o}
...
var valueTipList=L5.DomQuery.select("*[@valueTip]",root);for(var i=0;i<valueTipList.length;i++){var ele=valueTipList[i];var valtip=ele.getAttribute("valueTip");if(valtip==true||valtip=="true"||valtip=="TRUE"){ele.setAttribute("title",L5.databind.getShowTextOfElement(ele));L5.get(ele).on("change",function(){ele.setAttribute("title",L5.databind.getShowTextOfElement(ele));});}}}
...
if(record==null){L5.databind.setValueOfElement(ele,"");}else{if(field){if(property==field){L5.databind.setValueOfElement(ele,record.get(property));L5.Validator.validate(record,property,record.get(property));L5.databind.setValidateInfoOnElement(ele,record,true);}}else{var htmlValue=/\<|\>/;var dataValue=record.get(property);if(htmlValue.test(dataValue)){dataValue=L5.util.Format.htmlEncode(dataValue);}
...
if(nored){extEle.setStyle("border-color","");}else{extEle.setStyle("border-color","#FA8072");ele.setAttribute("L5:qtitle","校验错误");ele.setAttribute("L5:qtip",msg);}}else{if(ele.nodeName.toLowerCase()=="input"){if(ele.type=="radio"||ele.type=="checkbox"){ele=ele.parentNode;extEle=L5.get(ele);}}
extEle.setStyle("border-color","");ele.removeAttribute("L5:qtitle");ele.removeAttribute("L5:qtip");}}
L5.databind.dealWidthRender=function(ele,val,render){var eleCopy=ele.cloneNode(true);ele.parentNode.insertBefore(eleCopy,ele);ele.style.display="none";eleCopy.removeAttribute("field");eleCopy.removeAttribute("renderer");ele.setAttribute("rendererDealed",true);eleCopy.id=ele.id+"_copy";L5.get(eleCopy).on("focus",function(){eleCopy.style.display="none";ele.style.display="";L5.get(ele).focus();});L5.get(ele).on("blur",function(){ele.style.display="none";eleCopy.style.display="";L5.databind.dealWidthRender.changeCopyEl(eleCopy,ele,render);});L5.get(ele).on("change",L5.databind.dealWidthRender.changeCopyEl.createCallback(eleCopy,ele,render));}
...
for(var i=0;i<formList.length;i++){var datasetId=formList[i].getAttribute("dataset");var fieldList=L5.DomQuery.select("*[@field]",formList[i]);for(var j=0;j<fieldList.length;j++){var property=fieldList[j].getAttribute("field");if(recObj[datasetId]&&recObj[datasetId].validate[property]==false){L5.get(fieldList[j]).setElFalseStyle("校验错误",recObj[datasetId].msgLable[property]
...
for(var i=0;i<formList.length;i++){var datasetId=formList[i].getAttribute("dataset");var fieldList=L5.DomQuery.select("*[@field]",formList[i]);for(var j=0;j<fieldList.length;j++){if(!L5.get(fieldList[j]).isVisible()||fieldList[j].getAttribute("type")=="hidden"){continue;}
var property=fieldList[j].getAttribute("field");if(recObj[datasetId]&&recObj[datasetId].validate[property]==false&&recObj[datasetId+"~"+property]!=true){msg+=recObj[datasetId].msgLable[property]
...
var onchange1=ele.getAttribute("onchange1");eval(onchange1);}
...
+"(:[0-9]{1,4})?"
+"((/?)|"
+"(/[0-9a-z_!~*'().;?:@&=+$,%#-]+)+/?)$"),ip:/^(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5]).(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5]).(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5]).(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5])$/,currency:/^(-)?\d+(\.\d+)?$/,number:/^\d+$/,zip:/^[1-9]\d{5}$/,qq:/^[1-9]\d{4,9}$/,english:/^\w+$/,chinese:/^[\u0391-\uFFE5]+$/,username:/^[a-z]\w{3,19}$/i,integer:/^[-\+]?\d+$/,'double':/^[-\+]?\d+(\.\d+)?$/};L5.isIdCard={vcity:{11:"北京",12:"天津",13:"河北",14:"山西",15:"内蒙古",21:"辽宁",22:"吉林",23:"黑龙江",31:"上海",32:"江苏",33:"浙江",34:"安徽",35:"福建",36:"江西",37:"山东",41:"河南",42:"湖北",43:"湖南",44:"广东",45:"广西",46:"海南",50:"重庆",51:"四川",52:"贵州",53:"云南",54:"西藏",61:"陕西",62:"甘肃",63:"青海",64:"宁夏",65:"新疆",71:"台湾",81:"香港",82:"澳门",91:"国外"},checkIdCard:function(card){if(card===''){return"身份证号码不能为空!";}
if(this.isCardNo(card)===false){return"身份证号码长度或类型错误!";}
if(this.checkProvince(card)===false){return"身份证号码错误!";}
if(this.checkBirthday(card)===false){return"身份证号码错误!";}
if(this.checkParity(card)===false){return"身份证号码错误!";}
...
o[f1[i]]=f2[i];if(!o.Y)
o.Y=o.y;o.yyyy=s(o.yyyy,o.Y,d.getFullYear(),9999,4);o.MM=s(o.MM,o.m,d.getMonth()+1,12);o.dd=s(o.dd,o.d,d.getDate(),31);o.hh=s(o.hh,o.h,d.getHours(),24);o.mm=s(o.mm,o.m,d.getMinutes());o.ss=s(o.ss,o.s,d.getSeconds());o.ms=s(o.ms,o.ms,d.getMilliseconds(),999,3);if(o.yyyy+o.MM+o.dd+o.hh+o.mm+o.ss+o.ms<0)
...
reVal=s2*1;}
return(reVal==s1&&s1.length!=s5||reVal>s4)?-10000:reVal;}};L5.Validator={validate:function(record,name,value){var isValidate=true;var result=new Array();for(var i=0;i<record.fields.items.length;i++){if(record.fields.items[i].name===name&&record.fields.items[i].rule!=null){var rules=record.fields.items[i].rule.split("|");for(var j=0;j<rules.length;j++){var rule=rules[j];var ruleName=rule.substring(0,rule.indexOf("{"));var ruleParm=rule.substring(rule.indexOf("{")+1,rule.lastIndexOf("}"));if(rule.indexOf("{")<0){ruleName=rule;ruleParm="";}
...
return result;},empty:function(){return true;},require:function(value,record){if(value===null||value===""){return"不能为空";}else{if(typeof value=='string'&&value.trim()==="")
return"不能为空";return true;}},date:function(value,record,dateFormat){if(value===null||value==="")
return true;var isValidate=(value===undefined)?false:L5.isDateTime.call(value,dateFormat);if(isValidate==false){var msg="不是正确的日期格式";if(dateFormat!==null)
...
return true;var isValidate=pattern.test(value);if(isValidate==false){var msg="格式不正确";return msg;}else{return true;}},equal:function(value,record,target){if(value===null||value==="")
return true;var targetVal=record.get(target)||target;if(value==targetVal){return true;}else{var msg;if(record.get(target)!==null){msg="不等于"+target+"域的值";}else{msg="不等于"+target;}
return msg;}},compare:function(value,record,ruleParm){if(value===null||value==="")
...
break;default:}
if(result==false){var msg="必须";if(minVal!==null){msg=msg+"大于"+minVal;}
if(maxVal!==null){msg=msg+"小于"+maxVal;}
...
continue;if(val==record.dataset.getAt(j).get(unions[i])){return"违反唯一性约束";}}}
return true;},length:function(value,record,len){if(value===null||value==="")
return true;var len=len*1;var strVal=value.toString()+"";var curLen=strVal.length;var chiArr=strVal.match(/[\u4E00-\u9FA5\uF900-\uFA2D]|[\uFF00-\uFFFF]/g);if(chiArr!=null)curLen+=chiArr.length;if(curLen<=len){return true;}else{if(chiArr!=null){return("英文(中文)长度不能超过"+len+"("+Math.floor(len/2)+")个字符");}else{return("长度不能超过"+len+"个字符");}}},lengthUTF8:function(value,record,len){if(value===null||value==="")
return true;var len=len*1;var strVal=value.toString()+"";var curLen=strVal.length;var chiArr=strVal.match(/[\u4E00-\u9FA5\uF900-\uFA2D]|[\uFF00-\uFFFF]/g);if(chiArr!=null)curLen+=(chiArr.length*2);if(curLen<=len){return true;}else{if(chiArr!=null){return("英文(中文)长度不能超过"+len+"("+Math.floor(len/3)+")个字符");}else{return("长度不能超过"+len+"个字符");}}},lengthGBK:function(value,record,len){if(value===null||value==="")
return true;var len=len*1;var strVal=value.toString()+"";var curLen=strVal.length;var chiArr=strVal.match(/[\u4E00-\u9FA5\uF900-\uFA2D]|[\uFF00-\uFFFF]/g);if(chiArr!=null)curLen+=chiArr.length;if(curLen<=len){return true;}else{if(chiArr!=null){return("英文(中文)长度不能超过"+len+"("+Math.floor(len/2)+")个字符");}else{return("长度不能超过"+len+"个字符");}}},custom:function(value,record,fn){var parms=fn.split(",");var fnName;var args=new Array();args.push(value);args.push(record);if(parms.length>0){fnName=parms[0].substring(parms[0].indexOf(":")+1);for(var i=1;i<parms.length;i++){argname=parms[i].substring(0,parms[i].indexOf(":"));argvalue=parms[i].substring(parms[i].indexOf(":")+1);args.push(argvalue);}
...
L5.HtmlExt={errorSwitch:function(rule){var result=new Array();var rules=rule.split("|");for(var j=0;j<rules.length;j++){var rule=rules[j];var ruleName=rule.substring(0,rule.indexOf("{"));switch(ruleName){case"require":result[j]="不能为空";break;case"date":result[j]="不是正确的日期格式";break;case"equal":result[j]="不相等";break;case"regex":result[j]="格式不正确";break;case"compare":result[j]="不符合校验规则";break;case"length":result[j]="长度不正确";break;case"lengthUTF8":result[j]="长度不正确";break;case"lengthGBK":result[j]="长度不正确";break;case"custom":result[j]="不正确";break;default:result[j]="格式不正确";break;}}
...
return true;var isValidate=pattern.test(value);if(isValidate==false){var msg="格式不正确";return msg;}else{return true;}},customValidator:function(value,field,ruleParm){var parms=ruleParm.split(",");var fnName;var args=new Array();args.push(value);args.push(field);if(parms.length>0){fnName=parms[0].substring(parms[0].indexOf(":")+1);for(var i=1;i<parms.length;i++){argname=parms[i].substring(0,parms[i].indexOf(":"));argvalue=parms[i].substring(parms[i].indexOf(":")+1);args.push(argvalue);}
return(new Function("return "+fnName)())(args);}
...
if(value==val){return true;}else{var msg;if(val!==null){msg="不等于"+ruleParm+"域的值";}else{msg="不等于"+ruleParm;}
...
break;default:}
if(result1==false){var msg="必须";if(minVal!==null){msg=msg+"大于"+minVal;}
if(maxVal!==null){msg=msg+"小于"+maxVal;}
...
{extEle.setStyle("border-color","#FA8072");field.setAttribute("L5:qtitle","校验错误");field.setAttribute("L5:qtip",msg);}else{extEle.setStyle("border-color","");field.removeAttribute("L5:qtitle");field.removeAttribute("L5:qtip");}}}}
