...
data=data.substr(0,data.length-1);return data;};L5.lib.Ajax.setPollingInterval=function(i){if(typeof i=='number'&&isFinite(i)){L5.lib.Ajax.pollInterval=i;}};L5.lib.Ajax.createXhrObject=function(transactionId){var obj,http;try{http=new XMLHttpRequest();obj={conn:http,tId:transactionId};}catch(e){for(var i=0;i<this.activeX.length;++i){try{http=new ActiveXObject(this.activeX[i]);obj={conn:http,tId:transactionId};break;}catch(e){}}}finally{return obj;}};L5.lib.Ajax.getConnectionObject=function(){var o;var tId=this.transactionId;try{o=this.createXhrObject(tId);if(o){this.transactionId++;}}catch(e){}finally{return o;}};L5.lib.Ajax.asyncRequest=function(method,uri,callback,postData,headers){var o=this.getConnectionObject();if(!o){return null;}else{o.conn.open(method,uri,true);this.setHeader(headers,o);this.handleReadyState(o,callback);try{o.conn.send(postData||null);}catch(e){throw new L5.Exception(550,L5.connectionError?L5.connectionError:"connect to server failed");}
return o;}};L5.lib.Ajax.syncRequest=function(method,uri,callback,postData,headers){var o=this.getConnectionObject();if(!o){return null;}else{o.conn.open(method,uri,false);this.setHeader(headers,o);try{o.conn.send(postData||null);}catch(e){throw new L5.Exception(550,L5.connectionError?L5.connectionError:"connect to server failed");}
...
L5.DomHelper=function(){};(function(){var tempTableEl=null;var emptyTags=/^(?:br|frame|hr|img|input|link|meta|range|spacer|wbr|area|param|col)$/i;var tableRe=/^table|tbody|tr|td$/i;var createHtml=function(o){if(typeof o=='string'){return o;}
...
var n=valueCache[path](root);n=n[0]?n[0]:n;var v=(n&&n.firstChild?n.firstChild.nodeValue:null);return((v===null||v===undefined||v==='')?defaultValue:v);};L5.DomQuery.selectNumber=function(path,root,defaultValue){var v=L5.DomQuery.selectValue(path,root,defaultValue||0);return parseFloat(v);};L5.DomQuery.is=function(el,ss){if(typeof el=="string"){el=document.getElementById(el);}
...
a.push("}");return a.join("");}};this.decode=function(json){try{return eval("("+json+')');}catch(e){throw new L5.Exception(550,L5.parseReturnError?L5.parseReturnError:"parse return error");}};})();L5.encode=L5.util.JSON.encode;L5.decode=L5.util.JSON.decode;
...
command.setParameter("template",window.printjsp);command.setParameter("positions",window.billpos);command.execute("savePosition");if(!command.error){alert("保存成功!");}else{alert(command.error);}};L5.util.BillPrint.loadPosition=function(){var command=new L5.Command("org.loushang.next.web.cmd.billprint.BillPrintCmd");command.setParameter("template",window.printjsp);command.execute("loadPosition");window.billpos=command.getReturn("positions");for(name in window.billpos.map){var pos=window.billpos.map[name].split(',');var ele=document.getElementById(name);if(ele){ele.style.top=pos[0];ele.style.left=pos[1];}}};L5.util.BillPrint.afterDrag=function(){var ele=this.getEl();if(!window.billpos){window.billpos=new L5.Map();}
...
str=str.substring(0,str.lastIndexOf("&"));window.location.href=L5.webPath+"/"+url+str;};L5.grid2excel=function(dataset,gridObj,fileName,classNamePath){var frame=document.getElementById("L5_excel_frame");if(!frame){frame=document.createElement('iframe');frame.id="L5_excel_frame";frame.name="L5_excel_frame";frame.className='l-hidden';if(L5.isIE){frame.src=L5.SSL_SECURE_URL;}
...
return info;}else{alert("用户没有登录！");}}
L5.server={executeSysCmd:function(method){var command=new L5.Command("org.loushang.next.common.cmd.SystemInfoCmd");command.execute(method);if(!command.error){return command;}else{alert("获取服务器端信息出错！");return null;}}};L5.server.getSysDate=function(){var command=this.executeSysCmd("getSysDate");if(command!=null)
