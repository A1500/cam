...
function fPopUpCalendarDlg_New(ctrlobj,rowno)
{
	if(N){
	    showx = 220 ; // + deltaX;
	    showy = 220; // + deltaY;
	...
	if(N){
	    ...
	    retval = window.showModalDialog(EcgapForm.webPath+"/skins/ecgapform/calendar/masterref/CalendarDlg.htm", "", "dialogWidth:197px; dialogHeight:230px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
        }
...
function fPopUpCalendarDlg_Time(ctrlobj,rowno)
{
	if(N){
	    showx = 220 ; // + deltaX;
	    showy = 220; // + deltaY;
	...
	if(N){
	    ...
	    retval = window.showModalDialog(EcgapForm.webPath+"/skins/ecgapform/calendar/masterref/CalendarDlgTime.htm", "", "dialogWidth:197px; dialogHeight:235px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
        }
	...
	if( retval != null ){
		if(rowno=="rowIndex"||rowno==null){
          if(document.all.item(ctrlobj).getAttribute("CanWrite")!="0")
		   {
		      if(retval!="")
		          document.all.item(ctrlobj).value = retval+":00";
		      else
		          document.all.item(ctrlobj).value = "";
		    }
		}else{
		   var row = parseInt(rowno); 
		   if(document.all.item(ctrlobj)(row-1).getAttribute("CanWrite")!="0")
		    {
		      if(retval!="")
		           document.all.item(ctrlobj)(row-1).value = retval+":00";
		      else
		           document.all.item(ctrlobj)(row-1).value ="";
		    }
...
function getDateTime(elName,rowno){
	//fPopUpCalendarDlg(document.getElementsByName(elName)[0]);
	fPopUpCalendarDlg_Time(elName,rowno);
}
