 ...
 */
public class DetailExcelImportCmdCL extends BaseAjaxCommand {
	...
	private void processImportJzfamxExcel(String jzfaid, InputStream importExcel) {
		...
		try {
			...
			for(int i=3; i<sheet.getRows(); i++) {
				...
				//受助对象类型
				Cell szdxlxdmCell = sheet.getCell(4, i);
				...
				String cjzh = sheet.getCell(2, i).getContents().trim();//受助对象残疾证号
				jzfamx.setIdCard(sfzh);//设置受助对象身份证号		
				jzfamx.setHandicapNum(cjzh);//设置受助对象残疾证号	
...
//				}
				...
				Cell jzfxCell = sheet.getCell(3, i);//救助方向
				...
				//处理日期单元格
				Cell jzrqCell = sheet.getCell(5, i);
				...
				jzfamx.setGoodsName(sheet.getCell(7, i).getContents());//救助物品名称	
				
				try{
					jzfamx.setCaptial(new BigDecimal(sheet.getCell(6, i).getContents()));//救助资金现金金额
					
					jzfamx.setGoodsNum(new BigDecimal(sheet.getCell(8, i).getContents()));//救助物品数量
					jzfamx.setGoodsNums(new BigDecimal(sheet.getCell(9, i).getContents()));//救助物品折价金额
					jzfamx.setTotalFund(new BigDecimal(sheet.getCell(10, i).getContents()));//救助资金合计金额
				}catch(Exception e) {
					e.printStackTrace();
