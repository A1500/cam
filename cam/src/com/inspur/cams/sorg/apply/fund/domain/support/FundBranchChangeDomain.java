 ...
 * @date 2011-8-17
 */
public class FundBranchChangeDomain implements IFundBranchChangeDomain{
	@Reference
	...
	public static final String PROC_DEF_UNIQUE_ID = "402881e731dfcd840131dfd26e14000c";
	...
	public void savePeopleAndMemberOnline(SomDuty people, Map<String, String> map) {
		...
		// 维护任职信息
		ParameterSet pset = new ParameterSet();
		pset.setParameter("DUTY_ID@=",map.get("leaPeopleId"));
		...
		if(dutyds.getCount() > 0){
			...
			people.setDutyId(map.get("leaPeopleId"));
		...
		if(somResumeList.size() > 0 ){
			for (int j = 0; j < somResumeList.size(); j++) {
				...
				somResume.setDutyId(map.get("leaPeopleId"));
