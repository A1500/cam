...
import com.inspur.cams.sorg.apply.fund.domain.IFundBranchBuildDomain;
 ...
 * @date 2011-8-23
 */
public class FundBranchBuildDomain implements IFundBranchBuildDomain {
	...
	public static final String PROC_DEF_UNIQUE_ID = "8a898b9d363e970301363ea7cdbf002d";
	...
	public Map create(SomApply somApply) {
		...
		// 调用工作流发送接口
		Map map = WfTaskRequestWrapper.getInstance().create(somApply.getTaskCode(),somApply.getSomOrgan().getCnName() + "的成立申请");
		String processId = (String) map.get("processId");
		somApply.setProcessId(processId);

		//3.保存业务信息
		somApply.setSorgId(organBean.getSorgId());
		somApply.setApplyType(ApplyType.SOM_FUND_BRANCH_BUILD);
		somApply.setSorgType(organBean.getSorgType());
		somApply.setSorgName(organBean.getCnName());
		somApply.setBorgName(somApply.getSomOrgan().getBorgName());
	...
	public void send(SomApply somApply) {
		...
		if(organBean==null){
			...
			organBean = (SomOrgan) organDS.getRecord(0).toBean(SomOrgan.class);
		}
		//1.记录流程历史意见
		somApplyDomain.curSomApply(somApply);
