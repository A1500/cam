 ...
 * @date 2011-8-17
 */
public class UngovChangeDomain implements IUngovChangeDomain{
	@Reference
	...
	public static final String PROC_DEF_UNIQUE_ID = "402881e631e4e3630131e516c226000d";
	 ...
	 * @param map	所需参数
	 */
	public void savePeopleAndMember(SomDuty people,List<SomPeopleResume> peopleResumes, Map<String, String> map) {
	...
	public String savePeopleAndMemberOnline(SomChangeApply somChangeApply) {
		...
		if (dset==null||dset.getCount()<1) {
			throw new RuntimeException("获取当前届次信息出错！");
		}
	...
	public void deleteForChange(String id, String changeItem, String sorgId,String taskCode) {
		 ...
		 if(changeItem.equals("0")){
			 ...
			 name="住所产权证件,住所租（借）合同";
		 }else if(changeItem.equals("2")){
			 name="原法定代表人任期内的财务审计报告,新法定代表人的简历和身份证明,关于债权债务的说明";
		 }else if(changeItem.equals("3")){
			 name="业务范围变更修改后的章程（草案）";
		 }else if(changeItem.equals("4")){
			 name="会计师所出具的验资报告";
		 }else if(changeItem.equals("5")){
			 name="原业务主管单位意见,新业务主管单位意见,业务主管单位变更修改后的章程（草案）";
		 }else if(changeItem.equals("7")){
			 name="新出资人身份证明,出资人变更修改后的章程（草案）";
		 }
	...
	public void updateSomApply(SomChangeApply somChangeApply) {
		...
		somDutyDomain.reNewPeopleInfo(map.get("taskCode1"));//清空任职信息
		SomDuty people=somChangeApply.getSomLegalPeople();
		if(people != null){
			savePeopleAndMember(people,peopleResumes,map);
		}
	...
	private void updateChangeInfo(SomApply somApply, Map<String, String> map,String flag) {
		...
		//章程
		ParameterSet rulePset=new ParameterSet();
		rulePset.setParameter("TASK_CODE",somApply.getTaskCode());
		DataSet ruleDataset = somRuleDomain.queryRule(rulePset);
		RecordSet ruleRecords=ruleDataset.getRecordSet();
		Iterator<Record> rules= ruleRecords.iterator();//变更
			...
			while(changes.hasNext()){
				...
				if(somchange.getChangeItem().equals("0")){//名称
					...
					if(s[0]!=null && !"".equals(s[0])){
						...
						somOrgan.setLeasePeriod(s[1]);//租借期限
						...
						somOrgan.setResidence(s[4]);//变更住所
					}
					...
					somApply.setBorgName(somchange.getChangeAfter());
				}else if(somchange.getChangeItem().equals("7")){//出资人变更
