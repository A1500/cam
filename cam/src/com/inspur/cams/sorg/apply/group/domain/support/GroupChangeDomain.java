...
import com.inspur.cams.sorg.apply.group.domain.IGroupChangeDomain;
...
public class GroupChangeDomain implements IGroupChangeDomain{
	@Reference
	...
	public static final String PROC_DEF_UNIQUE_ID = "8ac896bb31928781013192949e20009f";
	public static final String START_ACT_DEF_ID = "act2";
	...
	public String savePeopleAndMemberOnline(SomChangeApply somChangeApply) {
		...
		if (dset == null||dset.getCount()<1) {
			throw new  RuntimeException("数据异常！未获取当前届次！");
		}
		...
		List<SomPeopleResume> peopleResumes=people.getSomPeopleResumeList();
	...
	public void deleteForChange(String id, String changeItem, String sorgId,String taskCode) {
		 ...
		 if(changeItem.equals("0")){
			 name="名称变更修改后的章程（草案）,名称变更召开会员(代表)大会后提交通过的章程";
		 }else if(changeItem.equals("1")){
			 name="新住所产权或使用权证明";
		 }else if(changeItem.equals("2")){
			 name="原法定代表人在任期内的财务审计报告,新选举的负责人名单";
		 }else if(changeItem.equals("3")){
			 name="业务范围变更修改后的章程（草案）,会员（代表）大会决议";
		 }else if(changeItem.equals("4")){
			 name="会计师所出具的验资报告";
		 }else if(changeItem.equals("5")){
			 name="业务主管单位交接双方意见,业务主管单位变更修改后的章程（草案）,业务主管单位变更召开会员（代表）大会后提交通过的章程";
		 }
		...
		if(flag.equals("update")){
			while(changes.hasNext()){
				...
				if(somchange.getChangeItem().equals("0")){//名称
					...
					String[] s1=somchange.getChangeBefore().split(";",-1);
					if(s1[0]!=null &&!"".endsWith(s1[0]))
					{
						changgebefore=s1[0];
					}
					...
					}if(s[3]!=null &&!"".endsWith(s[3])){
						somOrgan.setHousingArea(new BigDecimal(s[3]));//用房面积
					}if(s[4]!=null &&!"".endsWith(s[4])){
