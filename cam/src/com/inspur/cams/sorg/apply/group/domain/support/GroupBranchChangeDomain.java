...
import com.inspur.cams.sorg.apply.group.domain.IGroupBranchChangeDomain;
...
public class GroupBranchChangeDomain implements IGroupBranchChangeDomain{
	@Reference
	...
	public static final String PROC_DEF_UNIQUE_ID = "8ac896bb318d9b2901318e80be43002e";
	...
	public void endForChange(SomChangeApply somChangeApply) {
		...
		somCert.setActArea(map.get("actArea"));
	...
	public void deleteForChange(String id, String changeItem, String sorgId,String taskCode) {
		...
		somChangeDomain.deleteChange(id);
	}
	/**
	 * 修改页面的保存按钮
	 * @param somApply
	...
	private void createInsertChangeInfo(SomChangeApply somChangeApply) {
		...
		SomDuty people=somChangeApply.getSomLegalPeople();
		if(people != null){
				//people.setTaskCode(somChangeApply.getSomApply().getTaskCode());
		...
		if(flag.equals("update")){
			while(changes.hasNext()){
				...
				if(somchange.getChangeItem().equals("0")){//名称
					...
					String[] s=somchange.getChangeAfter().split(";",-1);
					if(s[0]!=null &&!"".endsWith(s[0])){
						somOrgan.setFundingPeople(s[0]);//出资人
					}if(s[1]!=null &&!"".endsWith(s[1])){
						somOrgan.setCheckCapitalOrgan(s[1]);//验资单位
					}if(s[2]!=null &&!"".endsWith(s[2])){
						somOrgan.setMoneySource(s[2]);//资金来源
					}if(s[3]!=null &&!"".endsWith(s[3])){
						somOrgan.setRegMon(s[3]);//变更注册资金
					}
	...
	private void createInsertChangeInfoOnline(SomChangeApply somChangeApply) {
		...
		if(people != null){
			if(map.get("legalDutyId") != null && map.get("taskCode1") != null ){
				savePeopleAndMemberOnline(people,map);
			}
		}
		List<SomChange> changes=somChangeApply.getSomChange();
		for(int i=0;i<changes.size();i++){
			...
			change.setSorgId(map.get("sorgId"));
			change.setChangeStatus("0");
			if("2".equals(change.getChangeItem())){//更新法定代表人的dutyId
				change.setLegalDutyId(people.getDutyId());
			}
			somChangeDomain.insert(change);//插入变更信息
		}
		SomRule somRule=somChangeApply.getSomRule();
	 ...
	 * @param map	所需参数
	 */
	public void savePeopleAndMemberOnline(SomDuty people, Map<String, String> map) {
