...
import com.inspur.cams.sorg.apply.group.domain.IGroupElectionApplyDomain;
 ...
 * @date 2012年6月25日
 */
public class GroupElectionApplyCmd extends BaseAjaxCommand {

	private IGroupElectionApplyDomain groupElectionApplyDomain = ScaComponentFactory.getService(
			IGroupElectionApplyDomain.class, "groupElectionApplyDomain/groupElectionApplyDomain");

	// 保存
	public void create() {
		Record somApplyRecord = (Record) getParameter("somApplyRecord");
		SomApply somApply = (SomApply) somApplyRecord.toBean(SomApply.class);
		groupElectionApplyDomain.create(somApply, getSomElection());
	}
	// 保存并提交
	public void createAndSend() {
		Record somApplyRecord = (Record) getParameter("somApplyRecord");
		SomApply somApply = (SomApply) somApplyRecord.toBean(SomApply.class);
		groupElectionApplyDomain.createAndSend(somApply, getSomElection());
	}
	public void send(){
		Record somApplyRecord = (Record) getParameter("somApplyRecord");
		SomApply somApply = (SomApply) somApplyRecord.toBean(SomApply.class);
		if(Activity.ACT_SUPPLY_CN.equals(somApply.getCurActivity())||Activity.ACT_ACCEPT_CN.equals(somApply.getCurActivity())){
			groupElectionApplyDomain.update(somApply, getSomElection());
		}
		groupElectionApplyDomain.send(somApply);
