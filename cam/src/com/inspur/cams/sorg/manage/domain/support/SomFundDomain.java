 ...
 * @author Jiangzhaobao
 *
 * @date 2011年5月24日19:27:44
 */
public class SomFundDomain implements ISomFundDomain{
	@Reference
	...
	private void updatePeopleName(SomPeople peopleBean) {
		...
		for (int i = 0; i < ds.getCount(); i++) {
			SomMember memberBean = (SomMember) ds.getRecord(i).toBean(SomMember.class);
			memberBean.setPeopleName(peopleBean.getName());
	...
	public void saveMain(SomOrgan organBean, String sorgType) {
		// 人员类型(L:法定代表人,C:负责人,S:理事,J:监事,Z:理事长,F:副理事长,I:秘书长)
		// set社会组织信息
		...
		if (somPeopleList.size() == 1) {
			...
			if (currentPeopleId == null) {
				...
				if (ds.getCount() == 1) {
					...
					SomMember memberBean = (SomMember) ds.getRecord(0).toBean(
							SomMember.class);
					updateMember(memberBean, peopleBean, organBean,
							currentSorgId, "L");
					somMemberDomain.update(memberBean);
				}
			}
	...
	public void savePeople(SomPeople peopleBean) {
		...
		updateOrgan(sorgId, organBean, organBean.getSorgStatus());
		somOrganDomain.update(organBean);
		// 新建人员
		if (peopleId == null) {
			// 保存人员信息
	...
	public void saveCancel(SomOrgan organBean) {
		...
		if (organBean.getState() == Record.STATE_MODIFIED) {
			...
			updateOrgan(sorgId ,organBean, _sorgStatus);
			// 更新社会组织的基本信息
			somOrganDomain.update(organBean);
			// 注销组织的所有会议和人员
	...
	public void updateBranch(SomOrgan organBean) {
		...
		if (organBean.getState() == Record.STATE_MODIFIED) {
			// 更新社团信息
			updateOrgan(branchSorgId, organBean, "22");
			somOrganDomain.update(organBean);
			// 更新成员信息
			List<SomMember> somMemberList = new ArrayList<SomMember>();
			ParameterSet pset = new ParameterSet();
	...
	public DataSet queryPeopleByIdCard(ParameterSet pset) {
		...
		for (int i = 0; i < peopleDs.getCount(); i++) {
			...
			int count = somMemberDomain.queryLegalPeople(peopleId,null);
			String isLegal = "0";
			if(count > 0){
				isLegal = "1";
			}
...
//----------------------------------jiangzhaobao------------------------------------------------------	
	/**
	 * 基金会换届（成立）选举报告的查询，保存和更新操作
	 */
	public DataSet queryElection(ParameterSet pset){
		pset.setParameter("SOM_MEETING.MEETING_TYPE", "X");
		pset.setParameter("SOM_MEETING.SORG_TYPE", "J");
...
//		somMeeting.setSorgType("J");
	...
	public DataSet queryAccount(ParameterSet pset) {
		pset.setParameter("SOM_ACCOUNT.SORG_TYPE", "J");
...
//		somMeeting.setSorgType("J");
	...
	public String queryAccountForQuery(ParameterSet pset) {
		return somAccountDomain.queryForQuery(pset);
	}
}
