...
public class DisConditionTongJiDao extends BaseJdbcDao {

    public DisConditionTongJiDao() {
    ...
    private static class Proc extends Procedure {
        ...
        public Proc(DataSource ds) {
            ...
            super(ds, "BP_DIS_CONDITION_TONGJI");
            // 声明参数
            declareParameter(new SqlParameter("in_xzqh", Types.VARCHAR));
            declareParameter(new SqlParameter("in_ffsj_q", Types.VARCHAR));
            declareParameter(new SqlParameter("in_ffsj_z", Types.VARCHAR));
            // 声明结果集的个数
            setNumberOfResultSet(1);
            compile();
        }

        public Map call(String xzqh, String ffsj_q, String ffsj_z) {
            Map in = new HashMap();
            // 设置入参的值，key要和声明参数的名称一致
            in.put("in_xzqh", xzqh);
            in.put("in_ffsj_q", ffsj_q);
            in.put("in_ffsj_z", ffsj_z);
    ...
    public DataSet getDataSet(ParameterSet pset) {
        ...
        String ffsj_q = (String) pset.getParameter("ffsjq");// 查询时间_起
        String ffsj_z = (String) pset.getParameter("ffsjz");// 查询时间_止
        object = procduce.call(xzqu,ffsj_q,ffsj_z).get("results");
