package com.inspur.cams.bpt.base.dao.jdbc;

import java.sql.Types;
import java.util.ArrayList;
import java.util.List;

import org.loushang.next.dao.EntityDao;
import org.loushang.next.data.DataSet;
import org.loushang.next.data.ParameterSet;
import org.loushang.persistent.jdbc.datasource.DataSourceFactory;

import com.inspur.cams.bpt.base.dao.IBptPeopleDemobilizedDao;
import com.inspur.cams.bpt.base.data.BptPeopleDemobilized;
import com.inspur.cams.bpt.util.BptConstant;
import com.inspur.cams.bpt.util.BptGetAge;
import com.inspur.cams.bpt.util.FilterLimit;
import com.inspur.cams.comm.util.DateUtil;

/**
 * @title:BptPeopleDemobilizedDao
 * @description:
 * @author:
 * @since:2011-05-20
 * @version:1.0
*/
 public class BptPeopleDemobilizedDao extends EntityDao<BptPeopleDemobilized> implements IBptPeopleDemobilizedDao{

 	public BptPeopleDemobilizedDao() throws Exception {
			this.setDataSource(DataSourceFactory.defaultFactory
					.getDataSource("dataSource"));
			this.afterPropertiesSet();
	}

  	@Override
	public Class<BptPeopleDemobilized> getEntityClass() {
		return BptPeopleDemobilized.class;
	}
   
   /**
	 * 查询在乡复员军人信息和相应的优抚人员信息
	 */
   public DataSet queryDemobilizedPeople(ParameterSet pset){
	    DataSet ds = new DataSet();
		List<Integer> typeList = new ArrayList<Integer>();
		List<Object> argsList = new ArrayList<Object>();
		String domicileCode = (String)pset.getParameter("domicileCode");
		String name = (String)pset.getParameter("name");
		String idCard = (String)pset.getParameter("idCard");
		String belongDate = (String)pset.getParameter("belongDate");
		String isKoreanwar = (String)pset.getParameter("isKoreanwar");
		String objectState = (String)pset.getParameter("objectState");
		String domicileType = (String)pset.getParameter("domicileType");
		String organCode = (String) pset.getParameter("organCode");
		String startTime = (String) pset.getParameter("startTime");
		String endTime = (String) pset.getParameter("endTime");
		String ageBegin = (String) pset.getParameter("ageBegin");
		String ageEnd = (String) pset.getParameter("ageEnd");
		String isprovinceMigrate = (String) pset.getParameter("zisprovinceMigrate");
		StringBuffer sql = new StringBuffer();
		StringBuffer sqlfind = new StringBuffer() ;
		sqlfind.append("SELECT A.BIRTHDAY,'' PEOPLE_AGE, A.FAMILY_ID, A.PEOPLE_ID,\n");
		sqlfind.append("       A.DOMICILE_CODE, A.NAME, A.DEMOBILIZED_STATE_CODE, A.ID_CARD, A.SEX,\n"); 
		sqlfind.append("       A.NATION, C.BELONGDATE, C.IS_KOREANWAR, C.CONSCRIPT_DATE,\n"); 
		sqlfind.append("       C.DECRUITMENT_DATE, D.FULL_NAME DOMICILE_NAME, A.DOMICILE_TYPE,\n"); 
		sqlfind.append("       A.TEL_MOBILE, C.NOTE\n"); 
		if (isprovinceMigrate != null && !isprovinceMigrate.equals("")&& !isprovinceMigrate.equals("0")){
			sqlfind.append("  ,'1' as flag\n"); 
		}
		sqlfind.append("  FROM BASEINFO_PEOPLE A, BPT_PEOPLE B, BPT_PEOPLE_DEMOBILIZED C, DIC_CITY D\n");
		if (isprovinceMigrate != null && !isprovinceMigrate.equals("")&& !isprovinceMigrate.equals("0")){
			sqlfind.append("  ,bpt_province_migrate e\n"); 
		}
		sqlfind.append(" WHERE A.PEOPLE_ID = B.PEOPLE_ID AND A.PEOPLE_ID = C.PEOPLE_ID AND\n"); 
		sqlfind.append("       A.DEMOBILIZED_FLAG = '1' AND A.PERSONAL_STATS_TAG != '4' AND\n"); 
		sqlfind.append("       A.DOMICILE_CODE = D.ID");
		if (isprovinceMigrate != null && !isprovinceMigrate.equals("")&& !isprovinceMigrate.equals("0")){
			sqlfind.append("  and A.people_id = e.people_id and e.service_type='43'\n"); 
		}
		StringBuffer sqlExcel = new StringBuffer();
		sqlExcel.append("SELECT A.BIRTHDAY,'' PEOPLE_AGE, A.DOMICILE_CODE, A.NAME,\n");
		sqlExcel.append("       (CASE A.DEMOBILIZED_STATE_CODE\n"); 
		sqlExcel.append("          WHEN '1' THEN '受理'\n"); 
		sqlExcel.append("          WHEN '2' THEN '享受待遇'\n"); 
		sqlExcel.append("          WHEN '3' THEN '身份认定不享受待遇'\n"); 
		sqlExcel.append("          WHEN '4' THEN '停发'\n"); 
		sqlExcel.append("          WHEN '5' THEN '迁移'\n"); 
		sqlExcel.append("          WHEN '6' THEN '不予认定'\n"); 
		sqlExcel.append("          WHEN '7' THEN '去世'\n"); 
		sqlExcel.append("          WHEN '8' THEN '取消'\n"); 
		sqlExcel.append("          WHEN '9' THEN '中止'\n"); 
		sqlExcel.append("        END) AS DEMOBILIZED_STATE_CODE,  A.ID_CARD,\n"); 
		sqlExcel.append("       (CASE A.SEX\n"); 
		sqlExcel.append("          WHEN '1' THEN '男'\n"); 
		sqlExcel.append("          WHEN '2' THEN '女'\n"); 
		sqlExcel.append("        END) AS SEX,  N.NAME AS NATION,  W.NAME AS BELONGDATE,\n"); 
		sqlExcel.append("       (CASE C.IS_KOREANWAR\n"); 
		sqlExcel.append("          WHEN '0' THEN '否'\n"); 
		sqlExcel.append("          WHEN '1' THEN '是'\n"); 
		sqlExcel.append("        END) AS IS_KOREANWAR, C.CONSCRIPT_DATE, C.DECRUITMENT_DATE,\n"); 
		sqlExcel.append("       D.FULL_NAME DOMICILE_NAME, A.DOMICILE_TYPE, A.TEL_MOBILE, C.NOTE\n"); 
		sqlExcel.append("  FROM BASEINFO_PEOPLE A, BPT_PEOPLE B, BPT_PEOPLE_DEMOBILIZED C, DIC_CITY D,\n"); 
		sqlExcel.append("       DIC_NATION N, DIC_BPT_WAR W\n"); 
		if (isprovinceMigrate != null && !isprovinceMigrate.equals("")&& !isprovinceMigrate.equals("0")){
			sqlExcel.append("  ,bpt_province_migrate e\n"); 
		}
		sqlExcel.append(" WHERE A.PEOPLE_ID = B.PEOPLE_ID AND A.PEOPLE_ID = C.PEOPLE_ID AND\n"); 
		sqlExcel.append("       A.DEMOBILIZED_FLAG = '1' AND A.PERSONAL_STATS_TAG != '4' AND\n"); 
		sqlExcel.append("       A.DOMICILE_CODE = D.ID AND C.BELONGDATE = W.CODE AND\n"); 
		sqlExcel.append("       A.NATION = N.CODE");
		if (isprovinceMigrate != null && !isprovinceMigrate.equals("")&& !isprovinceMigrate.equals("0")){
			sqlExcel.append("  and A.people_id = e.people_id and e.service_type='43'\n"); 
		}
		String isAll = (String) pset.getParameter("isAll");
		if("all".equals(isAll) || !pset.containsKey("limit")){
			sql.append(sqlExcel);
		}else{
			sql.append(sqlfind);
		}
		if (isprovinceMigrate != null && !isprovinceMigrate.equals("")&& !isprovinceMigrate.equals("0")){
			if (domicileCode != null && !domicileCode.equals("")) {
				if (!domicileCode.substring(0, 2).equals("00")
						&& domicileCode.substring(2, 4).equals("00")) {
					sql.append(" AND e.OUT_DOMICILE_CODE like ?");// 省局
					typeList.add(Types.VARCHAR);
					argsList.add(domicileCode.substring(0, 2)+"%");
				} else if (!domicileCode.substring(2, 4).equals("00")
						&& domicileCode.substring(4).equals("00000000")) {
					sql.append(" AND e.OUT_DOMICILE_CODE like ?");// 市局
					typeList.add(Types.VARCHAR);
					argsList.add(domicileCode.substring(0, 4)+"%");
				} else if (!domicileCode.substring(4, 6).equals("00")
						&& domicileCode.substring(6, 9).equals("000")) {
					sql.append(" AND e.OUT_DOMICILE_CODE like ?");// 区县
					typeList.add(Types.VARCHAR);
					argsList.add(domicileCode.substring(0, 6)+"%");
				} else if (!domicileCode.substring(6, 9).equals("000")
						&& domicileCode.substring(9, 12).equals("000")) {
					sql.append(" AND e.OUT_DOMICILE_CODE like ?");// 街道
					typeList.add(Types.VARCHAR);
					argsList.add(domicileCode.substring(0, 9)+"%");
				} else {
					sql.append(" AND e.OUT_DOMICILE_CODE = ?");// 社区
					typeList.add(Types.VARCHAR);
					argsList.add(domicileCode);
				}
			}else{
				if (!organCode.substring(0, 2).equals("00")
						&& organCode.substring(2, 4).equals("00")) {
					sql.append(" AND  e.OUT_DOMICILE_CODE like ?");// 省局
					typeList.add(Types.VARCHAR);
					argsList.add(organCode.substring(0, 2));
				} else if (!organCode.substring(2, 4).equals("00")
						&& organCode.substring(4, 6).equals("00")) {
					sql.append(" AND e.OUT_DOMICILE_CODE like ?");// 市局
					typeList.add(Types.VARCHAR);
					argsList.add(organCode.substring(0, 4));
				} else {
					sql.append(" AND e.OUT_DOMICILE_CODE like ?");// 区县
					typeList.add(Types.VARCHAR);
					argsList.add(organCode.substring(0, 6));
				}
			}
		}else {
			if (domicileCode != null && !domicileCode.equals("")) {
				if(!"00".equals(domicileCode.substring(2, 4))) { //非省级
					if ("00".equals(domicileCode.substring(4, 6))
							&& "00000000".equals(domicileCode.substring(4))) { // 市级
						sql.append(" and domicile_code like  ? ");
						typeList.add(Types.VARCHAR);
						argsList.add(domicileCode.substring(0, 4)+"%");
					} else if("000000".equals(domicileCode.substring(6))) { //县级
						sql.append(" and domicile_code like  ? ");
						typeList.add(Types.VARCHAR);
						argsList.add(domicileCode.substring(0, 6)+"%");
					} else if("000".equals(domicileCode.substring(9))) { //街道
						sql.append(" and domicile_code like  ? ");
						typeList.add(Types.VARCHAR);
						argsList.add(domicileCode.substring(0, 9)+"%");
					} else { //村
						sql.append(" and domicile_code = ? ");
						typeList.add(Types.VARCHAR);
						argsList.add(domicileCode);
					}
				} else { //省级
					sql.append(" and domicile_code like  ? ");
					typeList.add(Types.VARCHAR);
					argsList.add(domicileCode.substring(0, 2)+"%");
				}
			}  else {
				organCode=FilterLimit.filterByOrganCode(sql,organCode);	
				typeList.add(Types.VARCHAR);
				argsList.add(organCode);
			}
		}
		if(name != null && !name.equals("")){
			sql.append(" AND A.NAME LIKE ?");
			typeList.add(Types.VARCHAR);
			argsList.add("%"+name+"%");
		}
		
		if(idCard !=null && !idCard.equals("")){
			sql.append(" AND A.ID_CARD LIKE ?");
			typeList.add(Types.VARCHAR);
			argsList.add("%"+idCard+"%");
		}
		
		if(belongDate != null && !belongDate.equals("")){
			if(belongDate.indexOf(",") != -1) {
				String[] types = belongDate.split(",");
				for(int i=0;i<types.length;i++) {
					 if(i == 0) {
						 sql.append(" AND (C.BELONGDATE=? ");
					 } else {
						 sql.append(" OR C.BELONGDATE=?");
						 if(i == types.length - 1) {
							 sql.append(" )");
						 }
					 } 
					 typeList.add(Types.VARCHAR);
					 argsList.add(types[i]);
				}
			} else {
				sql.append("AND C.BELONGDATE=?");
				typeList.add(Types.VARCHAR);
				argsList.add(belongDate);
			}
			
		}
		
		if(isKoreanwar != null && !isKoreanwar.equals("")){
			sql.append("AND C.IS_KOREANWAR=?");
			typeList.add(Types.VARCHAR);
			argsList.add(isKoreanwar);
		}
		
		if(objectState != null && !objectState.equals("")){
			sql.append(" AND A.DEMOBILIZED_STATE_CODE=?");
			typeList.add(Types.VARCHAR);
			argsList.add(objectState);
		}
		if(domicileType != null && !domicileType.equals("")){
			sql.append(" AND A.DOMICILE_TYPE=?");
			typeList.add(Types.VARCHAR);
			argsList.add(domicileType);
		}
		
		if ((endTime != null && !"".equals(endTime))
				|| (startTime != null && !"".equals(startTime))) {
			if(endTime == null || "".equals(endTime)) {
				endTime = DateUtil.getDay();
			}
			if(startTime == null || "".equals(startTime)) {
				startTime = "1900-01-01";
			}
			sql.append(" AND TO_DATE(C.APPROVAL_DATE, 'YYYY-MM-DD') BETWEEN TO_DATE(?, 'YYYY-MM-DD')\n");
			sql.append(" AND TO_DATE(?, 'YYYY-MM-DD') ");
			typeList.add(Types.VARCHAR);
			argsList.add(startTime);
			typeList.add(Types.VARCHAR);
			argsList.add(endTime);
		}
		
		if (ageBegin != null && !"".equals(ageBegin)) {
			sql.append(" AND getage(A.birthday) >= ?");
			typeList.add(Types.INTEGER);
			argsList.add(Integer.parseInt(ageBegin));
		}

		if (ageEnd != null && !"".equals(ageEnd)) {
			sql.append(" AND getage(A.birthday) <= ?");
			typeList.add(Types.INTEGER);
			argsList.add(Integer.parseInt(ageEnd));
		}
		
		if(typeList.size() != 0 && argsList.size()!= 0) {
			int[] types = new int[typeList.size()];
			for(int i=0;i<typeList.size();i++) {
				types[i]=(Integer)(typeList.get(i));
			}
			Object[] args = argsList.toArray(new Object[argsList.size()]);
			if("all".equals(isAll) || !pset.containsKey("limit")){
				ds = this.executeDataset(sql.toString(), types,args,true);
			} else {
				ds = this.executeDataset(sql.toString(),types,args, pset.getPageStart(), pset
						.getPageLimit(), true);
			}
		} else {
			if("all".equals(isAll) || !pset.containsKey("limit")){
				ds = this.executeDataset(sql.toString(),true);
			} else {
				ds = this.executeDataset(sql.toString(), pset.getPageStart(), pset
						.getPageLimit(), true);
			}
		}
		for (int i = 0; i < ds.getCount(); i++) {
			
			String birth=(String)ds.getRecord(i).get("BIRTHDAY");
			ds.getRecord(i).set("PEOPLE_AGE",BptGetAge.getAgeByBirthday(birth));
		}
		return ds;
	}
	
	
	/**
	 * 查询未提交的乡复员军人
	 */
	public DataSet queryUnCimDemPeo(ParameterSet pset){
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT P.NAME, P.SEX, C.FULL_NAME AS DOMI_NAME, P.NATION, P.TEL_MOBILE,\n");
		sql.append("       P.DOMICILE_TYPE, P.FAMILY_ID, P.DOMICILE_CODE, D.CONSCRIPT_DATE,\n"); 
		sql.append("       P.ID_CARD, D.DECRUITMENT_DATE, D.IS_KOREANWAR, D.BELONGDATE,\n"); 
		sql.append("       P.REG_TIME, T.APPLY_ID, B.DEMOBILIZED_TYEP_CODE, D.PEOPLE_ID\n"); 
		sql.append("  FROM BASEINFO_PEOPLE P, BPT_PEOPLE B, DIC_CITY C, BPT_APPLY_DEMOBILIZED T,\n"); 
		sql.append("       BPT_PEOPLE_DEMOBILIZED D\n"); 
		sql.append(" WHERE P.PEOPLE_ID = B.PEOPLE_ID AND B.PEOPLE_ID = D.PEOPLE_ID AND\n"); 
		sql.append("       B.PEOPLE_ID = T.PEOPLE_ID AND P.DOMICILE_CODE = C.ID AND\n"); 
		sql.append("       T.SERVICE_TYPE = '41' AND T.COMMIT_FLAG = '0'");

		String organCode = (String) pset.getParameter("organCode");
		organCode=FilterLimit.filterByOrganCode(sql,organCode);	
		return this.executeDataset(sql.toString(),new int[]{Types.VARCHAR},new Object[]{organCode},pset.getPageStart(),pset.getPageLimit(), true);
	}

	public DataSet queryDemobilizedFlowHeader(String peopleId) {
		String sql = "  SELECT P.NAME,\n" +
		"       P.SEX,\n" + 
		"		C.NAME                  AS DOMI_NAME,\n" +
		"       P.NATION,\n" + 
		"       P.TEL_MOBILE,\n" +
		"       P.DOMICILE_TYPE,\n" +
		" 		P.FAMILY_ID,\n" +  
		" 		P.DOMICILE_CODE,\n" + 
		"       D.CONSCRIPT_DATE ,\n" + 
		"       ID_CARD ,\n" +
		"       D.DECRUITMENT_DATE ,\n" +
		"       D.IS_KOREANWAR ,\n" +
		"       D.BELONGDATE ,\n" +
		"       P.REG_TIME,\n" + 
		"       b.demobilized_tyep_code,\n" + 
		"       D.PEOPLE_ID\n" + 
		"  FROM BASEINFO_PEOPLE P, BPT_PEOPLE B,  DIC_CITY C, bpt_people_demobilized D\n" + 
		"  WHERE P.PEOPLE_ID = " 
		+  peopleId +
		"   AND B.PEOPLE_ID = " 
		+  peopleId +
		"   AND P.DOMICILE_CODE = C.ID\n" +
		//"   AND P.Demobilized_Flag = '1'\n" + 
		//"   AND b.demobilized_tyep_code='11'\n" + 
		"   AND D.COMMIT_FLAG = '1' ";
		return null;
	}

	public String queryByProcessId(String peopleId) {
		String family_id = "";
		String sql="select family_id from BASEINFO_PEOPLE where people_id= ? ";
		DataSet ds=executeDataset(sql,new int[]{Types.VARCHAR},new Object[]{peopleId},true);
		if(ds.getCount()>0){
			family_id =  (String) ds.getRecord(0).get("FAMILY_ID");
		}
		return family_id;
	}

	public DataSet queryChangedemo(ParameterSet pset) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT P.NAME, P.SEX, C.NAME AS DOMI_NAME, P.NATION, P.TEL_MOBILE,\n");
		sql.append("       P.DOMICILE_TYPE, P.FAMILY_ID, P.DOMICILE_CODE, D.CONSCRIPT_DATE,\n"); 
		sql.append("       P.ID_CARD, D.DECRUITMENT_DATE, D.IS_KOREANWAR, D.BELONGDATE,\n"); 
		sql.append("       P.REG_TIME, T.APPLY_ID, B.DEMOBILIZED_TYEP_CODE, D.ASDP_NO,\n"); 
		sql.append("       D.PEOPLE_ID\n"); 
		sql.append("  FROM BASEINFO_PEOPLE P, BPT_PEOPLE B, DIC_CITY C, BPT_APPLY_DEMOBILIZED T,\n"); 
		sql.append("       BPT_PEOPLE_DEMOBILIZED D\n"); 
		sql.append(" WHERE P.PEOPLE_ID = B.PEOPLE_ID AND B.PEOPLE_ID = D.PEOPLE_ID AND\n"); 
		sql.append("       B.PEOPLE_ID = T.PEOPLE_ID AND P.DOMICILE_CODE = C.ID AND\n"); 
		sql.append("       T.COMMIT_FLAG = '0'");
	return this.executeDataset(sql.toString(),pset.getPageStart(),pset.getPageLimit(), true);
	}
	/**
	 * 查询未提交的在乡复员换证军人列表
	 */
	public DataSet queryDemobilizedPeopleList(ParameterSet pset) {
		
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT P.NAME, P.SEX, C.NAME AS DOMI_NAME, P.NATION, P.TEL_MOBILE,\n");
		sql.append("       P.DOMICILE_TYPE, P.FAMILY_ID, P.DOMICILE_CODE, P.PEOPLE_ID,\n"); 
		sql.append("       D.CONSCRIPT_DATE, P.ID_CARD, D.DECRUITMENT_DATE, D.IS_KOREANWAR,\n"); 
		sql.append("       D.BELONGDATE, P.REG_TIME, T.APPLY_ID, T.EXCHANGE_REASON,\n"); 
		sql.append("       B.DEMOBILIZED_TYEP_CODE, P.NOTE, P.DEMOBILIZED_NO, D.FORCES_NO,\n"); 
		sql.append("       D.POSITION\n"); 
		sql.append("  FROM BASEINFO_PEOPLE P, BPT_PEOPLE B, DIC_CITY C, BPT_APPLY_DEMOBILIZED T,\n"); 
		sql.append("       BPT_PEOPLE_DEMOBILIZED D\n"); 
		sql.append(" WHERE P.PEOPLE_ID = B.PEOPLE_ID AND B.PEOPLE_ID = D.PEOPLE_ID AND\n"); 
		sql.append("       B.PEOPLE_ID = T.PEOPLE_ID AND P.DOMICILE_CODE = C.ID AND\n"); 
		sql.append("       T.EXCHACERTI_COMMIT_FLAG = '0' AND T.SERVICE_TYPE = '44'");
		DataSet ds= executeDataset(sql.toString(),pset.getPageStart(),pset.getPageLimit(), true);
		return	ds;
	}
	
	/**
	 * 查询在乡复员换证军人IdCard
	 */
	public DataSet queryDemobilizedBptPeopleGetIdCard(ParameterSet pset) {
		List<Integer> typeList = new ArrayList<Integer>();
		List<Object> argsList = new ArrayList<Object>();
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT P.NAME, P.SEX, C.NAME AS DOMI_NAME, P.NATION, P.TEL_MOBILE,\n");
		sql.append("       P.DOMICILE_TYPE, P.FAMILY_ID, P.DOMICILE_CODE, D.CONSCRIPT_DATE,\n"); 
		sql.append("       P.ID_CARD, D.DECRUITMENT_DATE, D.IS_KOREANWAR, D.BELONGDATE,\n"); 
		sql.append("       P.REG_TIME, P.PEOPLE_ID, B.DEMOBILIZED_TYEP_CODE, P.NOTE,\n"); 
		sql.append("       P.DEMOBILIZED_NO, D.FORCES_NO, D.POSITION\n"); 
		sql.append("  FROM BASEINFO_PEOPLE P, BPT_PEOPLE B, DIC_CITY C, BPT_PEOPLE_DEMOBILIZED D\n"); 
		sql.append(" WHERE P.PEOPLE_ID = B.PEOPLE_ID AND B.PEOPLE_ID = D.PEOPLE_ID AND\n"); 
		sql.append("       P.DOMICILE_CODE = C.ID");
		String domicileCode = (String) pset.getParameter("domicileCode");
		String name = (String) pset.getParameter("name");
		String idCard = (String) pset.getParameter("idCard");
		String belongDate = (String) pset.getParameter("belongDate");
		String isKoeanwar = (String) pset.getParameter("isKoeanwar");
		String demobilizedNo = (String) pset.getParameter("demobilizedNo");
		String organCode = (String) pset.getParameter("organCode");
		
		if (domicileCode != null && !domicileCode.equals("")) {
			domicileCode=FilterLimit.filterByOrganCode(sql,domicileCode);
			argsList.add(domicileCode);
		}else{
			organCode=FilterLimit.filterByOrganCode(sql,organCode);
			argsList.add(organCode);
		}
		typeList.add(Types.VARCHAR);

		if (name != null && !name.equals("")) {
			sql.append(" AND P.NAME LIKE ? ");
			typeList.add(Types.VARCHAR);
			argsList.add(name+"%");
		}

		if (idCard != null && !idCard.equals("")) {
			sql.append(" AND P.ID_CARD = ? ");
			typeList.add(Types.VARCHAR);
			argsList.add(idCard);		
		}

		if (belongDate != null && !belongDate.equals("")) {
			sql.append(" AND D.BELONGDATE = ? ");
			typeList.add(Types.VARCHAR);
			argsList.add(belongDate);
		}

		if (isKoeanwar != null && !isKoeanwar.equals("")) {
			sql.append(" AND IS_KOREANWAR = ? ");
			typeList.add(Types.VARCHAR);
			argsList.add(isKoeanwar);
		}

		if (demobilizedNo != null && !demobilizedNo.equals("")) {
			sql.append(" AND P.DEMOBILIZED_NO = ? ");
			typeList.add(Types.VARCHAR);
			argsList.add(demobilizedNo);
		}
		DataSet ds = new DataSet();
		if (typeList.size() != 0 && argsList.size() != 0) {
			int[] types = new int[typeList.size()];
			for (int i = 0; i < typeList.size(); i++) {
				types[i] = (Integer) (typeList.get(i));
			}
			Object[] args = argsList.toArray(new Object[argsList.size()]);
			ds = executeDataset(sql.toString(),types,args,pset.getPageStart(),pset.getPageLimit(), true);
		}	
		return ds;
	}
	 /**
	 * 查询未提交的在乡复员人员抚恤关系跨省迁出人员列表
	 * @param pset
	 * @return
	 */
	public DataSet queryDemoEmigrationPeopleList(ParameterSet pset) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT C.NAME AS DOMI_NAME, P.FAMILY_ID AS FAMILY_ID, P.NAME, P.SEX,\n");
		sql.append("       P.NATION, P.TEL_MOBILE, P.DOMICILE_TYPE, P.DOMICILE_CODE,\n"); 
		sql.append("       D.CONSCRIPT_DATE, P.ID_CARD, D.DECRUITMENT_DATE, D.IS_KOREANWAR,\n"); 
		sql.append("       D.BELONGDATE, P.REG_TIME, B.DEMOBILIZED_TYEP_CODE, P.NOTE,\n"); 
		sql.append("       P.DEMOBILIZED_NO, N.APPLY_ID, D.FORCES_NO, D.PEOPLE_ID,\n"); 
		sql.append("       A.SERVICE_TYPE, D.POSITION\n"); 
		sql.append("  FROM BASEINFO_PEOPLE P, BPT_PEOPLE B, BPT_PEOPLE_DEMOBILIZED D, DIC_CITY C,\n"); 
		sql.append("       BPT_APPLY_EMIGRATION N, BPT_APPLY_EMIGRATION A\n"); 
		sql.append(" WHERE P.PEOPLE_ID = B.PEOPLE_ID AND B.PEOPLE_ID = D.PEOPLE_ID AND\n"); 
		sql.append("       P.PEOPLE_ID = A.PEOPLE_ID AND B.PEOPLE_ID = N.PEOPLE_ID AND\n"); 
		sql.append("       P.DOMICILE_CODE = C.ID AND A.COMMIT_FLAG = '0'");
		sql.append(" AND A.SERVICE_TYPE = ? ");
		String organCode = (String)pset.getParameter("organCode");
		organCode=FilterLimit.filterByOrganCode(sql,organCode);
		return this.executeDataset(sql.toString(),new int[]{Types.VARCHAR,Types.VARCHAR},new Object[]{BptConstant.BPT_DEMOBILIZED_EMIGRATION,organCode},pset.getPageStart(), pset.getPageLimit(), true);
	}
	/**
	 * 查询未提交的外省迁入在乡复员军人
	 */
	public DataSet queryDemoProv(ParameterSet pset) {
			StringBuffer sql = new StringBuffer();
			sql.append("SELECT C.NAME AS DOMI_NAME, P.FAMILY_ID AS FAMILY_ID, P.NAME, P.SEX,\n");
			sql.append("       P.NATION, P.TEL_MOBILE, P.DOMICILE_TYPE, P.DOMICILE_CODE,\n"); 
			sql.append("       D.CONSCRIPT_DATE, P.ID_CARD, D.DECRUITMENT_DATE, D.IS_KOREANWAR,\n"); 
			sql.append("       D.BELONGDATE, P.REG_TIME, B.DEMOBILIZED_TYEP_CODE, P.NOTE,\n"); 
			sql.append("       P.DEMOBILIZED_NO, N.APPLY_ID, D.FORCES_NO, D.PEOPLE_ID,\n"); 
			sql.append("       N.SERVICE_TYPE, D.POSITION\n"); 
			sql.append("  FROM BASEINFO_PEOPLE P, BPT_PEOPLE B, BPT_PEOPLE_DEMOBILIZED D, DIC_CITY C,\n"); 
			sql.append("       BPT_APPLY_INTOPROVINCE N\n"); 
			sql.append(" WHERE P.PEOPLE_ID = B.PEOPLE_ID AND B.PEOPLE_ID = D.PEOPLE_ID AND\n"); 
			sql.append("       P.PEOPLE_ID = N.PEOPLE_ID AND B.PEOPLE_ID = N.PEOPLE_ID AND\n"); 
			sql.append("       P.DOMICILE_CODE = C.ID AND N.COMMIT_FLAG = '0'");
			sql.append(" AND N.SERVICE_TYPE = ? ");
			String organCode = (String)pset.getParameter("organCode");
			organCode=FilterLimit.filterByOrganCode(sql,organCode);
			return this.executeDataset(sql.toString(),new int[]{Types.VARCHAR,Types.VARCHAR},new Object[]{BptConstant.BPT_DEMOBILIZED_INGOINGPROVINCE,organCode},pset.getPageStart(), pset.getPageLimit(), true);
	}

	/**
	 * 查询带病回乡军人IdCard
	 */
	public DataSet queryDemobilizedIllBptPeopleGetIdCard(ParameterSet pset) {
		List<Integer> typeList = new ArrayList<Integer>();
		List<Object> argsList = new ArrayList<Object>();
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT P.NAME, P.SEX, C.NAME AS DOMI_NAME, P.NATION, P.TEL_MOBILE,\n");
		sql.append("       P.DOMICILE_TYPE, P.FAMILY_ID, P.PEOPLE_ID, P.DOMICILE_CODE,\n"); 
		sql.append("       D.CONSCRIPT_DATE, P.ID_CARD, D.DECRUITMENT_DATE, P.REG_TIME,\n"); 
		sql.append("       B.DEMOBILIZED_TYEP_CODE, P.NOTE, P.DEMOBILIZED_NO, D.FORCES_NO,\n"); 
		sql.append("       D.POSITION\n"); 
		sql.append("  FROM BASEINFO_PEOPLE P, BPT_PEOPLE B, DIC_CITY C,\n"); 
		sql.append("       BPT_PEOPLE_DEMOBILIZED_ILLNESS D\n"); 
		sql.append(" WHERE P.PEOPLE_ID = B.PEOPLE_ID AND B.PEOPLE_ID = D.PEOPLE_ID AND\n"); 
		sql.append("       P.DOMICILE_CODE = C.ID");
		String domicileCode = (String) pset.getParameter("domicileCode");
		String name = (String) pset.getParameter("name");
		String idCard = (String) pset.getParameter("idCard");
		String belongDate = (String) pset.getParameter("belongDate");
		String isKoeanwar = (String) pset.getParameter("isKoeanwar");
		String demobilizedNo = (String) pset.getParameter("demobilizedNo");
		String organCode = (String) pset.getParameter("organCode");
		if (domicileCode != null && !domicileCode.equals("")) {
			domicileCode=FilterLimit.filterByOrganCode(sql,domicileCode);	
			argsList.add(domicileCode);
		}else{
			organCode=FilterLimit.filterByOrganCode(sql,organCode);	
			argsList.add(organCode);
		}
		typeList.add(Types.VARCHAR);
		if (name != null && !name.equals("")) {
			sql.append(" AND P.NAME LIKE '%'|| ? ||'%'");// LIKE
			typeList.add(Types.VARCHAR);
			argsList.add(name);
		}
		if (idCard != null && !idCard.equals("")) {
			sql.append(" AND P.ID_CARD = ? ");// LIKE
			typeList.add(Types.VARCHAR);
			argsList.add(idCard);
		}
		if (belongDate != null && !belongDate.equals("")) {
			sql.append(" AND D.BELONGDATE = ? ");
			typeList.add(Types.VARCHAR);
			argsList.add(belongDate);
		}
		if (isKoeanwar != null && !isKoeanwar.equals("")) {
			sql.append(" AND IS_KOREANWAR = ? ");
			typeList.add(Types.VARCHAR);
			argsList.add(isKoeanwar);
		}
		if (demobilizedNo != null && !demobilizedNo.equals("")) {
			sql.append(" AND P.DEMOBILIZED_NO = ? ");
			typeList.add(Types.VARCHAR);
			argsList.add(demobilizedNo);
		}
		DataSet ds = new DataSet();
		if (typeList.size() != 0 && argsList.size() != 0) {
			int[] types = new int[typeList.size()];
			for (int i = 0; i < typeList.size(); i++) {
				types[i] = (Integer) (typeList.get(i));
			}
			Object[] args = argsList.toArray(new Object[argsList.size()]);
			ds = executeDataset(sql.toString(),types,args,pset.getPageStart(),pset.getPageLimit(), true);
		}
		return ds;
	}

	public DataSet queryIllEmigrationPeopleList(ParameterSet pset) {
		String organCode = (String) pset.getParameter("organCode");
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT T.SERVICE_TYPE, C.ASDP_NO, D.NAME AS DOMI_NAME, A.FAMILY_ID,\n");
		sql.append("       A.DEMOBILIZED_STATE_CODE, A.PEOPLE_ID, A.DOMICILE_CODE, A.NAME,\n"); 
		sql.append("       A.ID_CARD, A.SEX, A.NATION, C.IS_WAR, C.IS_INNUCLEAR,\n"); 
		sql.append("       C.CONSCRIPT_DATE, C.DECRUITMENT_DATE,\n"); 
		sql.append("       E.NAME || F.NAME || D.NAME DOMICILE_NAME, A.DOMICILE_TYPE,\n"); 
		sql.append("       A.TEL_MOBILE, C.NOTE, T.APPLY_ID\n"); 
		sql.append("  FROM BPT_APPLY_EMIGRATION T, BASEINFO_PEOPLE A, BPT_PEOPLE B,\n"); 
		sql.append("       BPT_PEOPLE_DEMOBILIZED_ILLNESS C, DIC_CITY D, DIC_CITY E, DIC_CITY F\n"); 
		sql.append(" WHERE A.PEOPLE_ID = C.PEOPLE_ID AND A.PEOPLE_ID = B.PEOPLE_ID AND\n"); 
		sql.append("       T.SERVICE_TYPE = '4B' AND A.DEMOBILIZED_FLAG = '1' AND\n"); 
		sql.append("       A.PERSONAL_STATS_TAG != '4' AND A.DOMICILE_CODE = D.ID AND\n"); 
		sql.append("       SUBSTR(DOMICILE_CODE, 1, 4) || '00000000' = E.ID AND\n"); 
		sql.append("       T.PEOPLE_ID = A.PEOPLE_ID AND T.COMMIT_FLAG = '0' AND\n"); 
		sql.append("       SUBSTR(DOMICILE_CODE, 1, 6) || '000000' = F.ID");
		organCode=FilterLimit.filterByOrganCode(sql,organCode);
		DataSet ds = this.executeDataset(sql.toString(),new int[]{Types.VARCHAR},new Object[]{organCode},pset.getPageStart(),pset.getPageLimit(), true);
		return ds;
	}
	
	public DataSet queryillprovinceEmigrationPeopleList(ParameterSet pset) {
		String organCode = (String) pset.getParameter("organCode");
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT T.SERVICE_TYPE, C.ASDP_NO, D.NAME AS DOMI_NAME, A.FAMILY_ID,\n");
		sql.append("       A.DEMOBILIZED_STATE_CODE, A.PEOPLE_ID, A.DOMICILE_CODE, A.NAME,\n"); 
		sql.append("       A.ID_CARD, A.SEX, A.NATION, C.IS_WAR, C.IS_INNUCLEAR,\n"); 
		sql.append("       C.CONSCRIPT_DATE, C.DECRUITMENT_DATE,\n"); 
		sql.append("       E.NAME || F.NAME || D.NAME DOMICILE_NAME, A.DOMICILE_TYPE,\n"); 
		sql.append("       A.TEL_MOBILE, C.NOTE, T.APPLY_ID\n"); 
		sql.append("  FROM BPT_PROVINCE_MIGRATE T, BASEINFO_PEOPLE A, BPT_PEOPLE B,\n"); 
		sql.append("       BPT_PEOPLE_DEMOBILIZED_ILLNESS C, DIC_CITY D, DIC_CITY E, DIC_CITY F\n"); 
		sql.append(" WHERE A.PEOPLE_ID = C.PEOPLE_ID AND A.PEOPLE_ID = B.PEOPLE_ID AND\n"); 
		sql.append("       A.DEMOBILIZED_FLAG = '1' AND A.PERSONAL_STATS_TAG != '4' AND\n"); 
		sql.append("       A.DOMICILE_CODE = D.ID AND\n"); 
		sql.append("       SUBSTR(DOMICILE_CODE, 1, 4) || '00000000' = E.ID AND\n"); 
		sql.append("       T.SERVICE_TYPE = '47' AND T.PEOPLE_ID = A.PEOPLE_ID AND\n"); 
		sql.append("       T.COMMIT_FLAG = '0' AND\n"); 
		sql.append("       SUBSTR(DOMICILE_CODE, 1, 6) || '000000' = F.ID");
		organCode=FilterLimit.filterByOrganCode(sql,organCode);
		DataSet ds = this.executeDataset(sql.toString(),new int[]{Types.VARCHAR},new Object[]{organCode}, pset.getPageStart(),pset.getPageLimit(), true);
		return ds;
	}
}
