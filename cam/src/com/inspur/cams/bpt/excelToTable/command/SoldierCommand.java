...
public class SoldierCommand extends BaseAjaxCommand{
	...
	public void saveDisability() {
		...
		DataSet ds = dao.queryMarkDatabyUser("BPT_IMP_SOLDIER_MAKE", user);
		...
		if(flag!=null&&"1".equals(flag)){//多重身份时只添加类别信息更新人员信息
			...
			if(d.getCount()>0){
				...
				if(dp.getCount()>0){
					...
					baseinfoPeople.setRetiredNo((String)r.get("RETIRED_NO"));
					baseinfoPeople.setRedaFlag("1");
					baseinfoPeople.setPersonalStatsTag("0");
					baseinfoPeople.setRetiredStateCode((String)r.get("RETIRED_STATE_CODE"));
			...
			BptCountRetiredSoldier countRetiredSoldier = getBptCountRetiredSoldier(r);
			countRetiredSoldier.setPeopleId((String)d.getRecord(0).get("peopleId"));
			countRetiredSoldier.setRetiredNo((String)r.get("PRETIRED_NO"));
			countRetiredSoldier.setState(1);
			dao.insert(countRetiredSoldier);
		}else{
			...
			countRetiredSoldier.setPeopleId(peopleIdRe);
			countRetiredSoldier.setRetiredNo((String)r.get("PRETIRED_NO"));
	 ...
	 *  获取退役士兵信息
	 */
	private BptCountRetiredSoldier getBptCountRetiredSoldier(Record r){
		BptCountRetiredSoldier bean = (BptCountRetiredSoldier) Until.toBean(r,BptCountRetiredSoldier.class);
        bean.setRegId(BspUtil.getEmpOrganId());
		bean.setRegTime(DateUtil.getTime());
        bean.setModId(BspUtil.getEmpOrganId());
		bean.setModTime(DateUtil.getTime());
