...
public class MartyrCommand extends BaseAjaxCommand{
	...
	IBptPeopleMartyrOffspringDomain martyrOffspringDomain = (IBptPeopleMartyrOffspringDomain)ScaComponentFactory.getService(IBptPeopleMartyrOffspringDomain.class, 
				"bptPeopleMartyrOffspringDomain/bptPeopleMartyrOffspringDomain");
	@Trans
	public void saveDisability() {
		String user = (String) getParameter("user");
		MessageDao dao = new MessageDao();
		DataSet ds = dao.queryMarkDatabyUser("BPT_IMP_MARTYROFFSPRING_MAKE", user);
		...
		if(flag!=null&&"1".equals(flag)){//多重身份时只添加类别信息更新人员信息
			...
			if(d.getCount()>0){
				...
				if(dp.getCount()>0){
					...
					baseinfoPeople.setIncomeYear(new BigDecimal((String)r.get("INCOME_YEAR")));
					...
					baseinfoPeople.setMartyrOffspringNo((String)r.get("MARTYR_OFFSPRING_NO"));
					baseinfoPeople.setMartyrOffspringFlag("1");
					baseinfoPeople.setPersonalStatsTag("0");
					baseinfoPeople.setMartyrOffspringStateCode((String)r.get("MARTYR_OFFSPRING_STATE_CODE"));
			...
			BptPeopleMartyrOffspring martyrOffspringInfor = getMartyrOffspringInfor(r);
			martyrOffspringInfor.setPeopleId((String)d.getRecord(0).get("peopleId"));
			martyrOffspringInfor.setState(1);
			...
			martyrOffspringInfor.setPeopleId(peopleIdRe);
	 ...
	 * @return
	 */
	public BptPeopleMartyrOffspring getMartyrOffspringInfor(Record r){
		BptPeopleMartyrOffspring bean = (BptPeopleMartyrOffspring)Until.toBean(r,BptPeopleMartyrOffspring.class);
		bean.setRegId(BspUtil.getEmpOrganId());
		bean.setRegTime(DateUtil.getTime());
		bean.setModId(BspUtil.getEmpOrganId());
		bean.setModTime(DateUtil.getTime());
