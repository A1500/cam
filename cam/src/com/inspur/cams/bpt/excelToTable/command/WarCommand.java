...
public class WarCommand extends BaseAjaxCommand{
	...
	public void saveDisability() {
		...
		DataSet ds = dao.queryMarkDatabyUser("bpt_imp_war_make", user);
	...
	public void saveDisability(Record r) {
		...
		if(flag!=null&&"1".equals(flag)){//多重身份时只添加类别信息更新人员信息
			...
			BptPeopleWar bptPeopleWar = getBptPeopleWar(r);
			bptPeopleWar.setPeopleId((String)d.getRecord(0).get("peopleId"));
			bptPeopleWar.setState(1);
	 ...
	 *  获取参战人员信息
	 */
	private BptPeopleWar getBptPeopleWar(Record r){
		BptPeopleWar bean = (BptPeopleWar) Until.toBean(r,BptPeopleWar.class);
		bean.setRegId(BspUtil.getEmpOrganId());
		bean.setRegTime(DateUtil.getTime());
		bean.setModId(BspUtil.getEmpOrganId());
		bean.setModTime(DateUtil.getTime());
