...
public class DemobilizedIllnessCommand extends BaseAjaxCommand{
	...
	public void saveDisability() {
		...
		DataSet ds = dao.queryMarkDatabyUser("bpt_imp_demillness_make", user);
	...
	public void saveDisability(Record r) {
		...
		if(flag!=null&&"1".equals(flag)){//多重身份时只添加类别信息更新人员信息
			...
			BptPeopleDemobilizedIllness bptPeopleDemobilizedIllness = getBptPeopleDemobilizedIllness(r);
			bptPeopleDemobilizedIllness.setPeopleId((String)d.getRecord(0).get("peopleId"));
			bptPeopleDemobilizedIllness.setState(1);
	 ...
	 */
	private BptPeopleDemobilizedIllness getBptPeopleDemobilizedIllness(Record r){
		BptPeopleDemobilizedIllness bean = (BptPeopleDemobilizedIllness) Until.toBean(r,BptPeopleDemobilizedIllness.class);
		bean.setRegId(BspUtil.getEmpOrganId());
		bean.setRegTime(DateUtil.getTime());
		bean.setModId(BspUtil.getEmpOrganId());
		bean.setModTime(DateUtil.getTime());
