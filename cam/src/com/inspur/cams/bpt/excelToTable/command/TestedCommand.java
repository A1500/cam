...
public class TestedCommand extends BaseAjaxCommand{
	...
	public void saveDisability() {
		...
		DataSet ds = dao.queryMarkDatabyUser("bpt_imp_tested_make", user);
		...
		if(flag!=null&&"1".equals(flag)){//多重身份时只添加类别信息更新人员信息
			...
			if(d.getCount()>0){
				...
				if(dp.getCount()>0){
					...
					baseinfoPeople.setWarNo((String)r.get("WAR_NO"));
					baseinfoPeople.setWarFlag("1");
					baseinfoPeople.setPersonalStatsTag("0");
					baseinfoPeople.setWarStateCode((String)r.get("WAR_STATE_CODE"));
			...
			BptPeopleTested bptPeopleTested = getBptPeopleTested(r);
			bptPeopleTested.setPeopleId((String)d.getRecord(0).get("peopleId"));
			bptPeopleTested.setState(1);
	 ...
	 *  获取参试信息
	 */
	private BptPeopleTested getBptPeopleTested(Record r){
		BptPeopleTested bean = (BptPeopleTested) Until.toBean(r,BptPeopleTested.class);
