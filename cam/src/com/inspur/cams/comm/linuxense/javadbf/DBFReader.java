...
package com.inspur.cams.comm.linuxense.javadbf;
		...
		try {
			...
			for (int i = 0; i < this.header.fieldArray.length; i++) {
				...
				case 'C':
					...
					if (!Utils.contains(b_array, (byte) '-')) {
				...
				case 'N':
					...
					try {
						...
						if (t_numeric.length > 0
								&& !"".equals(new String(t_numeric).trim())
				...
				case 'M':
					// TODO Later
					byte b_array_M[] = new byte[this.header.fieldArray[i]
							.getFieldLength()];
					dataInputStream.read(b_array_M);
					if (!Utils.contains(b_array_M, (byte) '-')) {
						recordObjects[i] = new String(b_array_M, characterSetName);
					} else {
						recordObjects[i] = null;
					}
					break;
