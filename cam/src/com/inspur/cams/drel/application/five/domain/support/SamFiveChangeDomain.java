package com.inspur.cams.drel.application.five.domain.support;
...
import com.inspur.cams.drel.application.five.domain.ISamFiveChangeDomain;
 ...
 */
public class SamFiveChangeDomain implements ISamFiveChangeDomain{
	@Reference
	...
	public static final String PROC_DEF_UNIQUE_ID = "8a898bd3376e5f2301376e87154c0006";
	 ...
	 * 更新低保家庭成员状态信息
	 */
	public BaseinfoFamily updatePeopleStats(String peopleId,String familyId,String personalStatsTag){
		//更新人员状态
		ParameterSet pset=new ParameterSet();
		...
		//根据人员年收入更新家庭收入
		baseinfoDomain.updateIncomeByYear(familyId);
		//更新家庭人口信息		
...
//TODO ###############################                私有方法                ##########################################################
	...
	private void create(SamApplyInfo applyInfo) {
		...
		//创建流程
		Map wfMap = WfTaskRequestWrapper.getInstance().create(applyInfo.getApplyId(), applyInfo.getFamilyName() + "五保复查变更业务申请");
		//得到委派实例ID
	...
	private void fillApplyInfo(SamApplyInfo applyInfo) {
		...
		applyInfo.setAsisstanceClass(baseinfoFamily.getAssistanceClass());
		//申请业务类别
		applyInfo.setApplyType(SamApplyType.SAM_FIVE_CHANGE);
	 ...
	 * @return
	 */
	private void resolveTreatment(SamApplyInfo applyInfo,String dataSource) {
		...
		//五保
		samFamilyTreatment.setAssistanceType("03");
		if(applyInfo.getBeginDate().trim() == null || "".equals(applyInfo.getBeginDate().trim())){
			throw new  RuntimeException("数据异常！未获取开始时间 at line 641！");
		}
		...
		samFamilyTreatment.setSupportWay(applyInfo.getSupportWay());//供养方式
		samFamilyTreatment.setSupportOrg(applyInfo.getSupportOrg());//供养机构
	...
	private void toFillCurInfo(SamApplyInfo applyInfo) {
		...
		//如果是审批环节
			...
			//如果是审批同意
				...
				if("06".equals(applyInfo.getChangeItem())){
					//更新家庭信息
					BaseinfoFamily baseinfoFamily=applyInfo.getBaseInfoFamily();
					...
					samFamilyAssistanceDomain.deleteAssistance(applyInfo.getFamilyId(), "03");//农村五保
					applyInfo.setCurActivityId("audit");
					applyInfo.setCurActivityName("审批");
					...
					if(ds.getCount() == 1){
						...
						//维护一个结束日期
						if(applyInfo.getBeginDate().trim() == null || "".equals(applyInfo.getBeginDate().trim())){
							throw new  RuntimeException("数据异常！未获取结束时间 at line 767！");
						}
						samFamilyTreatment.setEndDate(applyInfo.getBeginDate());
						samFamilyTreatment.setChangeItem("06");
					...
					if(ds.getCount() == 1){
						SamFamilyTreatment samFamilyTreatment=(SamFamilyTreatment)ds.getRecord(0).toBean(SamFamilyTreatment.class);
						if(applyInfo.getBeginDate().trim() == null || "".equals(applyInfo.getBeginDate().trim())){
							throw new  RuntimeException("数据异常！未获取开始时间 at line 792！");
						}
					...
					resolveTreatment(applyInfo,dataSource);
					//插入特定的救助类型：农村五保				
	...
	private void getFamilyCardNo(SamApplyInfo applyInfo) {
		...
		pset.setParameter("APPLY_TYPE@IN","030,032");
	...
	private void insertSpecialAssistance(SamApplyInfo applyInfo){
		ParameterSet pset=new ParameterSet();
		pset.setParameter("ASSISTANCE_TYPE","03");
		pset.setParameter("FAMILY_ID",applyInfo.getFamilyId());
		...
		if(ds.getCount() > 0 ){
			...
			samFamilyAssistance.setAssistanceType("03");
			samFamilyAssistance.setCardNo(applyInfo.getCardNo());
			samFamilyAssistance.setAssistanceMon(applyInfo.getAssistanceMoney());
			samFamilyAssistance.setAssistanceClassMon(applyInfo.getAssistanceClassMoney());
			samFamilyAssistance.setAssistanceBegin(applyInfo.getBeginDate());
