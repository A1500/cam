...
import com.inspur.cams.drel.application.five.domain.ISamFiveApplyDomain;
 ...
 * 农村五保新申请业务Domain接口
 */
public class SamFiveApplyDomain implements ISamFiveApplyDomain{
	...
	public static final String PROC_DEF_UNIQUE_ID = "8a898bd3373fc71501373ffa262f0057";
	...
	public void sendApply(SamApplyInfo applyInfo) {
		...
		if("1".equals(samApplyOptionsHis.getOpinionId()) &&
			...
			if(applyInfo.getFamilyId() != null && !"".equals(applyInfo.getFamilyId())){
				BaseinfoFamily baseinfoFamily=baseinfoDomain.getFamily(applyInfo.getFamilyId());
	...
	private void toFillCurInfo(SamApplyInfo applyInfo) {
		...
		if("check".equals(applyInfo.getCurActivityId())){//如果是审核环节
			...
			if("3".equals(applyInfo.getAuditOpinionId())){
				...
				applyInfo.setIsFinish("1");
				applyInfo.setFinishTime(DateUtil.getTime());
				applyInfo.setCardNo(samCardNoDomain.getCardNo("W",applyInfo.getDomicileCode()));//农村五保
	...
	private void create(SamApplyInfo applyInfo) {
		...
		//创建流程
		Map wfMap = WfTaskRequestWrapper.getInstance().create(applyInfo.getApplyId(), applyInfo.getFamilyName() + "五保业务申请");
		//得到委派实例ID
	...
	private void fillApplyInfo(SamApplyInfo applyInfo) {
		...
		applyInfo.setApplyType(SamApplyType.SAM_FIVE_NEW_APPLY);//申请业务类别
	...
	private void resolveTreatment(SamApplyInfo applyInfo) {
		...
		samFamilyTreatment.setAssistanceType("03");//五保
		if(applyInfo.getBeginDate().trim() == null || "".equals(applyInfo.getBeginDate().trim())){
			throw new  RuntimeException("数据异常！未获取开始时间 at line 652！");
		}
	...
	public BaseinfoFamily insertPeopleInfo(BaseinfoPeople baseinfoPeople,String applyId){
		...
		//根据人员年收入更新家庭收入
		baseinfoDomain.updateIncomeByYear(baseinfoPeople.getFamilyId());
		//更新家庭人口信息
	 ...
	 * 删除低保家庭成员信息
	 */
	public BaseinfoFamily deletePeopleInfo(String peopleId,String familyId){
		//处理人员成员信息
	...
	private String handlePeople(BaseinfoPeople baseinfoPeople) {
		...
		if("".equals(peopleId) || peopleId == null){//新增
			baseinfoPeople.setRuralFiveguaFlag("1");
