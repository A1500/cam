 ...
 * 农村低保新申请业务Domain接口
 */
public class SamCountryApplyDomain implements ISamCountryApplyDomain{
	...
	public static final String PROC_DEF_UNIQUE_ID = "8a898bd3373f8ddd01373f8ddd090000";
	...
	private void toFillCurInfo(SamApplyInfo applyInfo) {
		...
		if("check".equals(applyInfo.getCurActivityId())){//如果是审核环节
			...
			if("3".equals(applyInfo.getAuditOpinionId())){
				...
				applyInfo.setIsFinish("1");
				applyInfo.setFinishTime(DateUtil.getTime());
				applyInfo.setCardNo(samCardNoDomain.getCardNo("N",applyInfo.getDomicileCode()));//农村低保
	...
	private void create(SamApplyInfo applyInfo) {
		...
		//创建流程
		Map wfMap = WfTaskRequestWrapper.getInstance().create(applyInfo.getApplyId(), applyInfo.getFamilyName() + "农村低保业务申请");
		//得到委派实例ID
	...
	private void fillApplyInfo(SamApplyInfo applyInfo) {
		...
		applyInfo.setApplyType(SamApplyType.SAM_COUNTRY_NEW_APPLY);//申请业务类别
	...
	private void resolveTreatment(SamApplyInfo applyInfo) {
		...
		if(applyInfo.getBeginDate().trim() == null || "".equals(applyInfo.getBeginDate().trim())){
			throw new  RuntimeException("数据异常！未获取开始时间 at line 667！");
		}
	...
	private String handlePeople(BaseinfoPeople baseinfoPeople) {
		...
		if("".equals(peopleId) || peopleId == null){//新增
			baseinfoPeople.setRuralResiFlag("1");//农村低保标志
