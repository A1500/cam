package com.inspur.cams.drel.application.country.domain.support;
 ...
 */
public class SamCountryChangeDomain implements ISamCountryChangeDomain {
	@Reference
	...
	public static final String PROC_DEF_UNIQUE_ID = "8a898bd3376e5f2301376e86dbfc0004";
		...
		if( !"".equals(applyId) && applyId != null){
			...
			if(ds.getCount() > 0 ){
				...
				if("10".equals(assistanceModeValue)){//如果救助金计算方式是补差，才进行户月保障金的计算
					//户月保障金的计算
					BigDecimal assistanceMoney=samApplyInfoDomain.getCountryAssistanceMoney(baseinfoFamily);
...
//TODO ###############################                私有方法                ##########################################################
	...
	private void create(SamApplyInfo applyInfo) {
		...
		//创建流程
		Map wfMap = WfTaskRequestWrapper.getInstance().create(applyInfo.getApplyId(), applyInfo.getFamilyName() + "农村低保复查变更业务申请");
		//得到委派实例ID
	...
	private void fillApplyInfo(SamApplyInfo applyInfo) {
		...
		applyInfo.setAsisstanceClass(baseinfoFamily.getAssistanceClass());
		//申请业务类别
		applyInfo.setApplyType(SamApplyType.SAM_COUNTRY_CHANGE);
	...
	private void resolveTreatment(SamApplyInfo applyInfo,String dataSource, BigDecimal beforeMoney) {
		...
		//农村低保
		samFamilyTreatment.setAssistanceType("02");
		if(applyInfo.getBeginDate().trim() == null || "".equals(applyInfo.getBeginDate().trim())){
			throw new  RuntimeException("数据异常！未获取开始时间 at line 661！");
		}
...
//		}
		...
		//如果是审批环节
			...
			//如果是审批同意
				...
				if("03".equals(applyInfo.getChangeItem())){
					...
					samFamilyAssistanceDomain.deleteAssistance(applyInfo.getFamilyId(), "02");//农村低保
					applyInfo.setCurActivityId("audit");
					applyInfo.setCurActivityName("审批");
					...
					if(ds.getCount() == 1){
						...
						//维护一个结束日期
						if(applyInfo.getBeginDate().trim() == null || "".equals(applyInfo.getBeginDate().trim())){
							throw new  RuntimeException("数据异常！未获取结束时间 at line 802！");
						}
					...
					if(ds.getCount() == 1){
						SamFamilyTreatment samFamilyTreatment=(SamFamilyTreatment)ds.getRecord(0).toBean(SamFamilyTreatment.class);
						if(applyInfo.getBeginDate().trim() == null || "".equals(applyInfo.getBeginDate().trim())){
							throw new  RuntimeException("数据异常！未获取结束时间 at line 825！");
						}
	...
	private void getFamilyCardNo(SamApplyInfo applyInfo) {
		...
		pset.setParameter("APPLY_TYPE@IN","020,022");
