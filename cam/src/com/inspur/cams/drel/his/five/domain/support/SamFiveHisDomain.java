package com.inspur.cams.drel.his.five.domain.support;
...
import com.inspur.cams.drel.his.five.domain.ISamFiveHisDomain;
 ...
 * @date 
 */
public class SamFiveHisDomain implements ISamFiveHisDomain{	
	@Reference
	...
	private void resolveTreatment(SamApplyInfo applyInfo) {
		...
		if(ds.getCount() == 1){//如果已经存在，更新
			...
			samFamilyTreatment.setSupportWay(applyInfo.getSupportWay());
			samFamilyTreatment.setSupportOrg(applyInfo.getSupportOrg());
			...
			samFamilyTreatment.setBeginDate(applyInfo.getBeginDate());
			samFamilyTreatment.setCardNo(applyInfo.getCardNo());
			samFamilyTreatment.setAssistanceType("03");//农村五保
	...
	private void fillApplyInfo(SamApplyInfo applyInfo) {
		...
		applyInfo.setApplyType(SamApplyType.SAM_FIVE_HIS);//申请业务类别
	}
	/**
	 * 保存照片
	 * @param photoText
