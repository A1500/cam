package com.inspur.cams.drel.his.city.cmd;
 ...
 * @date 
 */
public class SamCityHisCmd  extends BaseAjaxCommand {
	// 获取SCA组件
	 ...
	 * 调用存储过程删除历史采集的信息
	 */
	public void deleteHisInfoByApplyAndFamilyId(){
		String applyId=(String)getParameter("applyId");
		String familyId=(String)getParameter("familyId");
		if(applyId!=null && familyId !=null && !"".equals(applyId) && !"".equals(familyId)){
			service.deleteHisInfoByApplyAndFamilyId(applyId,familyId);
		}
	}
	
	public void deleteHisPeople(){
		String peopleId=(String)getParameter("peopleId");
		String familyId=(String)getParameter("familyId");
		service.deleteHisPeople(peopleId,familyId);
	}
	/**
	 * 封装数据
	 * @return
