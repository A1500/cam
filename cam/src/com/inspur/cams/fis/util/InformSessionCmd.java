...
public class InformSessionCmd extends BaseAjaxCommand {
	private static IOrganProvider IOrgan = (IOrganProvider) OrganFactory
			.getIOrganProvider();
	...
	public void getUserInfo() {
		...
		if (obj.getEmployeeStruId() != null) {
			info.setUserId(obj.getEmployeeOrganId() == null ? "nobodyid" : obj
					.getEmployeeOrganId());
			...
			InformPubOrganDao pubOrganDao = (InformPubOrganDao) DaoFactory
					.getDao(InformPubOrganDao.class);
			String organId = obj.getDepartmentOrganId();
			String cantCode = IOrgan.getOrganByOrganId(organId).getOrganCode();
			InformPubOrgan pubOrgan = pubOrganDao.getPubOrganByOrgId(obj
					.getCorporationOrganId());
			info.setCorporationId(cantCode);
			info.setUserDepOrganName(OrganFactory.getIOrganProvider()
					.getLoginUserDepartName());
			...
			info.setUserDepstruId(IOrgan.getParent(obj.getEmployeeStruId())
					.getStruId());
			info.setUserDepstrutype(IOrgan.getParent(obj.getEmployeeStruId())
					.getOrganType());
			...
			if (pubOrgan != null) {
				// info.setCantCode(pubOrgan.getOrganId());
